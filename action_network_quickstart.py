...
class ActionNetworkQuickStart:
    ...
    def _extract_game_urls(self, date: str) -> Optional[Path]:
        ...
        try:
            ...
            logger.info("✅ Game URLs extracted", total_games=total_games, file=urls_file.name)
    ...
    async def _check_game_outcomes(self):
        ...
        try:
            if check_game_outcomes is None:
                logger.warning("⚠️ Game outcome service not available - skipping outcome check")
                return
            ...
            results = await check_game_outcomes(date_range=date_range, force_update=False)
            
            # Log results
            if results["updated_outcomes"] > 0:
                logger.info("✅ Updated game outcomes", 
    ...
    def _display_games_and_opportunities(self, urls_file: Path):
        ...
        try:
            ...
            print("="*80)
            print("🎯 NEXT STEPS:")
            print("1. Run the complete Action Network pipeline to collect historical data:")
            print("   uv run python -m src.interfaces.cli action-network pipeline")
            print()
            print("2. Or use the movement analysis commands (after collecting historical data):")
            print("   uv run python -m src.interfaces.cli movement analyze -i output/historical_line_movement_*.json")
            print()
            print("3. Check for RLM opportunities:")
            print("   uv run python -m src.interfaces.cli movement rlm -i output/historical_line_movement_*.json")
            print()
            print("4. Detect steam moves:")
            print("   uv run python -m src.interfaces.cli movement steam -i output/historical_line_movement_*.json")
            print()
