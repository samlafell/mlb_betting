...
class ActionNetworkStandalonePipeline:
    ...
    async def run_complete_pipeline(
        ...
        pipeline_results = {
            ...
            "configuration": {"date": date, "max_games": max_games},
            "phases": {},
        }
        ...
        try:
            ...
            if not urls_result["success"]:
                logger.error(
                    "❌ Failed to extract game URLs", error=urls_result["error"]
                )
                return False

            logger.info(
                "✅ Game URLs extracted", total_games=urls_result["total_games"]
            )

            # Phase 2: Collect Historical Data
            logger.info("📊 Phase 2: Collecting historical data")
            history_result = await self._collect_historical_data(
                urls_result["games"], max_games
            )
            ...
            analysis_result = await self._analyze_opportunities(
                history_result["historical_data"]
            )
            pipeline_results["phases"]["opportunity_analysis"] = analysis_result

            if not analysis_result["success"]:
                logger.error(
                    "❌ Failed to analyze opportunities", error=analysis_result["error"]
                )
                return False
            ...
            await self._save_results(
                pipeline_results, history_result["historical_data"], analysis_result
            )
    ...
    async def _extract_game_urls(self, date: str) -> dict:
        ...
        try:
            ...
            return {"success": False, "error": str(e), "total_games": 0, "games": []}

    async def _collect_historical_data(
        self, games: list[dict], max_games: int | None
    ) -> dict:
        ...
        try:
            ...
            logger.info(
                "🔄 Collecting historical data from games", total_games=len(games)
            )
            ...
            async with aiohttp.ClientSession(headers=self.headers) as session:
                for i, game in enumerate(games, 1):
                    try:
                        ...
                        logger.info(
                            f"📊 Collecting data for game {i}/{len(games)}",
                            game_id=game_id,
                            teams=f"{game.get('away_team', 'Unknown')} @ {game.get('home_team', 'Unknown')}",
                        )

                        # Collect historical data
                        game_history = await self._collect_game_history(
                            session, game_id, history_url, game
                        )
                        if game_history:
            ...
            total_movements = sum(
                len(game.get("historical_entries", [])) for game in historical_data
            )
    ...
    async def _collect_game_history(
        self,
        ...
        try:
            ...
            for game in historical_data:
                ...
                for entry in entries:
                    ...
                    analysis_results["market_breakdown"][market_type] = (
                        analysis_results["market_breakdown"].get(market_type, 0) + 1
                    )

                    book_id = entry.get("book_id")
                    book_name = book_mapping.get(book_id, f"Book {book_id}")
                    analysis_results["book_breakdown"][book_name] = (
                        analysis_results["book_breakdown"].get(book_name, 0) + 1
                    )
    ...
    def _detect_rlm_opportunities(self, game: dict, entries: list[dict]) -> list[dict]:
        ...
        for market_type, market_entries in market_groups.items():
            ...
            entries_with_bet_percent = [
                e for e in market_entries if e.get("bet_percent") is not None
            ]
    ...
    async def _save_results(
        self, pipeline_results: dict, historical_data: list[dict], analysis_result: dict
    ):
        """Save all results to files."""
        try:
            # Save historical data
            history_file = (
                self.output_dir / f"historical_line_movement_full_{self.timestamp}.json"
            )
            history_output = {
                "collected_at": datetime.now().isoformat(),
                "total_games": len(historical_data),
            ...
            # Save opportunities
            opportunities_file = (
                self.output_dir / f"betting_opportunities_{self.timestamp}.json"
            )
            opportunities_data = {
                "generated_at": datetime.now().isoformat(),
                "total_games": analysis_result["games_analyzed"],
                "rlm_opportunities": analysis_result["analysis_data"][
                    "rlm_opportunities"
                ],
                "steam_moves": analysis_result["analysis_data"]["steam_moves"],
                "high_movement_games": analysis_result["analysis_data"][
                    "high_movement_games"
                ],
                "summary": {
                    "total_rlm_opportunities": len(
                        analysis_result["analysis_data"]["rlm_opportunities"]
                    ),
                    "total_steam_moves": len(
                        analysis_result["analysis_data"]["steam_moves"]
                    ),
                    "total_high_movement_games": len(
                        analysis_result["analysis_data"]["high_movement_games"]
                    ),
                },
            }
    ...
    def _display_summary(self, pipeline_results: dict, analysis_result: dict):
        ...
        if "url_extraction" in phases:
            print(
                f"📡 URLs Extracted: {phases['url_extraction'].get('total_games', 0)} games"
            )
        if "history_collection" in phases:
            print(
                f"📊 Historical Data: {phases['history_collection'].get('total_movements', 0)} movements"
            )
        if "opportunity_analysis" in phases:
            print(
                f"🔍 Games Analyzed: {phases['opportunity_analysis'].get('games_analyzed', 0)}"
            )
        ...
        print(
            f"  • High Movement Games: {analysis_result.get('high_movement_games', 0)}"
        )
        ...
        if rlm_opportunities:
            ...
            for i, opp in enumerate(rlm_opportunities[:5], 1):
                print(
                    f"  {i}. {opp['teams']} - {opp['market_type']} ({opp['strength']})"
                )
        ...
        if steam_moves:
            ...
            for i, move in enumerate(steam_moves[:5], 1):
                print(
                    f"  {i}. {move['teams']} - {move['market_type']} ({move['book_count']} books)"
                )
