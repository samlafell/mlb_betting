...
async def demo_backtesting_engine_functionality():
    ...
    try:
        ...
        print("✅ BacktestingEngine initialized with consolidated modules")
        ...
        try:
            accuracy_metrics = (
                await backtesting_engine.accuracy_monitor.get_recent_metrics()
            )
...
async def demo_historical_backtesting():
    ...
    try:
        ...
        print(
            f"📅 Testing date range: {start_date.strftime('%Y-%m-%d')} to {end_date.strftime('%Y-%m-%d')}"
        )
        ...
        if isinstance(results, dict):
            ...
            if top_performers:
                ...
                for i, performer in enumerate(top_performers[:3], 1):
                    ...
                    print(
                        f"   ROI: {performer.get('roi', 0):.1f}% | Win Rate: {performer.get('win_rate', 0):.1%}"
                    )
        else:
