...
async def test_game_outcome_service():
    ...
    try:
        ...
        if results["updated_outcomes"] > 0:
            print(f"🎯 Successfully updated {results['updated_outcomes']} game outcomes!")
        else:
    ...
    try:
        ...
        if recent_outcomes:
            ...
            for outcome in recent_outcomes[:3]:  # Show first 3
                print(f"   {outcome['away_team']} {outcome['away_score']} - {outcome['home_score']} {outcome['home_team']}")
                print(f"   Winner: {outcome['home_team'] if outcome['home_win'] else outcome['away_team']}")
                if outcome['over'] is not None:
                    print(f"   Total: {'Over' if outcome['over'] else 'Under'} {outcome['total_line']}")
                print()
