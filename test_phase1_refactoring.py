...
from mlb_sharp_betting.services.config_service import (
    get_config_service, get_service_config, get_rate_limits, get_retry_config
)
from mlb_sharp_betting.services.retry_service import (
    get_retry_service, OperationType, retry_operation
)
from mlb_sharp_betting.services.rate_limiter import (
    get_rate_limiter, RateLimitStatus
)
...
from mlb_sharp_betting.services.feature_flags import (
    get_feature_flags, is_enabled, get_flag_value, TemporaryFlagOverride
)
...
class Phase1RefactoringTester:
    ...
    async def test_configuration_service(self) -> bool:
        ...
        try:
            ...
            assert config_service1 is config_service2, "ConfigurationService should be singleton"
            ...
            assert "service_name" in vsin_config, "Service config should include service_name"
            assert "rate_limits" in vsin_config, "Service config should include rate_limits"
            ...
            assert "max_requests_per_minute" in rate_limits, "Rate limits should include max_requests_per_minute"
            ...
            assert "max_attempts" in retry_config, "Retry config should include max_attempts"
            assert "base_delay_seconds" in retry_config, "Retry config should include base_delay_seconds"
            print("✓ Retry config retrieval working")
            
            # Test cache functionality
            config_service1.clear_cache()
            cache_stats = config_service1.get_cache_stats()
            assert cache_stats["cached_configs"] == 0, "Cache should be empty after clear"
            print("✓ Cache management working")
            
            # Test configuration update
            config_service1.update_service_config("test_service", {"custom_setting": "test_value"})
            test_config = get_service_config("test_service")
            assert test_config["custom"]["custom_setting"] == "test_value", "Custom config should be retrievable"
    ...
    async def test_retry_service(self) -> bool:
        ...
        try:
            ...
            assert result == "success_after_retries", "Retry operation should eventually succeed"
            ...
            assert "operation_metrics" in metrics, "Metrics should include operation_metrics"
            assert "circuit_breaker_states" in metrics, "Metrics should include circuit_breaker_states"
    ...
    async def test_rate_limiter(self) -> bool:
        ...
        try:
            ...
            result = await rate_limiter.check_rate_limit("odds_api", "test_operation", 1)
            assert result.status in [RateLimitStatus.ALLOWED, RateLimitStatus.QUOTA_EXCEEDED], "Rate limit check should return valid status"
            print("✓ Rate limit checking working")
            
            # Test request recording
            if result.allowed:
                await rate_limiter.record_request("odds_api", "test_operation", 1, "test_endpoint", {"test": "metadata"})
            ...
            assert "service_name" in status, "Service status should include service_name"
            ...
            wait_result = await rate_limiter.wait_for_rate_limit("test_service", "test_op", 1, 0.1)
            # Should either be allowed immediately or timeout quickly
            assert wait_result.status in [RateLimitStatus.ALLOWED, RateLimitStatus.RATE_LIMITED], "Wait should return valid status"
    ...
    async def test_accuracy_monitor(self) -> bool:
        ...
        try:
            ...
            with tempfile.TemporaryDirectory() as temp_dir:
                from mlb_sharp_betting.services.betting_accuracy_monitor import BettingAccuracyMonitor
                ...
                try:
                    ...
                    if "Failed to establish baseline" in str(e) or "connection" in str(e).lower():
                        print("✓ Baseline establishment handles missing database gracefully")
                    else:
                        raise
                ...
                assert monitor.current_refactoring_phase == "test_phase", "Phase should be set"
                ...
                assert "monitoring_active" in status, "Status should include monitoring_active"
    ...
    async def test_feature_flags(self) -> bool:
        ...
        try:
            ...
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                assert isinstance(use_config_service, bool), "Flag value should be boolean"
                ...
                assert flags.is_enabled("use_config_service"), "Flag should be enabled after setting"
                ...
                assert not flags.is_enabled("use_config_service"), "Flag should be disabled"
                ...
                with TemporaryFlagOverride({"use_config_service": False}):
                    assert not flags.is_enabled("use_config_service"), "Flag should be overridden in context"
                
                # Should be restored after context
                assert flags.is_enabled("use_config_service"), "Flag should be restored after context"
        ...
        try:
            ...
            with TemporaryFlagOverride({"use_unified_retry": True}):
                if is_enabled("use_unified_retry"):
                    ...
                    assert result == "integration_success", "Integrated operation should succeed"
            ...
            with TemporaryFlagOverride({"use_unified_rate_limiting": True}):
                if is_enabled("use_unified_rate_limiting"):
                    ...
                    assert hasattr(rate_result, 'allowed'), "Rate limit result should have allowed property"
