...
WITH team_splits_data AS (
    SELECT 
        ...
        -- Extract line values for context
        CASE 
...
strategy_performance AS (
    SELECT 
        ...
        -- Performance metrics
        ROUND(AVG(overall_performance) * 100, 1) as avg_win_rate_pct,
        
        -- ROI estimation (simplified, assumes even betting across all games)
        ROUND(((AVG(overall_performance) * 100) - ((1 - AVG(overall_performance)) * 110)) / 110 * 100, 1) as estimated_roi_per_100,
...
SELECT 
    ...
    -- Supporting data
    ROUND(avg_public_bet_pct, 1) as avg_public_bet_pct,
    ROUND(avg_sharp_differential, 1) as avg_sharp_diff,
