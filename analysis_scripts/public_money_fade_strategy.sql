...
SELECT 
    ...
    ROUND(AVG(fade_successful) * 100, 1) as win_rate,
    ROUND(((SUM(fade_successful) * 100.0) - ((COUNT(*) - SUM(fade_successful)) * 110.0)) / (COUNT(*) * 110.0) * 100, 1) as roi_per_100_unit,
    
    -- Compare to following public
    SUM(follow_public_successful) as follow_public_wins,
    ROUND(AVG(follow_public_successful) * 100, 1) as follow_public_win_rate_pct,
    ROUND(((SUM(follow_public_successful) * 100.0) - ((COUNT(*) - SUM(follow_public_successful)) * 110.0)) / (COUNT(*) * 110.0) * 100, 1) as follow_public_roi_per_100,
    
    -- Signal quality metrics
    ROUND(AVG(public_consensus_strength), 1) as avg_public_consensus_pct,
    ROUND(AVG(num_books), 1) as avg_books_per_signal,
    ROUND(AVG(money_pct_stddev), 1) as avg_book_agreement,
    COUNT(DISTINCT home_team || '-' || away_team || '-' || DATE(game_datetime)) as unique_games,
    
    -- Performance edge calculation
    ROUND(AVG(fade_successful) - AVG(follow_public_successful), 3) as fade_edge_over_public,
    
    -- Recent examples for validation
    STRING_AGG(
        home_team || ' vs ' || away_team || ' (Fade: ' || fade_recommendation || 
        ', Public: ' || ROUND(avg_money_pct, 0) || '%, Result: ' || 
