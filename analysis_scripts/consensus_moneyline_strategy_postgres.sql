...
SELECT 
    ...
    ROUND((AVG(consensus_follow_correct) * 100)::NUMERIC, 1) as win_rate_pct,
    ROUND((((SUM(consensus_follow_correct) * 100.0) - ((COUNT(*) - SUM(consensus_follow_correct)) * 110.0)) / (COUNT(*) * 110.0) * 100)::NUMERIC, 1) as roi_per_100,
    ROUND((AVG(consensus_strength))::NUMERIC, 1) as avg_consensus_strength,
    ROUND((AVG(consensus_alignment))::NUMERIC, 1) as avg_consensus_alignment,
...
SELECT 
    ...
    ROUND((AVG(consensus_fade_correct) * 100)::NUMERIC, 1) as win_rate_pct,
    ROUND((((SUM(consensus_fade_correct) * 100.0) - ((COUNT(*) - SUM(consensus_fade_correct)) * 110.0)) / (COUNT(*) * 110.0) * 100)::NUMERIC, 1) as roi_per_100,
    ROUND((AVG(consensus_strength))::NUMERIC, 1) as avg_consensus_strength,
    ROUND((AVG(consensus_alignment))::NUMERIC, 1) as avg_consensus_alignment,
    COUNT(DISTINCT home_team || '-' || away_team || '-' || DATE(game_datetime)) as unique_games,
    -- Show recent example bets  
