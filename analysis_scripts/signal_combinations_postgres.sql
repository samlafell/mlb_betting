...
game_aggregated AS (
    SELECT 
        ...
        -- Get outcomes
        BOOL_OR(home_win) as home_win,
        BOOL_OR(home_cover_spread) as home_cover_spread,
        BOOL_OR(over) as over,
...
SELECT 
    ...
    CASE WHEN multi_consensus_bets > 0 
         THEN ROUND((100.0 * multi_consensus_wins / multi_consensus_bets)::NUMERIC, 1) 
...
SELECT 
    ...
    CASE WHEN ml_spread_bets > 0 
         THEN ROUND((100.0 * ml_spread_wins / ml_spread_bets)::NUMERIC, 1) 
         ELSE 0 END as win_rate,
...
SELECT 
    ...
    CASE WHEN triple_consensus_bets > 0 
         THEN ROUND((100.0 * triple_consensus_wins / triple_consensus_bets)::NUMERIC, 1) 
         ELSE 0 END as win_rate,
...
SELECT 
    ...
    CASE WHEN fade_conflicts_bets > 0 
         THEN ROUND((100.0 * fade_conflicts_wins / fade_conflicts_bets)::NUMERIC, 1) 
         ELSE 0 END as win_rate,
