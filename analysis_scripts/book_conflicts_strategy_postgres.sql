...
WITH enhanced_latest_splits AS (
    SELECT 
        ...
        COALESCE((COALESCE(home_or_over_bets, 0) + COALESCE(away_or_under_bets, 0)), 0) as total_volume,
        ...
        CASE 
            WHEN COALESCE((COALESCE(home_or_over_bets, 0) + COALESCE(away_or_under_bets, 0)), 0) >= 1000 THEN 'HIGH_VOLUME'
            WHEN COALESCE((COALESCE(home_or_over_bets, 0) + COALESCE(away_or_under_bets, 0)), 0) >= 300 THEN 'MEDIUM_VOLUME'
            WHEN COALESCE((COALESCE(home_or_over_bets, 0) + COALESCE(away_or_under_bets, 0)), 0) >= 50 THEN 'LOW_VOLUME'
            ELSE 'MINIMAL_VOLUME'
        END as volume_tier,
...
enhanced_book_signals AS (
    SELECT 
        ...
        CASE 
            WHEN COALESCE(money_pct - bet_pct, 0) > 10  THEN 'VALIDATED_SHARP_HOME'
            WHEN COALESCE(money_pct - bet_pct, 0) < -10  THEN 'VALIDATED_SHARP_AWAY'
            WHEN ABS(COALESCE(money_pct - bet_pct, 0)) > 10 AND 1 = 1 THEN 'UNVALIDATED_SIGNAL'
...
SELECT 
    ...
    ROUND((COALESCE(AVG(enhanced_action_successful), 0) * 100)::NUMERIC, 1) as win_rate,
    ...
    ROUND((CASE 
        ...
        ELSE 0
    END)::NUMERIC, 1) as roi_per_100_unit
