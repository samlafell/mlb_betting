...
strategy_performance AS (
    SELECT 
        ...
        -- Performance metrics
        ROUND(((AVG(overall_performance) * 100)::NUMERIC)::NUMERIC, 1) as avg_win_rate_pct,
        
        -- ROI estimation (simplified, assumes even betting across all games)
        ROUND(((((AVG(overall_performance) * 100) - ((1 - AVG(overall_performance)) * 110)) / 110 * 100)::NUMERIC)::NUMERIC, 1) as estimated_roi_per_100,
...
SELECT 
    ...
    -- Supporting data
    ROUND(((avg_public_bet_pct)::NUMERIC)::NUMERIC, 1) as avg_public_bet_pct,
    ROUND(((avg_sharp_differential)::NUMERIC)::NUMERIC, 1) as avg_sharp_diff,
