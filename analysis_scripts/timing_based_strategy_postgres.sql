...
multi_book_consensus AS (
    SELECT 
        ...
        BOOL_OR(ema.home_win) as home_win,
        BOOL_OR(ema.home_cover_spread) as home_cover_spread,
        BOOL_OR(ema.over) as over
...
SELECT 
    ...
    ROUND((((wins * 100 * avg_bet_multiplier) - ((total_bets - wins) * 110 * avg_bet_multiplier)) / total_bets)::NUMERIC, 2) as profit_per_bet_enhanced,
    
    -- Enhanced validation metrics
    ROUND((avg_credibility_score)::NUMERIC, 2) as avg_credibility,
    ROUND((avg_line_movement)::NUMERIC, 2) as avg_movement,
    ROUND((avg_consensus_books)::NUMERIC, 1) as avg_books,
    ROUND((avg_closing_differential)::NUMERIC, 1) as avg_closing_diff,
    ROUND((avg_bet_multiplier)::NUMERIC, 1) as avg_multiplier,
