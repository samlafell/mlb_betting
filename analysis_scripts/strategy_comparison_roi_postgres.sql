...
SELECT 
    ...
    ROUND((100.0 * wins / total_bets)::NUMERIC, 1) as win_rate,
    
    -- ROI Calculations for $100 unit bets
    -- Assuming standard -110 odds (risk $110 to win $100)
    ROUND(((wins * 100) - ((total_bets - wins) * 110))::NUMERIC, 2) as roi_dollars_110_odds,
    
    -- Alternative: Assuming -105 odds (slightly better)
    ROUND(((wins * 100) - ((total_bets - wins) * 105))::NUMERIC, 2) as roi_dollars_105_odds,
    
    -- Break-even analysis
    ROUND((110.0 / 210.0 * 100)::NUMERIC, 1) as breakeven_rate_110_odds,
    ROUND((105.0 / 205.0 * 100)::NUMERIC, 1) as breakeven_rate_105_odds,
    
    -- Profit per bet
    ROUND((((wins * 100) - ((total_bets - wins) * 110)) / total_bets)::NUMERIC, 2) as profit_per_bet_110,
    ROUND((((wins * 100) - ((total_bets - wins) * 105)) / total_bets)::NUMERIC, 2) as profit_per_bet_105,
    
    -- ROI Percentage 
    ROUND((((wins * 100) - ((total_bets - wins) * 110))::NUMERIC / (total_bets * 110) * 100), 1) as roi_percentage_110,
    ROUND((((wins * 100) - ((total_bets - wins) * 105))::NUMERIC / (total_bets * 105) * 100), 1) as roi_percentage_105,
    
    -- Expected field name for backtesting service
    ROUND((((wins * 100) - ((total_bets - wins) * 110))::NUMERIC / (total_bets * 110) * 100), 1) as roi_per_100_unit,
    
    -- Supporting metrics
    ROUND((avg_line_movement)::NUMERIC, 1) as avg_line_move,
    ROUND((avg_differential)::NUMERIC, 1) as avg_diff,
    ...
    CASE 
        WHEN (100.0 * wins / total_bets) > 52.4 THEN 
            ROUND((((100.0 * wins / total_bets / 100) * 2.1 - 1) / 1.1 * 100)::NUMERIC, 1)
        ELSE 0
