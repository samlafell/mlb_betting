...
SELECT 
    ...
    ROUND((SUM(mixed_consensus_follow_correct) * 100.0 - (COUNT(*) - SUM(mixed_consensus_follow_correct)) * 110.0) / COUNT(*), 1) as roi_per_100,
...
SELECT 
    ...
    ROUND((SUM(mixed_consensus_fade_correct) * 100.0 - (COUNT(*) - SUM(mixed_consensus_fade_correct)) * 110.0) / COUNT(*), 1) as roi_per_100,
    ROUND(AVG(consensus_average), 1) as avg_consensus_pct,
    ROUND(AVG(bet_money_gap), 1) as avg_bet_money_gap,
    ROUND(AVG(sharp_public_diff), 1) as avg_sharp_public_diff,
    COUNT(DISTINCT home_team || '-' || away_team || '-' || DATE(g.game_datetime)) as unique_games
