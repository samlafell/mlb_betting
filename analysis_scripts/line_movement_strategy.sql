...
SELECT 
    ...
    CASE WHEN significant_moves > 0 
         THEN ROUND(100.0 * follow_significant_wins / significant_moves, 1) 
         ELSE 0 END as win_rate,
    CASE WHEN significant_moves > 0 
         THEN ROUND(((follow_significant_wins * 100) - ((significant_moves - follow_significant_wins) * 110)) / (significant_moves * 110) * 100, 1)
...
SELECT 
    ...
    CASE WHEN significant_moves > 0 
         THEN ROUND(100.0 * fade_significant_wins / significant_moves, 1) 
         ELSE 0 END as win_rate,
    CASE WHEN significant_moves > 0 
         THEN ROUND(((fade_significant_wins * 100) - ((significant_moves - fade_significant_wins) * 110)) / (significant_moves * 110) * 100, 1)
         ELSE 0 END as roi_per_100_unit
FROM strategy_results
WHERE significant_moves >= 3
