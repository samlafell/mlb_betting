    ...
    try:
        ...
        if results:
            ...
            for i, row in enumerate(results, 1):
                ...
                print(
                    f"   🎯 Strategy Edge: {edge:.1%} {'advantage' if edge > 0 else 'disadvantage'}"
                )
                print()
            ...
            if overall_roi > 10.0:
                ...
                print(
                    "   🔧 RECOMMENDATION: Consider refining or increasing sample size"
                )
                print(f"   💰 Expected profit: ${overall_roi:.2f} per $100 wagered")
            else:
            ...
            cross_market_results = [
                r for r in results if r[0] == "CROSS_MARKET_CONTRADICTION"
            ]
            ...
            total_cross_market_results = [
                r for r in results if r[0] == "TOTAL_CROSS_MARKET"
            ]

            print("\n🔍 FLIP TYPE BREAKDOWN:")
            if cross_market_results:
                cm_bets = sum(
                    r[1] for r in cross_market_results
                )  # total_bets is now index 1
                cm_wins = sum(
                    r[2] for r in cross_market_results
                )  # strategy_wins is now index 2
                cm_roi = (
                    sum(r[6] * r[1] for r in cross_market_results) / cm_bets
                    if cm_bets > 0
                    else 0
                )  # strategy_roi is index 6
                print(
                    f"🔀 Cross-Market Contradictions: {cm_wins}/{cm_bets} ({cm_wins / cm_bets:.1%}) | ROI: {cm_roi:.2f}%"
                )
            ...
            if same_market_results:
                ...
                sm_roi = (
                    sum(r[6] * r[1] for r in same_market_results) / sm_bets
                    if sm_bets > 0
                    else 0
                )
                print(
                    f"🔁 Same Market Flips: {sm_wins}/{sm_bets} ({sm_wins / sm_bets:.1%}) | ROI: {sm_roi:.2f}%"
                )
            ...
            if cross_book_results:
                ...
                cb_roi = (
                    sum(r[6] * r[1] for r in cross_book_results) / cb_bets
                    if cb_bets > 0
                    else 0
                )
                print(
                    f"📚 Cross-Book Flips: {cb_wins}/{cb_bets} ({cb_wins / cb_bets:.1%}) | ROI: {cb_roi:.2f}%"
                )
            ...
            if cross_source_results:
                ...
                cs_roi = (
                    sum(r[6] * r[1] for r in cross_source_results) / cs_bets
                    if cs_bets > 0
                    else 0
                )
                print(
                    f"🌐 Cross-Source Flips: {cs_wins}/{cs_bets} ({cs_wins / cs_bets:.1%}) | ROI: {cs_roi:.2f}%"
                )
            ...
            if total_market_results:
                ...
                tm_roi = (
                    sum(r[6] * r[1] for r in total_market_results) / tm_bets
                    if tm_bets > 0
                    else 0
                )
                print(
                    f"🎯 Total Market Flips: {tm_wins}/{tm_bets} ({tm_wins / tm_bets:.1%}) | ROI: {tm_roi:.2f}%"
                )
            ...
            if total_cross_market_results:
                ...
                tcm_roi = (
                    sum(r[6] * r[1] for r in total_cross_market_results) / tcm_bets
                    if tcm_bets > 0
                    else 0
                )
                print(
                    f"🔀 Total Cross-Market: {tcm_wins}/{tcm_bets} ({tcm_wins / tcm_bets:.1%}) | ROI: {tcm_roi:.2f}%"
                )
            ...
            if profitable_patterns:
                for i, pattern in enumerate(profitable_patterns[:5], 1):  # Top 5
                    print(
                        f"   #{i}. {pattern[0]} | ROI: {pattern[6]:.2f}% | {pattern[2]}/{pattern[1]} wins"
                    )
            else:
            ...
            if basic_results and basic_results[0]:
                ...
                print(
                    f"📅 Period: {basic_results[0][2].date()} to {basic_results[0][3].date()}"
                )
