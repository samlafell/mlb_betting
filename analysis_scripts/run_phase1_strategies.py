...
def analyze_results(all_results: dict) -> dict:
    ...
    for strategy_name, data in all_results.items():
        if data["status"] == "SUCCESS":
            ...
            excellent = [r for r in results if "🟢" in str(r.get("strategy_rating", ""))]
            
            summary["strategy_performance"][strategy_name] = {
                "total_variants": len(results),
                "profitable_variants": len(profitable),
                "excellent_variants": len(excellent),
                "best_roi": max([r.get("roi_per_100_unit", -100) for r in results]) if results else -100,
    ...
    if summary["best_opportunities"]:
        ...
        summary["key_insights"].append(f"🏆 Best opportunity: {best_opp['strategy']} - {best_opp['variant']} with {best_opp['roi']:.1f}% ROI")
    
    high_win_rate = [opp for opp in summary["best_opportunities"] if opp["win_rate"] > 60]
    if high_win_rate:
        summary["key_insights"].append(f"🎯 {len(high_win_rate)} strategies with >60% win rate found")
...
async def main():
        ...
        if result["status"] == "SUCCESS":
            ...
            if result["results"]:
                ...
                for i, res in enumerate(sorted_results[:2]):
                    ...
                    print(f"   {i+1}. {variant}: {win_rate:.1f}% win rate, {roi:.1f}% ROI")
        else:
    ...
    print(f"✅ Successful Strategies: {summary['successful_strategies']}/{summary['total_strategies']}")
    ...
    if summary["best_opportunities"]:
        ...
        print("-" * 40)
        top_opportunities = sorted(summary["best_opportunities"], 
    ...
    # Save results
    output_file = Path("analysis_results") / f"phase1_strategies_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    ...
    if summary["best_opportunities"]:
        ...
        print(f"   Start with small unit sizes to validate")
        
        print("\n2. 📈 PORTFOLIO APPROACH:")
        profitable_count = len([opp for opp in summary["best_opportunities"] if opp["roi"] > 10])
