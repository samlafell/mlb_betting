...
class DatabaseConsolidationAnalyzer:
    ...
    def __init__(self, project_root: Path):
        ...
        self.service_files = {
            "database_coordinator.py": self.src_root
            / "services"
            / "database_coordinator.py",
            "postgres_database_coordinator.py": self.src_root
            / "services"
            / "postgres_database_coordinator.py",
            "database_service_adapter.py": self.src_root
            / "services"
            / "database_service_adapter.py",
        ...
        try:
            ...
            for node in ast.walk(tree):
                if isinstance(node, ast.ClassDef):
                    ...
                    node.lineno >= cls_node.lineno
                    and node.lineno <= getattr(cls_node, "end_lineno", float("inf"))
            ...
            patterns = {
                ...
                "duckdb": [r"duckdb", r"DuckDB", r"\.duckdb", r"PRAGMA"],
                "connection_pool": [
                    r"connection.*pool",
                    r"ThreadedConnectionPool",
                    r"QueuePool",
                ...
                "async": [r"async def", r"await ", r"asyncio", r"@asynccontextmanager"],
                "transaction": [
                    r"\.commit\(\)",
                    r"\.rollback\(\)",
                    r"transaction",
    ...
    def _generate_summary(self, results: dict) -> dict:
        ...
        service_files = [
            name
            for name, analysis in results["service_files"].items()
            if analysis.get("exists", False)
        ]
    ...
    def generate_report(self) -> str:
        ...
        report.append(
            f"- **Total lines of database code**: {summary['total_database_lines']}"
        )
        report.append(
            f"- **Service files using database**: {summary['service_files_found']}"
        )
        report.append(
            f"- **Files with database dependencies**: {summary['dependent_files']}"
        )
        report.append(f"- **Estimated effort**: {summary['estimated_effort']}")
        report.append(
            f"- **Estimated code reduction**: {summary['estimated_reduction']}"
        )
        report.append("")
        ...
        for name, analysis in results["db_files"].items():
            ...
            report.append(
                f"- **Classes**: {', '.join([cls['name'] for cls in analysis.get('classes', [])])}"
            )
        ...
        if plan["files_to_merge"]:
            ...
            for file_info in plan["files_to_merge"]:
                ...
                if file_info["key_features"]:
                    report.append(
                        f"  - Features: {', '.join(file_info['key_features'])}"
                    )
            report.append("")
...
def main():
    ...
    try:
        ...
        print("Database consolidation analysis complete!")
