...
class DuckDBCleanupManager:
    ...
    def _update_sharp_action_analyzer(self, file_path: Path) -> None:
        ...
        content = re.sub(
            r"self\.conn = duckdb\.connect.*\n",
            "from ..db.connection import get_db_manager\n        self.db_manager = get_db_manager()\n",
            content,
        )
        content = re.sub(
            r'"data/raw/mlb_betting\.duckdb"', '"PostgreSQL database"', content
        )
        ...
        content = content.replace(
            "self.conn.execute(query).df()", "self._execute_query_to_dataframe(query)"
        )
    ...
    def _update_analysis_command(self, file_path: Path) -> None:
        ...
        content = re.sub(
            r'"""Connect to the DuckDB database"""',
    ...
    def _update_migrate_command(self, file_path: Path) -> None:
        ...
        content = re.sub(
            r"# Removed DuckDB-specific optimized connection.*\n", "", content
        )

        # Update remaining references
        content = content.replace(
            "DuckDB Performance Benchmark", "PostgreSQL Performance Benchmark"
        )
    ...
    def _update_table_registry(self, file_path: Path) -> None:
        ...
        content = re.sub(
            r"\s+elif self\.database_type == DatabaseType\.DUCKDB:\s*\n\s+# DuckDB mappings.*\n",
            "",
            content,
            flags=re.MULTILINE,
        )
    ...
    def _update_repositories(self, file_path: Path) -> None:
        ...
        content = content.replace(
            "# Use PostgreSQL information_schema instead of DuckDB PRAGMA",
    ...
    def _update_postgres_db_manager(self, file_path: Path) -> None:
        ...
        content = content.replace(
            "as the original DuckDB DatabaseManager", "as a PostgreSQL DatabaseManager"
        )
        content = content.replace(
            "drop-in replacement for the DuckDB DatabaseManager",
            "PostgreSQL DatabaseManager",
        )
        content = content.replace(
            "same interface as the DuckDB manager", "PostgreSQL manager interface"
        )
        ...
        content = content.replace(
            "DuckDB cursor interface", "PostgreSQL cursor interface"
        )
        content = content.replace(
            "DuckDB connection interface", "PostgreSQL connection interface"
        )
    ...
    def _update_postgres_connection(self, file_path: Path) -> None:
        ...
        content = content.replace(
            "This replaces DuckDB to eliminate all concurrency issues.",
    ...
    def _update_database_coordinator(self, file_path: Path) -> None:
        ...
        content = re.sub(
            r"Database Coordinator Service for Multi-Process DuckDB Access",
            "Database Coordinator Service (Legacy - Use PostgreSQL Connection Manager)",
            content,
        )
        content = content.replace(
            "DuckDB's single-writer limitation", "legacy database limitations"
        )
        content = content.replace("duckdb_coordinator.lock", "legacy_coordinator.lock")
        content = content.replace(
            "Falls back to DuckDB only if PostgreSQL fails",
            "Legacy fallback - use PostgreSQL connection manager instead",
        )
        content = content.replace(
            "PostgreSQL coordinator failed, falling back to DuckDB",
            "PostgreSQL coordinator failed, using legacy fallback",
        )
        content = content.replace(
            "make PostgreSQL coordinator work with existing DuckDB API",
    ...
    def _update_postgres_coordinator(self, file_path: Path) -> None:
        ...
        content = content.replace(
            "using PostgreSQL instead of DuckDB", "using PostgreSQL"
        )
        content = content.replace(
            "replaces the legacy DuckDB coordinator", "replaces the legacy coordinator"
        )
    ...
    def _update_data_dedup(self, file_path: Path) -> None:
        ...
        content = content.replace(
            "For DuckDB, we'll use a different approach",
    ...
    def _update_migration_helper(self, file_path: Path) -> None:
        ...
        content = content.replace(
            "# Legacy DuckDB patterns", "# Legacy database patterns"
        )
    ...
    def _update_entrypoint(self, file_path: Path) -> None:
        ...
        content = content.replace(
            "3. Store validated data in DuckDB", "3. Store validated data in PostgreSQL"
        )
    ...
    def _update_phase2_demo(self, file_path: Path) -> None:
        ...
        content = content.replace(
            "Database connection manager with DuckDB",
    ...
    def _update_config_toml(self, file_path: Path) -> None:
        ...
        content = re.sub(
            r'path = "data/raw/mlb_betting\.duckdb"',
    ...
    def _update_readme(self, file_path: Path) -> None:
        ...
        content = content.replace(
            "duckdb data/raw/mlb_betting.duckdb", "psql -h localhost -d mlb_betting"
        )
        content = content.replace(
        ...
        content = content.replace(
            "follows DuckDB best practices", "follows PostgreSQL best practices"
        )
    ...
    def _update_refactoring_plan(self, file_path: Path) -> None:
        ...
        content = content.replace(
            "Full DuckDB-based implementation",
    ...
    def _update_master_detector(self, file_path: Path) -> None:
        ...
        content = content.replace("data/raw/mlb_betting.duckdb", "PostgreSQL database")
        content = content.replace(
            'logging.getLogger("duckdb")', 'logging.getLogger("postgresql")'
        )
    ...
    def _update_analysis_readme(self, file_path: Path) -> None:
        ...
        content = content.replace(
            "duckdb data/raw/mlb_betting.duckdb", "psql -h localhost -d mlb_betting -f"
        )
        content = content.replace(
            "Connect to your DuckDB database", "Connect to your PostgreSQL database"
        )
        content = content.replace(
            "tables in your DuckDB database", "tables in your PostgreSQL database"
        )
    ...
    def _update_backtesting_schema(self, file_path: Path) -> None:
        ...
        content = content.replace(
            "DuckDB may not support triggers", "PostgreSQL supports triggers"
        )
    ...
    def _update_postgresql_functions(self, file_path: Path) -> None:
        ...
        content = content.replace(
            "Replaces DuckDB-specific functionality",
            "PostgreSQL-specific functionality",
        )
        content = content.replace(
            "replacing DuckDB-specific aggregates", "PostgreSQL aggregates"
        )
        content = content.replace(
            "replacing DuckDB TRY_CAST", "PostgreSQL safe casting"
        )
    ...
    def _update_backtesting_guide(self, file_path: Path) -> None:
        ...
        content = content.replace(
            "duckdb.connect('data/raw/mlb_betting.duckdb')",
            'psycopg2.connect(database="mlb_betting")',
        )
        content = content.replace(
            "data/raw/mlb_betting.duckdb", "PostgreSQL database mlb_betting"
        )
    ...
    def _update_phase2_summary(self, file_path: Path) -> None:
        ...
        content = content.replace(
            "appropriate for DuckDB architecture",
            "appropriate for PostgreSQL architecture",
        )
        content = content.replace(
            "Single DuckDB connection", "PostgreSQL connection pooling"
        )
    ...
    def _update_optimized_database_test(self, file_path: Path) -> None:
        ...
        content = content.replace(
            "DuckDB Optimization Performance Benchmark",
    ...
    def _update_gitignore(self) -> None:
        ...
        if gitignore_path.exists():
            ...
            content = content.replace(
                "*.duckdb", "# *.duckdb  # No longer using DuckDB"
            )
            content = content.replace(
                "**/*.duckdb", "# **/*.duckdb  # No longer using DuckDB"
            )
            content = content.replace(
                "*.duckdb.wal", "# *.duckdb.wal  # No longer using DuckDB"
            )
