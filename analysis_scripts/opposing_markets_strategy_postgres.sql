...
strategy_performance AS (
    SELECT 
        ...
        ROUND((AVG(stronger_signal_win) * 100)::NUMERIC, 2) as stronger_signal_win_rate,
        
        -- Strategy 2: ML preference
        SUM(ml_preference_win) as ml_preference_wins,
        ROUND((AVG(ml_preference_win) * 100)::NUMERIC, 2) as ml_preference_win_rate,
        
        -- Strategy 3: Spread preference
        SUM(spread_preference_win) as spread_preference_wins,
        ROUND((AVG(spread_preference_win) * 100)::NUMERIC, 2) as spread_preference_win_rate,
        
        -- Strategy 4: Contrarian
        SUM(contrarian_win) as contrarian_wins,
        ROUND((AVG(contrarian_win) * 100)::NUMERIC, 2) as contrarian_win_rate
