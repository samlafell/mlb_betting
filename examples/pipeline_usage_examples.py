...
async def example_smart_pipeline():
    ...
    try:
        ...
        print(
            f"⏱️  Estimated Runtime: {recommendations['estimated_runtime_minutes']} min"
        )
        ...
        results = await orchestrator.execute_smart_pipeline(
            detection_minutes=60, force_fresh_data=False
        )
        ...
        if results["detection_results"]:
            ...
            print(
                f"🎯 Found {len(results['detection_results'].games)} games with {total_opportunities} opportunities"
            )
