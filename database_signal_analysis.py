...
async def analyze_database_signals():
    ...
    try:
        # Connect to database
        conn = await asyncpg.connect('postgresql://sam:password123@localhost:5432/mlb_betting')
        ...
        print(f'  75th percentile: {p75:.1f}% (good signals - RECOMMENDED for most processors)')
        ...
        for threshold in thresholds:
            ...
            print(f'  {threshold:2d}%: {count:4d} signals ({pct:4.1f}% coverage){status}')
        ...
        current_65_signals = sum(1 for row in rows if row['home_or_over_stake_percentage'] and row['home_or_over_stake_percentage'] >= 65)
        ...
        print(f'  PublicFadeProcessor: 58% consensus (down from 65%)')
        ...
        for split_type, diffs in split_types.items():
            if len(diffs) > 10:  # Only show types with decent sample size
                ...
                print(f'  {split_type}: {len(diffs)} signals, 75th percentile = {p75_type:.1f}%')
