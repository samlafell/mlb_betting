        ...
        async with session.get(url) as response:
            ...
            try:
                ...
                for i, game_data in enumerate(games_data):
                    ...
                    game_dict = game_data.model_dump() if hasattr(game_data, 'model_dump') else game_data.__dict__
                    
                    print(f"Game ID: {game_dict.get('sbr_game_id', 'N/A')}")
                    print(f"Teams: {game_dict.get('away_team', 'N/A')} @ {game_dict.get('home_team', 'N/A')}")
                    ...
                    if odds_data:
                        for j, odds in enumerate(odds_data[:3]):  # Show first 3
                            print(f"  Sportsbook {j+1}: {odds.get('sportsbook', 'N/A')}")
                            if odds.get('moneyline_home'):
                                print(f"    Moneyline: {odds.get('moneyline_home')}/{odds.get('moneyline_away')}")
                            if odds.get('spread_home'):
                                print(f"    Spread: {odds.get('spread_home')}/{odds.get('spread_away')}")
                            if odds.get('total_line'):
                                print(f"    Total: {odds.get('total_line')} O/U {odds.get('over_price')}/{odds.get('under_price')}")
