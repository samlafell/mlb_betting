...
async def test_sbr_historical_availability():
            ...
            for bet_type in bet_types:
                ...
                try:
                    async with session.get(url, timeout=10) as response:
                        ...
                        if status == 200:
                            if content_length > 50000:  # Reasonable threshold for content with data
                                print(f"   ✅ {bet_type}: {status} ({content_length:,} bytes) - Likely has data")
                            elif content_length > 10000:
                                print(f"   ⚠️  {bet_type}: {status} ({content_length:,} bytes) - Minimal content")
                            else:
                                print(f"   ❌ {bet_type}: {status} ({content_length:,} bytes) - Very little content")
                        else:
    ...
    print(f"\n🎯 INTERPRETATION:")
    print(f"   ✅ = Likely has betting data (large content)")
    print(f"   ⚠️  = May have some data (medium content)")
    print(f"   ❌ = Likely no data (small content or error)")
    print(f"\n📝 NOTE:")
    print(f"   If historical dates show ❌ but current/recent dates show ✅,")
    print(f"   it suggests SportsbookReview.com doesn't maintain historical betting data.")
