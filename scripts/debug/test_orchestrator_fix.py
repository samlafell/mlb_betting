...
async def test_orchestrator_processing():
    ...
    try:
        ...
        async with storage.pool.acquire() as conn:
            ...
            logger.info(
                f"BEFORE Processing - Total: {before_stats['total_rows']}, New: {before_stats['new_rows']}, Loaded: {before_stats['loaded_rows']}, Failed: {before_stats['failed_rows']}, Duplicates: {before_stats['duplicate_rows']}"
            )
        ...
        async with storage.pool.acquire() as conn:
            ...
            logger.info(
                f"AFTER Processing - Total: {after_stats['total_rows']}, New: {after_stats['new_rows']}, Loaded: {after_stats['loaded_rows']}, Failed: {after_stats['failed_rows']}, Duplicates: {after_stats['duplicate_rows']}"
            )
            ...
            logger.info(
                f"SCHEMA DATA - Games: {games_count}, Moneyline: {moneyline_count}, Spread: {spread_count}, Totals: {totals_count}"
            )
            ...
            logger.info(
                f"RECENT INSERTIONS (last 5 min) - Games: {recent_games}, Moneyline: {recent_moneyline}, Spread: {recent_spread}, Totals: {recent_totals}"
            )
        ...
        if (
            ...
            logger.warning(
                "⚠️  No recent insertions detected - may need further investigation"
            )
