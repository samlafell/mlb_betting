...
class Phase2AMigrationTester:
    ...
    async def test_table_registry(self) -> dict:
        ...
        try:
            ...
            primary_tables = ['moneyline', 'spreads', 'totals', 'games', 'betting_recommendations']
        ...
        try:
            ...
            try:
                ...
                for table in [moneyline_table, spreads_table, totals_table]:
                    ...
                    result = await storage.pool.fetchval(exists_query, schema, table_name)
                    if not result:
                        return {
                            'success': False,
    ...
    async def test_migration_monitor(self) -> dict:
        ...
        try:
            ...
            if not report:
                return {'success': False, 'error': 'Migration monitor returned no report'}
            
            # Verify report structure
            required_fields = ['report_timestamp', 'migration_phase', 'overall_status', 
                             'table_comparisons', 'recommendations']
...
async def main():
    ...
    try:
        ...
        # Save results
        output_file = Path(f"phase2a_test_results_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
