    ...
    try:
        ...
        for odds in game_dict.get("odds_data", []):
            ...
            if record["bet_type"] == "moneyline":
                ...
                print(
                    f"    Moneyline mapping: moneyline_home={odds.get('moneyline_home')} -> home_ml={record['home_ml']}"
                )
                print(
                    f"    Moneyline mapping: moneyline_away={odds.get('moneyline_away')} -> away_ml={record['away_ml']}"
                )
                ...
                record["home_spread_price"] = odds.get("home_spread_price") or odds.get(
                    "moneyline_home"
                )
                record["away_spread_price"] = odds.get("away_spread_price") or odds.get(
                    "moneyline_away"
                )
                print(
                    f"    Spread mapping: spread_home={odds.get('spread_home')} -> home_spread={record['home_spread']}"
                )
                print(
                    f"    Spread mapping: spread_away={odds.get('spread_away')} -> away_spread={record['away_spread']}"
                )
                print(
                    f"    Spread price mapping: moneyline_home={odds.get('moneyline_home')} -> home_spread_price={record['home_spread_price']}"
                )
                print(
                    f"    Spread price mapping: moneyline_away={odds.get('moneyline_away')} -> away_spread_price={record['away_spread_price']}"
                )
                ...
                print(
                    f"    Total mapping: total_line={odds.get('total_line')} -> total_line={record['total_line']}"
                )
                print(
                    f"    Total mapping: total_over={odds.get('total_over')} -> over_price={record['over_price']}"
                )
                print(
                    f"    Total mapping: total_under={odds.get('total_under')} -> under_price={record['under_price']}"
                )
