...
async def debug_real_parsing_chain():
    ...
    try:
        ...
        parsed_games = scraper.parser.parse_bet_type_page(html_content, 'totals', test_date, totals_url)
        ...
        if parsed_games:
            ...
            if first_game.odds_data:
                for i, odds_record in enumerate(first_game.odds_data[:2]):  # Show first 2
                    print(f"\n  📊 Odds Record {i+1}:")
                    ...
                    print(f"    🔍 Totals Check:")
                    print(f"      total_line: {total_line}")
                    print(f"      total_over: {total_over}")
                    print(f"      total_under: {total_under}")
                    print(f"      Has totals: {total_line is not None and (total_over is not None or total_under is not None)}")
                ...
                if match:
                    try:
                        ...
                        if valid_odds_views:
                            ...
                            formatted = scraper.parser._format_odds_line(first_odds_json, 'totals')
