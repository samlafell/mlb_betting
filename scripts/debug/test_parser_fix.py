...
async def test_parser_fix():
    ...
    for bet_type, url in urls.items():
        ...
        try:
            ...
            parsed_games = scraper.parser.parse_bet_type_page(html_content, bet_type, test_date, url)
            ...
            if parsed_games:
                ...
                if odds_data:
                    ...
                    has_ml = 'moneyline_home' in odds_dict and 'moneyline_away' in odds_dict
                    has_spread = 'spread_home' in odds_dict and 'spread_away' in odds_dict
                    has_total = 'total_line' in odds_dict and odds_dict['total_line'] is not None
                    ...
                    # Debug totals fields
                    print(f"🔍 Debug totals: total_line={odds_dict.get('total_line')}, total_over={odds_dict.get('total_over')}, total_under={odds_dict.get('total_under')}")
                    
                    if has_ml:
                        print(f"   ML: {odds_dict.get('moneyline_away')} / {odds_dict.get('moneyline_home')}")
                    if has_spread:
                        print(f"   Spread: {odds_dict.get('spread_away')} / {odds_dict.get('spread_home')}")
                    if has_total:
                        print(f"   Total: {odds_dict.get('total_line')} - {odds_dict.get('total_over')}/{odds_dict.get('total_under')}")
                else:
