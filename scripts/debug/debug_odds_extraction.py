    ...
    try:
        ...
        if failed_record:
            ...
            for key, value in game_data.items():
                if 'odds' in key.lower() or 'line' in key.lower() or 'price' in key.lower():
                    print(f"{key}: {value}")
        ...
        try:
            ...
            for i, script in enumerate(script_tags):
                ...
                if script_content and '"props":' in script_content and len(script_content) > 1000:
                    print(f"Found large script tag #{i} with props ({len(script_content)} chars)")
                        ...
                        try:
                            ...
                            if 'pageProps' in data['props']:
                                ...
                                if 'oddsTables' in data['props']['pageProps']:
                                    ...
                                    print(f"  📈 Number of odds tables: {len(odds_tables)}")
                                    ...
                                    if odds_tables and len(odds_tables) > 0:
                                        ...
                                        if 'oddsTableModel' in first_table:
                                            ...
                                            if 'gameRows' in model:
                                                ...
                                                print(f"  🎯 Number of game rows: {len(game_rows)}")
                                                ...
                                                if game_rows:
                                                    ...
                                                    if 'oddsViews' in first_game:
                                                        ...
                                                        print(f"  💰 Number of odds views: {len(odds_views)}")
                                                        
                                                        if odds_views:
                                                            # Filter out None values
                                                            valid_odds_views = [ov for ov in odds_views if ov is not None]
                                                            print(f"  💰 Valid odds views: {len(valid_odds_views)} out of {len(odds_views)}")
                                                            
                                                            if valid_odds_views:
                                                                first_odds = valid_odds_views[0]
                                                                print(f"  💰 First valid odds keys: {list(first_odds.keys())}")
                                                                
                                                                # Check sportsbook
                                                                provider = first_odds.get('sportsbook') or first_odds.get('provider')
                                                                print(f"  🏪 Sportsbook: {provider}")
                                                                
                                                                # Check for actual odds values
                                                                if 'currentLine' in first_odds:
                                                                    current_line = first_odds['currentLine']
                                                                    print(f"  📊 Current line: {current_line}")
                                                                
                                                                if 'openingLine' in first_odds:
                                                                    opening_line = first_odds['openingLine']
                                                                    print(f"  📊 Opening line: {opening_line}")
                                                                    
                                                                # Show all odds views to understand the pattern
                                                                print(f"\n  🔍 All valid odds views:")
                                                                for i, odds_view in enumerate(valid_odds_views):
                                                                    provider = odds_view.get('sportsbook') or odds_view.get('provider')
                                                                    current = odds_view.get('currentLine', {})
                                                                    opening = odds_view.get('openingLine', {})
                                                                    print(f"    {i+1}. {provider}: current={current}, opening={opening}")
                                                            else:
                                                                print(f"  ❌ All {len(odds_views)} odds views are None!")
                                                                # Show the actual content
                                                                print(f"  🔍 Raw odds views: {odds_views}")
                                                    else:
                                                        print("  ❌ No oddsViews found in game row")
                                                else:
            ...
            if not json_found:
                ...
                for i, script in enumerate(script_tags):
                    ...
                    print(f"  Script {i}: {len(content)} chars, has props: {'props' in content}")
