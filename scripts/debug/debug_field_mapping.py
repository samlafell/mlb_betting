...
async def debug_field_mapping():
    ...
    if record:
        ...
        game_data = (
            json.loads(record["game_data"])
            if isinstance(record["game_data"], str)
            else record["game_data"]
        )
        ...
        print("🔄 Collection Orchestrator Transformation:")
        ...
        for i, odds in enumerate(odds_data[:3]):
            ...
            if bet_type == "moneyline":
                record_transformed["home_ml"] = odds.get("moneyline_home") or odds.get(
                    "home_ml"
                )
                record_transformed["away_ml"] = odds.get("moneyline_away") or odds.get(
                    "away_ml"
                )
                print(
                    f"    home_ml: {record_transformed['home_ml']} (from moneyline_home: {odds.get('moneyline_home')}, home_ml: {odds.get('home_ml')})"
                )
                print(
                    f"    away_ml: {record_transformed['away_ml']} (from moneyline_away: {odds.get('moneyline_away')}, away_ml: {odds.get('away_ml')})"
                )
        ...
        print("🏦 Database Records for this time period:")
        ...
        for i, db_record in enumerate(db_records):
            print(
                f"  DB Record {i + 1}: ID={db_record['id']}, Game={db_record['game_id']}, Book={db_record['sportsbook']}"
            )
            print(
                f"    home_ml: {db_record['home_ml']}, away_ml: {db_record['away_ml']}"
            )
