...
async def debug_staging_data():
    ...
    try:
            ...
            if game_data:
                ...
                if isinstance(game_data, str):
                    ...
                    try:
                        ...
                        print(f"    Failed to parse game_data as JSON")
                        continue
                
                if isinstance(game_data, dict):
                    print(f"    Teams: {game_data.get('away_team', 'N/A')} @ {game_data.get('home_team', 'N/A')}")
        ...
        for record in odds_with_data:
            ...
            print(f"  {away_team} @ {home_team}: {record['has_moneyline']}, {record['has_spread']}, {record['has_total']}")
...
async def main():
    """Main function."""
    print(f"🔍 DEBUGGING STAGING DATA PROCESSING")
    print(f"=" * 50)
