...
async def debug_backtesting_data():
    ...
    print(
        f"📅 Backtesting looks for data from: {start_date.strftime('%Y-%m-%d')} to {end_date.strftime('%Y-%m-%d')}"
    )
        ...
        with db_manager.get_cursor() as cursor:
            ...
            if sample_data:
                ...
                for row in sample_data:
                    print(
                        f"   • Game {row[0]}: {row[1]} vs {row[2]} ({row[3]}) - {row[6]}"
                    )
    ...
    try:
        ...
        if core_engine.processor_factory:
            ...
            print(
                f"✅ Processor factory initialized: {len(all_processors)} processors available"
            )
