...
async def test_season_start_detection():
    ...
    try:
        async with SportsbookReviewScraper(storage_service=storage) as scraper:
            ...
            recent_url = scraper.url_patterns['moneyline'].format(date=recent_date.strftime("%Y-%m-%d"))
            ...
            old_url = scraper.url_patterns['moneyline'].format(date=old_date.strftime("%Y-%m-%d"))
            ...
            if season_start:
                ...
                print(f"   📊 Days from search start: {(season_start - search_start).days}")
                
                # Verify the found date actually has data
                verify_url = scraper.url_patterns['moneyline'].format(date=season_start.strftime("%Y-%m-%d"))
                has_data = await scraper.check_url_exists(verify_url)
                print(f"   ✅ Verified season start has data: {has_data}")
            else:
                print(f"   ❌ No season start found")
            ...
            if scraper.unified_rate_limiter:
                service_status = scraper.unified_rate_limiter.get_service_status("sportsbookreview_scraper")
                rate_limits = service_status.get('rate_limits', {})
                
                print(f"   ⚡ Max requests per minute: {rate_limits.get('max_requests_per_minute', 'N/A')}")
                print(f"   ⚡ Request delay: {rate_limits.get('request_delay_seconds', 'N/A')}s")
                print(f"   ⚡ Max requests per hour: {rate_limits.get('max_requests_per_hour', 'N/A')}")
            else:
                print(f"   ⚠️  UnifiedRateLimiter not available")
...
async def test_rate_limiting_improvements():
    ...
    try:
        async with SportsbookReviewScraper(storage_service=storage) as scraper:
            ...
            test_dates = [
                date.today() - timedelta(days=i) for i in range(1, 6)  # Last 5 days
            ]
            ...
            for i, test_date in enumerate(test_dates, 1):
                test_url = scraper.url_patterns['moneyline'].format(date=test_date.strftime("%Y-%m-%d"))
                
                request_start = time.time()
                exists = await scraper.check_url_exists(test_url)
                request_duration = time.time() - request_start
                
                print(f"   📡 Request {i}: {test_date} | {request_duration:.2f}s | Exists: {exists}")
...
async def main():
    ...
    try:
        ...
        print(f"   python collect_2025_season_optimized.py --concurrent-dates 4")
        print(f"   python collect_2025_season_optimized.py --aggressive --concurrent-dates 6")
