...
async def debug_bet_type_pipeline():
    ...
    try:
        ...
        for row in staging_data:
            print(f"  ID {row['id']}: bet_type='{row['bet_type']}', game={row['game_id']}, odds={row['odds_count']}")
        ...
        for game in july_7_games[:3]:  # Show first 3
            ...
            ml_count = await conn.fetchval('SELECT COUNT(*) FROM mlb_betting.moneyline WHERE game_id = $1', game_id)
            spread_count = await conn.fetchval('SELECT COUNT(*) FROM mlb_betting.spreads WHERE game_id = $1', game_id)
            total_count = await conn.fetchval('SELECT COUNT(*) FROM mlb_betting.totals WHERE game_id = $1', game_id)
            
            print(f"    Moneyline: {ml_count}, Spreads: {spread_count}, Totals: {total_count}")
