...
async def test_bet_type_flow():
    ...
    try:
        ...
        conn = await asyncpg.connect(
            "postgresql://samlafell@localhost:5432/mlb_betting"
        )
        ...
        print("🎯 After quality processing:")
        ...
        if game_dict.get("odds_data"):
            print(
                f"   First odds record bet_type: '{game_dict['odds_data'][0].get('bet_type')}'"
            )
        ...
        print("🎯 Final betting records to be stored:")
        ...
        print(
            f"🎯 Calling store_betting_data with {len(odds_records_final)} records..."
        )
        ...
        for i, bet_record in enumerate(odds_records_final):
            ...
            print(
                f"   Processing record {i + 1}: bet_type='{bet_type}', sportsbook='{bet_record.get('sportsbook')}'"
            )

            if not bet_type:
                print("   ⚠️  Empty bet_type detected!")
                break
