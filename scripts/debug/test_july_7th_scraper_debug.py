    ...
    async with SportsbookReviewScraper() as scraper:
        ...
        for bet_type, url in test_urls.items():
            ...
            try:
                ...
                try:
                    json_games = parser._extract_games_from_json(
                    ...
                    if json_games:
                        ...
                        for i, game in enumerate(json_games):
                            ...
                            if hasattr(game, "away_team") and hasattr(
                                game, "home_team"
                            ):
                                print(f"    Teams: {game.away_team} @ {game.home_team}")
                                print(
                                    f"    Game Time: {getattr(game, 'game_time', 'N/A')}"
                                )
                                print(
                                    f"    Odds Data: {len(getattr(game, 'odds_data', []))} sportsbooks"
                                )
                ...
                try:
                    html_games = parser._parse_html_fallback(
                        html_content, bet_type, test_date, url
                    )
                    ...
                    if html_games:
                        ...
                        for i, game in enumerate(html_games[:3]):  # Show first 3 games
                            ...
                            print(
                                f"    Teams: {game_data.get('away_team')} @ {game_data.get('home_team')}"
                            )
                            print(f"    Game Time: {game_data.get('game_time')}")
                            print(
                                f"    Odds Data: {len(game_data.get('odds_data', {}))}"
                            )
                    else:
                ...
                try:
                    full_games = parser.parse_bet_type_page(
                        html_content, bet_type, test_date, url
                    )
                    ...
                    if full_games:
                        ...
                        for i, game in enumerate(full_games[:3]):  # Show first 3 games
                            ...
                            if hasattr(game, "away_team") and hasattr(
                                ...
                                print(f"    Teams: {game.away_team} @ {game.home_team}")
                                print(
                                    f"    Bet Type: {getattr(game, 'bet_type', 'N/A')}"
                                )
                                print(
                                    f"    Odds Data: {len(getattr(game, 'odds_data', []))} sportsbooks"
                                )
                ...
                json_scripts = [
                    s for s in script_tags if s.string and '"props":' in s.string
                ]
                print(
                    f"Found {len(script_tags)} script tags, {len(json_scripts)} with JSON data"
                )
                ...
                if json_scripts:
                    sample_script = (
                        ...
                        if len(json_scripts[0].string) > 500
                        else json_scripts[0].string
                    )
