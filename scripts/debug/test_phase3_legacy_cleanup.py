...
from src.mlb_sharp_betting.services.strategy_manager import StrategyManager
from src.mlb_sharp_betting.services.confidence_scorer import ConfidenceScorer
from src.mlb_sharp_betting.services.timing_analysis_service import TimingAnalysisService
from src.mlb_sharp_betting.services.dynamic_threshold_manager import DynamicThresholdManager
...
class Phase3TestSuite:
    ...
    async def run_all_tests(self) -> Dict[str, Any]:
        ...
        self.logger.info("=" * 40)
        self.logger.info(f"Overall Status: {'✅' if success_rate >= 95 else '⚠️' if success_rate >= 80 else '❌'} {self.test_results['overall_status']}")
        ...
        if success_rate >= 95:
            ...
            self.logger.info(f"✅ System ready for legacy cleanup")
        elif success_rate >= 80:
            self.logger.warning(f"\n⚠️ Phase 3 validation needs review")
            self.logger.warning(f"🔍 Some tests failed - investigate before cleanup")
        else:
            self.logger.error(f"\n❌ Phase 3 validation failed")
            self.logger.error(f"🚫 System NOT ready for legacy cleanup")
    ...
    async def _test_performance_benchmarking(self) -> Dict[str, Any]:
        ...
        try:
            ...
            new_schema_queries = [
                ("moneyline_count", "SELECT COUNT(*) FROM core_betting.betting_lines_moneyline"),
                ("spreads_count", "SELECT COUNT(*) FROM core_betting.betting_lines_spreads"),
                ("totals_count", "SELECT COUNT(*) FROM core_betting.betting_lines_totals"),
            ...
            try:
                legacy_queries = [
                    ("legacy_moneyline_count", "SELECT COUNT(*) FROM mlb_betting.moneyline"),
                    ("legacy_spreads_count", "SELECT COUNT(*) FROM mlb_betting.spreads"),
                ...
                benchmarks["legacy_benchmark_note"] = f"Legacy tables not accessible: {e}"
    ...
    async def _test_migration_monitor(self) -> Dict[str, Any]:
        ...
        try:
            ...
            monitor_tests = {
                ...
                "migration_status": report.get("overall_status") if report else "UNKNOWN",
                "new_schema_percentage": report.get("migration_percentage", 0) if report else 0
            }
            ...
            try:
                ...
                self.logger.error(f"Migration monitor test failed (both async and sync): {e}, {e2}")
                return {
    ...
    async def _test_table_registry_functionality(self) -> Dict[str, Any]:
        ...
        try:
            ...
            key_tables = [
                "moneyline", "spreads", "totals", "games", 
                "betting_recommendations", "strategy_performance",
                "raw_betting_splits", "timing_analysis_results"
            ]
...
async def main():
    ...
    if status == "READY_FOR_CLEANUP":
        ...
        print(f"✅ All tests passed - system ready for legacy table removal")
    elif status == "NEEDS_REVIEW":
        print(f"\n⚠️ RECOMMENDATION: REVIEW ISSUES BEFORE CLEANUP")
        print(f"🔍 Some tests failed - investigate before proceeding")
    else:
        print(f"\n❌ RECOMMENDATION: DO NOT PROCEED WITH CLEANUP")
        print(f"🚫 Critical issues found - system not ready")
