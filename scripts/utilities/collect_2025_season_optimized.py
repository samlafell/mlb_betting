...
class OptimizedCollectionOrchestrator(CollectionOrchestrator):
    ...
    async def initialize_services(self):
        ...
        try:
            ...
            print(f"✅ Initialized optimized scraper with:")
            print(f"   - Rate limit delay: {scraper_config['rate_limit_delay']}s")
            print(f"   - Max concurrent requests: {scraper_config['max_concurrent_requests']}")
    ...
    async def scrape_with_progress_optimized(
        ...
        for i in range(0, total_dates, self.concurrent_dates):
            ...
            for j, result in enumerate(results):
                if isinstance(result, Exception):
                    ...
                    self.stats.errors_encountered.append(f"Date {batch_dates[j]}: {result}")
                else:
                    successful_dates += 1
            ...
            print(f"📊 Batch {i//self.concurrent_dates + 1}/{(total_dates + self.concurrent_dates - 1)//self.concurrent_dates} completed:")
        ...
        print(f"   🚀 Performance improvement: {(2.0 * 3 * total_dates) / total_duration:.1f}x faster")
    ...
    async def _scrape_date_with_monitoring(self, game_date: date) -> None:
        ...
        try:
            ...
            self.avg_response_time = sum(self.response_times[-10:]) / min(len(self.response_times), 10)
    ...
    async def _adjust_rate_limiting_if_needed(self):
        ...
        elif recent_avg < 3.0 and not self.stats.errors_encountered[-5:]:  # Fast and no recent errors
            current_delay = self.scraper.rate_limit_delay
            new_delay = max(current_delay * 0.9, 0.3)  # Min 0.3s
            self.scraper.rate_limit_delay = new_delay
            if new_delay != current_delay:
                print(f"🚀 Responses fast and stable, reducing delay to {new_delay:.1f}s")
...
async def optimized_collection(
        ...
        try:
            async with SportsbookReviewScraper(storage_service=storage) as scraper:
                ...
                if actual_start:
                    ...
                    print(f"⚠️  Could not find season start, using default: {start_date}")
        except Exception as e:
    ...
    if dry_run:
        ...
        print(f"   📊 Expected improvement: 4-5x faster than standard collection")
        return {"status": "dry_run_complete", "estimated_time_minutes": estimated_time/60}
...
def main():
    """Main entry point."""
    parser = argparse.ArgumentParser(description="Optimized 2025 MLB Season Data Collection")
    parser.add_argument("--start-date", type=str, help="Start date (YYYY-MM-DD)")
    parser.add_argument("--end-date", type=str, help="End date (YYYY-MM-DD)")
    parser.add_argument("--concurrent-dates", type=int, default=2, help="Number of dates to process concurrently")
    parser.add_argument("--aggressive", action="store_true", help="Use aggressive optimization settings")
    parser.add_argument("--test-run", action="store_true", help="Test with last 7 days only")
    parser.add_argument("--output-dir", type=str, help="Output directory for results")
    parser.add_argument("--dry-run", action="store_true", help="Test configuration without scraping")
    parser.add_argument("--no-auto-start", action="store_true", help="Disable automatic season start detection")
    ...
    if args.concurrent_dates < 1 or args.concurrent_dates > 8:
        print(f"❌ Concurrent dates must be between 1 and 8, got: {args.concurrent_dates}")
        sys.exit(1)
    
    # Run collection
    try:
        result = asyncio.run(optimized_collection(
            start_date=start_date,
            end_date=end_date,
