...
async def validate_fixes():
    ...
    try:
        ...
        print(
            f"   ✅ Caching {'WORKING' if stats['cache_hit_rate_pct'] > 0 else 'FAILED'}"
        )
        ...
        batch_data = await repository.get_batch_signal_data(
            start_time, end_time, {"sharp_action", "opposing_markets", "book_conflicts"}
        )
        ...
        print(
            f"   ✅ Batch optimization {'WORKING' if len(batch_data) > 1 else 'FAILED'}"
        )
