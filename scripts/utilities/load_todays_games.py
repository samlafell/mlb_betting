...
from src.mlb_sharp_betting.models.actionnetwork import GameFact, TeamDimension
...
def get_team_mapping(db_config: Dict) -> Dict[str, int]:
    ...
    with psycopg2.connect(cursor_factory=RealDictCursor, **db_config) as conn:
        with conn.cursor() as cursor:
            ...
            logger.info(f"Loaded team mapping for {len(results)} teams with {len(team_mapping)} name variations")
            return team_mapping


def parse_game_datetime(game_data: Dict) -> tuple[date, Optional[time], Optional[datetime]]:
    """
        ...
        if field in game_data and game_data[field]:
            try:
                if isinstance(game_data[field], str):
                    ...
                    utc_datetime = datetime.fromisoformat(game_data[field].replace('Z', '+00:00'))
    ...
    if game_datetime:
        ...
        logger.warning(f"Could not parse datetime from game data, using today's date in EST")
...
def extract_team_info(game_data: Dict, team_mapping: Dict[str, int]) -> tuple[Optional[int], Optional[int]]:
    """
...
def validate_game_data(game_data: Dict, team_mapping: Dict[str, int]) -> Optional[GameFact]:
    """
    ...
    try:
        ...
        model_data = {
            ...
            'venue_name': game_data.get('venue', {}).get('name') if 'venue' in game_data else None,
        ...
        logger.error(f"Validation failed for game {game_data.get('id', 'unknown')}: {e}")
        return None
    ...
    try:
        ...
        for game_data in games:
            ...
            if game_fact:
                ...
                logger.warning(f"Failed to validate game: {game_data.get('id', 'unknown')}")
        
        logger.info(f"Successfully validated {len(validated_games)} out of {len(games)} games")
        ...
        with psycopg2.connect(cursor_factory=RealDictCursor, **db_config) as conn:
            with conn.cursor() as cursor:
                ...
                # Commit transaction
                conn.commit()
                logger.info(f"Successfully loaded {success_count} games into action.fact_games")
                ...
                for game in loaded_games:
                    logger.info(f"  Game {game['id_action']}: {game['matchup']} - {game['game_status']}")
