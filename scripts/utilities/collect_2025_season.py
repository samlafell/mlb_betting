...
from sportsbookreview.services.collection_orchestrator import CollectionOrchestrator, run_historical_collection
...
class Season2025Collector:
    ...
    async def verify_collection_results(self) -> Dict[str, Any]:
        ...
        try:
            ...
            print(f"💰 Moneyline Records: {verification_results['moneyline_records']:,}")
            ...
            if verification_results["games_collected"] > 0:
                avg_records_per_game = total_betting_records / verification_results["games_collected"]
...
def parse_arguments():
    ...
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Test run without actual scraping"
    )
...
async def main():
    ...
    try:
        ...
        results_file = args.output_dir / f"collection_results_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
