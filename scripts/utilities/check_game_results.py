...
def check_game_results(away_team, home_team, game_date):
    ...
    try:
        outcomes = db_mgr.execute_query(
            outcome_query, (home_team, away_team, game_date)
        )
    ...
    for split in splits:
        ...
        if abs(diff) >= 10:  # Sharp signal threshold
            ...
            print(
                f"{signal_strength} {split['split_type'].upper()} SIGNAL ({split['source']}-{split['book']}):"
            )
            print(f"   🎯 Recommendation: {rec}")
            print(
                f"   📊 Differential: {diff:+.1f}% (Stake: {split['stake_pct']}%, Bets: {split['bets_pct']}%)"
            )

            # Check if recommendation won (if we have outcome data)
            if outcome:
                result = check_recommendation_result(
                    split, outcome, away_team, home_team
                )
                print(f"   {result}")
                ...
                print("   ⏳ Result: Game outcome not available")
    ...
    if outcome and recommendations:
        ...
        print(
            f"   🎯 Game Result: {away_team} {outcome['away_score']}, {home_team} {outcome['home_score']}"
        )

    elif outcome:
        print("📈 GAME RESULT:")
        print(
            f"   🎯 Final Score: {away_team} {outcome['away_score']}, {home_team} {outcome['home_score']}"
        )
