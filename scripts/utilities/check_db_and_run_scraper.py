...
async def check_database_state():
    ...
    try:
        ...
        moneyline_count = db.execute_query("SELECT COUNT(*) as count FROM mlb_betting.moneyline")
        ...
        for row in recent_moneyline:
            print(f"  {row['game_date']}: {row['away_team']} @ {row['home_team']} - {row['home_ml']}/{row['away_ml']}")
        ...
        # Check spreads table
        spreads_count = db.execute_query("SELECT COUNT(*) as count FROM mlb_betting.spreads")
        print(f"Spreads records: {spreads_count[0]['count']}")
        
        # Check totals table
        totals_count = db.execute_query("SELECT COUNT(*) as count FROM mlb_betting.totals")
        print(f"Totals records: {totals_count[0]['count']}")
        
        # Check staging data
        staging_count = db.execute_query("SELECT COUNT(*) as count FROM sportsbookreview.sbr_parsed_games")
                ...
                try:
                    ...
                    if result:
                        ...
                        if result:
                            ...
                            print(f"  Sample game: {sample_game.get('away_team', 'Unknown')} @ {sample_game.get('home_team', 'Unknown')}")
