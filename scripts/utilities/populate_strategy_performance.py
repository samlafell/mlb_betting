...
class StrategyPerformancePopulator:
    ...
    async def get_available_data_ranges(self) -> dict[str, Any]:
        ...
        with self.db_manager.get_cursor() as cursor:
            ...
            return {
                ...
                "overlap_range": {"start": overlap_start, "end": overlap_end},
            }
    ...
    async def run_comprehensive_population(self):
        ...
        self.logger.info(
            f"📅 Processing overlap period: {overlap_start} to {overlap_end}"
        )
    ...
    async def _populate_weekly_chunks(self, start_date, end_date):
        ...
        self.logger.info(
            "📅 Processing data in weekly chunks for comprehensive coverage"
        )
        ...
        while current_date <= end_date:
            ...
            try:
                self.logger.info(
                    f"🔄 Processing chunk {chunk_count + 1}: {current_date} to {chunk_end}"
                )
                ...
                if (
                    "backtest_results" in result
                    and "strategy_results" in result["backtest_results"]
                ):
                    chunk_results = result["backtest_results"]["strategy_results"]

                    self.total_results_added += len(chunk_results)
                    self.logger.info(
                        f"✅ Chunk {chunk_count + 1}: Added {len(chunk_results)} results"
                    )
                ...
                self.logger.error(
                    f"❌ Failed to process chunk {current_date} to {chunk_end}: {e}"
                )
                current_date = chunk_end + timedelta(days=1)
                continue
    ...
    async def _populate_full_range(self, start_date, end_date):
        ...
        try:
            ...
            if (
                ...
                self.logger.info(
                    f"✅ Full range: Added {len(full_range_results)} strategy results"
                )
    ...
    async def _generate_population_report(self):
        ...
        with self.db_manager.get_cursor() as cursor:
            cursor.execute(
                "SELECT COUNT(*) as total_count FROM backtesting.strategy_performance"
            )
...
"""
        ...
        for i, strategy in enumerate(top_strategies, 1):
            report += (
                f"{i:2d}. {strategy['strategy_name']}: {strategy['count']} records "
            )
