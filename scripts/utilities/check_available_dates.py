...
async def check_available_dates():
    ...
    try:
        print(f"🔍 Checking available dates in our database...")
...
async def verify_results(target_date):
    ...
    try:
        ...
        if total_records > 0:
            ...
            if ml_samples:
                print(f"  Moneyline (with odds):")
                for record in ml_samples:
                    print(f"    {record['sportsbook']}: {record['home_ml']} / {record['away_ml']}")
...
async def main():
    """Main function."""
    print(f"🔍 CHECKING AVAILABLE DATES AND TESTING FIXES")
    print(f"=" * 50)
