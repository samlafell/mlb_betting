...
class ActionNetworkURLBuilder:
    ...
    def __init__(self, cache_build_id: bool = True, db_config: Optional[Dict] = None):
        """
        ...
        self._cached_build_id: Optional[str] = None
        self._build_id_timestamp: Optional[float] = None
        ...
        self._team_slug_map: Optional[Dict[str, str]] = None
        self._team_slug_cache_time: Optional[float] = None
    ...
    def _get_db_config(self) -> Dict:
        ...
        try:
            ...
            self.logger.warning("Could not import settings, using environment variables")
            return {
    ...
    def _load_team_slug_map(self) -> Dict[str, str]:
        ...
        try:
            with psycopg2.connect(cursor_factory=RealDictCursor, **self.db_config) as conn:
                with conn.cursor() as cursor:
                    ...
                    self.logger.info(f"Loaded {len(slug_map)} team slug mappings from database")
                    return slug_map
    ...
    def _get_team_slug_map(self) -> Dict[str, str]:
        ...
        if (self._team_slug_map and 
            self._team_slug_cache_time and
            (now - self._team_slug_cache_time) < self.team_slug_ttl):
    ...
    def _get_build_id(self) -> str:
        ...
        if (self.cache_build_id and 
            self._cached_build_id and 
            self._build_id_timestamp and
            (now - self._build_id_timestamp) < self.build_id_ttl):
        ...
        if not build_id:
            if self._cached_build_id:
                ...
                raise ValueError("Failed to extract build ID and no cached version available")
    ...
    def _normalize_team_name(self, team_name: str) -> str:
        ...
        name_variations = [
            ...
            re.sub(r'^(Oakland|Los Angeles|New York|San Francisco|San Diego|St\.|Saint)\s+', '', team_name),
        ...
        name_variations = list(set([v for v in name_variations if v and v != team_name]))
        
        # Try each variation
        for variation in name_variations:
            if variation in team_slug_map:
                self.logger.debug(f"Found team '{team_name}' using variation '{variation}'")
        ...
        self.logger.warning(f"No mapping found for team '{team_name}', using generated slug: {slug}")
        return slug
    ...
    def build_game_data_url(self, game_data: Dict, date: datetime, build_id: Optional[str] = None) -> str:
        """
        ...
        if not away_team or not home_team:
            raise ValueError(f"Could not extract team names from game data: {game_data}")
        ...
        self.logger.debug("Built game data URL", 
                         game_id=game_id, 
    ...
    def build_all_game_urls(self, date: datetime) -> List[Tuple[Dict, str]]:
        ...
        for game in games:
            try:
                ...
                self.logger.warning("Failed to build URL for game", 
                                   game_id=game.get('id'), 
                                   error=str(e))
                continue
    ...
    def get_game_url_by_id(self, game_id: str, date: datetime) -> Optional[str]:
        """
    ...
    def fetch_game_data(self, url: str) -> Optional[Dict]:
        """
