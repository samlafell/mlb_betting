...
class ActionNetworkEnhancedFetcher:
    ...
    def fetch_game_data(self, url: str) -> Optional[Dict]:
        ...
        try:
            ...
            logger.error(
                "Failed to fetch game data with Firefox headers", 
                url=url,
                error=str(e)
            )
            return None
    
    def fetch_scoreboard_api(self, date_str: str) -> Optional[Dict]:
        """
    ...
    def fetch_history_data(self, game_id: str) -> Optional[Dict]:
        """
        ...
        """
        history_url = f"https://api.actionnetwork.com/web/v2/markets/event/{game_id}/history"
    ...
    if data:
        ...
        if game_info:
            ...
            if len(teams) >= 2:
                home_team = teams[0]
                away_team = teams[1]
                print(f"   🏠 {home_team.get('full_name')} vs ✈️  {away_team.get('full_name')}")
