...
def extract_game_urls(date_option: str = "today") -> List[Tuple[Dict, str]]:
    ...
    logger.info("🔗 Extracting Action Network game URLs", 
    ...
    try:
        ...
        logger.info("✅ Successfully extracted game URLs", 
...
def test_urls_with_enhanced_fetcher(game_urls: List[Tuple[Dict, str]]) -> Dict[str, bool]:
    """
    ...
    if old_files:
        ...
        for file_path in old_files:
            try:
                ...
                logger.warning("Failed to delete old file", file=file_path, error=str(e))
    else:
        logger.debug("No old JSON files to clean up")


def save_urls_to_file(game_urls: List[Tuple[Dict, str]], date_option: str = "today", filename: str = None) -> str:
    """
...
def print_urls_summary(game_urls: List[Tuple[Dict, str]], date_option: str = "today") -> None:
    """Print a summary of extracted URLs."""
    target_date = get_target_date(date_option)
    print("\n" + "="*80)
    print(f"🔗 ACTION NETWORK GAME URLs - {target_date.strftime('%Y-%m-%d')} ({date_option.upper()})")
    print("="*80)
...
Examples:
  ...
  %(prog)s --no-test          # Skip URL testing
    ...
    parser.add_argument(
        "--no-test",
        action="store_true",
        help="Skip URL testing with enhanced fetcher"
    )
...
def main():
    ...
    try:
        ...
        if not game_urls:
            logger.warning("⚠️  No games found for the specified date", date_option=args.date)
            return
