        ...
        if game_data:
            ...
            if game_info:
                ...
                if len(teams) >= 2:
                    home_team = teams[0]
                    away_team = teams[1] 
                    print(f"   🏠 Home: {home_team.get('full_name')} ({home_team.get('abbr')})")
                    print(f"   ✈️  Away: {away_team.get('full_name')} ({away_team.get('abbr')})")
                ...
                if boxscore:
                    ...
                    if stats:
                        ...
                        print(f"   📊 Score: {away_stats.get('runs', 0)} - {home_stats.get('runs', 0)}")
...
def main():
    ...
    print("3. Use the API to get games and build URLs automatically (with browser headers)")
