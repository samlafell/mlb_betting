...
def analyze_historical_data(
    data_file: str = "output/historical_line_movement_full_20250711_165111.json",
):
            ...
            try:
                ...
                for market_type, market_data in event_data.items():
                    ...
                    for outcome in market_data:
                        ...
                        rlm_detected = detect_rlm_in_outcome(
                            outcome, game_id, teams, market_type, book_name
                        )
                        if rlm_detected:
    ...
    analysis_results["games_by_movement"].sort(
...
def detect_rlm_in_outcome(
    outcome: dict, game_id: int, teams: str, market_type: str, book_name: str
) -> list[dict]:
...
def display_analysis_results(results: dict):
    ...
    for i, game in enumerate(results["games_by_movement"][:10], 1):
        print(
            f"  {i:2d}. {game['teams']:<40} {game['movements']:,} movements ({game['markets']} markets, {game['books']} books)"
        )

    # Market breakdown
    print("\n📊 MARKET BREAKDOWN:")
    for market, count in sorted(
        results["market_breakdown"].items(), key=lambda x: x[1], reverse=True
    ):
    ...
    for book, count in sorted(
        results["book_breakdown"].items(), key=lambda x: x[1], reverse=True
    ):
    ...
    if results["rlm_opportunities"]:
        ...
        for i, rlm in enumerate(results["rlm_opportunities"][:10], 1):
            print(
                f"  {i:2d}. {rlm['teams']:<35} {rlm['market_type']:<10} {rlm['book']:<12} ({rlm['strength']})"
            )
            print(
                f"      Tickets: {rlm['ticket_percent']}% | Money: {rlm['money_percent']}% | Disparity: {rlm['disparity']}%"
            )
    ...
    if results["steam_moves"]:
        ...
        for i, steam in enumerate(results["steam_moves"][:10], 1):
            print(
                f"  {i:2d}. {steam['teams']:<35} {steam['movements']:,} movements ({steam['books']} books, {steam['strength']})"
            )
...
def main():
    ...
    if not Path(data_file).exists():
        ...
        print(
            "Please run the Action Network pipeline first to collect historical data."
        )
        return
    ...
    if results:
        ...
        print(
            f"📊 Found {results['total_movements']:,} total movements across {results['games_with_movements']} games"
        )
        ...
        if results["games_by_movement"]:
            ...
            print(
                f"  • Most active game: {top_game['teams']} ({top_game['movements']:,} movements)"
            )

        if results["rlm_opportunities"]:
            strong_rlm = [
                r for r in results["rlm_opportunities"] if r["strength"] == "Strong"
            ]
            print(f"  • Strong RLM opportunities: {len(strong_rlm)}")

        if results["steam_moves"]:
            strong_steam = [
                s for s in results["steam_moves"] if s["strength"] == "Strong"
            ]
