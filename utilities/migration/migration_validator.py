...
class MigrationValidator:
            ...
            async with connection_manager.get_async_connection() as conn:
                if phase in ["all", "pre-migration"]:
                    self.validation_results["validations"][
                        "pre_migration"
                    ] = await self._validate_pre_migration(conn)

                if phase in ["all", "raw-zone", "phase2"]:
                    self.validation_results["validations"][
                        "raw_zone"
                    ] = await self._validate_raw_zone_migration(conn)

                if phase in ["all", "staging-zone", "phase3"]:
                    self.validation_results["validations"][
                        "staging_zone"
                    ] = await self._validate_staging_zone_migration(conn)

                if phase in ["all", "curated-zone", "phase4"]:
                    self.validation_results["validations"][
                        "curated_zone"
                    ] = await self._validate_curated_zone_migration(conn)

                if phase in ["all", "pipeline"]:
                    self.validation_results["validations"][
                        "pipeline"
                    ] = await self._validate_pipeline_functionality(conn)
    ...
    async def _check_schema_existence(self, conn) -> dict[str, Any]:
        ...
        missing_schemas = [
            schema for schema in required_schemas if schema not in existing_schema_names
        ]
    ...
    async def _check_raw_zone_record_counts(self, conn) -> dict[str, Any]:
        ...
        if source_counts["source_moneylines"] != raw_counts["raw_moneylines"]:
            issues.append(
                f"Moneyline count mismatch: source={source_counts['source_moneylines']}, raw={raw_counts['raw_moneylines']}"
            )

        if source_counts["source_spreads"] != raw_counts["raw_spreads"]:
            issues.append(
                f"Spread count mismatch: source={source_counts['source_spreads']}, raw={raw_counts['raw_spreads']}"
            )

        if source_counts["source_totals"] != raw_counts["raw_totals"]:
            issues.append(
                f"Totals count mismatch: source={source_counts['source_totals']}, raw={raw_counts['raw_totals']}"
            )
    ...
    def _generate_validation_summary(self) -> dict[str, Any]:
        ...
        for validation_name, validation_data in self.validation_results[
            "validations"
        ].items():
            if isinstance(validation_data, dict):
                total_checks += validation_data.get("passed", 0) + validation_data.get(
                    "failed", 0
                )
    ...
    def _generate_recommendations(self) -> list[dict[str, Any]]:
        ...
        for validation_name, validation_data in self.validation_results[
            ...
            if (
                ...
                and validation_data.get("failed", 0) > 0
            ):
...
async def main():
    ...
    try:
        ...
        # Save results
        output_file = Path(
            f"utilities/migration/validation_results_{phase}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
