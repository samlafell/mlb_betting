...
class RawZoneMigrator:
    ...
    async def migrate_all_to_raw_zone(self) -> Dict[str, Any]:
        ...
        try:
            ...
            async with connection_manager.get_async_connection() as conn:
                ...
                migration_results['tables_migrated']['unified'] = await self._create_unified_betting_lines(conn)
            ...
            while offset < total_records:
                ...
                for record in batch_records:
                    try:
                        ...
                        logger.error(f"Failed to migrate moneyline record {record['external_id']}: {e}")
            ...
            while offset < total_records:
                ...
                for record in batch_records:
                    try:
                        ...
                        logger.error(f"Failed to migrate spread record {record['external_id']}: {e}")
                        table_stats['failed'] += 1
            ...
            while offset < total_records:
                ...
                for record in batch_records:
                    try:
                        ...
                        logger.error(f"Failed to migrate totals record {record['external_id']}: {e}")
                        table_stats['failed'] += 1
    ...
    async def _create_unified_betting_lines(self, conn) -> Dict[str, Any]:
        ...
        try:
            ...
            ml_count = int(ml_result.split()[-1]) if ml_result.split()[-1].isdigit() else 0
            spread_count = int(spread_result.split()[-1]) if spread_result.split()[-1].isdigit() else 0
            totals_count = int(totals_result.split()[-1]) if totals_result.split()[-1].isdigit() else 0
