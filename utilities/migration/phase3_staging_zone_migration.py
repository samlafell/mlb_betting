...
class TeamNormalizer:
    ...
    TEAM_MAPPINGS = {
        ...
        'CLE': ['Guardians', 'Cleveland Guardians', 'Cleveland', 'Indians', 'Cleveland Indians'],
        ...
        'STL': ['Cardinals', 'St. Louis Cardinals', 'St Louis Cardinals', 'StL Cardinals'],
    ...
    def normalize_team_name(cls, team_name: str) -> Optional[str]:
        ...
        for code, variants in cls.TEAM_MAPPINGS.items():
            if team_clean in variants or team_clean.upper() in [v.upper() for v in variants]:
                return code
...
class StagingZoneMigrator:
    ...
    async def migrate_all_to_staging_zone(self) -> Dict[str, Any]:
        ...
        try:
            ...
            async with connection_manager.get_async_connection() as conn:
                ...
                migration_results['tables_migrated']['games'] = await self._migrate_games(conn)
    ...
    async def _clear_staging_tables(self, conn):
        ...
        tables = ['staging.moneylines', 'staging.spreads', 'staging.totals', 'staging.games']
        ...
        try:
            ...
            for game in games:
                try:
                    ...
                    logger.error(f"Failed to migrate game {game['game_external_id']}: {e}")
            ...
            while offset < total_records:
                ...
                for record in batch_records:
                    try:
                        ...
                        quality_score, errors = self.quality_scorer.calculate_quality_score(record_dict, 'moneyline')
                        
                        # Normalize team names
                        home_team_raw = record.get('home_team_name', '')
                        away_team_raw = record.get('away_team_name', '')
                        home_team_norm = self.team_normalizer.normalize_team_name(home_team_raw)
                        away_team_norm = self.team_normalizer.normalize_team_name(away_team_raw)
                        
                        # Determine validation status (updated for 0-9.99 scale)
                        validation_status = 'validated' if quality_score >= 8.0 else 'needs_review' if quality_score >= 6.0 else 'invalid'
            ...
            while offset < total_records:
                ...
                for record in batch_records:
                    try:
                        ...
                        record_dict = dict(record)
                        quality_score, errors = self.quality_scorer.calculate_quality_score(record_dict, 'spread')
                        
                        # Normalize team names
                        favorite_team_norm = self.team_normalizer.normalize_team_name(record.get('favorite_team', ''))
                        underdog_team_norm = self.team_normalizer.normalize_team_name(record.get('underdog_team', ''))
            ...
            while offset < total_records:
                ...
                for record in batch_records:
                    try:
                        ...
                        record_dict = dict(record)
                        quality_score, errors = self.quality_scorer.calculate_quality_score(record_dict, 'total')
