...
class DataAnalyzer:
    ...
    async def analyze_all_data(self) -> Dict[str, Any]:
        ...
        try:
            ...
            async with connection_manager.get_async_connection() as conn:
                ...
                analysis_results['tables']['games'] = await self._analyze_games_table(conn)
                analysis_results['tables']['moneylines'] = await self._analyze_betting_table(
                    conn, 'betting_lines_moneyline', 'Moneyline'
                )
                analysis_results['tables']['spreads'] = await self._analyze_betting_table(
                    conn, 'betting_lines_spread', 'Spread'
                )
                analysis_results['tables']['totals'] = await self._analyze_betting_table(
                    conn, 'betting_lines_totals', 'Totals'
                )
                analysis_results['tables']['sharp_actions'] = await self._analyze_sharp_actions(conn)
                
                # Generate summary and recommendations
                analysis_results['summary'] = self._generate_summary(analysis_results['tables'])
    ...
    async def _analyze_betting_table(self, conn, table_name: str, bet_type: str) -> Dict[str, Any]:
        ...
        return {
            ...
            'completeness_stats': dict(completeness_stats) if completeness_stats else {},
            'migration_targets': {
    ...
    def _generate_summary(self, tables: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            'estimated_migration_time_hours': max(1, total_records // 10000),  # ~10k records per hour
    ...
    def _generate_migration_recommendations(self, tables: Dict[str, Any], summary: Dict[str, Any]) -> List[Dict[str, Any]]:
...
async def main():
    ...
    try:
        ...
        if 'summary' in results:
            ...
            print(f"📈 Total Records to Migrate: {summary.get('total_records_to_migrate', 0):,}")
            ...
            print(f"⏱️ Estimated Migration Time: {summary.get('estimated_migration_time_hours', 0)} hours")
