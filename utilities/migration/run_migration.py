...
class MigrationOrchestrator:
        ...
        try:
            ...
            if results.get('summary', {}).get('overall_status') != 'passed':
                ...
                if critical_issues > 0:
                    raise Exception(f"Pre-migration validation failed with {critical_issues} critical issues")
    ...
    async def _execute_phase1(self):
        ...
        try:
            ...
            if not self.dry_run:
                ...
                logger.info(f"✅ Phase 1 completed: {results.get('summary', {}).get('total_records_to_migrate', 0):,} records analyzed")
            else:
        ...
        try:
            ...
            if not self.dry_run:
                ...
                if results.get('status') == 'completed':
                    ...
                    logger.info(f"✅ Phase 2 completed: {summary.get('total_records_successful', 0):,} records migrated")
                else:
                    phase_log['status'] = 'failed'
                    logger.error(f"❌ Phase 2 failed: {results.get('error', 'Unknown error')}")
    ...
    def _generate_final_summary(self) -> Dict[str, Any]:
        ...
        completed_phases = [p for p in self.migration_log['phases_executed'] if p.get('status') == 'completed']
        failed_phases = [p for p in self.migration_log['phases_executed'] if p.get('status') == 'failed']
        ...
        if len(completed_phases) > 0:
            for phase in completed_phases:
                ...
                if 'summary' in results:
                    total_records_migrated += results['summary'].get('total_records_successful', 0)
        
        return {
            'migration_status': 'completed' if len(failed_phases) == 0 else 'partial_failure',
    ...
    def _calculate_duration_minutes(self) -> float:
        ...
        if self.migration_log.get('completed_at') and self.migration_log.get('started_at'):
...
async def main():
    ...
    parser = argparse.ArgumentParser(description='MLB Betting System Pipeline Migration')
    parser.add_argument('--phase', choices=['all', '1', '2', '3', '4', 'phase1', 'phase2', 'phase3', 'phase4'], 
    ...
    if args.validate_only:
        ...
        try:
            ...
            phase_map = {'1': 'pre-migration', '2': 'raw-zone', '3': 'staging-zone', '4': 'curated-zone'}
    ...
    try:
        ...
        print(f"{status_emoji} Migration Status: {summary.get('migration_status', 'unknown').upper()}")
        print(f"📈 Phases Completed: {summary.get('phases_completed', 0)}/{summary.get('phases_attempted', 0)}")
