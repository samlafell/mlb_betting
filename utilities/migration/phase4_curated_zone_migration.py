...
class MLFeatureEngineer:
    ...
    def calculate_expected_value(
        true_prob: float, odds: int, stake: float = 100.0
    ) -> float:
    ...
    def create_betting_features(cls, record: dict, bet_type: str) -> dict:
        ...
        features = {
            "bet_type": bet_type,
            "sportsbook_tier": cls._get_sportsbook_tier(
                record.get("sportsbook_name", "")
            ),
        ...
        if bet_type == "moneyline":
            ...
            no_vig_home, no_vig_away = cls.calculate_no_vig_probability(
                home_prob, away_prob
            )
            ...
            features.update(
                {
                    ...
                    "market_efficiency": cls.calculate_market_efficiency_score(
                        home_prob, away_prob
                    ),
                    "home_expected_value": cls.calculate_expected_value(
                        no_vig_home, home_odds
                    ),
                    "away_expected_value": cls.calculate_expected_value(
                        no_vig_away, away_odds
                    ),
                    "odds_differential": abs(home_odds - away_odds)
            ...
            features.update(
                {
                    ...
                    "spread_expected_value": cls.calculate_expected_value(
                        0.5, spread_odds
                    ),  # Assume 50% true prob
                    "key_number_proximity": cls._calculate_key_number_proximity(
                        spread_value
                    ),
                }
            )
            ...
            no_vig_over, no_vig_under = cls.calculate_no_vig_probability(
            ...
            features.update(
                {
                    ...
                    "over_expected_value": cls.calculate_expected_value(
                        no_vig_over, over_odds
                    ),
                    "under_expected_value": cls.calculate_expected_value(
                        no_vig_under, under_odds
                    ),
...
class SharpActionDetector:
    ...
    def analyze_sharp_indicators(cls, record: dict, bet_type: str) -> dict:
        ...
        return {
            ...
            "market_inefficiency_score": cls._calculate_market_inefficiency(
                record, bet_type
            ),
        }
...
class CuratedZoneMigrator:
        ...
        try:
            ...
            async with connection_manager.get_async_connection() as conn:
                ...
                migration_results["tables_migrated"][
                    "enhanced_games"
                ] = await self._migrate_enhanced_games(conn)
                ...
                migration_results["tables_migrated"][
                    "feature_vectors"
                ] = await self._create_feature_vectors(conn)
    ...
    async def _migrate_enhanced_games(self, conn) -> dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"✅ Enhanced games migration completed: {batch_successful:,} games"
            )
    ...
    def _create_market_features(self, game: dict) -> dict:
        ...
        return {
            ...
            "total_betting_records": (
                game.get("total_moneyline_records", 0)
            ...
            if any(
                [
                    game.get("avg_moneyline_quality"),
                    game.get("avg_spread_quality"),
                    game.get("avg_total_quality"),
                ]
            )
            else 5.0,
        }
    ...
    def _create_sharp_action_summary(self, game: dict) -> dict:
        ...
        return {
            ...
            if game.get("data_quality_score", 0) >= 8.0
            else "medium"
            if game.get("data_quality_score", 0) >= 6.0
            else "low",
            "market_efficiency_estimate": min(
                9.99, game.get("data_quality_score", 5.0)
            ),
            "liquidity_score": min(
                9.99,
                (
            ...
            while offset < total_records:
                ...
                for record in batch_records:
                    try:
                        ...
                        ml_features = self.ml_engineer.create_betting_features(
                            record_dict, "moneyline"
                        )
                        sharp_analysis = self.sharp_detector.analyze_sharp_indicators(
                            record_dict, "moneyline"
                        )
                        ...
                        home_prob = self.ml_engineer.calculate_implied_probability(
                            home_odds
                        )
                        away_prob = self.ml_engineer.calculate_implied_probability(
                            away_odds
                        )
                        no_vig_home, no_vig_away = (
                            self.ml_engineer.calculate_no_vig_probability(
                                home_prob, away_prob
                            )
                        )

                        home_ev = self.ml_engineer.calculate_expected_value(
                            no_vig_home, home_odds
                        )
                        away_ev = self.ml_engineer.calculate_expected_value(
                            no_vig_away, away_odds
                        )
                        market_efficiency = (
                            self.ml_engineer.calculate_market_efficiency_score(
                                home_prob, away_prob
                            )
                        )
            ...
            while offset < total_records:
                ...
                for record in batch_records:
                    try:
                        ...
                        implied_prob = self.ml_engineer.calculate_implied_probability(
                            spread_odds
                        )
                        expected_value = self.ml_engineer.calculate_expected_value(
                            0.5, spread_odds
                        )  # Assume 50% true prob

                        # Detect patterns
                        rlm_detected = self.sharp_detector.detect_reverse_line_movement(
                            record_dict, "spread"
                        )
                        steam_detected = self.sharp_detector.detect_steam_moves(
                            record_dict, "spread"
                        )
            ...
            while offset < total_records:
                ...
                for record in batch_records:
                    try:
                        ...
                        over_prob = self.ml_engineer.calculate_implied_probability(
                            over_odds
                        )
                        under_prob = self.ml_engineer.calculate_implied_probability(
                            under_odds
                        )
                        ...
                        over_ev = self.ml_engineer.calculate_expected_value(
                            no_vig_over, over_odds
                        )
                        under_ev = self.ml_engineer.calculate_expected_value(
                            no_vig_under, under_odds
                        )
        ...
        try:
            ...
            for bet_type in bet_types:
                ...
                for record in feature_records:
                    try:
                        feature_vector = (
                            json.loads(record["feature_vector"])
                            if record["feature_vector"]
                            else {}
                        )

                        # Add game-level features
                        game_features = await self._get_game_features(
                            conn, record["game_id"]
                        )
                        ...
                        logger.error(
                            f"Failed to create feature vector for game {record['game_id']}, bet_type {bet_type}: {e}"
                        )
                        table_stats["failed"] += 1
            ...
            logger.info(
                f"✅ Feature vectors created: {table_stats['successful']:,} vectors"
            )
    ...
    async def _get_game_features(self, conn, game_id: int) -> dict:
        ...
        if game_info["market_features"]:
            ...
            features.update({f"market_{k}": v for k, v in market_features.items()})

        # Add sharp action features
        if game_info["sharp_action_summary"]:
            sharp_features = json.loads(game_info["sharp_action_summary"])
            features.update({f"sharp_{k}": v for k, v in sharp_features.items()})
...
async def main():
    ...
    try:
        ...
        if results["status"] == "completed" and "summary" in results:
            ...
            print(
                f"🧠 ML Features: {'✅ Created' if summary.get('ml_features_created') else '❌ Failed'}"
            )
            print(
                f"📈 Analytics: {'✅ Enabled' if summary.get('analytics_enabled') else '❌ Failed'}"
            )
