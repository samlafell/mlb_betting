    ...
    try:
        ...
        if all_results:
            ...
            for row in all_results[:10]:
                print(
                    f"{row['strategy_name'][:25]:25} | {row['source_book_type'][:10]:10} | "
        ...
        if max_date:
            ...
            """
            latest_count_result = repository.coordinator.execute_read(
                latest_date_query, (max_date,)
            )
        ...
        qualifying_strategies = [
            r for r in filter_results if r["filter_reason"] != "Does not meet criteria"
        ]
        non_qualifying_strategies = [
            r for r in filter_results if r["filter_reason"] == "Does not meet criteria"
        ]

        print(f"✅ Strategies that MEET filter criteria: {len(qualifying_strategies)}")
        print(
            f"❌ Strategies that DON'T MEET filter criteria: {len(non_qualifying_strategies)}"
        )

        if qualifying_strategies:
            print("\n🎯 QUALIFYING STRATEGIES:")
            print(
                "Strategy Name | Source-Book | Split | Win Rate | ROI | Bets | Filter Reason"
            )
            print("-" * 100)
            for row in qualifying_strategies[:10]:
                print(
                    f"{row['strategy_name'][:20]:20} | {row['source_book_type'][:8]:8} | "
                    f"{row['split_type'][:8]:8} | {row['win_rate_pct']:6.1f}% | "
        ...
        print(
            f"Strategies loaded by repository.get_profitable_strategies(): {len(loaded_strategies)}"
        )

        if loaded_strategies:
            print("\n📋 LOADED STRATEGIES:")
            print(
                "Strategy Name | Source-Book | Split | Win Rate | ROI | Bets | Confidence"
            )
            print("-" * 80)
            for strategy in loaded_strategies[:10]:
                print(
                    f"{strategy.strategy_name[:20]:20} | {strategy.source_book[:8]:8} | "
        ...
        if len(loaded_strategies) == 0:
            print(
                "🚨 ISSUE: No strategies are being loaded due to strict filtering criteria"
            )
            print("💡 SOLUTION: Relax the filter criteria or check data quality")
        else:
            print(
                f"✅ SUCCESS: {len(loaded_strategies)} strategies loaded successfully"
            )
