    ...
    if record:
        ...
        if odds_data:
            ...
            if record_transform["bet_type"] in ("total", "totals"):
                ...
                record_transform["over_price"] = first_odds.get(
                    "total_over"
                ) or first_odds.get("over_price")
                record_transform["under_price"] = first_odds.get(
                    "total_under"
                ) or first_odds.get("under_price")
            ...
            if sbr_game_id:
                ...
                if game_entry:
                    ...
                    if betting_records:
                        print(
                            f"  ✅ Found {len(betting_records)} totals betting records"
                        )
                        first_betting = betting_records[0]
                        print("  📊 First betting record:")
                        for key, value in dict(first_betting).items():
                            if value is not None:
                                print(f"    {key}: {value}")
                    else:
                        print("  ❌ No totals betting records found")
                else:
                    print("  ❌ Game not found in main table")
