...
def cleanup_output(
    ...
    for category, file_list in files_by_type.items():
        if file_list:
            ...
            print(f"  {category:12}: {len(file_list):3} files ({format_size(total_size)})")
        ...
        if archive:
            for category in ["reports", "opportunities", "pipeline", "historical"]:
                if files_by_type[category]:
                    print(f"📦 Would archive {len(files_by_type[category])} {category} files")
        
        # Show URL file management
        if len(files_by_type["urls"]) > keep_recent:
            excess = len(files_by_type["urls"]) - keep_recent
            print(f"🔗 Would keep {keep_recent} most recent URL files, archive {excess} old ones")
        else:
            print(f"🔗 Would keep all {len(files_by_type['urls'])} URL files (within limit)")
    ...
    if migrate_to_db and files_by_type["reports"]:
        ...
        migrated_count = asyncio.run(migrate_reports_to_database(files_by_type["reports"]))
    ...
    url_files = sorted(files_by_type["urls"], key=lambda f: f.stat().st_mtime, reverse=True)
    if len(url_files) > keep_recent:
        ...
        if archive:
            ...
            print(f"🔗 Archived {archived_urls} old URL files, kept {keep_recent} most recent")
        else:
            print(f"🔗 Found {len(excess_urls)} excess URL files (use --archive to move them)")
    else:
    ...
    print(f"   Reduction:    {format_size(size_reduction)} ({size_reduction/initial_size*100:.1f}%)")
    
    # Show recommendations
    print(f"\n💡 Recommendations:")
    print(f"   • Use database-first approach: `uv run -m src.interfaces.cli action-network pipeline`")
    print(f"   • View opportunities from database: `uv run -m src.interfaces.cli action-network opportunities`")
    print(f"   • URL files preserved for manual evaluation in {output_dir}")
    
    if not migrate_to_db:
        print(f"   • Run with --migrate-to-db to move analysis reports to PostgreSQL")
    
    if not archive:
        print(f"   • Run with --archive to organize old files")
