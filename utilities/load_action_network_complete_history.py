...
class CompleteHistoryCollector:
    ...
    async def _process_all_games(
        self, games: list[dict[str, Any]], game_mappings: dict[str, int]
    ) -> None:
        ...
        for game in games:
            ...
            for book_id_str, book_data in markets.items():
                ...
                sportsbook_mapping = (
                    await self.sportsbook_resolver.resolve_action_network_id(book_id)
                )
    ...
    async def _process_moneyline_markets(
        ...
        home_entry = next(
            (entry for entry in moneyline_data if entry.get("side") == "home"), None
        )
        away_entry = next(
            (entry for entry in moneyline_data if entry.get("side") == "away"), None
        )
    ...
    async def _insert_single_moneyline(
        ...
        try:
            ...
            """,
                ...
                home_odds,
                away_odds,
                prepare_for_postgres(odds_timestamp),
            )
            ...
            if not existing:
                ...
                """,
                    ...
                    "API",
                    "ACTION_NETWORK",
                    prepare_for_postgres(game_datetime),
                    now_est(),
                    prepare_for_postgres(odds_timestamp),
                )
    ...
    async def _process_spread_markets(
        ...
        home_entry = next(
            (entry for entry in spread_data if entry.get("side") == "home"), None
        )
        ...
        if history:
            ...
            for hist_point in history:
                ...
                if spread_line and spread_price and timestamp:
                    odds_timestamp = datetime.fromisoformat(
    ...
    async def _process_totals_markets(
        ...
        over_entry = next(
            (entry for entry in totals_data if entry.get("side") == "over"), None
        )
...
async def main():
    ...
    if results["total_inserted"] > 0:
        ...
        print("   -- See all moneyline records for a team:")
        print(
        ...
        print("⚠️  No records inserted - likely no historical data available")
        print("   This is normal for future games without line movement")
