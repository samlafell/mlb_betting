...
CREATE TABLE IF NOT EXISTS curated.sportsbook_mappings (
    ...
    sportsbook_id INTEGER NOT NULL REFERENCES curated.sportsbooks(id) ON DELETE CASCADE,
...
ON curated.sportsbook_mappings(external_source, external_id);

CREATE INDEX IF NOT EXISTS idx_sportsbook_external_mappings_name 
ON curated.sportsbook_mappings(external_source, external_name);
...
CREATE OR REPLACE FUNCTION curated.resolve_sportsbook_id(
    p_external_id TEXT,
...
CREATE OR REPLACE FUNCTION curated.auto_resolve_sportsbook_id()
RETURNS TRIGGER AS $$
BEGIN
...
DROP TRIGGER IF EXISTS auto_resolve_sportsbook_moneyline ON curated.betting_lines_unified -- NOTE: Add WHERE market_type = 'moneyline';
CREATE TRIGGER auto_resolve_sportsbook_moneyline
    ...
    FOR EACH ROW EXECUTE FUNCTION curated.auto_resolve_sportsbook_id();

DROP TRIGGER IF EXISTS auto_resolve_sportsbook_spreads ON curated.betting_lines_unified -- NOTE: Add WHERE market_type = 'spread's;
...
DROP TRIGGER IF EXISTS auto_resolve_sportsbook_totals ON curated.betting_lines_unified -- NOTE: Add WHERE market_type = 'totals';
...
CREATE OR REPLACE VIEW curated.unmapped_sportsbook_analysis AS
SELECT 
    source,
    sportsbook,
...
FROM (
    SELECT source, sportsbook, created_at FROM curated.betting_lines_unified -- NOTE: Add WHERE market_type = 'moneyline' WHERE sportsbook_id IS NULL
    UNION ALL
    SELECT source, sportsbook, created_at FROM curated.betting_lines_unified -- NOTE: Add WHERE market_type = 'spread's WHERE sportsbook_id IS NULL
    UNION ALL
    SELECT source, sportsbook, created_at FROM curated.betting_lines_unified -- NOTE: Add WHERE market_type = 'totals' WHERE sportsbook_id IS NULL
) unmapped
...
CREATE OR REPLACE FUNCTION curated.update_mapping_timestamp()
RETURNS TRIGGER AS $$
BEGIN
    NEW.updated_at = NOW();
    RETURN NEW;
...
CREATE TRIGGER update_sportsbook_mapping_timestamp
    BEFORE UPDATE ON curated.sportsbook_mappings
    FOR EACH ROW EXECUTE FUNCTION curated.update_mapping_timestamp();