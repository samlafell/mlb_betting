-- Final fix for Phase 2B migration - filter out test data
...
-- Migrate game outcomes data with proper filtering and type casting
...
INNER JOIN game_id_mapping gim ON go.game_id::integer = gim.old_game_id
WHERE go.game_id SIMILAR TO '[0-9]+'  -- Only include numeric game IDs
AND NOT EXISTS (
...
SET 
    records_processed = (SELECT COUNT(*) FROM public.game_outcomes WHERE game_id SIMILAR TO '[0-9]+'),
    records_migrated = (SELECT COUNT(*) FROM core_betting.game_outcomes),
    completed_at = NOW(),
    status = 'completed'
WHERE migration_step = 'GAME_OUTCOMES_MIGRATION_START';

-- Continue with analytics migration if not already done
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM migration_log_phase2b WHERE migration_step = 'ANALYTICS_MIGRATION_START') THEN
...
-- Update analytics migration log
UPDATE migration_log_phase2b 
SET 
...
-- Continue with operational migration if not already done
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM migration_log_phase2b WHERE migration_step = 'OPERATIONAL_MIGRATION_START') THEN
        INSERT INTO migration_log_phase2b (migration_step, table_name) 
...
-- Update operational migration log
UPDATE migration_log_phase2b 
SET 
    records_processed = (
...
-- Update sequences if not already done
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM migration_log_phase2b WHERE migration_step = 'SEQUENCE_UPDATE_START') THEN
        INSERT INTO migration_log_phase2b (migration_step, table_name) 
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM migration_log_phase2b WHERE migration_step = 'VALIDATION_START') THEN
        INSERT INTO migration_log_phase2b (migration_step, table_name) 
...
-- Mark migration as complete if not already done
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM migration_log_phase2b WHERE migration_step = 'PHASE2B_COMPLETE') THEN
