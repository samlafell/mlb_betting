-- ============================================================================
-- CORRECTED GAME ENTITY UNIFICATION - P0 Critical Fix
-- ============================================================================
-- Purpose: Resolve game entity fragmentation with proper table/view handling
-- Fix: Drop tables before creating views with the same names
-- Risk Level: LOW (focused fix, full rollback capability)
-- 
-- CRITICAL: This solves the core P0 issue with proper sequence
-- ============================================================================

BEGIN;

-- Create transaction savepoint for rollback capability
SAVEPOINT corrected_game_unification_start;
...
SELECT 'CORRECTED GAME ENTITY UNIFICATION STARTING:' as status;
...
-- Check for other tables with unique game_id constraints and drop them
DO $$
DECLARE
...
-- ============================================================================
-- 9. BACKUP AND REPLACE ORIGINAL TABLES WITH VIEWS
-- ============================================================================

SELECT 'Backing up original tables and creating views...' as status;

-- First, backup the original tables
ALTER TABLE curated.enhanced_games RENAME TO enhanced_games_backup_corrected_unification;
ALTER TABLE curated.games_complete RENAME TO games_complete_backup_corrected_unification;

-- Now create the backward compatibility views
...
VALUES (
    ...
    'COMPLETED',
    FORMAT('Successfully unified %s games from enhanced_games and games_complete into master_games, updated 9 FK relationships, resolved P0 schema fragmentation crisis with proper table/view handling',
           (SELECT COUNT(*) FROM curated.master_games))
);

SELECT '🎉 CORRECTED GAME ENTITY UNIFICATION SUCCESSFULLY COMPLETED' as status;
...
SELECT '✅ Original tables backed up for rollback safety' as safety;
