-- ============================================================================
-- FINAL GAME ENTITY UNIFICATION - P0 Critical Fix (CORRECTED VERSION)
-- ============================================================================
-- Purpose: Resolve game entity fragmentation with proper duplicate handling
-- Fix: Handle overlapping games and unique constraint conflicts
-- Risk Level: LOW (focused fix, full rollback capability)
-- 
-- CRITICAL: This solves the core P0 issue with proper duplicate resolution
-- ============================================================================

BEGIN;

-- Create transaction savepoint for rollback capability
SAVEPOINT final_game_unification_start;
...
-- Analyze overlapping games between the two tables
SELECT 'OVERLAP ANALYSIS:' as status;
WITH overlap_analysis AS (
    SELECT 
        COUNT(DISTINCT eg.mlb_stats_api_game_id) as enhanced_unique,
        COUNT(DISTINCT gc.mlb_stats_api_game_id) as complete_unique,
        COUNT(DISTINCT COALESCE(eg.mlb_stats_api_game_id, gc.mlb_stats_api_game_id)) as total_unique_games,
        COUNT(*) as overlap_matches
...
SELECT 'Merging game data with proper deduplication...' as status;
...
-- ============================================================================
-- 4. CREATE COMPREHENSIVE ID MAPPINGS 
-- ============================================================================

SELECT 'Creating comprehensive ID mappings...' as status;
...
-- Validation: Check for mapping duplicates
SELECT 'Validating ID mappings for duplicates...' as status;
SELECT 
    CASE WHEN dup_enhanced > 0 THEN 'ERROR: Enhanced games mapping has duplicates'
         WHEN dup_complete > 0 THEN 'ERROR: Games complete mapping has duplicates'
         ELSE 'SUCCESS: No duplicate mappings found'
    END as mapping_validation
FROM (
    SELECT 
        (SELECT COUNT(*) FROM (
            SELECT new_master_id FROM enhanced_games_id_mapping 
            GROUP BY new_master_id HAVING COUNT(*) > 1
        ) dup_enh) as dup_enhanced,
        (SELECT COUNT(*) FROM (
            SELECT new_master_id FROM games_complete_id_mapping 
            GROUP BY new_master_id HAVING COUNT(*) > 1  
        ) dup_comp) as dup_complete
) counts;

-- ============================================================================
-- 5. UPDATE FK REFERENCES SAFELY
-- ============================================================================

SELECT 'Updating FK references with conflict resolution...' as status;
...
-- Games complete FK updates (these should be 1:1 due to unique constraints)
...
-- ============================================================================
-- 9. BACKUP OLD TABLES FOR ROLLBACK SAFETY
-- ============================================================================

SELECT 'Creating backup tables for rollback safety...' as status;

-- Rename original tables for rollback safety
ALTER TABLE curated.enhanced_games_backup 
RENAME TO enhanced_games_backup_final_unification;

ALTER TABLE curated.games_complete_backup 
RENAME TO games_complete_backup_final_unification;
...
VALUES (
    ...
    'SUCCESS',
    FORMAT('Unified %s games from enhanced_games and games_complete into master_games, updated 9 FK relationships, resolved P0 schema fragmentation crisis',
           (SELECT COUNT(*) FROM curated.master_games))
);

SELECT '🎉 GAME ENTITY UNIFICATION SUCCESSFULLY COMPLETED' as status;
...
SELECT 'P0 DATABASE SCHEMA FRAGMENTATION CRISIS RESOLUTION: COMPLETE ✅' as final_status;