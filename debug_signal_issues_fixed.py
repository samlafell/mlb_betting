#!/usr/bin/env python3
"""
Fixed debug script that uses the actual database schema to identify why strategies aren't generating signals.
"""
...
class StrategySignalDebugger:
    ...
    def debug_strategy_signal_generation(self, strategy_name: str) -> Dict[str, Any]:
        ...
        try:
            # Step 1: Check raw data availability using correct schema
            with self.db_manager.get_cursor() as cursor:
                # Check betting splits data - using actual column names
                cursor.execute("""
                    SELECT 
    ...
    def _check_strategy_specific_data_quality(self, cursor, strategy_name: str, debug_info: Dict):
        """Check data quality specific to strategy requirements using actual schema."""
        
        # Different strategies have different data requirements
        strategy_data_requirements = {
            'opposing_markets': {
                'min_books': 2,
                'required_split_types': ['moneyline', 'Moneyline'],
                'description': 'Needs moneyline odds from multiple books for comparison'
            },
            'book_conflicts': {
                'min_books': 3,
                'required_split_types': ['moneyline', 'Moneyline'],
                'description': 'Needs moneyline odds conflicts between 3+ books'
            },
            'public_money_fade': {
                'min_books': 1,
                'required_split_types': ['moneyline', 'Moneyline'],
                'required_fields': ['home_or_over_bets_percentage', 'away_or_under_bets_percentage'],
                'description': 'Needs public betting percentages for moneyline'
            },
            'late_sharp_flip': {
                'min_books': 1,
                'required_split_types': ['moneyline', 'Moneyline'],
                'description': 'Needs time-series moneyline data for line movement'
            },
            'consensus_moneyline': {
                'min_books': 3,
                'required_split_types': ['moneyline', 'Moneyline'],
                'description': 'Needs consensus moneyline from multiple books'
            },
            'underdog_ml_value': {
                'min_books': 1,
                'required_split_types': ['moneyline', 'Moneyline'],
            ...
            'line_movement': {
                'min_books': 1,
                'required_split_types': ['moneyline', 'Moneyline'],
                'description': 'Needs historical moneyline odds for movement analysis'
            }
        }
        ...
        cursor.execute("""
            SELECT COUNT(DISTINCT book) as book_count
            FROM splits.raw_mlb_betting_splits 
            WHERE last_updated >= NOW() - INTERVAL '7 days'
            AND split_type IN ('moneyline', 'Moneyline')
        """)
        ...
        # Check required fields availability (if any)
        ...
        try:
            if strategy_name == 'opposing_markets':
                ...
                if not opposing_markets:
                    debug_info['data_quality_issues'].append("No games with multiple books for opposing markets")
                    debug_info['potential_fixes'].append("Need games with odds from multiple sportsbooks")
                else:
                    debug_info['multi_book_games'] = len(opposing_markets)
                ...
                cursor.execute("""
                    ...
                    FROM splits.raw_mlb_betting_splits 
                    WHERE (home_or_over_bets_percentage IS NOT NULL OR away_or_under_bets_percentage IS NOT NULL)
                    AND split_type IN ('moneyline', 'Moneyline')
                    AND last_updated >= NOW() - INTERVAL '7 days'
                """)
                ...
                if percentage_result and percentage_result['games_with_percentages'] == 0:
                    ...
                    debug_info['percentage_records'] = percentage_result['games_with_percentages']
                ...
                if not movement_data:
                    ...
                    debug_info['movement_games'] = len(movement_data)
        except Exception as e:
    ...
    def _test_signal_generation_logic(self, cursor, strategy_name: str, debug_info: Dict):
        ...
        try:
            # Get a small sample of moneyline data to test with
            cursor.execute("""
                SELECT 
                    game_id,
                    book,
    ...
    def _analyze_signal_generation_failure(self, sample_data: List, strategy_name: str, debug_info: Dict):
        ...
        # Check for missing percentage data
        no_percentages = [
            row for row in sample_data 
            if not row['home_or_over_bets_percentage'] and not row['away_or_under_bets_percentage']
        ]
        if no_percentages:
            failure_patterns.append(f"{len(no_percentages)} rows missing betting percentages")
        
        # Check for missing stake percentages
        no_stakes = [
            row for row in sample_data 
            if not row['home_or_over_stake_percentage'] and not row['away_or_under_stake_percentage']
        ]
        if no_stakes:
            failure_patterns.append(f"{len(no_stakes)} rows missing stake percentages")
        
        # Strategy-specific checks
        if strategy_name == 'public_money_fade':
            if no_percentages:
                failure_patterns.append("Public fade strategy needs betting percentages")
        ...
        if failure_patterns:
            debug_info['potential_fixes'].extend([
                ...
                "Ensure VSIN scraper is collecting betting percentages correctly"
            ])
...
def debug_all_failing_strategies():
    ...
    print("🔍 STRATEGY SIGNAL GENERATION DEBUG REPORT (FIXED)")
    print("=" * 80)
    ...
    for strategy in failing_strategies:
        ...
        if debug_results.get('strategy_requirements'):
            ...
            print(f"   📊 Split types: {', '.join(req['required_split_types'])}")
        
        # Add specific metrics
        if debug_results.get('multi_book_games'):
            print(f"✅ Multi-book games: {debug_results['multi_book_games']}")
        
        if debug_results.get('percentage_records'):
            print(f"✅ Records with betting percentages: {debug_results['percentage_records']}")
        
        if debug_results.get('movement_games'):
            print(f"✅ Games with line movement data: {debug_results['movement_games']}")
