...
class UnifiedUnderdogValueProcessor(BaseStrategyProcessor, StrategyProcessorMixin):
    ...
    def __init__(self, repository: UnifiedRepository, config: Dict[str, Any]):
        ...
        self.heavy_favorite_threshold = config.get('heavy_favorite_threshold', 65.0)  # Public on favorite
        self.moderate_favorite_threshold = config.get('moderate_favorite_threshold', 60.0)
        ...
        self.min_sharp_underdog_support = config.get('min_sharp_underdog_support', 40.0)  # Sharp money on dog
        
        # Value confidence modifiers
        self.value_modifiers = config.get('value_modifiers', {
        ...
        self.odds_ranges = config.get('odds_ranges', {
            'sweet_spot': (120, 200),    # Sweet spot for value
            'moderate': (200, 250),      # Moderate value range
            'long_shot': (250, 300)      # Long shot range
        })
        
        self.logger.info(f"Initialized UnifiedUnderdogValueProcessor with thresholds: "
    ...
    def get_strategy_description(self) -> str:
        ...
        return ("Underdog value strategy: Identify systematic value on underdogs "
    ...
    async def process_signals(self, 
        ...
        self.logger.info(f"Processing underdog value signals for {len(game_data)} games")
        ...
        try:
            ...
            value_opportunities = await self._find_underdog_value_opportunities(value_data)
            ...
            for value_data in value_opportunities:
                try:
                    ...
                    if not self._is_valid_value_data(value_data, processing_time, minutes_ahead):
                        continue
    ...
    async def _get_underdog_value_data(self, 
                                     game_data: List[Dict[str, Any]], 
        ...
        try:
            ...
            for game in game_data:
                ...
                mock_value_data = {
                    ...
                    'home_money_pct': game.get('money_percentage', 72.0),  # Heavy public on favorite
    ...
    async def _find_underdog_value_opportunities(self, value_data: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """
        ...
        for data in value_data:
            try:
                ...
                if value_analysis and self._is_significant_value_opportunity(value_analysis):
        ...
        self.logger.info(f"Found {len(value_opportunities)} potential value opportunities from {len(value_data)} games")
        return value_opportunities
    
    async def _analyze_underdog_value(self, data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        """
        ...
        try:
            ...
            if favorite_money_pct >= self.heavy_favorite_threshold:
                value_type = UnderdogValueType.VALUE_AWAY_DOG if value_type_base == 'AWAY' else UnderdogValueType.VALUE_HOME_DOG
            ...
            # Moderate favorite public bias (≥60%)
                ...
                value_type = UnderdogValueType.MODERATE_VALUE_AWAY if value_type_base == 'AWAY' else UnderdogValueType.MODERATE_VALUE_HOME
                value_confidence = ValueConfidence.LOW_VALUE
            ...
            sharp_underdog_support = sharp_money_pct if sharp_money_pct < 50 else (100 - sharp_money_pct)
    ...
    def _classify_odds_range(self, odds: int) -> str:
        ...
        if self.odds_ranges['sweet_spot'][0] <= odds <= self.odds_ranges['sweet_spot'][1]:
            return 'sweet_spot'
        elif self.odds_ranges['moderate'][0] <= odds <= self.odds_ranges['moderate'][1]:
            return 'moderate'
        elif self.odds_ranges['long_shot'][0] <= odds <= self.odds_ranges['long_shot'][1]:
            return 'long_shot'
        else:
            return 'other'
    ...
    def _is_significant_value_opportunity(self, value_analysis: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            # High value criteria
            if (public_bias >= 75.0 and 
                underdog_odds <= 200 and 
                sharp_support >= 40.0):
                return True
            
            # Moderate value criteria
            if (public_bias >= self.heavy_favorite_threshold and 
                underdog_odds <= 250 and 
                sharp_support >= 35.0):
                return True
            
            # Minimum value criteria
            if (public_bias >= self.moderate_favorite_threshold and 
                underdog_odds <= self.max_underdog_odds):
                return True
    ...
    async def _calculate_value_confidence(self, value_data: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if public_bias >= 75.0:
                ...
                applied_modifiers['extreme_public_bias'] = self.value_modifiers['extreme_public_bias']
            elif public_bias >= self.heavy_favorite_threshold:
                base_confidence *= self.value_modifiers['heavy_favorite_public']
                applied_modifiers['heavy_favorite_public'] = self.value_modifiers['heavy_favorite_public']
            ...
            if sharp_support >= self.min_sharp_underdog_support:
                ...
                applied_modifiers['sharp_underdog_support'] = self.value_modifiers['sharp_underdog_support']
            ...
            if odds_range == 'sweet_spot':
                ...
                applied_modifiers['moderate_odds_range'] = self.value_modifiers['moderate_odds_range']
            
            # Contrarian value bonus
            base_confidence *= self.value_modifiers['contrarian_value']
            applied_modifiers['contrarian_value'] = self.value_modifiers['contrarian_value']
    ...
    async def _create_value_signal(self, 
    ...
    def _is_valid_value_data(self, value_data: Dict[str, Any], 
                           current_time: datetime, minutes_ahead: int) -> bool:
        """Validate value opportunity data"""
        try:
            # Check required fields
            required_fields = ['underdog_team', 'underdog_odds', 'public_bias_strength', 'raw_data']
    ...
    async def _apply_value_filtering(self, signals: List[UnifiedBettingSignal]) -> List[UnifiedBettingSignal]:
        ...
        for signal in signals:
            ...
            if game_key not in unique_signals or current_priority > value_priority(unique_signals[game_key]):
                unique_signals[game_key] = signal
        
        # Sort by value priority (highest first)
        filtered_signals = sorted(unique_signals.values(), key=value_priority, reverse=True)
    ...
    def validate_strategy_data(self, raw_data: List[Dict[str, Any]]) -> bool:
        ...
        for row in raw_data:
            ...
            if not ((home_odds < 0 and away_odds > 0) or (home_odds > 0 and away_odds < 0)):
                return False
