...
class MovementAnalyzer:
    ...
    async def analyze_game_movements(self, game_data: Dict) -> GameMovementAnalysis:
        ...
        game_datetime = datetime.fromisoformat(game_data.get('game_datetime', '').replace('Z', '+00:00'))
        ...
        moneyline_summary = await self._create_market_summary(line_movements, MarketType.MONEYLINE)
        spread_summary = await self._create_market_summary(line_movements, MarketType.SPREAD)
        total_summary = await self._create_market_summary(line_movements, MarketType.TOTAL)
        
        # Generate insights
        sharp_money_indicators = await self._detect_sharp_money_patterns(
            line_movements, rlm_indicators, cross_book_movements
        )
        arbitrage_opportunities = await self._detect_arbitrage_opportunities(line_movements)
    ...
    async def _extract_line_movements(self, game_data: Dict) -> List[LineMovementDetail]:
    ...
    async def _create_movement_detail(
        ...
        prev_entry: Dict, 
        curr_entry: Dict
    ) -> Optional[LineMovementDetail]:
        """Create a detailed movement record from two history entries."""
        try:
            timestamp = datetime.fromisoformat(curr_entry.get('updated_at', '').replace('Z', '+00:00'))
            ...
            if market_type == MarketType.MONEYLINE:
                ...
                if prev_value is not None and curr_value is not None:
                    if curr_value > prev_value:
                        ...
                        direction = MovementDirection.DOWN
                    movement_amount = Decimal(str(abs(float(curr_value) - float(prev_value))))
    ...
    async def _extract_betting_snapshots(self, game_data: Dict) -> List[BettingPercentageSnapshot]:
    ...
    async def _detect_rlm(
        ...
        for (sportsbook_id, market_type), movement_list in grouped_movements.items():
            ...
            recent_movements = sorted(movement_list, key=lambda x: x.timestamp)[-5:]  # Last 5 movements
    ...
    def _get_dominant_direction(self, movements: List[LineMovementDetail]) -> MovementDirection:
    ...
    def _get_public_direction(self, snapshot: BettingPercentageSnapshot) -> MovementDirection:
        """Determine public betting direction based on percentages."""
    ...
    async def _analyze_cross_book_movements(self, movements: List[LineMovementDetail]) -> List[CrossBookMovement]:
        ...
        for movement in movements:
            ...
            rounded_time = movement.timestamp.replace(minute=movement.timestamp.minute // 5 * 5, second=0, microsecond=0)
        ...
        for timestamp, market_groups in time_groups.items():
            for market_type, market_movements in market_groups.items():
                if len(market_movements) < 3:  # Need at least 3 books for cross-book analysis
                    continue
                
                participating_books = list(set(m.sportsbook_id for m in market_movements))
                ...
                direction_counts = {
                    ...
                    MovementDirection.STABLE: directions.count(MovementDirection.STABLE)
                }
                ...
                divergent_books = [
                    m.sportsbook_id for m in market_movements 
                ...
                cross_book = CrossBookMovement(
                    ...
                    consensus_strength="strong" if consensus_strength > 0.8 else "moderate" if consensus_strength > 0.6 else "weak",
                    divergent_books=divergent_books,
                    average_movement=Decimal(str(sum(
                        float(m.movement_amount or 0) for m in market_movements
    ...
    async def _detect_sharp_money_patterns(
        ...
        major_movements = [m for m in movements if m.magnitude == MovementMagnitude.MAJOR]
        if len(major_movements) > 5:
            indicators.append(f"High volume of major movements ({len(major_movements)})")
        
        # Rapid consecutive movements
        rapid_movements = self._detect_rapid_movements(movements)
        if rapid_movements:
            indicators.append(f"Rapid consecutive movements detected in {len(rapid_movements)} markets")
    ...
    def _detect_rapid_movements(self, movements: List[LineMovementDetail]) -> List[str]:
        ...
        for (sportsbook_id, market_type), movement_list in grouped.items():
            ...
            for i in range(len(sorted_movements) - 2):
                ...
                time_span = (window_movements[-1].timestamp - window_movements[0].timestamp).total_seconds()
                
                if time_span <= 600:  # 10 minutes
                    rapid_patterns.append(f"{self.sportsbook_names.get(sportsbook_id, sportsbook_id)} {market_type.value}")
        
        return rapid_patterns
    
    async def _detect_arbitrage_opportunities(self, movements: List[LineMovementDetail]) -> List[Dict]:
        ...
        for movement in movements:
            if movement.timestamp > datetime.now().replace(hour=datetime.now().hour - 1):  # Last hour
    ...
    async def _generate_recommendations(
        ...
        if len(sharp_indicators) >= 3:
            recommendations.append("HIGH SHARP ACTIVITY: Exercise caution, professional money heavily involved")
