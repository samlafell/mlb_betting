...
class UnifiedLateFlipProcessor(BaseStrategyProcessor, StrategyProcessorMixin):
    ...
    def __init__(self, repository: UnifiedRepository, config: dict[str, Any]):
        ...
        self.flip_detection_hours = config.get(
            "flip_detection_hours", 3.0
        )  # Hours before game
        self.min_flip_magnitude = config.get(
            "min_flip_magnitude", 15.0
        )  # Minimum % change for flip
        self.early_window_hours = config.get(
            "early_window_hours", 24.0
        )  # Early action window
        self.min_early_confidence = config.get(
            "min_early_confidence", 60.0
        )  # Min early action confidence

        # Flip timing thresholds
        self.timing_thresholds = config.get(
            "timing_thresholds",
            {
        ...
        self.flip_modifiers = config.get(
            "flip_modifiers",
            {
        ...
        self.logger.info(
            f"Initialized UnifiedLateFlipProcessor with flip detection: "
    ...
    def get_strategy_description(self) -> str:
        ...
        return (
            "Late flip strategy: Detect late sharp money direction changes "
    ...
    async def process_signals(
        ...
        try:
            ...
            betting_timeline = await self._get_betting_timeline_data(
            ...
            for flip_data in flip_opportunities:
                try:
                    ...
                    if not self._is_valid_flip_data(
                        flip_data, processing_time, minutes_ahead
                    ):
                        continue
    ...
    async def _get_betting_timeline_data(
    ...
    async def _detect_late_flips(
        ...
        for game_key, game_timeline in grouped_data.items():
            try:
                ...
                self.logger.warning(
                    f"Error analyzing flip patterns for game {game_key}: {e}"
                )
                continue

        self.logger.info(
            f"Detected {len(flip_opportunities)} potential flip opportunities from {len(grouped_data)} games"
        )
        return flip_opportunities

    def _group_timeline_by_game(
        self, timeline_data: list[dict[str, Any]]
    ...
    async def _analyze_flip_patterns(
        self, game_timeline: list[dict[str, Any]]
    ) -> dict[str, Any] | None:
        """
        ...
        try:
            ...
            timeline = sorted(
                game_timeline, key=lambda x: x.get("timestamp", datetime.now())
            )
            ...
            if early_money_pct < 50 and late_money_pct > 50:
                ...
                fade_recommendation = early_action.get(
                    "away_team"
                )  # Fade late flip, follow early
            ...
            return {
                ...
                "volume_increase": late_action.get("volume", 0)
                - early_action.get("volume", 0),
            }
    ...
    def _is_significant_flip(self, flip_analysis: dict[str, Any]) -> bool:
        ...
        try:
            ...
            if (
                flip_magnitude >= 25.0
                and early_confidence >= 70.0
                and hours_to_game <= 2.0
            ):
                return True
            ...
            if (
                flip_magnitude >= 20.0
                and early_confidence >= 65.0
                and hours_to_game <= 3.0
            ):
                return True

            # Minimum flip criteria
            if (
                flip_magnitude >= self.min_flip_magnitude
                and early_confidence >= self.min_early_confidence
            ):
                return True
    ...
    async def _calculate_flip_confidence(
        self, flip_data: dict[str, Any]
    ) -> dict[str, Any]:
        """
        ...
        try:
            ...
            if flip_timing == FlipTiming.ULTRA_LATE:
                ...
                applied_modifiers["ultra_late_flip"] = self.flip_modifiers[
                    "ultra_late_flip"
                ]
            ...
            if early_confidence >= 75.0:
                ...
                applied_modifiers["strong_early_action"] = self.flip_modifiers[
                    "strong_early_action"
                ]
    ...
    async def _create_flip_signal(
        ...
        try:
            ...
            signal = UnifiedBettingSignal(
                ...
                game_id=late_action.get(
                    "game_id",
                    f"{late_action.get('home_team', 'unknown')}_vs_{late_action.get('away_team', 'unknown')}",
                ),
                ...
                game_date=self._normalize_game_time(
                    late_action.get("game_datetime", processing_time)
                ),
    ...
    def _is_valid_flip_data(
        self, flip_data: dict[str, Any], current_time: datetime, minutes_ahead: int
    ) -> bool:
    ...
    async def _apply_flip_filtering(
        ...
        for signal in signals:
            ...
            if game_key not in unique_signals or current_priority > flip_priority(
        ...
        filtered_signals = sorted(
            unique_signals.values(), key=flip_priority, reverse=True
        )
