...
class UnifiedBookConflictProcessor(BaseStrategyProcessor, StrategyProcessorMixin):
    ...
    def __init__(self, repository: UnifiedRepository, config: Dict[str, Any]):
        ...
        self.min_arbitrage_profit = config.get('min_arbitrage_profit', 0.02)  # 2% minimum profit
        self.min_discrepancy_threshold = config.get('min_discrepancy_threshold', 5.0)  # 5 cent line difference
        self.significant_discrepancy_threshold = config.get('significant_discrepancy_threshold', 10.0)  # 10 cents
        self.min_books_required = config.get('min_books_required', 2)
        
        # Book reliability scoring for conflict analysis
        self.book_reliability = config.get('book_reliability', {
        ...
        self.book_speed = config.get('book_speed', {
        ...
        self.logger.info(f"Initialized UnifiedBookConflictProcessor with {len(self.book_reliability)} books")
    ...
    def get_strategy_description(self) -> str:
        ...
        return ("Book conflict detection with arbitrage identification, "
    ...
    async def process_signals(self, 
        ...
        try:
            ...
            multi_book_data = await self._get_multi_book_odds_data(game_data, minutes_ahead)
            
            if not multi_book_data:
                self.logger.info("No multi-book odds data available for conflict analysis")
                return signals
            ...
            for game_odds in multi_book_data:
                try:
                    ...
                    for conflict in conflicts:
                        ...
                        conflict_metrics = self._calculate_conflict_metrics(conflict, game_odds)
                        
                        if not self._meets_conflict_thresholds(conflict_metrics):
                            continue
                        
                        # Calculate confidence with conflict-specific factors
                        confidence_data = self._calculate_conflict_confidence(conflict, conflict_metrics)
    ...
    async def _get_multi_book_odds_data(self, 
                                      game_data: List[Dict[str, Any]], 
                ...
                mock_odds = {
                    ...
                    'books': {
                        'pinnacle': {
                            ...
                            'last_updated': datetime.now(self.est) - timedelta(minutes=5)
                        },
                        'draftkings': {
                            'moneyline': {'home': -105, 'away': -105},  # Conflict opportunity
                            'spread': {'home': -1.5, 'away': +1.5, 'home_odds': -105, 'away_odds': -115},
                            'total': {'over': 8.5, 'under': 8.5, 'over_odds': -110, 'under_odds': -110},
                            'last_updated': datetime.now(self.est) - timedelta(minutes=2)
                        },
                        'fanduel': {
                            'moneyline': {'home': -115, 'away': +105},  # Different line
                            'spread': {'home': -1.0, 'away': +1.0, 'home_odds': -110, 'away_odds': -110},  # Spread conflict
                            'total': {'over': 8.0, 'under': 8.0, 'over_odds': -110, 'under_odds': -110},  # Total conflict
                            'last_updated': datetime.now(self.est) - timedelta(minutes=1)
                        }
                    },
    ...
    async def _detect_book_conflicts(self, game_odds: Dict[str, Any]) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            for market_type in market_types:
                market_conflicts = await self._analyze_market_conflicts(books, market_type, game_odds)
    ...
    async def _analyze_market_conflicts(self, 
                                      books: Dict[str, Any], 
                                      market_type: str,
                                      game_odds: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if market_type == 'moneyline':
                conflicts.extend(self._detect_moneyline_conflicts(book_lines, game_odds))
    ...
    def _detect_moneyline_conflicts(self, book_lines: Dict[str, Any], game_odds: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if best_home_book and best_away_book and best_home_book != best_away_book:
                arbitrage_profit = self._calculate_arbitrage_profit(best_home_odds, best_away_odds)
            ...
            if max_discrepancy >= self.min_discrepancy_threshold:
                ...
                conflicts.append({
                    ...
                    'recommended_side': self._determine_recommended_side(book_lines, discrepancy_books),
                    'all_book_lines': book_lines
                })
    ...
    def _detect_spread_conflicts(self, book_lines: Dict[str, Any], game_odds: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if spread_discrepancy >= 0.5:  # Half point or more difference
                severity = ConflictSeverity.HIGH if spread_discrepancy >= 1.0 else ConflictSeverity.MEDIUM
                ...
                conflicts.append({
                    ...
                    'recommended_side': 'home' if max_spread > min_spread else 'away',
                    'all_book_lines': book_lines
                })
    ...
    def _detect_total_conflicts(self, book_lines: Dict[str, Any], game_odds: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if total_discrepancy >= 0.5:  # Half run or more difference
                severity = ConflictSeverity.HIGH if total_discrepancy >= 1.0 else ConflictSeverity.MEDIUM
                ...
                conflicts.append({
                    ...
                    'max_total': max_total,
                    'min_total': min_total,
                    'recommended_side': 'under' if max_total > min_total else 'over',
                    'all_book_lines': book_lines
                })
    ...
    def _determine_recommended_side(self, book_lines: Dict[str, Any], discrepancy_books: List[str]) -> str:
    ...
    def _calculate_conflict_metrics(self, conflict: Dict[str, Any], game_odds: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if conflict_type == ConflictType.ARBITRAGE_OPPORTUNITY:
                metrics.update({
                    ...
                    'profitability_score': conflict.get('arbitrage_profit', 0) * 10,  # Scale to 0-1
                ...
                metrics.update({
                    ...
                    'profitability_score': min(conflict.get('max_discrepancy', 0) / 20.0, 1.0)
                })
            ...
            metrics['avg_book_reliability'] = sum(book_reliability_scores) / len(book_reliability_scores) if book_reliability_scores else 2.0
            metrics['max_book_reliability'] = max(book_reliability_scores) if book_reliability_scores else 2.0
    ...
    def _meets_conflict_thresholds(self, conflict_metrics: Dict[str, Any]) -> bool:
        ...
        if conflict_metrics.get('conflict_type') == ConflictType.ARBITRAGE_OPPORTUNITY:
            return conflict_metrics.get('arbitrage_profit', 0) > self.min_arbitrage_profit
        
        return True
    
    def _calculate_conflict_confidence(self, 
                                     conflict: Dict[str, Any], 
                                     conflict_metrics: Dict[str, Any]) -> Dict[str, Any]:
        """Calculate confidence with conflict-specific factors"""
        ...
        modifiers = {
            'book_reliability': conflict_metrics.get('avg_book_reliability', 2.0) / 5.0,  # Normalize to 0-1
            'book_count': min(conflict_metrics.get('book_count', 2) / 5.0, 1.0),  # More books = higher confidence
            'severity_bonus': self._get_severity_bonus(conflict_metrics.get('severity')),
            'conflict_type_bonus': self._get_conflict_type_bonus(conflict_metrics.get('conflict_type'))
        }
    ...
    def _create_conflict_signal(self, 
        ...
        try:
            ...
            signal = UnifiedBettingSignal(
                ...
                timing_category=self._get_timing_category(self._calculate_minutes_to_game(
                    self._normalize_game_time(game_odds['game_datetime']),
                    processing_time
                )),
    ...
    async def _apply_conflict_filtering(self, signals: List[UnifiedBettingSignal]) -> List[UnifiedBettingSignal]:
        """Apply conflict-specific filtering and ranking"""
        if not signals:
            return signals
        ...
        def conflict_priority(signal):
            ...
            if strategy_data.get('conflict_type') == ConflictType.ARBITRAGE_OPPORTUNITY.value:
                priority_score += 0.5
        ...
        for signal in signals:
            ...
            if game_key not in unique_signals or current_priority > conflict_priority(unique_signals[game_key]):
                unique_signals[game_key] = signal
        
        # Sort by conflict priority (highest first)
        filtered_signals = sorted(unique_signals.values(), key=conflict_priority, reverse=True)
        ...
        if len(filtered_signals) > max_signals:
            ...
            self.logger.info(f"Limited signals to top {max_signals} by conflict priority")
