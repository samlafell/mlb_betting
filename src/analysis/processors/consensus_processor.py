...
class UnifiedConsensusProcessor(BaseStrategyProcessor, StrategyProcessorMixin):
    ...
    def __init__(self, repository: UnifiedRepository, config: dict[str, Any]):
        ...
        self.mixed_consensus_money_threshold = config.get(
            "mixed_consensus_money_threshold", 80.0
        )
        self.mixed_consensus_bet_threshold = config.get(
        ...
        self.consensus_modifiers = config.get(
            "consensus_modifiers",
            {
        ...
        self.logger.info(
            f"Initialized UnifiedConsensusProcessor with thresholds: "
    ...
    def get_strategy_description(self) -> str:
        ...
        return (
            "Consensus analysis: Follow or fade when public and sharp money align "
    ...
    async def process_signals(
        ...
        try:
            ...
            consensus_data = await self._get_consensus_splits_data(
            ...
            for split_data in consensus_data:
                try:
                    ...
                    if not self._is_valid_consensus_data(
                        split_data, processing_time, minutes_ahead
                    ):
                        continue

                    # Analyze consensus patterns
                    consensus_analysis = await self._analyze_consensus_patterns(
                        split_data
                    )
    ...
    async def _get_consensus_splits_data(
                ...
                mock_splits = [
                    {
                        ...
                        "sharp_public_diff": game.get("money_percentage", 85.0)
                        - game.get("bet_percentage", 78.0),
                    }
                ]
    ...
    async def _analyze_consensus_patterns(
        ...
        try:
            ...
            if (
                money_pct >= self.heavy_consensus_threshold
                and bet_pct >= self.heavy_consensus_threshold
            ) or (
                money_pct <= (100 - self.heavy_consensus_threshold)
                and bet_pct <= (100 - self.heavy_consensus_threshold)
            ):
                consensus_type = (
                    ConsensusType.CONSENSUS_HEAVY_HOME
                    if money_pct >= self.heavy_consensus_threshold
                    else ConsensusType.CONSENSUS_HEAVY_AWAY
                )

                return {
                    "consensus_type": consensus_type,
                    "recommended_side": split_data.get("home_team")
                    if money_pct >= self.heavy_consensus_threshold
            ...
            # Mixed consensus patterns (moderate alignment)
                ...
                money_pct >= self.mixed_consensus_money_threshold
                and bet_pct >= self.mixed_consensus_bet_threshold
            ) or (
                money_pct <= (100 - self.mixed_consensus_money_threshold)
                and bet_pct <= (100 - self.mixed_consensus_bet_threshold)
            ):
                consensus_type = (
                    ConsensusType.MIXED_CONSENSUS_HOME
                    if money_pct >= self.mixed_consensus_money_threshold
    ...
    async def _calculate_consensus_confidence(
        self, split_data: dict[str, Any], consensus_analysis: dict[str, Any]
    ) -> dict[str, Any]:
        """
        ...
        try:
            ...
            if consensus_analysis["pattern_strength"] == "HEAVY":
                ...
                applied_modifiers["heavy_consensus"] = self.consensus_modifiers[
                    "heavy_consensus"
                ]
            else:
                base_confidence *= self.consensus_modifiers["mixed_consensus"]
                applied_modifiers["mixed_consensus"] = self.consensus_modifiers[
                    "mixed_consensus"
                ]
            ...
            if alignment_diff <= 5.0:  # Very tight alignment
                ...
                applied_modifiers["perfect_alignment"] = self.consensus_modifiers[
                    "perfect_alignment"
                ]
    ...
    async def _create_consensus_signal(
        self,
    ...
    def _is_valid_consensus_data(
        self, split_data: dict[str, Any], current_time: datetime, minutes_ahead: int
    ) -> bool:
        ...
        try:
            ...
            consensus_strength = (money_pct + bet_pct) / 2
            if (
                consensus_strength < self.min_consensus_strength
                and consensus_strength > (100 - self.min_consensus_strength)
            ):
                return False
    ...
    async def _apply_consensus_filtering(
        ...
        for signal in signals:
            ...
            if game_key not in unique_signals or current_priority > consensus_priority(
        ...
        filtered_signals = sorted(
            unique_signals.values(), key=consensus_priority, reverse=True
        )
        ...
        if len(filtered_signals) > max_signals:
            ...
            self.logger.info(
                f"Limited signals to top {max_signals} by consensus priority"
            )
