...
class UnifiedHybridSharpProcessor(BaseStrategyProcessor, StrategyProcessorMixin):
    ...
    def __init__(self, repository: UnifiedRepository, config: Dict[str, Any]):
        ...
        self.min_line_movement = config.get('min_line_movement', 0.5)        # Minimum line movement
        self.min_sharp_threshold = config.get('min_sharp_threshold', 15.0)   # Minimum sharp action differential
        self.confirmation_threshold = config.get('confirmation_threshold', 0.7)  # Confirmation correlation threshold
        self.steam_play_threshold = config.get('steam_play_threshold', 25.0)     # Steam play sharp action threshold
        
        # Hybrid confidence modifiers
        self.hybrid_modifiers = config.get('hybrid_modifiers', {
        ...
        self.logger.info(f"Initialized UnifiedHybridSharpProcessor with thresholds: "
    ...
    def get_strategy_description(self) -> str:
        ...
        return ("Hybrid sharp strategy: Combine line movement with sharp action "
        ...
        try:
            ...
            for hybrid_data in hybrid_opportunities:
                try:
                    ...
                    if not self._is_valid_hybrid_data(hybrid_data, processing_time, minutes_ahead):
                        continue
                    
                    # Calculate hybrid confidence
                    confidence_data = await self._calculate_hybrid_confidence(hybrid_data)
    ...
    async def _get_hybrid_sharp_data(self, 
                                   game_data: List[Dict[str, Any]], 
    ...
    async def _detect_hybrid_opportunities(self, hybrid_data: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """
        ...
        for data in hybrid_data:
            try:
                ...
                if hybrid_analysis and self._is_significant_hybrid_opportunity(hybrid_analysis):
        ...
        self.logger.info(f"Found {len(hybrid_opportunities)} potential hybrid opportunities from {len(hybrid_data)} games")
        return hybrid_opportunities
    
    async def _analyze_hybrid_patterns(self, data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        """
        ...
        try:
            ...
            if (has_line_movement and has_sharp_action and 
                line_direction == sharp_direction and 
                correlation >= self.correlation_thresholds['strong']):
            ...
            elif (has_line_movement and has_sharp_action and 
                  correlation >= self.correlation_thresholds['moderate']):
            ...
            elif (has_sharp_action and not has_line_movement and 
                  sharp_differential >= self.steam_play_threshold):
                
                hybrid_type = HybridSignalType.STEAM_PLAY
                confirmation_strength = ConfirmationStrength.STRONG
                
            # Reverse line movement: Line moves opposite to public betting
            elif (has_line_movement and 
                  self._is_reverse_line_movement(data)):
            ...
            if abs(sharp_differential) > abs(line_movement):
                # Follow sharp action
                recommended_side = data.get('home_team') if sharp_direction == 'home' else data.get('away_team')
                signal_basis = 'sharp_action'
            else:
                # Follow line movement
                recommended_side = data.get('home_team') if line_direction == 'home' else data.get('away_team')
    ...
    def _calculate_signal_correlation(self, line_movement: float, sharp_differential: float,
                                    line_direction: str, sharp_direction: str) -> float:
        ...
        try:
            ...
            sharp_magnitude = abs(sharp_differential) / 30.0  # Normalize sharp differential
            
            magnitude_correlation = min(line_magnitude, sharp_magnitude) / max(line_magnitude, sharp_magnitude, 0.1)
    ...
    def _is_reverse_line_movement(self, data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            return line_direction != public_direction and line_direction != 'none' and public_direction != 'none'
            
        except Exception:
            return False
    
    def _is_significant_hybrid_opportunity(self, hybrid_analysis: Dict[str, Any]) -> bool:
    ...
    async def _calculate_hybrid_confidence(self, hybrid_data: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if hybrid_type == HybridSignalType.STRONG_CONFIRMATION:
                ...
                applied_modifiers['strong_confirmation'] = self.hybrid_modifiers['strong_confirmation']
            elif hybrid_type == HybridSignalType.MODERATE_CONFIRMATION:
                base_confidence *= self.hybrid_modifiers['moderate_confirmation']
                applied_modifiers['moderate_confirmation'] = self.hybrid_modifiers['moderate_confirmation']
            elif hybrid_type == HybridSignalType.STEAM_PLAY:
                ...
                applied_modifiers['reverse_line_movement'] = self.hybrid_modifiers['reverse_line_movement']
            ...
            if confirmation_strength == ConfirmationStrength.PERFECT:
                ...
                applied_modifiers['perfect_alignment'] = self.hybrid_modifiers['perfect_alignment']
            ...
            if book_consensus >= 3:
                ...
                applied_modifiers['multi_signal_consensus'] = self.hybrid_modifiers['multi_signal_consensus']
    ...
    async def _create_hybrid_signal(self, 
        ...
        try:
            ...
            signal = UnifiedBettingSignal(
                ...
                game_id=raw_data.get('game_id', f"{raw_data.get('home_team', 'unknown')}_vs_{raw_data.get('away_team', 'unknown')}"),
                home_team=raw_data.get('home_team', 'unknown'),
                away_team=raw_data.get('away_team', 'unknown'),
                game_date=self._normalize_game_time(raw_data.get('game_datetime', processing_time)),
                recommended_side=recommended_side,
                ...
                minutes_to_game=int(self._calculate_minutes_to_game(
                    self._normalize_game_time(raw_data.get('game_datetime', processing_time)),
                    processing_time
                )),
    ...
    def _is_valid_hybrid_data(self, hybrid_data: Dict[str, Any], 
                            current_time: datetime, minutes_ahead: int) -> bool:
        """Validate hybrid opportunity data"""
        try:
            # Check required fields
            required_fields = ['hybrid_type', 'recommended_side', 'correlation', 'raw_data']
    ...
    async def _apply_hybrid_filtering(self, signals: List[UnifiedBettingSignal]) -> List[UnifiedBettingSignal]:
        ...
        def hybrid_priority(signal):
            ...
            if strategy_data.get('hybrid_type') == HybridSignalType.STRONG_CONFIRMATION.value:
                priority_score += 0.3
            
            # Perfect alignment bonus
            if strategy_data.get('confirmation_strength') == ConfirmationStrength.PERFECT.value:
                priority_score += 0.2
        ...
        for signal in signals:
            ...
            if game_key not in unique_signals or current_priority > hybrid_priority(unique_signals[game_key]):
                unique_signals[game_key] = signal
        
        # Sort by hybrid priority (highest first)
        filtered_signals = sorted(unique_signals.values(), key=hybrid_priority, reverse=True)
    ...
    def validate_strategy_data(self, raw_data: List[Dict[str, Any]]) -> bool:
        ...
        required_fields = ['line_movement', 'sharp_differential', 'money_pct', 'bet_pct']
        for row in raw_data:
