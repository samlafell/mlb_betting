...
class UnifiedTimingBasedProcessor(BaseStrategyProcessor, StrategyProcessorMixin):
    ...
    def __init__(self, repository: UnifiedRepository, config: Dict[str, Any]):
        ...
        self.closing_window_threshold = config.get('closing_window_threshold', 2.0)  # hours
        self.min_volume_threshold = config.get('min_volume_threshold', 50)
        
        # Book credibility scoring (from legacy SQL - enhanced)
        self.book_credibility = config.get('book_credibility', {
        ...
        self.context_multipliers = config.get('context_multipliers', {
        ...
        self.major_market_teams = config.get('major_market_teams', {
            'New York Yankees', 'New York Mets', 'Los Angeles Dodgers', 
            'Los Angeles Angels', 'Boston Red Sox', 'Chicago Cubs',
            'Chicago White Sox', 'Philadelphia Phillies', 'San Francisco Giants'
        })
        
        self.logger.info(f"Initialized UnifiedTimingBasedProcessor with {len(self.timing_multipliers)} timing categories")
    ...
    def get_strategy_description(self) -> str:
        ...
        return ("Advanced timing-based sharp action with 9 timing categories, "
        ...
        try:
            ...
            for split_data in splits_data:
                try:
                    ...
                    timing_metrics = await self._calculate_timing_metrics(split_data, processing_time)
                    ...
                    confidence_data = self._calculate_timing_confidence(split_data, timing_metrics)
    ...
    async def _get_timing_splits_data(self, 
                                    game_data: List[Dict[str, Any]], 
                ...
                mock_splits = [
                    {
                        ...
                        'last_updated': datetime.now(self.est) - timedelta(minutes=30),  # 30 mins ago
                        'differential': abs(game.get('money_percentage', 68.0) - game.get('bet_percentage', 42.0)),
    ...
    async def _calculate_timing_metrics(self, 
                                      split_data: Dict[str, Any], 
                                      processing_time: datetime) -> Optional[Dict[str, Any]]:
        """
        ...
        try:
            ...
            sharp_direction = self._determine_sharp_direction(money_pct, bet_pct, split_data)
            ...
            timing_credibility = book_credibility * timing_multiplier * context_multiplier
            ...
            return {
                ...
                'raw_timing_strength': min(differential / 30.0, 1.0)  # Normalize to 0-1
            }
    ...
    def _classify_game_context(self, split_data: Dict[str, Any], game_datetime: datetime) -> GameContext:
        ...
        try:
            ...
            if home_team in self.major_market_teams or away_team in self.major_market_teams:
    ...
    def _analyze_line_movement_correlation(self, split_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _determine_sharp_direction(self, money_pct: float, bet_pct: float, split_data: Dict[str, Any]) -> str:
    ...
    def _meets_timing_thresholds(self, timing_metrics: Dict[str, Any]) -> bool:
        ...
        if timing_metrics['is_ultra_late']:
            return timing_metrics['differential'] >= 5.0  # Lower threshold for ultra-late
    ...
    def _calculate_timing_confidence(self, 
        ...
        modifiers = {
            ...
            'volume_reliability': self._get_volume_reliability_multiplier(timing_metrics['volume_reliability']),
    ...
    def _get_volume_reliability_multiplier(self, volume_reliability: VolumeReliability) -> float:
    ...
    def _create_timing_signal(self, 
                            split_data: Dict[str, Any], 
    ...
    async def _apply_timing_filtering(self, signals: List[UnifiedBettingSignal]) -> List[UnifiedBettingSignal]:
        """Apply timing-specific filtering and ranking"""
        if not signals:
            return signals
        ...
        for signal in signals:
            ...
            if game_key not in unique_signals or current_priority > timing_priority(unique_signals[game_key]):
                unique_signals[game_key] = signal
        
        # Sort by timing priority (highest first)
        filtered_signals = sorted(unique_signals.values(), key=timing_priority, reverse=True)
    ...
    def validate_strategy_data(self, raw_data: List[Dict[str, Any]]) -> bool:
        ...
        required_fields = ['game_datetime', 'last_updated', 'money_percentage', 'bet_percentage', 'volume']
        for row in raw_data:
