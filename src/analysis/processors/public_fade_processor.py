...
class UnifiedPublicFadeProcessor(BaseStrategyProcessor, StrategyProcessorMixin):
    ...
    def __init__(self, repository: UnifiedRepository, config: Dict[str, Any]):
        ...
        self.min_public_consensus = config.get('min_public_consensus', 58.0)  # Lowered from 65% based on analysis
        self.heavy_consensus_threshold = config.get('heavy_consensus_threshold', 80.0)
        self.extreme_consensus_threshold = config.get('extreme_consensus_threshold', 85.0)
        self.min_books_for_consensus = config.get('min_books_for_consensus', 1)
        self.max_variance_for_strong_consensus = config.get('max_variance_for_strong_consensus', 50.0)
        
        # Fade confidence modifiers
        self.fade_modifiers = config.get('fade_modifiers', {
        ...
        self.logger.info(f"Initialized UnifiedPublicFadeProcessor with thresholds: "
    ...
    def get_strategy_description(self) -> str:
        ...
        return ("Public fade strategy: Identify heavy public betting consensus "
        ...
        try:
            ...
            for fade_data in fade_opportunities:
                try:
                    ...
                    if not self._is_valid_fade_data(fade_data, processing_time, minutes_ahead):
                        continue
    ...
    async def _get_public_betting_data(self, 
                                     game_data: List[Dict[str, Any]], 
                ...
                mock_public_data = [
                    {
                        ...
                        'home_or_over_stake_percentage': game.get('money_percentage', 82.0),  # Heavy public
                        'home_or_over_bets_percentage': game.get('bet_percentage', 75.0),    # Strong public
                        ...
                        'books_showing_consensus': game.get('books_showing_consensus', 5)
                    },
    ...
    async def _find_public_fade_opportunities(self, public_data: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """
        ...
        self.logger.info(f"Found {len(fade_opportunities)} potential fade opportunities from {len(grouped_data)} games")
        return fade_opportunities
    
    def _group_by_game_and_split_type(self, data: List[Dict[str, Any]]) -> Dict[tuple, List[Dict[str, Any]]]:
    ...
    async def _analyze_public_consensus(self, book_data: List[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
        """
        ...
        try:
            ...
            money_variance = statistics.variance(money_percentages) if len(money_percentages) > 1 else 0
            
            # Count books showing heavy consensus
            books_heavy_home = sum(1 for pct in money_percentages if pct >= self.heavy_consensus_threshold)
            books_heavy_away = sum(1 for pct in money_percentages if pct <= (100 - self.heavy_consensus_threshold))
            books_extreme_home = sum(1 for pct in money_percentages if pct >= self.extreme_consensus_threshold)
            books_extreme_away = sum(1 for pct in money_percentages if pct <= (100 - self.extreme_consensus_threshold))
            ...
            if avg_money_pct >= self.heavy_consensus_threshold or max_money_pct >= self.extreme_consensus_threshold:
                consensus_type = PublicConsensusType.HEAVY_PUBLIC_HOME
                fade_recommendation = book_data[0].get('away_team')  # Fade the public, bet away
                
                if books_extreme_home >= 1 or (books_heavy_home >= 2 and money_variance <= self.max_variance_for_strong_consensus):
                    fade_confidence = FadeConfidence.HIGH
                elif books_heavy_home >= 1:
                    fade_confidence = FadeConfidence.MEDIUM
                    
            elif avg_money_pct <= (100 - self.heavy_consensus_threshold) or min_money_pct <= (100 - self.extreme_consensus_threshold):
                consensus_type = PublicConsensusType.HEAVY_PUBLIC_AWAY
                fade_recommendation = book_data[0].get('home_team')  # Fade the public, bet home
                
                if books_extreme_away >= 1 or (books_heavy_away >= 2 and money_variance <= self.max_variance_for_strong_consensus):
                    fade_confidence = FadeConfidence.HIGH
            ...
            if consensus_type and fade_recommendation:
                return {
                    ...
                    'public_consensus_strength': max(avg_money_pct, 100 - avg_money_pct),
    ...
    def _is_significant_public_consensus(self, consensus_analysis: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if consensus_strength >= self.heavy_consensus_threshold and variance <= self.max_variance_for_strong_consensus:
                return True
    ...
    async def _calculate_fade_confidence(self, fade_data: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if consensus_strength >= self.extreme_consensus_threshold:
                ...
                applied_modifiers['extreme_consensus'] = self.fade_modifiers['extreme_consensus']
            elif consensus_strength >= self.heavy_consensus_threshold:
                base_confidence *= self.fade_modifiers['heavy_consensus']
                applied_modifiers['heavy_consensus'] = self.fade_modifiers['heavy_consensus']
            ...
            if book_count >= 3:
                ...
                applied_modifiers['multi_book_consensus'] = self.fade_modifiers['multi_book_consensus']
            ...
            applied_modifiers['contrarian_signal'] = self.fade_modifiers['contrarian_signal']
    ...
    async def _create_fade_signal(self, 
        ...
        try:
            ...
            signal = UnifiedBettingSignal(
                ...
                game_id=game_data.get('game_id', f"{game_data.get('home_team', 'unknown')}_vs_{game_data.get('away_team', 'unknown')}"),
                home_team=game_data.get('home_team', 'unknown'),
                away_team=game_data.get('away_team', 'unknown'),
                game_date=self._normalize_game_time(game_data.get('game_datetime', processing_time)),
                ...
                minutes_to_game=int(self._calculate_minutes_to_game(
                    self._normalize_game_time(game_data.get('game_datetime', processing_time)),
                    processing_time
                )),
    ...
    def _is_valid_fade_data(self, fade_data: Dict[str, Any], 
                          current_time: datetime, minutes_ahead: int) -> bool:
        """Validate fade opportunity data"""
        try:
            # Check required fields
            required_fields = ['fade_recommendation', 'public_consensus_strength', 'sample_game_data']
    ...
    async def _apply_fade_filtering(self, signals: List[UnifiedBettingSignal]) -> List[UnifiedBettingSignal]:
        ...
        for signal in signals:
            ...
            if game_key not in unique_signals or current_priority > fade_priority(unique_signals[game_key]):
                unique_signals[game_key] = signal
        
        # Sort by fade priority (highest first)
        filtered_signals = sorted(unique_signals.values(), key=fade_priority, reverse=True)
    ...
    def validate_strategy_data(self, raw_data: List[Dict[str, Any]]) -> bool:
        ...
        required_fields = ['home_or_over_stake_percentage', 'home_or_over_bets_percentage', 'source']
        for row in raw_data:
