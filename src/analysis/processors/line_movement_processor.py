...
class UnifiedLineMovementProcessor(BaseStrategyProcessor, StrategyProcessorMixin):
    ...
    def __init__(self, repository: UnifiedRepository, config: Dict[str, Any]):
        ...
        self.min_movement_threshold = config.get('min_movement_threshold', 0.5)  # Minimum line movement
        self.steam_move_threshold = config.get('steam_move_threshold', 1.0)     # Steam move threshold
        self.late_movement_hours = config.get('late_movement_hours', 3.0)       # Late movement window
        self.min_book_consensus = config.get('min_book_consensus', 3)           # Min books for consensus
        
        # Movement confidence modifiers
        self.movement_modifiers = config.get('movement_modifiers', {
        ...
        self.movement_thresholds = config.get('movement_thresholds', {
            'moneyline': 5,      # 5 cents for moneyline
            'spread': 0.5,       # 0.5 points for spread
            'total': 0.5         # 0.5 points for total
        })
        
        self.logger.info(f"Initialized UnifiedLineMovementProcessor with thresholds: "
    ...
    def get_strategy_description(self) -> str:
        ...
        return ("Line movement analysis: Detect sharp action through line movement patterns "
        ...
        try:
            ...
            for movement_data in movement_opportunities:
                try:
                    ...
                    if not self._is_valid_movement_data(movement_data, processing_time, minutes_ahead):
                        continue
                    
                    # Calculate movement confidence
                    confidence_data = await self._calculate_movement_confidence(movement_data)
    ...
    async def _get_line_movement_data(self, 
                                    game_data: List[Dict[str, Any]], 
    ...
    async def _detect_line_movements(self, movement_data: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """
        ...
        for game_key, game_movements in grouped_data.items():
            try:
                ...
                movement_analysis = await self._analyze_movement_patterns(game_movements)
                
                if movement_analysis and self._is_significant_movement(movement_analysis):
                ...
                self.logger.warning(f"Error analyzing movement patterns for game {game_key}: {e}")
                continue
        
        self.logger.info(f"Detected {len(movement_opportunities)} significant movements from {len(grouped_data)} games")
        return movement_opportunities
    
    def _group_movement_by_game(self, movement_data: List[Dict[str, Any]]) -> Dict[str, List[Dict[str, Any]]]:
        """Group movement data by game and bet type"""
        grouped = {}
    ...
    async def _analyze_movement_patterns(self, game_movements: List[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
        """
        ...
        try:
            ...
            movements = sorted(game_movements, key=lambda x: x.get('timestamp', datetime.now()))
            ...
            movement_type = self._classify_movement_type(opening_line, current_line, movement_magnitude)
            
            if not movement_type:
                return None
            
            # Calculate movement speed
            time_diff = (current_line.get('timestamp', datetime.now()) - 
                        opening_line.get('timestamp', datetime.now())).total_seconds() / 3600
            movement_speed = self._classify_movement_speed(movement_magnitude, time_diff)
            ...
            return {
                ...
                'volume_increase': current_line.get('volume', 0) - opening_line.get('volume', 0),
    ...
    def _classify_movement_type(self, opening_line: Dict[str, Any], 
                              current_line: Dict[str, Any], 
                              magnitude: float) -> Optional[MovementType]:
        """Classify the type of line movement"""
        try:
            # Check for steam move (rapid movement across books)
            if (magnitude >= self.steam_move_threshold and 
                current_line.get('book_count', 1) >= self.min_book_consensus):
            ...
            if hours_to_game <= self.late_movement_hours and magnitude >= self.min_movement_threshold:
            ...
            line_moved_home = current_line.get('home_line', 0) < opening_line.get('home_line', 0)
            public_moved_home = current_public > opening_public
            
            if line_moved_home != public_moved_home and magnitude >= self.min_movement_threshold:
    ...
    def _classify_movement_speed(self, magnitude: float, hours_elapsed: float) -> MovementSpeed:
    ...
    def _is_significant_movement(self, movement_analysis: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if movement_type == MovementType.STEAM_MOVE and book_consensus >= self.min_book_consensus:
                return True
            
            # Reverse line movements are significant
            if movement_type == MovementType.REVERSE_LINE_MOVEMENT and movement_magnitude >= 0.5:
                return True
            
            # Late movements are significant
            if movement_type == MovementType.LATE_MOVEMENT and movement_magnitude >= 0.5:
                return True
            
            # Sharp movements with multi-book consensus
            if (movement_type == MovementType.SHARP_MOVEMENT and 
                movement_magnitude >= 0.75 and 
                book_consensus >= 2):
                return True
    ...
    async def _calculate_movement_confidence(self, movement_data: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if movement_type == MovementType.REVERSE_LINE_MOVEMENT:
                ...
                applied_modifiers['reverse_line_movement'] = self.movement_modifiers['reverse_line_movement']
                ...
                applied_modifiers['late_movement'] = self.movement_modifiers['late_movement']
            elif movement_type == MovementType.SHARP_MOVEMENT:
                base_confidence *= self.movement_modifiers['sharp_movement']
                applied_modifiers['sharp_movement'] = self.movement_modifiers['sharp_movement']
            ...
            if book_consensus >= self.min_book_consensus:
                ...
                applied_modifiers['multi_book_consensus'] = self.movement_modifiers['multi_book_consensus']
            ...
            if movement_speed == MovementSpeed.RAPID:
                ...
                applied_modifiers['rapid_movement'] = self.movement_modifiers['rapid_movement']
    ...
    async def _create_movement_signal(self, 
        ...
        try:
            ...
            signal = UnifiedBettingSignal(
                ...
                game_id=current_line.get('game_id', f"{current_line.get('home_team', 'unknown')}_vs_{current_line.get('away_team', 'unknown')}"),
                home_team=current_line.get('home_team', 'unknown'),
                away_team=current_line.get('away_team', 'unknown'),
                game_date=self._normalize_game_time(current_line.get('game_datetime', processing_time)),
                recommended_side=recommended_side,
                ...
                minutes_to_game=int(self._calculate_minutes_to_game(
                    self._normalize_game_time(current_line.get('game_datetime', processing_time)),
                    processing_time
                )),
    ...
    def _is_valid_movement_data(self, movement_data: Dict[str, Any], 
                              current_time: datetime, minutes_ahead: int) -> bool:
        """Validate movement opportunity data"""
        try:
            # Check required fields
            required_fields = ['movement_type', 'recommended_side', 'movement_magnitude', 'current_line']
    ...
    async def _apply_movement_filtering(self, signals: List[UnifiedBettingSignal]) -> List[UnifiedBettingSignal]:
        ...
        def movement_priority(signal):
            ...
            if strategy_data.get('movement_type') == MovementType.REVERSE_LINE_MOVEMENT.value:
                priority_score += 0.3
        ...
        for signal in signals:
            ...
            if game_key not in unique_signals or current_priority > movement_priority(unique_signals[game_key]):
                unique_signals[game_key] = signal
        
        # Sort by movement priority (highest first)
        filtered_signals = sorted(unique_signals.values(), key=movement_priority, reverse=True)
        ...
        if len(filtered_signals) > max_signals:
            ...
            self.logger.info(f"Limited signals to top {max_signals} by movement priority")
