...
from src.data.models.unified.base import UnifiedBaseModel, TimestampedModel
...
class UnifiedBettingSignal(UnifiedBaseModel):
    ...
    recommended_stake: Optional[Decimal] = Field(None, description="Recommended stake amount")
    expected_value: Optional[float] = Field(None, description="Expected value calculation")
    odds: Optional[float] = Field(None, description="Odds at signal generation")
    
    # Strategy-Specific Data
    strategy_data: Dict[str, Any] = Field(default_factory=dict, description="Strategy-specific data")
    signal_strength: float = Field(..., ge=0.0, le=1.0, description="Raw signal strength")
    
    # Timing Information
    minutes_to_game: int = Field(..., description="Minutes until game start")
    timing_category: str = Field(..., description="Timing category (ULTRA_LATE, LATE, etc.)")
    
    # Source Information
    data_source: str = Field(..., description="Primary data source")
    book_sources: List[str] = Field(default_factory=list, description="Contributing sportsbooks")
    
    # Validation and Quality
    validation_passed: bool = Field(True, description="Whether signal passed validation")
    quality_score: float = Field(..., ge=0.0, le=1.0, description="Data quality score")
    
    # Performance Tracking
    strategy_performance_id: Optional[str] = Field(None, description="Associated performance record")
...
class UnifiedStrategyData(UnifiedBaseModel):
    ...
    thresholds: Dict[str, float] = Field(default_factory=dict, description="Strategy thresholds")
    ...
    # Recent Performance
    recent_performance: List[Dict[str, Any]] = Field(default_factory=list, description="Recent performance data")
...
class UnifiedPerformanceMetrics(UnifiedBaseModel):
    ...
    confidence_accuracy: float = Field(0.0, description="Confidence calibration accuracy")
    
    def calculate_kelly_criterion(self, win_probability: float, average_odds: float) -> float:
    ...
    def calculate_profit_factor(self) -> float:
        ...
        if self.total_profit > 0:
            gross_profit = abs(self.total_profit) + abs(self.total_profit * 0.3)  # Estimate
...
class CrossStrategyComparison(UnifiedBaseModel):
    ...
    # Rankings
    roi_ranking: List[str] = Field(default_factory=list, description="Strategies ranked by ROI")
    win_rate_ranking: List[str] = Field(default_factory=list, description="Strategies ranked by win rate")
    sharpe_ranking: List[str] = Field(default_factory=list, description="Strategies ranked by Sharpe ratio")
    ...
    # Portfolio Analysis
    portfolio_metrics: Optional[UnifiedPerformanceMetrics] = Field(
    ...
    def calculate_portfolio_optimization(self) -> Dict[str, float]:
        ...
        if total_sharpe == 0:
            return {strategy: equal_weight for strategy in self.strategies}
    ...
    def generate_comparison_report(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "best_win_rate_strategy": self.win_rate_ranking[0] if self.win_rate_ranking else None,
