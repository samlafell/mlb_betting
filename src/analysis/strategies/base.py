...
class BaseStrategyProcessor(ABC):
    ...
    def __init__(self, repository: UnifiedRepository, config: Dict[str, Any]):
        ...
        self.logger.info(f"Initialized {self.strategy_name} processor with ID: {self.strategy_id}")
    ...
    async def process(self, minutes_ahead: int, 
                     profitable_strategies: List[Any]) -> List[Any]:
        """
    ...
    async def _get_game_data_for_legacy(self, minutes_ahead: int) -> List[Dict[str, Any]]:
    ...
    def _convert_to_legacy_signals(self, unified_signals: List[UnifiedBettingSignal]) -> List[Any]:
    ...
    async def process_with_error_handling(self, 
                                        game_data: List[Dict[str, Any]], 
                                        context: Dict[str, Any]) -> List[UnifiedBettingSignal]:
        """
        ...
        try:
            self.logger.info(f"Starting {self.strategy_name} processing", 
            ...
            if signals:
                self.logger.info(f"✅ {self.strategy_name}: {len(signals)} signals generated")
            else:
    ...
    def calculate_confidence_with_modifiers(self, 
    ...
    def _calculate_base_confidence(self, signal_data: Dict[str, Any]) -> float:
        ...
        if differential >= self.thresholds['high_confidence_threshold']:
            ...
            ratio = (differential - self.thresholds['min_differential']) / \
                   (self.thresholds['high_confidence_threshold'] - self.thresholds['min_differential'])
    ...
    def _normalize_game_time(self, game_datetime: Union[str, datetime]) -> datetime:
...
class StrategyProcessorMixin:
    ...
    def create_unified_signal(self, 
                            signal_data: Dict[str, Any], 
                            confidence_data: Dict[str, Any]) -> UnifiedBettingSignal:
        ...
        return UnifiedBettingSignal(
            ...
            game_id=signal_data.get('game_id', f"{signal_data['home_team']}_vs_{signal_data['away_team']}"),
            ...
            timing_category=self._get_timing_category(signal_data.get('minutes_to_game', 0)),
