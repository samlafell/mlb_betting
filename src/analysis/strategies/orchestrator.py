...
class StrategyOrchestrator:
    ...
    def __init__(
        ...
        self.logger.info(
            f"Initialized StrategyOrchestrator with max_concurrent={self.max_concurrent_strategies}"
        )
    ...
    async def execute_strategies(
        ...
        self.logger.info(
            "Starting strategy orchestration",
            extra={
        ...
        try:
            ...
            result.successful_strategies = sum(
                1
                for r in result.strategy_results.values()
                if r.status == ExecutionStatus.COMPLETED
            )
            result.failed_strategies = (
                result.total_strategies - result.successful_strategies
            )
            ...
            self.logger.info(
                "Strategy orchestration completed",
                extra={
                    "orchestration_id": orchestration_id,
    ...
    async def _create_execution_plan(
        self, strategy_names: list[str], context: dict[str, Any]
        ...
        self.logger.debug(
            f"Created execution plan {plan_id} with {len(execution_order)} execution groups"
        )
        return plan
    ...
    async def _execute_single_strategy(
        ...
        try:
            ...
            strategy_result.error_message = (
                f"Strategy timed out after {plan.timeout_seconds} seconds"
            )
    ...
    async def execute_strategies_by_category(
        ...
        strategy_names = [
            s.strategy_name.lower().replace("processor", "") for s in strategies
        ]
        return await self.execute_strategies(strategy_names, game_data, context)

    async def execute_high_priority_strategies(
        self, game_data: list[dict[str, Any]], context: dict[str, Any] | None = None
    ) -> OrchestrationResult:
        ...
        return await self.execute_strategies(
            high_priority_strategies, game_data, context
        )
    ...
    def _calculate_performance_summary(self) -> dict[str, Any]:
        ...
        avg_execution_time = sum(
            r.execution_time_seconds for r in recent_executions
