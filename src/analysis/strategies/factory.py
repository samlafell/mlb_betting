...
class StrategyLoadingStatus:
    ...
    def __init__(self):
        ...
        self.load_end_time: Optional[datetime] = None
...
class StrategyFactory:
    ...
    def __init__(self, repository: UnifiedRepository, config: Dict[str, Any]):
        ...
        self.prefer_unified = config.get('prefer_unified', True)  # Prefer migrated processors
        self.enable_legacy_bridge = config.get('enable_legacy_bridge', True)
        
        # Initialize migration status
        self._initialize_migration_tracking()
        
        self.logger.info(f"Initialized StrategyFactory with {len(self.STRATEGY_REGISTRY)} registered strategies")
    ...
    async def _load_migrated_strategies(self) -> None:
        ...
        for strategy_name in migrated_strategies:
            try:
                ...
                self.logger.warning(f"Failed to load migrated strategy {strategy_name}: {e}")
    ...
    async def _load_legacy_bridge_strategies(self) -> None:
        ...
        for strategy_name in bridge_strategies:
            try:
                ...
                if migration_target and migration_target not in self._strategy_instances:
                    await self.load_strategy(strategy_name)
            except Exception as e:
                self.logger.warning(f"Failed to load legacy bridge strategy {strategy_name}: {e}")
    
    async def load_strategy(self, strategy_name: str) -> Optional[BaseStrategyProcessor]:
        """
        ...
        try:
            ...
            self.logger.info(f"✅ Loaded strategy: {strategy_name} ({strategy_info.get('status', 'UNKNOWN')})")
    ...
    async def _load_strategy_class(self, strategy_info: Dict[str, Any]) -> Optional[Type[BaseStrategyProcessor]]:
    ...
    def _create_strategy_config(self, strategy_name: str, strategy_info: Dict[str, Any]) -> Dict[str, Any]:
        ...
        base_config = {
            ...
            'legacy_mode': strategy_info.get('status') in ['LEGACY_BRIDGE', 'LEGACY_PENDING']
        }
        
        # Add strategy-specific configuration
        strategy_specific_config = self.config.get('strategies', {}).get(strategy_name, {})
        
        return {**base_config, **strategy_specific_config}
    
    async def _create_legacy_strategy_instance(self, 
                                             strategy_class: Type[BaseStrategyProcessor], 
                                             strategy_config: Dict[str, Any]) -> BaseStrategyProcessor:
    ...
    def _log_loading_report(self) -> None:
        ...
        migrated_count = len([s for s in self._strategy_instances.keys() 
                             if self.STRATEGY_REGISTRY[s].get('status') == 'MIGRATED'])
        legacy_bridge_count = len([s for s in self._strategy_instances.keys() 
                                  if self.STRATEGY_REGISTRY[s].get('status') == 'LEGACY_BRIDGE'])
        legacy_pending_count = len([s for s in self.STRATEGY_REGISTRY.keys() 
        ...
        if self.loading_status.loading_errors:
            self.logger.warning(f"❌ LOADING ERRORS:")
    ...
    def get_strategy_by_signal_type(self, signal_type: SignalType) -> List[BaseStrategyProcessor]:
        ...
        for strategy_name, strategy_instance in self._strategy_instances.items():
            try:
                ...
                self.logger.warning(f"Error checking signal type for {strategy_name}: {e}")
        
        return matching_strategies
    
    def get_strategy_by_category(self, category: StrategyCategory) -> List[BaseStrategyProcessor]:
    ...
    def get_migration_report(self) -> Dict[str, Any]:
        ...
        report = {
            ...
            'migration_phases': {
                '5B': [name for name, info in self.STRATEGY_REGISTRY.items() 
                      if info.get('migration_phase') == '5B'],
                '5C': [name for name, info in self.STRATEGY_REGISTRY.items() 
