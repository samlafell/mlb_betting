...
from ..models.unified.game import GameStatus, Team, UnifiedGame
from ..models.unified.odds import BettingMarket, BookType, MarketType
...
class GameRepository(BaseRepository[UnifiedGame, GameCreateSchema, GameUpdateSchema]):
    ...
    async def find_by_criteria(self, **criteria) -> list[UnifiedGame]:
        ...
        start_time = self.logger.log_operation_start(
            "find_games_by_criteria", extra={"criteria": criteria}
        )
        ...
        try:
            ...
            results = await self.connection.execute_async(
                query, *params, fetch="all", table=self.table_name
            )
            ...
            self.logger.log_operation_end(
                "find_games_by_criteria", start_time, success=False, error=e
            )
            raise
    ...
    async def find_by_date_range(
        ...
        try:
            ...
            query = self._build_select_query(
                where_clause=where_clause, order_by="game_date ASC, game_time ASC"
            )
            ...
            self.logger.log_operation_end(
                "find_games_by_date_range", start_time, success=False, error=e
            )
            raise

    async def find_by_team(
        self, team: Team, start_date: datetime | None = None
    ) -> list[UnifiedGame]:
        """
        Find games by team.
        ...
        try:
            ...
            query = self._build_select_query(
                where_clause=where_clause, order_by="game_date DESC, game_time DESC"
            )
            ...
            self.logger.log_operation_end(
                "find_games_by_team", start_time, success=False, error=e
            )
            raise
    ...
    async def find_by_external_id(
        ...
        try:
            ...
            result = await self.connection.execute_async(
                query, external_id, fetch="one", table=self.table_name
            )
            ...
            self.logger.log_operation_end(
                "find_game_by_external_id", start_time, success=False, error=e
            )
            raise
...
class OddsRepository(BaseRepository[BettingMarket, OddsCreateSchema, OddsUpdateSchema]):
    ...
    async def find_by_criteria(self, **criteria) -> list[BettingMarket]:
        ...
        start_time = self.logger.log_operation_start(
            "find_odds_by_criteria", extra={"criteria": criteria}
        )
        ...
        try:
            ...
            where_clause = " AND ".join(where_clauses) if where_clauses else None
            query = self._build_select_query(
                where_clause=where_clause, order_by="timestamp DESC"
            )
            ...
            self.logger.log_operation_end(
                "find_odds_by_criteria", start_time, success=False, error=e
            )
            raise
    ...
    async def find_by_game(
        ...
        try:
            ...
            self.logger.log_operation_end(
                "find_odds_by_game", start_time, success=False, error=e
            )
            raise
    ...
    async def get_latest_odds(
        ...
        try:
            ...
            latest_result = await self.connection.execute_async(
                latest_query, *params, fetch="one", table=self.table_name
            )
            ...
            self.logger.log_operation_end(
                "get_latest_odds", start_time, success=False, error=e
            )
            raise
    ...
    async def get_line_movement(
        ...
        try:
            ...
            query = self._build_select_query(
                where_clause=where_clause, order_by="timestamp ASC"
            )
            ...
            self.logger.log_operation_end(
                "get_line_movement", start_time, success=False, error=e
            )
            raise


class BettingAnalysisRepository(
    BaseRepository[
        BettingAnalysis, BettingAnalysisCreateSchema, BettingAnalysisUpdateSchema
    ]
):
    ...
    async def find_by_criteria(self, **criteria) -> list[BettingAnalysis]:
        ...
        start_time = self.logger.log_operation_start(
            "find_analyses_by_criteria", extra={"criteria": criteria}
        )
        ...
        try:
            ...
            where_clause = " AND ".join(where_clauses) if where_clauses else None
            query = self._build_select_query(
                where_clause=where_clause, order_by="created_at DESC"
            )
            ...
            self.logger.log_operation_end(
                "find_analyses_by_criteria", start_time, success=False, error=e
            )
            raise
    ...
    async def get_high_confidence_analyses(
        self, min_confidence: float = 0.8, limit: int | None = None
        ...
        try:
            ...
            self.logger.log_operation_end(
                "get_high_confidence_analyses", start_time, success=False, error=e
            )
            raise


class SharpDataRepository(
    BaseRepository[SharpSignal, SharpDataCreateSchema, SharpDataUpdateSchema]
):
    ...
    async def find_by_criteria(self, **criteria) -> list[SharpSignal]:
        ...
        start_time = self.logger.log_operation_start(
            "find_sharp_signals_by_criteria", extra={"criteria": criteria}
        )
        ...
        try:
            ...
            self.logger.log_operation_end(
                "find_sharp_signals_by_criteria", start_time, success=False, error=e
            )
            raise
    ...
    async def get_strong_signals(
        self, min_strength: float = 0.8, confidence_level: str | None = None
    ) -> list[SharpSignal]:
        """
        ...
        try:
            ...
            query = self._build_select_query(
                where_clause=where_clause, order_by="strength DESC, created_at DESC"
            )
            ...
            self.logger.log_operation_end(
                "get_strong_signals", start_time, success=False, error=e
            )
            raise
...
class UnifiedRepository:
    ...
    async def health_check(self) -> dict[str, Any]:
        ...
        try:
            ...
            self.logger.log_operation_end(
                "unified_repository_health_check", start_time, success=True
            )
            ...
            self.logger.log_operation_end(
                "unified_repository_health_check", start_time, success=False, error=e
            )
            raise
