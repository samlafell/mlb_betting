...
class DatabaseConnection:
        ...
        async with self._connection_lock:
            ...
            try:
                ...
                logger.log_operation_end(
                    "database_connect", start_time, success=False, error=e
                )
                raise DatabaseError(
        ...
        async with self._connection_lock:
            ...
            try:
                ...
                if self._async_pool:
                    ...
                    logger.info(
                        "Closed async connection pool", operation="async_pool_close"
                    )
                ...
                if self._sync_pool:
                    ...
                    logger.info(
                        "Closed sync connection pool", operation="sync_pool_close"
                    )

                self._is_connected = False
                logger.log_operation_end(
                    "database_disconnect", start_time, success=True
                )

            except Exception as e:
                logger.log_operation_end(
                    "database_disconnect", start_time, success=False, error=e
                )
                raise DatabaseError(
    ...
    async def get_async_connection(self) -> AsyncContextManager[asyncpg.Connection]:
        ...
        if not self._async_pool:
            raise DatabaseError(
                "Async connection pool not available", operation="get_async_connection"
            )
    ...
    def get_sync_connection(self):
        ...
        if not self._sync_pool:
            raise DatabaseError(
                "Sync connection pool not available", operation="get_sync_connection"
            )
    ...
    async def execute_async(
        self, query: str, *args, fetch: str = "none", **kwargs
    ) -> Any:
        """
        Execute async query.
        ...
        try:
            async with self.get_async_connection() as conn:
                logger.log_database_query(
                    query, params=list(args) if args else None, **kwargs
                )
                ...
                logger.log_operation_end(
                    "async_query_execution", start_time, success=True
                )
                return result

        except Exception as e:
            logger.log_operation_end(
                "async_query_execution", start_time, success=False, error=e
            )
            raise DatabaseError(
        ...
        try:
            with self.get_sync_connection() as conn:
                with conn.cursor() as cursor:
                    logger.log_database_query(query, params=params, **kwargs)
                    ...
                    logger.log_operation_end(
                        "sync_query_execution", start_time, success=True
                    )
                    return result

        except Exception as e:
            logger.log_operation_end(
                "sync_query_execution", start_time, success=False, error=e
            )
            raise DatabaseError(
    ...
    async def health_check(self) -> dict[str, Any]:
        ...
        try:
            ...
            logger.log_operation_end(
                "database_health_check", start_time, success=False, error=e
            )
            raise DatabaseError(
...
class ConnectionPool:
    ...
    def __init__(self):
        ...
        logger.info(
            "Initialized connection pool manager", operation="pool_manager_init"
        )

    def add_connection(
        self, name: str, connection_string: str, *, is_default: bool = False, **kwargs
    ) -> None:
        """
    ...
    async def connect_all(self) -> None:
        ...
        try:
            ...
            logger.log_operation_end(
                "connect_all_connections", start_time, success=True
            )

        except Exception as e:
            logger.log_operation_end(
                "connect_all_connections", start_time, success=False, error=e
            )
            raise DatabaseError(
    ...
    async def disconnect_all(self) -> None:
        ...
        try:
            ...
            logger.log_operation_end(
                "disconnect_all_connections", start_time, success=True
            )

        except Exception as e:
            logger.log_operation_end(
                "disconnect_all_connections", start_time, success=False, error=e
            )
            raise DatabaseError(
    ...
    async def health_check_all(self) -> dict[str, Any]:
        ...
        try:
            ...
            logger.log_operation_end(
                "health_check_all_connections", start_time, success=True
            )
            return health_results

        except Exception as e:
            logger.log_operation_end(
                "health_check_all_connections", start_time, success=False, error=e
            )
            raise DatabaseError(
