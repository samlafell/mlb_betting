...
class ConstraintDefinition:
    ...
    def to_sql(self) -> str:
        ...
        sql_parts = ["ALTER TABLE", self.table_name, "ADD CONSTRAINT", self.name]
...
class DatabaseSchema:
    ...
    async def initialize_migration_tracking(self) -> None:
        ...
        try:
            ...
            version_count = await self.connection.execute_async(
                f"SELECT COUNT(*) FROM {self.version_table}", fetch="one"
            )
            ...
            self.logger.log_operation_end(
                "initialize_migration_tracking", start_time, success=True
            )

        except Exception as e:
            self.logger.log_operation_end(
                "initialize_migration_tracking", start_time, success=False, error=e
            )
            raise DatabaseError(
    ...
    async def create_table(self, table_def: TableDefinition) -> None:
        ...
        start_time = self.logger.log_operation_start(
            "create_table", extra={"table_name": table_def.name}
        )
        ...
        try:
            ...
            for statement in statements:
                await self.connection.execute_async(statement, table=table_def.name)
            ...
            self.logger.log_operation_end(
                "create_table", start_time, success=False, error=e
            )
            raise DatabaseError(
    ...
    async def drop_table(self, table_name: str, cascade: bool = False) -> None:
        ...
        """
        start_time = self.logger.log_operation_start(
            "drop_table", extra={"table_name": table_name, "cascade": cascade}
        )
        ...
        try:
            ...
            self.logger.log_operation_end(
                "drop_table", start_time, success=True, extra={"table_name": table_name}
            )

        except Exception as e:
            self.logger.log_operation_end(
                "drop_table", start_time, success=False, error=e
            )
            raise DatabaseError(
    ...
    async def create_index(self, index_def: IndexDefinition) -> None:
        ...
        try:
            ...
            self.logger.log_operation_end(
                "create_index", start_time, success=False, error=e
            )
            raise DatabaseError(
    ...
    async def drop_index(self, index_name: str, cascade: bool = False) -> None:
        ...
        """
        start_time = self.logger.log_operation_start(
            "drop_index", extra={"index_name": index_name, "cascade": cascade}
        )
        ...
        try:
            ...
            self.logger.log_operation_end(
                "drop_index", start_time, success=True, extra={"index_name": index_name}
            )

        except Exception as e:
            self.logger.log_operation_end(
                "drop_index", start_time, success=False, error=e
            )
            raise DatabaseError(
    ...
    async def apply_migration(self, migration: MigrationDefinition) -> None:
        ...
        try:
            ...
            execution_time = int(
                (datetime.now() - migration_start).total_seconds() * 1000
            )
            ...
            self.logger.log_operation_end(
                "apply_migration", start_time, success=False, error=e
            )
            raise DatabaseError(
    ...
    async def rollback_migration(self, migration: MigrationDefinition) -> None:
        ...
        try:
            ...
            self.logger.log_operation_end(
                "rollback_migration", start_time, success=False, error=e
            )
            raise DatabaseError(
...
def create_schema(
    connection: DatabaseConnection, schema_name: str = "public"
) -> DatabaseSchema:
    """
...
async def migrate_schema(
    ...
    try:
        ...
        if target_version is None:
            target_version = (
                sorted_migrations[-1].version if sorted_migrations else current_version
            )
