...
from ...core.exceptions import ValidationError as UnifiedValidationError
...
class BaseRepository(ABC, Generic[T, CreateSchemaType, UpdateSchemaType]):
    ...
    def _build_update_query(
        self, data: dict[str, Any], where_clause: str, where_params: list[Any]
    ) -> tuple[str, list[Any]]:
    ...
    async def create(self, data: CreateSchemaType) -> T:
        ...
        start_time = self.logger.log_operation_start(
            "repository_create", extra={"model": self.model_class.__name__}
        )
        ...
        try:
            ...
            result = await self.connection.execute_async(
                query, *values, fetch="one", table=self.table_name
            )
            ...
            self.logger.log_operation_end(
                "repository_create", start_time, success=False, error=e
            )
            raise DatabaseError(
                f"Failed to create {self.model_class.__name__}: {str(e)}",
                operation="repository_create",
                cause=e,
                details={"table": self.table_name, "model": self.model_class.__name__},
            )

    async def get_by_id(self, record_id: str | int | UUID) -> T | None:
        """
        Get record by ID.
        ...
        """
        start_time = self.logger.log_operation_start(
            "repository_get_by_id", extra={"record_id": str(record_id)}
        )

        try:
            query = self._build_select_query(where_clause=f"{self.primary_key} = $1")

            result = await self.connection.execute_async(
                query, record_id, fetch="one", table=self.table_name
            )
            ...
            self.logger.log_operation_end(
                "repository_get_by_id", start_time, success=False, error=e
            )
            raise DatabaseError(
                f"Failed to get {self.model_class.__name__} by ID: {str(e)}",
                operation="repository_get_by_id",
                cause=e,
                details={"table": self.table_name, "record_id": str(record_id)},
            )
        ...
        *,
        ...
        try:
            ...
            results = await self.connection.execute_async(
                query, fetch="all", table=self.table_name
            )
            ...
            self.logger.log_operation_end(
                "repository_get_all", start_time, success=False, error=e
            )
            raise DatabaseError(
    ...
    async def update(
        self, record_id: str | int | UUID, data: UpdateSchemaType
    ) -> T | None:
        """
        Update record by ID.
        ...
        """
        start_time = self.logger.log_operation_start(
            "repository_update", extra={"record_id": str(record_id)}
        )
        ...
        try:
            ...
            query, values = self._build_update_query(
                update_data, f"{self.primary_key} = $1", [record_id]
            )
            ...
            self.logger.log_operation_end(
                "repository_update", start_time, success=False, error=e
            )
            raise DatabaseError(
    ...
    async def delete(self, record_id: str | int | UUID) -> bool:
        """
        Delete record by ID.
        ...
        """
        start_time = self.logger.log_operation_start(
            "repository_delete", extra={"record_id": str(record_id)}
        )
        ...
        try:
            ...
            result = await self.connection.execute_async(
                query, record_id, table=self.table_name
            )
            ...
            self.logger.log_operation_end(
                "repository_delete", start_time, success=False, error=e
            )
            raise DatabaseError(
    ...
    async def count(self, where_clause: str | None = None) -> int:
        ...
        try:
            ...
            result = await self.connection.execute_async(
                query, fetch="one", table=self.table_name
            )

            count = result[0] if result else 0

            self.logger.log_operation_end(
                "repository_count", start_time, success=True, extra={"count": count}
            )
            ...
            self.logger.log_operation_end(
                "repository_count", start_time, success=False, error=e
            )
            raise DatabaseError(
    ...
    async def exists(self, record_id: str | int | UUID) -> bool:
        """
        ...
        """
        start_time = self.logger.log_operation_start(
            "repository_exists", extra={"record_id": str(record_id)}
        )

        try:
            query = (
                f"SELECT 1 FROM {self.table_name} WHERE {self.primary_key} = $1 LIMIT 1"
            )
            ...
            self.logger.log_operation_end(
                "repository_exists", start_time, success=False, error=e
            )
            raise DatabaseError(
...
class TransactionManager:
    ...
    async def __aenter__(self):
        ...
        try:
            ...
            self.logger.log_operation_end(
                "transaction_begin", start_time, success=False, error=e
            )
    ...
    async def __aexit__(self, exc_type, exc_val, exc_tb):
        ...
        if exc_type is None:
            ...
            try:
                ...
                self.logger.log_operation_end(
                    "transaction_commit", start_time, success=True
                )
            except Exception as e:
                self.logger.log_operation_end(
                    "transaction_commit", start_time, success=False, error=e
                )
                raise TransactionError(
            ...
            try:
                ...
                self.logger.log_operation_end(
                    "transaction_rollback", start_time, success=True
                )
            except Exception as e:
                self.logger.log_operation_end(
                    "transaction_rollback", start_time, success=False, error=e
                )
