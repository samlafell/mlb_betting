...
from ..models.unified.actionnetwork import ActionNetworkHistoricalData, ActionNetworkHistoricalEntry
from .base import BaseRepository, DatabaseError, QueryError, TransactionManager
...
class ActionNetworkRepository:
    ...
    async def _get_mlb_game_id(self, home_team: str, away_team: str, game_datetime: datetime) -> Optional[str]:
        """
        ...
        try:
            ...
            # Find matching game
            for game in games:
                if (game.home_team == home_normalized and game.away_team == away_normalized):
                    self.logger.info("Found MLB game match",
            ...
            self.logger.warning("No MLB game match found",
                              home_team=home_team,
                              away_team=away_team,
                              game_date=game_date,
    ...
    async def _ensure_game_exists(self, conn, historical_data: ActionNetworkHistoricalData) -> int:
        """
        ...
        existing_game = await conn.fetchval("""
            ...
            LIMIT 1
        """, home_abbr, away_abbr, historical_data.game_datetime)
        
        if existing_game:
            self.logger.info("Found existing game",
            ...
            """, historical_data.game_id, existing_game)
        ...
        self.logger.info("Creating new game record",
        ...
        self.logger.info("Created game record",
                       new_game_id=game_id,
                       action_game_id=historical_data.game_id,
                       mlb_game_id=mlb_game_id)
    ...
    async def save_historical_data(self, historical_data: ActionNetworkHistoricalData) -> Dict[str, Any]:
        """
        ...
        try:
            async with self.connection.get_async_connection() as conn:
                ...
                for entry in historical_data.historical_entries:
                    saved_count = await self._save_historical_entry(conn, game_id, historical_data, entry)
                ...
                self.logger.info("Historical data saved to core_betting", 
    ...
    async def _ensure_sportsbooks_exist(self, conn) -> None:
        ...
        for book_id, book_name in self.action_network_books.items():
            ...
            self.logger.info("Ensuring sportsbook exists",
                           book_id=book_id,
                           book_name=book_name,
            ...
            """, book_name, book_name, abbreviation)

    async def _save_historical_entry(self, conn, game_id: int, historical_data: ActionNetworkHistoricalData,
                                   entry: ActionNetworkHistoricalEntry) -> int:
        ...
        self.logger.info("Processing historical entry",
                       game_id=historical_data.game_id,
        ...
        for market_name in ['moneyline', 'spread', 'total']:
            if market_name in event_data and event_data[market_name]:
                ...
                self.logger.info("Found market data",
                               market_name=market_name,
                               data_type=type(market_data).__name__,
                               data_length=len(market_data) if isinstance(market_data, list) else 'N/A')
                
                if isinstance(market_data, list):
                    if market_name == 'moneyline':
                        saved_count += await self._save_moneyline_data(conn, game_id, historical_data, entry, market_data)
                    elif market_name == 'spread':
                        saved_count += await self._save_spread_data(conn, game_id, historical_data, entry, market_data)
                    elif market_name == 'total':
                        saved_count += await self._save_total_data(conn, game_id, historical_data, entry, market_data)
                
        self.logger.info("Completed historical entry processing",
                       game_id=historical_data.game_id,
                       saved_count=saved_count)
        return saved_count

    async def _save_moneyline_data(self, conn, game_id: int, historical_data: ActionNetworkHistoricalData,
                                 entry: ActionNetworkHistoricalEntry, market_data: List[Dict]) -> int:
        """Save moneyline data to core_betting.betting_lines_moneyline."""
        saved_count = 0
        
        self.logger.info("Processing moneyline data",
                       game_id=historical_data.game_id,
        ...
        for book_id, sides_data in sportsbook_data.items():
            ...
            home_bet_info = sides_data['home'].get('bet_info', {}) if sides_data['home'] else {}
            away_bet_info = sides_data['away'].get('bet_info', {}) if sides_data['away'] else {}
            ...
            self.logger.info("Inserting moneyline data",
                           game_id=historical_data.game_id,
            ...
            try:
                ...
                self.logger.info("Successfully inserted moneyline data",
                               game_id=historical_data.game_id,
                               sportsbook=sportsbook_name)
            except Exception as e:
                self.logger.error("Failed to insert moneyline data",
                                game_id=historical_data.game_id,
                                sportsbook=sportsbook_name,
                                error=str(e),
                                home_abbr=home_abbr,
    ...
    async def _save_spread_data(self, conn, game_id: int, historical_data: ActionNetworkHistoricalData,
                              entry: ActionNetworkHistoricalEntry, market_data: List[Dict]) -> int:
    ...
    async def _save_total_data(self, conn, game_id: int, historical_data: ActionNetworkHistoricalData,
                             entry: ActionNetworkHistoricalEntry, market_data: List[Dict]) -> int:
    ...
    async def _update_extraction_log(self, conn, game_id: int, home_team: str, away_team: str,
                                   game_datetime: datetime, total_lines_extracted: int,
                                   extraction_status: str, error_message: Optional[str] = None,
                                   history_url: Optional[str] = None) -> None:
        ...
        await conn.execute("""
            ...
            DO UPDATE SET
                ...
                error_message = $7,
                history_url = $8
        """, game_id, home_team, away_team, game_datetime, total_lines_extracted,
             extraction_status, error_message, history_url)

    async def get_last_extraction_time(self, game_id: int) -> Optional[datetime]:
        ...
        try:
            ...
            self.logger.error("Failed to get last extraction time", game_id=game_id, error=str(e))
            return None

    async def get_new_lines_since_last_extraction(self, game_id: int, book_id: Optional[int] = None,
        ...
        try:
            ...
            if not last_extraction:
                last_extraction = datetime.now() - timedelta(days=7)  # Default to 7 days ago
            ...
            async with self.connection.get_async_connection() as conn:
                ...
                if not market_type or market_type == 'moneyline':
                    ...
                    if book_id:
                        ...
                        rows = await conn.fetch(query, game_id, last_extraction, book_id)
                    else:
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        try:
            async with self.connection.get_async_connection() as conn:
                ...
                moneyline_count = await conn.fetchval("SELECT COUNT(*) FROM core_betting.betting_lines_moneyline WHERE source = 'ACTION_NETWORK'")
                spread_count = await conn.fetchval("SELECT COUNT(*) FROM core_betting.betting_lines_spreads WHERE source = 'ACTION_NETWORK'")
                total_count = await conn.fetchval("SELECT COUNT(*) FROM core_betting.betting_lines_totals WHERE source = 'ACTION_NETWORK'")
                sportsbook_count = await conn.fetchval("SELECT COUNT(*) FROM core_betting.sportsbooks WHERE name IN ('DraftKings', 'FanDuel', 'BetMGM', 'Caesars', 'BetRivers', 'ESPN BET')")
