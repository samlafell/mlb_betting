...
class RawDataRecord(DataRecord):
    ...
    sportsbook_key: Optional[str] = None
    ...
    game_pk: Optional[int] = None  # MLB Stats API
...
class RawZoneConsolidatedProcessor(BaseZoneProcessor):
    ...
    async def process_record(
        ...
        **kwargs
        ...
        try:
            ...
            logger.debug(f"Processed raw record: {raw_record.external_id} from {raw_record.source}")
            return raw_record
        ...
        try:
            if isinstance(record.raw_data, str):
                try:
                    ...
                    logger.error(f"Invalid JSON in raw_data for record {record.external_id}: {e}")
    ...
    async def _extract_action_network_metadata(self, record: RawDataRecord, raw_data: Dict) -> RawDataRecord:
        """Extract Action Network specific metadata."""
        # Game information
        record.game_external_id = str(raw_data.get('id', raw_data.get('game_id', record.external_id)))
    ...
    async def _extract_sbd_metadata(self, record: RawDataRecord, raw_data: Dict) -> RawDataRecord:
    ...
    async def _extract_vsin_metadata(self, record: RawDataRecord, raw_data: Dict) -> RawDataRecord:
    ...
    async def _extract_mlb_metadata(self, record: RawDataRecord, raw_data: Dict) -> RawDataRecord:
        ...
        if 'gameDate' in raw_data:
            try:
                record.game_datetime = datetime.fromisoformat(raw_data['gameDate'].replace('Z', '+00:00'))
        ...
        try:
            ...
            async with get_connection() as connection:
                for table_key, table_records in records_by_table.items():
                    await self._insert_records_by_source(connection, table_key, table_records)
    ...
    async def _insert_action_network_history(self, connection, records: List[DataRecord]) -> None:
    ...
    async def _insert_mlb_stats_api_games(self, connection, records: List[DataRecord]) -> None:
        ...
        if record.raw_data and isinstance(record.raw_data, dict):
            ...
            if data_type == 'game':
                ...
                if not any(market in raw_data for market in market_types):
                    record.validation_errors = record.validation_errors or []
                    record.validation_errors.append("Missing market data (moneyline, spread, or total)")
                    return False
