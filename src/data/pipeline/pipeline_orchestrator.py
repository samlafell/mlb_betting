...
from . import raw_zone, staging_zone
...
class DataPipelineOrchestrator:
    ...
    def _initialize_zones(self):
        ...
        try:
            ...
            if self.settings.pipeline.zones.raw_enabled:
                ...
                self.zones[ZoneType.RAW] = ZoneFactory.create_zone(ZoneType.RAW, raw_config)
            ...
            if self.settings.pipeline.zones.staging_enabled:
                ...
                self.zones[ZoneType.STAGING] = ZoneFactory.create_zone(ZoneType.STAGING, staging_config)
    ...
    async def run_full_pipeline(
        ...
        try:
            ...
            if (ZoneType.STAGING in self.zones and 
                raw_result.records_successful > 0):
                ...
                raw_records = await self._get_records_for_next_zone(
                    ZoneType.RAW, 
                    raw_result.records_successful
                )
                
                staging_result = await self._process_zone(ZoneType.STAGING, raw_records)
                execution.metrics.zone_metrics[ZoneType.STAGING] = staging_result.model_dump()
                
                # Stage 3: CURATED processing (if enabled and STAGING successful)
                if (ZoneType.CURATED in self.zones and 
                    staging_result.records_successful > 0):
                    ...
                    staging_records = await self._get_records_for_next_zone(
                        ZoneType.STAGING,
                        staging_result.records_successful
                    )
                    
                    curated_result = await self._process_zone(ZoneType.CURATED, staging_records)
                    execution.metrics.zone_metrics[ZoneType.CURATED] = curated_result.model_dump()
    ...
    async def run_single_zone_pipeline(
        ...
        try:
            ...
            logger.info(f"Starting {zone_type} pipeline execution {execution.execution_id}")
            ...
            logger.error(f"Zone pipeline execution {execution.execution_id} failed: {e}")
            execution.status = ProcessingStatus.FAILED
            execution.errors.append(str(e))
            return await self._complete_execution(execution)
    ...
    def _determine_execution_status(self, execution: PipelineExecution) -> ProcessingStatus:
    ...
    async def _complete_execution(self, execution: PipelineExecution) -> PipelineExecution:
    ...
    async def calculate_overall_metrics(self, execution: PipelineExecution) -> Dict[str, Any]:
            ...
            for zone_type, zone_result in execution.metrics.zone_metrics.items():
                if isinstance(zone_result, dict):
                    ...
                    overall_metrics["zone_breakdown"][zone_type.value] = {
                        ...
                        "processing_time": zone_result.get('processing_time', 0.0),
                        "quality_score": getattr(quality_metrics, 'quality_score', 0.0) if quality_metrics else 0.0
                    }
            
            if quality_scores:
                overall_metrics["overall_quality_score"] = sum(quality_scores) / len(quality_scores)
