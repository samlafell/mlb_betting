        ...
        try:
            ...
            if "spread" in raw_data:
                ...
                if isinstance(spread_data, dict):
                    record.line_value = self._safe_decimal_convert(
                        spread_data.get("point", spread_data.get("line"))
                    )
                    record.odds_american = spread_data.get(
                        "price", spread_data.get("odds")
                    )
            ...
            if "total" in raw_data:
                ...
                if isinstance(total_data, dict):
                    record.line_value = self._safe_decimal_convert(
                        total_data.get("point", total_data.get("line"))
                    )
    ...
    async def _insert_spreads(self, connection, records: list[DataRecord]) -> None:
        ...
        for record in records:
            await connection.execute(
                ...
                getattr(record, "odds_american", None),
                getattr(
                    record, "home_team_normalized", None
                ),  # Assuming home is favorite for now
                getattr(record, "away_team_normalized", None),
                getattr(record, "quality_score", None),
                record.validation_status.value
                if record.validation_status
