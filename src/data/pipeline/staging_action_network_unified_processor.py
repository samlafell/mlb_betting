...
class ActionNetworkUnifiedStagingProcessor:
    ...
    async def process_all_sources(self, limit: int = 100) -> dict[str, Any]:
        ...
        """
        logger.info(
            "Starting unified Action Network processing",
        ...
        try:
            ...
            results["combined_totals"]["historical_records_valid"] = odds_results.get(
                "historical_records_valid", 0
            ) + history_results.get("historical_records_valid", 0)
            results["combined_totals"]["mlb_games_resolved"] = odds_results.get(
                "mlb_games_resolved", 0
            ) + history_results.get("mlb_games_resolved", 0)

            results["processing_batch_id"] = self.processing_batch_id
            results["structure_type"] = (
                "Unified - complete temporal records from both sources"
            )

            logger.info(
                "Unified Action Network processing completed",
                **results["combined_totals"],
            )
            return results
    ...
    async def process_history_table(self, limit: int = 10) -> dict[str, Any]:
        ...
        try:
            ...
            return await self._process_records(
                conn, raw_history, "action_network_history"
            )
    ...
    async def _process_records(
        self, conn: asyncpg.Connection, raw_records: list[dict], source_table: str
    ) -> dict[str, Any]:
        ...
        for raw_record in raw_records:
            try:
                ...
                if source_table == "action_network_odds":
                    ...
                    historical_records = await self._extract_from_history(
                        raw_record, conn
                    )
                ...
                logger.error(
                    f"Error processing {source_table} record {raw_record['id']}: {e}"
                )
                continue
        ...
        for side_data in market_data:
            ...
            for hist_entry in history:
                ...
                final_line_value = (
                    ...
                    else (hist_line_value or line_value)
                )
