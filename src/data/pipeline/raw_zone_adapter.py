...
from .raw_zone import RawZoneProcessor, RawDataRecord
from .zone_interface import create_zone_config, ZoneType, ProcessingResult
        ...
        try:
            ...
            for game_data in games_data:
                ...
                record = RawDataRecord(
                    ...
                    game_external_id=str(game_data.get('id', game_data.get('game_id', ''))),
                    raw_data=game_data,
                    data_type='game',
                    endpoint_url=source_info.get('endpoint_url') if source_info else None,
                    response_status=source_info.get('response_status', 200) if source_info else 200,
            ...
            logger.info(f"Stored {result.records_successful} Action Network games to RAW zone")
            return result
    ...
    async def store_action_network_odds(
        ...
        try:
            ...
            for odds_entry in odds_data:
                ...
                sportsbook_key = odds_entry.get('sportsbook_key', odds_entry.get('key', ''))
            ...
            logger.info(f"Stored {result.records_successful} Action Network odds to RAW zone")
            return result
    ...
    async def store_sbd_betting_splits(
        ...
        try:
            ...
            for split_data in splits_data:
                # Create raw data record
                record = RawDataRecord(
                    external_id=split_data.get('matchup_id', f"sbd_{datetime.now().isoformat()}"),
                    source='sbd',
                    game_external_id=split_data.get('game_id'),
                    raw_data=split_data,
                    data_type='betting_splits',
                    api_endpoint=source_info.get('api_endpoint') if source_info else None,
                    collected_at=datetime.now(timezone.utc)
                )
                raw_records.append(record)
            ...
            logger.info(f"Stored {result.records_successful} SBD betting splits to RAW zone")
            return result
    ...
    async def store_vsin_data(
        ...
        try:
            ...
            for data_entry in vsin_data:
                # Create raw data record
                record = RawDataRecord(
                    external_id=data_entry.get('id', f"vsin_{datetime.now().isoformat()}"),
                    source='vsin',
            ...
            logger.info(f"Stored {result.records_successful} VSIN {data_type} records to RAW zone")
            return result
        ...
        try:
            ...
            for data_entry in mlb_data:
                ...
                record = RawDataRecord(
                    ...
                    game_external_id=str(data_entry.get('gamePk', data_entry.get('id', ''))),
                    raw_data=data_entry,
                    api_endpoint=source_info.get('api_endpoint') if source_info else None,
            ...
            logger.info(f"Stored {result.records_successful} MLB Stats API records to RAW zone")
            return result
    ...
    async def store_betting_lines(
        ...
        try:
            ...
            for line_data in lines_data:
                # Create raw data record
                record = RawDataRecord(
                    external_id=line_data.get('id', f"{source}_{bet_type}_{datetime.now().isoformat()}"),
                    source=source,
            ...
            logger.info(f"Stored {result.records_successful} {bet_type} lines from {source} to RAW zone")
            return result
    ...
    async def store_line_movements(
        ...
        try:
            ...
            for movement_data in movements_data:
                # Create raw data record
                record = RawDataRecord(
                    external_id=movement_data.get('id', f"{source}_movement_{datetime.now().isoformat()}"),
                    source=source,
            ...
            logger.info(f"Stored {result.records_successful} line movements from {source} to RAW zone")
            return result
