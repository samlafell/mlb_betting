...
from ...core.datetime_utils import safe_game_datetime_parse, now_est
...
class HistoricalOddsRecord(BaseModel):
    ...
    line_status: Optional[str] = "normal"   # opener, normal, suspended
...
class ActionNetworkHistoricalProcessor:
    ...
    async def initialize(self):
        ...
        logger.info("ActionNetworkHistoricalProcessor initialized with MLB Stats API integration")
    ...
    async def process_historical_odds(self, limit: int = 100) -> Dict[str, Any]:
        ...
        """
        logger.info(f"Starting historical odds processing", 
        ...
        try:
            ...
            logger.info(f"Found {len(raw_odds)} unprocessed odds records for historical processing")
            ...
            for raw_odds_record in raw_odds:
                try:
                    ...
                    historical_records = await self._extract_historical_records(raw_odds_record, conn)
                    ...
                    logger.error(f"Error processing historical odds record {raw_odds_record['id']}: {e}")
                    continue
    ...
    async def _extract_historical_records(self, raw_odds: Dict, conn: asyncpg.Connection) -> List[HistoricalOddsRecord]:
                ...
                for side_data in market_data:
                    ...
                    for hist_entry in history:
                        ...
                        try:
                            ...
                            logger.warning(f"Invalid timestamp in history: {updated_at_str}, error: {e}")
                            continue
    ...
    async def _resolve_mlb_game_id(self, record: HistoricalOddsRecord, conn: asyncpg.Connection) -> Optional[str]:
        ...
        try:
            ...
            if resolution_result.mlb_game_id:
                ...
                logger.debug(f"Resolved MLB game ID for {record.external_game_id}: {resolution_result.mlb_game_id}")
                return resolution_result.mlb_game_id
            
            return None
            
        except Exception as e:
            logger.error(f"Error resolving MLB game ID for {record.external_game_id}: {e}")
            return None
