...
class UnifiedStagingProcessor(BaseZoneProcessor):
    ...
    async def _populate_source_attribution(self, record: UnifiedStagingRecord, **kwargs):
        ...
        try:
            # Extract from kwargs or infer from context
            record.data_source = kwargs.get('data_source', 'action_network')  # Default assumption
            record.source_collector = kwargs.get('source_collector', 'consolidated_action_network_collector')
            
            # Try to infer from record metadata if not provided
            if hasattr(record, 'raw_data') and isinstance(record.raw_data, dict):
                metadata = record.raw_data.get('_metadata', {})
                if metadata:
                    record.data_source = metadata.get('source', record.data_source)
                    record.source_collector = metadata.get('collector', record.source_collector)
    ...
    async def _populate_team_names(self, record: UnifiedStagingRecord):
        ...
        try:
            ...
            # Strategy 1: Query Action Network games table for team names and metadata
            team_info = await self._query_action_network_game_data(external_game_id)
            ...
            if team_ids:
                team_names = await self._resolve_team_ids_to_names(team_ids)
    ...
    async def _add_data_lineage(self, record: UnifiedStagingRecord, source_record: DataRecord):
        ...
        try:
            ...
            record.raw_data_table = "raw_data.action_network_odds"  # Default, could be parameterized
    ...
    async def store_unified_records(self, records: List[UnifiedStagingRecord]) -> None:
        """Store unified records to the new unified staging table."""
        try:
                ...
                async with connection.transaction():
                    ...
                    for record in records:
                        try:
                            ...
                            logger.error(f"Error inserting unified record {record.external_game_id}: {e}")
                            raise
            
            logger.info(f"Successfully stored {len(records)} unified staging records")
            
        except Exception as e:
            logger.error(f"Error storing unified records: {e}")
            raise
    ...
    async def _query_action_network_game_data(self, external_game_id: str) -> Optional[Dict[str, Any]]:
        """Query Action Network games table for team names and metadata."""
        try:
            from ...data.database.connection import get_connection
    ...
    async def _resolve_team_ids_to_names(self, team_ids: Dict[str, int]) -> Optional[Dict[str, str]]:
        """Resolve Action Network team IDs to team names."""
        try:
            from ...data.database.connection import get_connection
            
            # Comprehensive Action Network team ID mapping
            ...
            home_team = action_network_team_mapping.get(home_team_id)
            away_team = action_network_team_mapping.get(away_team_id)
            
            if home_team and away_team:
                logger.debug(f"✅ Resolved team IDs via mapping: {home_team_id}→{home_team}, {away_team_id}→{away_team}")
                return {
                    'home_team': home_team,
                    'away_team': away_team
                }
            
            # Try to find team IDs in other games in the database
            async with get_connection() as connection:
                query = """
                SELECT DISTINCT 
                ...
                JOIN raw_data.action_network_games g ON r.external_game_id = g.external_game_id
                ...
                if rows:
                    ...
                    if home_team and away_team:
                        logger.debug(f"✅ Resolved team IDs via database lookup: {home_team_id}→{home_team}, {away_team_id}→{away_team}")
                        return {
                            'home_team': home_team,
                            'away_team': away_team
                        }
