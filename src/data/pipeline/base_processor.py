...
class BaseZoneProcessor(DataZone):
    ...
    async def process_batch(
        ...
        result = ProcessingResult(
            status=ProcessingStatus.IN_PROGRESS, records_processed=len(records)
        )
    ...
    def _calculate_batch_quality_score(self, records: list[DataRecord]) -> float:
        ...
        total_score = sum(record.quality_score or 0.0 for record in records)
    ...
    async def retry_failed_records(
        self, failed_records: list[DataRecord], max_retries: int | None = None
    ) -> ProcessingResult:
        """
        ...
        for attempt in range(max_retries):
            ...
            logger.info(
                f"Retry attempt {attempt + 1}/{max_retries} for {len(failed_records)} records"
            )
