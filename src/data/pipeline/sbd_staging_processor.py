...
class SBDBettingSplitRecord(BaseModel):
    ...
    public_bet_percentage: Decimal | None = None
    public_money_percentage: Decimal | None = None
    sharp_bet_percentage: Decimal | None = None
    raw_sportsbook_id: str | None = None
    data_source: str = "sbd"
...
class SBDStagingProcessor(BaseZoneProcessor):
    ...
    async def process_sbd_raw_records(
    ...
    async def _process_sbd_batch(
        self, db_connection, raw_records: list[dict[str, Any]]
    ) -> ProcessingResult:
                ...
                async with connection.transaction():
                    for raw_record in raw_records:
                        try:
                            ...
                            betting_record = self._extract_betting_split_data(
                                raw_response
                            )

                            if not game_record or not betting_record:
                                errors.append(
                                    f"Failed to extract data from record {raw_record['id']}"
                                )
                                processed += 1
                                continue
                            ...
                            sharp_signal = self._detect_sharp_action_signal(
                                betting_record
                            )
                            if sharp_signal:
                                sharp_signal.game_id = game_id
                                await self._store_sharp_action_signal(
                                    connection, sharp_signal
                                )
                            ...
                            logger.error(
                                f"Error processing SBD record {raw_record['id']}: {e}"
                            )
    ...
    def _extract_betting_split_data(
        ...
        try:
            ...
            normalized_sportsbook = self._normalize_sportsbook_name(
                sportsbook, sportsbook_id
            )

            # Extract percentages
            public_bet_pct = self._safe_decimal_convert(
                betting_record.get("home_bets_percentage")
            )
            public_money_pct = self._safe_decimal_convert(
                betting_record.get("home_money_percentage")
            )

            # Calculate sharp action percentage
            sharp_bet_pct = self._calculate_sharp_percentage(
    ...
    def _normalize_sportsbook_name(
        self, sportsbook: str, sportsbook_id: str | None = None
    ) -> str:
    ...
    def _calculate_sharp_percentage(
        self, public_bet_pct: Decimal | None, public_money_pct: Decimal | None
    ) -> Decimal | None:
    ...
    def _detect_sharp_action_signal(
        self, betting_record: SBDBettingSplitRecord
    ) -> SBDSharpActionSignal | None:
        """Detect and create sharp action signal if significant discrepancy exists."""
        try:
            if (
                not betting_record.public_bet_percentage
                or not betting_record.public_money_percentage
                or not betting_record.sharp_bet_percentage
            ):
                return None
    ...
    async def _get_or_create_game(self, connection, game_record: SBDGameRecord) -> int:
    ...
    async def _store_betting_split(
        self, connection, betting_record: SBDBettingSplitRecord
    ) -> None:
    ...
    async def _store_sharp_action_signal(
        self, connection, signal: SBDSharpActionSignal
    ) -> None:
    ...
    async def _mark_raw_records_processed(
        self, db_connection, raw_ids: list[int]
    ) -> None:
        """Mark raw records as processed."""
        try:
            async with db_connection.get_async_connection() as connection:
        ...
        try:
            ...
            async with db_connection.get_async_connection() as connection:
                ...
                return {
                    ...
                    "processing_rate": f"{raw_processed}/{raw_total}"
                    if raw_total > 0
                    else "0/0",
                }
...
async def main():
    ...
    from .zone_interface import create_zone_config
