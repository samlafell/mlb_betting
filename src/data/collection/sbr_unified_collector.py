...
class SBRUnifiedCollector(BaseCollector):
    ...
    def __init__(self, config=None):
        ...
        self.source_value = self.source.value if hasattr(self.source, 'value') else str(self.source)
    ...
    async def collect_data(self, request: CollectionRequest) -> list[dict[str, Any]]:
        ...
        try:
            ...
            if self.browser and not request.additional_params.get("keep_browser_alive", False):
    ...
    async def _collect_games_for_date(self, target_date: date) -> list[dict[str, Any]]:
        ...
        try:
            ...
            try:
                ...
                self.logger.info("Navigating to SBR MLB page", url=mlb_url, target_date=date_str)

                await page.goto(mlb_url, wait_until="domcontentloaded", timeout=self.page_timeout)
            ...
            self.logger.error("Error collecting games for date", date=target_date, error=str(e))
            return []

    async def _extract_games_from_page(self, page: Page, target_date: date) -> list[dict[str, Any]]:
        """Extract game data from SBR page using JavaScript evaluation."""
        try:
            # Wait for the betting table to load
            await page.wait_for_selector("table, .betting-table, [data-testid*='odds'], .odds-table", timeout=10000)
            ...
            for i, game_data in enumerate(games_data):
                try:
                    processed_game = await self._process_sbr_game_data(game_data, target_date, i)
                    if processed_game:
            ...
            self.logger.info(f"Processed {len(processed_games)} games from {len(games_data)} extracted")
    ...
    async def _process_sbr_game_data(self, game_data: dict[str, Any], target_date: date, index: int) -> dict[str, Any] | None:
        ...
        try:
            ...
            self.logger.error("Error processing SBR game data", game_index=index, error=str(e))
            return None
    ...
    def _extract_betting_markets(self, game_data: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            positive_odds = [n for n in numeric_values if n > 0 and n < 1000]  # Typical range for +odds
            negative_odds = [n for n in numeric_values if n < 0 and n > -1000]  # Typical range for -odds
            ...
            if spread_candidates:
                markets["spread"] = {
                    "line": float(spread_candidates[0]) / 10 if abs(spread_candidates[0]) > 10 else float(spread_candidates[0]),
            ...
            total_candidates = [n for n in numeric_values if 50 <= n <= 150]  # Expecting decimal representation
            if total_candidates:
    ...
    async def test_connection(self) -> bool:
        ...
        try:
            ...
            try:
                ...
                self.logger.info("SBR connection test completed", success=success, title=title)
                return success
