...
class VSINUnifiedCollector(BaseCollector):
    ...
    def build_vsin_url(self, sport: str, sportsbook: str = "dk") -> str:
        ...
        if not sport_path:
            ...
            raise ValueError(
                f"Sport '{sport}' not supported. Available sports: {supported_sports}"
            )
    ...
    async def collect_data(self, request: CollectionRequest) -> list[dict[str, Any]]:
        ...
        try:
            ...
            if live_data:
                self.logger.info(
                    f"Successfully collected {len(live_data)} live VSIN records"
                )
                return live_data
            else:
    ...
    def _collect_vsin_data_sync(self, sport: str, **kwargs) -> list[dict[str, Any]]:
        ...
        try:
            ...
            sportsbooks_to_collect = (
                [sportsbook] if sportsbook != "all" else ["dk", "circa", "fanduel"]
            )
            ...
            for book in sportsbooks_to_collect:
                try:
                    ...
                    self.logger.info(
                        f"Collecting VSIN data from {book.upper()}", url=url
                    )
                    ...
                    if html_content:
                        ...
                        if parsed_data:
                            ...
                            self.logger.info(
                                f"Successfully parsed {len(parsed_data)} records from {book.upper()}"
                            )
                        else:
                            self.logger.warning(
                                f"No data found in HTML for {book.upper()}"
                            )
                    else:
                        self.logger.warning(
                            f"Failed to fetch HTML content for {book.upper()}"
                        )

                except Exception as e:
                    self.logger.error(
                        f"Error collecting from {book.upper()}", error=str(e)
                    )
                    continue
    ...
    def _parse_vsin_html(
        self, html_content: str, sport: str, sportsbook: str
    ) -> list[dict[str, Any]]:
        """
    ...
    def _extract_main_content(self, html_content: str) -> str:
        ...
        try:
            ...
            for selector in selectors:
                main_content_div = soup.find("div", selector) or soup.find(
                    "table", selector
                )
                if main_content_div:
    ...
    def _parse_game_row(
        self, cells: list, sport: str, sportsbook: str
    ) -> dict[str, Any] | None:
        """
        ...
        try:
            ...
            game_record = {
                ...
                "spread_line": combined_data.get("away_rl")
                or combined_data.get("away_spread"),
                "raw_response": {
                    "away_team": away_team,
                    "home_team": home_team,
                    "sportsbook": sportsbook,
    ...
    def _extract_team_names(self, team_cell) -> tuple[str, str]:
        ...
        try:
            ...
            for item in teams:
                ...
                clean_item = re.sub(
                    r"History|VSiN Pro Picks|\d+ VSiN Pro Picks", "", clean_item
                ).strip()
        ...
        try:
            ...
            if len(cells) > ml_col:
                ...
                if len(odds_values) >= 2:
                    ...
                    away_odds = (
                        odds_values[0]
                        if odds_values[0].startswith(("+", "-"))
                        else f"+{odds_values[0]}"
                    )
                    home_odds = (
                        odds_values[1]
                        if odds_values[1].startswith(("+", "-"))
                        else odds_values[1]
                    )
                    data.update({"away_ml": away_odds, "home_ml": home_odds})
            ...
            if len(cells) > ml_col + 1:
                ...
                if len(handle_values) >= 2:
                    data.update(
                        {
                            "away_ml_handle_pct": float(
                                handle_values[0].replace("%", "")
                            ),
                            "home_ml_handle_pct": float(
                                handle_values[1].replace("%", "")
                            ),
                        }
                    )
    ...
    def _extract_spread_data(
        self, cells: list, spread_col: int, sport: str
    ) -> dict[str, Any]:
        """
        ...
        try:
            ...
            if len(cells) > spread_col + 1:
                ...
                if len(handle_values) >= 2:
                    data.update(
                        {
                            f"away_{spread_prefix}_handle_pct": float(
                                handle_values[0].replace("%", "")
                            ),
                            f"home_{spread_prefix}_handle_pct": float(
                                handle_values[1].replace("%", "")
                            ),
                        }
                    )
            ...
            if len(cells) > spread_col + 2:
                ...
                if len(bets_values) >= 2:
                    data.update(
                        {
                            f"away_{spread_prefix}_bets_pct": float(
                                bets_values[0].replace("%", "")
                            ),
                            f"home_{spread_prefix}_bets_pct": float(
                                bets_values[1].replace("%", "")
                            ),
                        }
                    )
    ...
    def _detect_sharp_action_comprehensive(self, betting_data: dict) -> dict[str, Any]:
        ...
        try:
            ...
            if sharp_indicators:
                sharp_count = len(
                    [v for v in sharp_indicators.values() if "SHARP" in v]
                )
                strong_count = len(
                    [v for v in sharp_indicators.values() if "STRONG" in v]
                )
    ...
    def _detect_percentage_divergence(
        self,
        handle1: float | None,
        bets1: float | None,
        handle2: float | None,
    ...
    def _calculate_data_completeness(self, betting_data: dict) -> float:
        ...
        try:
            ...
            return (
                (present_fields / len(required_fields)) * 100 if required_fields else 0
            )
    ...
    async def _collect_vsin_data_async(
        self, sport: str, **kwargs
    ) -> list[dict[str, Any]]:
    ...
    async def _extract_betting_data(
        self, game_data: dict[str, Any], bet_type: dict[str, Any]
    ) -> list[dict[str, Any]]:
        """
            ...
            try:
                ...
                raw_record = {
                    ...
                    "api_endpoint": game_data.get(
                        "api_endpoint", "vsin_unified_collector"
                    ),
                }
        ...
        for record in betting_data:
            try:
                ...
                sharp_action = self._detect_sharp_action(
                    unified_record, bet_type["unified_type"]
                )
                if sharp_action:
    ...
    def _process_sportsbook_data(
        ...
        try:
            ...
            if "moneyline" in sportsbook_data or "ml" in sportsbook_data:
                ml_data = sportsbook_data.get(
                    "moneyline", sportsbook_data.get("ml", {})
                )
                records.append(
                    {
                        ...
                        "home_bets_percentage": ml_data.get(
                            "home_bets_pct", ml_data.get("home_tickets")
                        ),
                        "away_bets_percentage": ml_data.get(
                            "away_bets_pct", ml_data.get("away_tickets")
                        ),
                        "home_money_percentage": ml_data.get(
                            "home_money_pct", ml_data.get("home_handle")
                        ),
                        "away_money_percentage": ml_data.get(
                            "away_money_pct", ml_data.get("away_handle")
                        ),
                        "timestamp": datetime.now().isoformat(),
                        "game_data": game_data,
                    }
            ...
            if "spread" in sportsbook_data or "runline" in sportsbook_data:
                spread_data = sportsbook_data.get(
                    "spread", sportsbook_data.get("runline", {})
                )
                records.append(
                    {
                        "sportsbook": sportsbook_name,
                        ...
                        "home_spread_odds": spread_data.get(
                            "home_odds", spread_data.get("home")
                        ),
                        "away_spread_odds": spread_data.get(
                            "away_odds", spread_data.get("away")
                        ),
                        "home_bets_percentage": spread_data.get(
                            "home_bets_pct", spread_data.get("home_tickets")
                        ),
                        "away_bets_percentage": spread_data.get(
                            "away_bets_pct", spread_data.get("away_tickets")
                        ),
                        "home_money_percentage": spread_data.get(
                            "home_money_pct", spread_data.get("home_handle")
                        ),
                        "away_money_percentage": spread_data.get(
                            "away_money_pct", spread_data.get("away_handle")
                        ),
                        "timestamp": datetime.now().isoformat(),
                        "game_data": game_data,
                    }
            ...
            if "totals" in sportsbook_data or "total" in sportsbook_data:
                totals_data = sportsbook_data.get(
                    "totals", sportsbook_data.get("total", {})
                )
                records.append(
                    {
                        "sportsbook": sportsbook_name,
                        ...
                        "over_odds": totals_data.get(
                            "over_odds", totals_data.get("over")
                        ),
                        "under_odds": totals_data.get(
                            "under_odds", totals_data.get("under")
                        ),
                        "over_bets_percentage": totals_data.get(
                            "over_bets_pct", totals_data.get("over_tickets")
                        ),
                        "under_bets_percentage": totals_data.get(
                            "under_bets_pct", totals_data.get("under_tickets")
                        ),
                        "over_money_percentage": totals_data.get(
                            "over_money_pct", totals_data.get("over_handle")
                        ),
                        "under_money_percentage": totals_data.get(
                            "under_money_pct", totals_data.get("under_handle")
                        ),
                        "timestamp": datetime.now().isoformat(),
                        "game_data": game_data,
                    }
    ...
    def _extract_json_from_html(
        self, html_text: str, api_url: str
    ) -> dict[str, Any] | None:
                ...
                for match in matches:
                    try:
                        ...
                        if isinstance(data, dict) and (
                            ...
                            self.logger.info(
                                f"Successfully extracted JSON from HTML at {api_url}"
                            )
                            return data
    ...
    def _generate_mock_data(self, sport: str) -> list[dict[str, Any]]:
        ...
        try:
            ...
            mock_scenarios = [
                {
                    ...
                    "betting_data": {
                        ...
                        "away_ml_bets_pct": 21.0,
                        "home_ml_bets_pct": 79.0,
                        # Totals with sharp action on over
                        "total_line": 8.5,
                        "over_handle_pct": 60.0,
                        "under_handle_pct": 40.0,
                        "over_bets_pct": 55.0,
                        "under_bets_pct": 45.0,
                        # Run line with strong sharp action on home
                        "away_rl": "+1.5",
                        "home_rl": "-1.5",
                        "away_rl_handle_pct": 8.0,
                        "home_rl_handle_pct": 92.0,
                        "away_rl_bets_pct": 30.0,
                        "home_rl_bets_pct": 70.0,
                    },
                    "sportsbook": "dk",
                },
                {
                    ...
                    "betting_data": {
                        ...
                        "away_ml": "+278",
                        "home_ml": "-361",
                        "away_ml_handle_pct": 14.0,
                        "home_ml_handle_pct": 86.0,
                        "away_ml_bets_pct": 8.0,
                        "home_ml_bets_pct": 92.0,
                        # Totals with moderate sharp action on under
                        "total_line": 9.0,
                        "over_handle_pct": 13.0,
                        "under_handle_pct": 87.0,
                        "over_bets_pct": 57.0,
                        "under_bets_pct": 43.0,
                        # Run line with typical pattern
                        "away_rl": "+1.5",
                        "home_rl": "-1.5",
                        "away_rl_handle_pct": 6.0,
                        "home_rl_handle_pct": 94.0,
                        "away_rl_bets_pct": 9.0,
                        "home_rl_bets_pct": 91.0,
                    },
                    "sportsbook": "dk",
                },
                {
                    ...
                    "betting_data": {
                        ...
                        "away_ml": "+134",
                        "home_ml": "-164",
                        "away_ml_handle_pct": 13.0,
                        "home_ml_handle_pct": 87.0,
                        "away_ml_bets_pct": 21.0,
                        "home_ml_bets_pct": 79.0,
                        # Totals with sharp action on over
                        "total_line": 7.5,
                        "over_handle_pct": 55.0,
                        "under_handle_pct": 45.0,
                        "over_bets_pct": 63.0,
                        "under_bets_pct": 37.0,
                        # Run line with reverse action
                        "away_rl": "+1.5",
                        "home_rl": "-1.5",
                        "away_rl_handle_pct": 9.0,
                        "home_rl_handle_pct": 91.0,
                        "away_rl_bets_pct": 41.0,
                        "home_rl_bets_pct": 59.0,
                    },
                    "sportsbook": "dk",
                },
                {
                    ...
                    "betting_data": {
                        ...
                        "away_ml": "+100",
                        "home_ml": "-121",
                        "away_ml_handle_pct": 82.0,
                        "home_ml_handle_pct": 18.0,
                        "away_ml_bets_pct": 53.0,
                        "home_ml_bets_pct": 47.0,
                        # High total with under sharp action
                        "total_line": 9.5,
                        "over_handle_pct": 25.0,
                        "under_handle_pct": 75.0,
                        "over_bets_pct": 48.0,
                        "under_bets_pct": 52.0,
                        # Run line with strong divergence
                        "away_rl": "+1.5",
                        "home_rl": "-1.5",
                        "away_rl_handle_pct": 15.0,
                        "home_rl_handle_pct": 85.0,
                        "away_rl_bets_pct": 45.0,
                        "home_rl_bets_pct": 55.0,
                    },
                    "sportsbook": "dk",
                },
            ]
            ...
            for i, scenario in enumerate(mock_scenarios):
                ...
                sharp_indicators = self._detect_sharp_action_comprehensive(
                    scenario["betting_data"]
                )

                # Calculate data quality score
                quality_score = self._calculate_data_completeness(
                    scenario["betting_data"]
                )
                ...
                mock_record = {
                    ...
                    "steam_move": False,  # Would need historical data
                    # Include betting percentages at top level for unified processing
                    "home_money_percentage": scenario["betting_data"].get(
                        "home_ml_handle_pct"
                    ),
                    "away_money_percentage": scenario["betting_data"].get(
                        "away_ml_handle_pct"
                    ),
                    "home_bets_percentage": scenario["betting_data"].get(
                        "home_ml_bets_pct"
                    ),
                    "away_bets_percentage": scenario["betting_data"].get(
                        "away_ml_bets_pct"
                    ),
                    "over_money_percentage": scenario["betting_data"].get(
                        "over_handle_pct"
                    ),
                    "under_money_percentage": scenario["betting_data"].get(
                        "under_handle_pct"
                    ),
                    "over_bets_percentage": scenario["betting_data"].get(
                        "over_bets_pct"
                    ),
                    "under_bets_percentage": scenario["betting_data"].get(
                        "under_bets_pct"
                    ),
                    ...
                    "spread_line": scenario["betting_data"].get("away_rl")
                    or scenario["betting_data"].get("away_spread"),
                    "raw_response": {
                        "away_team": scenario["away_team"],
                        "home_team": scenario["home_team"],
                        "sportsbook": scenario["sportsbook"],
            ...
            self.logger.info(
                f"Generated {len(processed_data)} realistic mock VSIN records with sharp action patterns"
            )
            return processed_data
    ...
    async def test_collection(self, sport: str = "mlb") -> dict[str, Any]:
        ...
        try:
            ...
            test_request = CollectionRequest(
                ...
                sport=sport,
                dry_run=True,
                additional_params={"sport": sport, "sportsbook": "dk"},
            )
    ...
    def _process_and_store_record(
        ...
        try:
            ...
            try:
                ...
                self.logger.error(
                    "Fallback storage also failed", error=str(fallback_error)
                )
                return False

    def _store_record_without_game_id(
        self, record: dict[str, Any], batch_id: uuid.UUID
    ) -> bool:
        """
    ...
    def _store_in_core_betting_schema(self, *args, **kwargs) -> bool:
        ...
        logger.warning(
            "_store_in_core_betting_schema is deprecated. All VSIN data should be stored in raw_data schema."
        )
        return False  # Always return False to force use of raw_data storage

    def _store_in_three_tier_pipeline(
        self, record: dict[str, Any], batch_id: uuid.UUID
    ) -> bool:
    ...
    def _store_in_raw_zone(self, record: dict[str, Any]) -> bool:
        ...
        try:
            ...
            self.logger.error(
                "Error storing in raw_data.vsin_data",
                error=str(e),
    ...
    def _store_moneyline_legacy(self, *args, **kwargs) -> bool:
        """DEPRECATED: Legacy method - use raw_data storage instead."""
        logger.warning(
            "_store_moneyline_legacy is deprecated. All VSIN data should be stored in raw_data schema."
        )
        return False
    ...
    def _store_spread_legacy(self, *args, **kwargs) -> bool:
        """DEPRECATED: Legacy method - use raw_data storage instead."""
        logger.warning(
            "_store_spread_legacy is deprecated. All VSIN data should be stored in raw_data schema."
        )
        return False
        try:
            with self.connection_pool.get_connection() as conn:
                with conn.cursor() as cur:
                    ...
                    """,
                        (
                            ...
                            -110,
                            betting_data.get(
                                f"home_{spread_key.split('_')[1]}_handle_pct"
                            ),
                            betting_data.get(
                                f"away_{spread_key.split('_')[1]}_handle_pct"
                            ),
                            betting_data.get(
                                f"home_{spread_key.split('_')[1]}_bets_pct"
                            ),
                            betting_data.get(
                                f"away_{spread_key.split('_')[1]}_bets_pct"
                            ),
    ...
    def test_live_collection(
        self, sport: str = "mlb", sportsbook: str = "dk"
    ) -> dict[str, Any]:
        """
        ...
        try:
            self.logger.info(
                f"Testing live VSIN collection for {sport.upper()} from {sportsbook.upper()}"
            )
            ...
            try:
                ...
                if live_data:
                    ...
                    quality_analysis = {
                        "average_quality_score": total_quality / len(live_data)
                        if live_data
                        else 0,
                        "games_with_sharp_action": sharp_games,
                        "sharp_action_percentage": (sharp_games / len(live_data)) * 100
                        if live_data
                        else 0,
                    }
            ...
            return {
                ...
                "url_generation": {"success": url_test, "test_url": test_url},
                "live_collection": {
                ...
                "summary": {
                    ...
                    "sharp_action_detection": "working"
                    if mock_sharp_analysis.get("games_with_sharp_action", 0) > 0
                    else "not_detected",
                    "data_quality_scoring": "working"
                    if quality_analysis.get("average_quality_score", 0) > 0
                    else "basic",
                },
            }
            ...
            return {
                ...
                "error": str(e),
                "timestamp": datetime.now().isoformat(),
                "test_configuration": {"sport": sport, "sportsbook": sportsbook},
            }
...
if __name__ == "__main__":
    ...
    print(
        f"Mock Fallback Working: {live_test_result['summary']['mock_fallback_working']}"
    )
    print(
        f"Sharp Action Detection: {live_test_result['summary']['sharp_action_detection']}"
    )

    if live_test_result["live_collection"]["success"]:
        print(
            f"Live Records Collected: {live_test_result['live_collection']['record_count']}"
        )
        print(
            f"Average Quality Score: {live_test_result['live_collection']['quality_analysis'].get('average_quality_score', 'N/A')}"
        )

    if live_test_result["mock_fallback"]["success"]:
        print(
            f"Mock Records Generated: {live_test_result['mock_fallback']['record_count']}"
        )
        print(
            f"Sharp Action Detection Rate: {live_test_result['mock_fallback']['sharp_action_analysis'].get('sharp_detection_rate', 'N/A')}%"
        )
