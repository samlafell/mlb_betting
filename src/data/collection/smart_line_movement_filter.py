...
class SmartLineMovementFilter:
    ...
    def _is_significant_movement(
        ...
        if current_odds is not None and previous_odds is not None:
            ...
            corrected_odds_change = self._calculate_american_odds_change(
        ...
        try:
            current_time = datetime.fromisoformat(
                current.get("updated_at", "").replace("Z", "+00:00")
            )
            previous_time = datetime.fromisoformat(
                previous.get("updated_at", "").replace("Z", "+00:00")
            )
    ...
    def _calculate_american_odds_change(
        self, previous_odds: int, current_odds: int
    ) -> int:
        """
        ...
        if (previous_odds > 0 and current_odds > 0) or (
            previous_odds < 0 and current_odds < 0
        ):
