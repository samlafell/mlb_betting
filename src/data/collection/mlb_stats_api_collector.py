...
class MLBStatsAPIClient:
    ...
    async def fetch_schedule(self, date: str, season: int = None) -> List[Dict[str, Any]]:
        ...
        async with session.get(url, params=params) as response:
            if response.status == 200:
                ...
                logger.error("MLB Stats API request failed", status=response.status, url=url)
                return []
    ...
    async def fetch_game_detail(self, game_pk: int) -> Dict[str, Any]:
        ...
        async with session.get(url) as response:
            if response.status == 200:
                ...
                logger.error("Game detail API request failed", status=response.status, game_pk=game_pk)
                return {}
...
class MLBStatsAPICollector(BaseCollector):
    ...
    async def collect_data(self, request: CollectionRequest) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.info("Starting MLB Stats API collection", date=date_str, season=season)
            
            # Fetch schedule for the date
            games = await self.client.fetch_schedule(date_str, season)
            
            if not games:
                logger.warning("No games found for MLB Stats API collection", date=date_str)
                return []
    ...
    async def _store_raw_game_data(self, games: List[Dict[str, Any]], date_str: str) -> None:
        ...
        try:
            ...
            for game in games:
                try:
                    ...
                    game_datetime = safe_game_datetime_parse(game_date_str) if game_date_str else None
                    ...
                    200,
                    game_datetime.date() if game_datetime else datetime.strptime(date_str, "%Y-%m-%d").date(),
                    ...
                    logger.debug("Stored MLB Stats API game", game_pk=game_pk, 
                               home_team=home_team, away_team=away_team)
                    
                except Exception as e:
                    logger.error("Error storing individual game", game_pk=game.get("gamePk"), error=str(e))
                    self.stats["errors"] += 1
            
            await conn.close()
            logger.info(f"Stored {self.stats['games_stored']} games to raw_data.mlb_stats_api_games")
