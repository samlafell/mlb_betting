...
class SharpActionDetector:
    ...
    async def detect_sharp_action(self, lookback_hours: int = 24) -> list[DetectedPattern]:
                ...
                with conn.cursor() as cur:
                    ...
                    for row in rows:
                        ...
                        if home_sharp_differential > 15:
                            severity = PatternSeverity.HIGH if home_sharp_differential > 25 else PatternSeverity.MEDIUM
                            confidence = min(0.95, (home_sharp_differential / 30) * 0.8 + 0.2)
                        ...
                        if away_sharp_differential > 15:
                            severity = PatternSeverity.HIGH if away_sharp_differential > 25 else PatternSeverity.MEDIUM
                            confidence = min(0.95, (away_sharp_differential / 30) * 0.8 + 0.2)
...
class LineMovementAnalyzer:
    ...
    async def analyze_line_movements(self, game_id: int, bet_type: str = 'moneyline') -> list[LineMovement]:
        ...
        try:
                ...
                with conn.cursor() as cur:
                    if bet_type == 'moneyline':
                        ...
                        for row in rows:
                            ...
                            home_movement_pct = (home_movement / abs(row['opening_home_ml'])) * 100
                            ...
                            away_movement_pct = (away_movement / abs(row['opening_away_ml'])) * 100
            ...
            self.logger.error("Error analyzing line movements", game_id=game_id, error=str(e))

        return movements

    async def detect_reverse_line_movement(self, lookback_hours: int = 6) -> list[DetectedPattern]:
                ...
                with conn.cursor() as cur:
                    ...
                    for row in rows:
                        ...
                        if (row['home_bets_percentage'] > 60 and home_line_movement > 0):
                            confidence = min(0.9, (row['home_bets_percentage'] - 60) / 40 * 0.6 + 0.3)

                            patterns.append(DetectedPattern(
                                pattern_type=PatternType.REVERSE_LINE_MOVEMENT,
                                severity=PatternSeverity.HIGH if row['home_bets_percentage'] > 75 else PatternSeverity.MEDIUM,
                                confidence=confidence,
                                game_id=row['game_id'],
                                sportsbook=row['sportsbook'],
                                bet_type='moneyline',
                        ...
                        if (row['away_bets_percentage'] > 60 and away_line_movement > 0):
                            confidence = min(0.9, (row['away_bets_percentage'] - 60) / 40 * 0.6 + 0.3)

                            patterns.append(DetectedPattern(
                                pattern_type=PatternType.REVERSE_LINE_MOVEMENT,
                                severity=PatternSeverity.HIGH if row['away_bets_percentage'] > 75 else PatternSeverity.MEDIUM,
                                confidence=confidence,
                                game_id=row['game_id'],
                                sportsbook=row['sportsbook'],
                                bet_type='moneyline',
...
class ArbitrageDetector:
    ...
    async def detect_arbitrage_opportunities(self, min_profit_percentage: float = 1.0) -> list[DetectedPattern]:
        """Detect arbitrage opportunities across sportsbooks."""
        patterns = []
            ...
            with psycopg2.connect(
                ...
                with conn.cursor() as cur:
                    ...
                    for game_id, odds_list in game_odds.items():
                        ...
                        best_home_odds = max(odds_list, key=lambda x: x['home_ml'] if x['home_ml'] > 0 else 1/abs(x['home_ml']))
                        best_away_odds = max(odds_list, key=lambda x: x['away_ml'] if x['away_ml'] > 0 else 1/abs(x['away_ml']))

                        # Calculate implied probabilities
                        home_prob = self._calculate_implied_probability(best_home_odds['home_ml'])
                        away_prob = self._calculate_implied_probability(best_away_odds['away_ml'])
                        ...
                        if total_prob < 1.0:
                            ...
                            if profit_percentage >= min_profit_percentage:
                                patterns.append(DetectedPattern(
                                    ...
                                    severity=PatternSeverity.CRITICAL if profit_percentage > 5 else PatternSeverity.HIGH,
...
class MarketAnalyzer:
    ...
    async def analyze_market_conditions(self, game_id: int, bet_type: str = 'moneyline') -> MarketAnalysis:
        ...
        try:
                ...
                with conn.cursor() as cur:
                    if bet_type == 'moneyline':
                        ...
                        market_consensus = {
                            ...
                            'home_odds_std': statistics.stdev(home_odds) if len(home_odds) > 1 else 0,
                            'away_odds_std': statistics.stdev(away_odds) if len(away_odds) > 1 else 0
                        }

                        # Calculate line spread
                        line_spread = (max(home_odds) - min(home_odds) + max(away_odds) - min(away_odds)) / 2
                        ...
                        home_bets = [row['home_bets_percentage'] for row in rows if row['home_bets_percentage'] is not None]
                        away_bets = [row['away_bets_percentage'] for row in rows if row['away_bets_percentage'] is not None]
                        ...
                        for row in rows:
                            if row['sharp_action']:
                                sharp_indicators.append(f"{row['sportsbook']}: {row['sharp_action']}")
            ...
            self.logger.error("Error analyzing market conditions", game_id=game_id, error=str(e))
            return MarketAnalysis(
                game_id=game_id,
                bet_type=bet_type,
                market_consensus={},
...
class UnifiedAnalyticsSystem:
    ...
    async def run_comprehensive_analysis(self, lookback_hours: int = 6) -> dict[str, Any]:
        """Run comprehensive analysis across all detectors."""
        try:
            # Detect patterns
            sharp_action_patterns = await self.sharp_action_detector.detect_sharp_action(lookback_hours)
            rlm_patterns = await self.line_movement_analyzer.detect_reverse_line_movement(lookback_hours)
            arbitrage_patterns = await self.arbitrage_detector.detect_arbitrage_opportunities()
            ...
            summary = {
                ...
                'critical_patterns': len([p for p in all_patterns if p.severity == PatternSeverity.CRITICAL]),
                'high_severity_patterns': len([p for p in all_patterns if p.severity == PatternSeverity.HIGH]),
                'medium_severity_patterns': len([p for p in all_patterns if p.severity == PatternSeverity.MEDIUM]),
                'low_severity_patterns': len([p for p in all_patterns if p.severity == PatternSeverity.LOW]),
                'avg_confidence': statistics.mean([p.confidence for p in all_patterns]) if all_patterns else 0
            }
    ...
    async def analyze_specific_game(self, game_id: int) -> dict[str, Any]:
        ...
        try:
            ...
            line_movements = await self.line_movement_analyzer.analyze_line_movements(game_id)

            # Analyze market conditions
            market_analysis = await self.market_analyzer.analyze_market_conditions(game_id)
