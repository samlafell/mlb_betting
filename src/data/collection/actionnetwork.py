...
class ActionNetworkHistoryParser:
    ...
    def parse_history_response(self, response_data: Dict[str, Any],
        ...
        try:
            ...
            self.logger.info("Successfully parsed Action Network history",
                           game_id=game_id,
                           total_entries=len(historical_entries),
                           pregame_entries=pregame_count,
                           live_entries=live_count)
            ...
            self.logger.error("Failed to parse historical response",
    ...
    def parse_multiple_histories(self, response_data_list: List[Tuple[Dict[str, Any], Dict[str, Any]]]) -> List[ActionNetworkHistoricalData]:
        """
        ...
        for response_data, game_metadata in response_data_list:
            try:
                ...
                self.logger.error("Failed to parse game history",
                                game_id=game_metadata.get('game_id'),
                                error=str(e))
        
        self.logger.info("Completed parsing multiple histories",
...
class ActionNetworkHistoryCollector:
        ...
        try:
            ...
            async with session.get(url) as response:
                if response.status == 200:
                    ...
                    self.logger.warning("HTTP error", 
                                      url=url, status=response.status)
                    return None
    ...
    async def collect_history_data(self, history_url: str, game_id: int,
                                  home_team: str, away_team: str,
                                  game_datetime: datetime) -> CollectionResult:
        """
        ...
        try:
            self.logger.info("Collecting Action Network history",
                           game_id=game_id, 
            ...
            self.logger.error("Action Network history collection failed",
                            game_id=game_id, url=history_url, error=str(e))
    ...
    async def collect_multiple_histories(self, game_data_list: List[Dict[str, Any]]) -> List[CollectionResult]:
        """
        ...
        self.logger.info("Collecting multiple Action Network histories",
        ...
        for i, result in enumerate(results):
            if isinstance(result, Exception):
                self.logger.error("Failed to collect game history",
        ...
        self.logger.info("Completed multiple history collection",
    ...
    async def collect(self, **kwargs: Any) -> CollectionResult:
        ...
        return CollectionResult(
            ...
            source=self.source_name,
            timestamp=datetime.now(),
            errors=["Use collect_history_data method for Action Network historical data"],
            metadata=kwargs
        ) 