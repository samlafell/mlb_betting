...
class RateLimitResult:
    ...
    next_token_time: Optional[datetime] = None
...
class AdaptiveRateLimiter:
    ...
    def get_adjusted_config(self) -> RateLimitConfig:
        ...
        adjusted = RateLimitConfig(
            ...
            requests_per_hour=int(self.base_config.requests_per_hour * self.current_multiplier),
            burst_limit=max(1, int(self.base_config.burst_limit * self.current_multiplier)),
            **{k: v for k, v in self.base_config.__dict__.items() 
               if k not in ['requests_per_second', 'requests_per_minute', 
...
class UnifiedRateLimiter:
    ...
    def configure_source(self, source: str, config: RateLimitConfig) -> None:
        ...
        with self._locks[source]:
            ...
            self.logger.info("Rate limiter configured for source",
                           source=source, strategy=config.strategy.value)
    ...
    async def acquire(self, source: str, tokens: int = 1) -> RateLimitResult:
        ...
        if not allowed and config.exponential_backoff:
            recent_failures = sum(
                1 for req in list(self.request_history[source])[-10:]
                if not req['allowed']
            )
        ...
        if not allowed and wait_time > 0:
            self.logger.debug("Rate limited, waiting",
                            source=source, wait_time=wait_time, reason=reason)
        ...
        if source in self.adaptive_limiters:
            ...
            metrics["adaptive"] = {
                ...
                "success_rate": (
                    ...
                    if adaptive.success_history else 1.0
                )
            }
    ...
    def get_global_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "active_circuit_breakers": len([
                source for source, cb in self.circuit_breakers.items()
