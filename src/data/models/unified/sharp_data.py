...
from .base import UnifiedEntity, AnalysisEntity, ValidatedModel
from .game import Team
from .odds import MarketType, BookType
...
class SharpSignal(AnalysisEntity):
    ...
    detected_at: datetime = Field(
        ...,
        description="When signal was detected (EST)"
    )
    
    detector_name: str = Field(
        ...,
        description="Name of the detector that found this signal",
        max_length=100
    )
    ...
    market_type: MarketType = Field(
        ...,
        description="Market where signal was detected"
    )
    
    market_side: Optional[str] = Field(
        default=None,
        description="Specific side of the market",
        max_length=20
    )
    ...
    sharp_books: List[BookType] = Field(
        ...
        description="Books showing sharp action"
    )
    
    non_sharp_books: List[BookType] = Field(
        default_factory=list,
        description="Books not showing sharp action"
    )
    ...
    signal_strength: float = Field(
        ...,
        description="Strength of the signal (0.0-1.0)",
        ge=0.0,
        le=1.0
    )
    ...
    line_movement: Optional[float] = Field(
        default=None,
        description="Line movement amount"
    )
    
    odds_movement: Optional[int] = Field(
        default=None,
        description="Odds movement in American format"
    )
    
    movement_speed: Optional[str] = Field(
        default=None,
    ...
    volume_spike: bool = Field(
        default=False,
        description="Whether volume spike was detected"
    )
    ...
    money_percentage: Optional[float] = Field(
        default=None,
    ...
    bet_percentage: Optional[float] = Field(
        default=None,
    ...
    # Timing context
    time_to_game: Optional[int] = Field(
        default=None,
        description="Minutes until game start when detected",
        ge=0
    )
    
    is_late_signal: bool = Field(
        default=False,
        description="Whether this is a late signal (within 2 hours)"
    )
    
    # Supporting evidence
    supporting_indicators: List[SharpIndicatorType] = Field(
        default_factory=list,
        description="Additional supporting indicators"
    )
    
    contradicting_indicators: List[SharpIndicatorType] = Field(
        default_factory=list,
        description="Contradicting indicators that weaken signal"
    )
    
    # Raw data
    raw_data: Dict[str, Any] = Field(
        default_factory=dict,
        description="Raw data used in signal detection"
    )
    
    feature_values: Dict[str, float] = Field(
        default_factory=dict,
        description="Feature values used in detection algorithm"
    )
    ...
    @computed_field
    @property
    def public_sharp_divergence(self) -> Optional[float]:
    ...
    def is_actionable(self, min_confidence: float = 0.65, min_strength: float = 0.6) -> bool:
        """
        ...
        return (
            ...
            self.signal_strength >= min_strength and
            not self.has_contradicting_evidence
        )
...
class SharpMoney(ValidatedModel):
    ...
    timestamp: datetime = Field(
        ...,
        description="When sharp money was detected (EST)"
    )
    
    # Market context
    market_type: MarketType = Field(
        ...,
        description="Market receiving sharp money"
    )
    
    sportsbook: BookType = Field(
        ...,
        description="Sportsbook receiving sharp money"
    )
    
    # Money flow data
    sharp_side: str = Field(
        ...,
        description="Side receiving sharp money",
        max_length=20
    )
    
    estimated_amount: Optional[float] = Field(
        default=None,
        description="Estimated amount of sharp money",
        ge=0.0
    )
    
    percentage_of_handle: Optional[float] = Field(
        default=None,
        description="Percentage of total handle (0.0-1.0)",
        ge=0.0,
        le=1.0
    )
    
    # Sharp characteristics
    bet_size_category: Optional[str] = Field(
        default=None,
        description="Category of bet size (small, medium, large, massive)",
        pattern="^(small|medium|large|massive)$"
    )
    
    betting_pattern: Optional[str] = Field(
        default=None,
        description="Betting pattern observed",
        max_length=100
    )
    
    is_coordinated: bool = Field(
        default=False,
        description="Whether this appears to be coordinated sharp action"
    )
    
    # Impact assessment
    line_impact: Optional[float] = Field(
        default=None,
        description="Impact on line movement"
    )
    
    odds_impact: Optional[int] = Field(
        default=None,
        description="Impact on odds (American format)"
    )
    
    market_impact_score: Optional[float] = Field(
        default=None,
    ...
    time_to_game: Optional[int] = Field(
        default=None,
        description="Minutes until game start",
        ge=0
    )
    
    related_signals: List[str] = Field(
        default_factory=list,
        description="Related sharp signal IDs"
    )
    ...
    def is_significant_amount(self) -> bool:
        ...
        return (
            self.bet_size_category in ["large", "massive"] or
...
class SharpConsensus(AnalysisEntity):
    ...
    market_type: MarketType = Field(
        ...,
        description="Market for this consensus"
    )
    
    # Consensus data
    sharp_signals: List[str] = Field(
        default_factory=list,
        description="Sharp signal IDs included in consensus"
    )
    
    signal_count: int = Field(
        default=0,
        description="Number of signals in consensus",
        ge=0
    )
    ...
    consensus_confidence: float = Field(
        ...,
        description="Confidence in consensus (0.0-1.0)",
        ge=0.0,
        le=1.0
    )
    ...
    signal_agreement: float = Field(
        ...,
        description="Agreement between signals (0.0-1.0)",
        ge=0.0,
        le=1.0
    )
    ...
    book_agreement: float = Field(
        ...,
        description="Agreement between books (0.0-1.0)",
        ge=0.0,
        le=1.0
    )
    
    # Timing analysis
    signal_timing_spread: Optional[int] = Field(
        default=None,
        description="Time spread of signals in minutes",
        ge=0
    )
    
    is_coordinated_consensus: bool = Field(
        default=False,
        description="Whether consensus represents coordinated action"
    )
    ...
    outlier_signals: List[str] = Field(
        default_factory=list,
        description="Signal IDs that are outliers"
    )
    ...
    def is_strong_consensus(self) -> bool:
        ...
        return (
            self.consensus_confidence >= 0.8 and
            self.signal_agreement >= 0.7 and
            self.signal_count >= 3
        )
