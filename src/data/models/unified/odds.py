...
from .base import UnifiedEntity, ValidatedModel, SourcedModel
...
class OddsData(SourcedModel, ValidatedModel):
    ...
    market_side: Optional[MarketSide] = Field(
        default=None,
        description="Side of the market (if applicable)"
    )
    
    sportsbook: BookType = Field(
        ...,
        description="Sportsbook offering the odds"
    )
    
    # Odds values
    odds_american: Optional[int] = Field(
        default=None,
    ...
    odds_decimal: Optional[float] = Field(
        default=None,
        description="Decimal odds format (2.50, 1.83)",
        gt=1.0,
        le=1000.0
    )
    
    odds_fractional: Optional[str] = Field(
        default=None,
        description="Fractional odds format (3/2, 5/6)",
        max_length=20
    )
    ...
    # Limits and volume
    max_bet_limit: Optional[float] = Field(
        default=None,
        description="Maximum bet limit in USD",
        ge=0.0
    )
    
    volume: Optional[int] = Field(
        default=None,
        description="Betting volume/handle",
        ge=0
    )
    ...
    timestamp: datetime = Field(
        ...,
        description="When odds were recorded (EST)"
    )
    
    # Opening/closing tracking
    is_opening_line: bool = Field(
        default=False,
        description="Whether this is the opening line"
    )
    
    is_closing_line: bool = Field(
        default=False,
        description="Whether this is the closing line"
    )
    ...
    def convert_to_decimal(self) -> Optional[float]:
    ...
    def convert_to_implied_probability(self) -> Optional[float]:
...
class OddsSnapshot(ValidatedModel):
    ...
    timestamp: datetime = Field(
        ...,
        description="When snapshot was taken (EST)"
    )
    
    odds_data: List[OddsData] = Field(
        default_factory=list,
        description="All odds data captured in this snapshot"
    )
    
    # Market summary
    total_books: int = Field(
        default=0,
        description="Number of sportsbooks in snapshot",
        ge=0
    )
    
    markets_available: List[MarketType] = Field(
        default_factory=list,
        description="Market types available in snapshot"
    )
...
class LineMovement(UnifiedEntity):
    ...
    sportsbook: BookType = Field(
        ...,
        description="Sportsbook where movement occurred"
    )
    
    # Previous state
    previous_odds: Optional[OddsData] = Field(
        default=None,
        description="Previous odds state"
    )
    
    # Current state
    current_odds: OddsData = Field(
        ...,
        description="Current odds state"
    )
    
    # Movement analysis
    odds_movement: OddsMovement = Field(
        ...,
    ...
    line_movement: Optional[float] = Field(
        default=None,
        description="Change in line value (current - previous)"
    )
    
    odds_change_american: Optional[int] = Field(
        default=None,
        description="Change in American odds (current - previous)"
    )
    
    movement_magnitude: Optional[float] = Field(
        default=None,
        description="Magnitude of movement (0.0-1.0)",
        ge=0.0,
        le=1.0
    )
    ...
    time_since_previous: Optional[int] = Field(
        default=None,
        description="Minutes since previous movement",
        ge=0
    )
    ...
    is_steam_move: bool = Field(
        default=False,
        description="Whether this is identified as a steam move"
    )
    
    is_reverse_line_movement: bool = Field(
        default=False,
        description="Whether this is reverse line movement"
    )
    
    volume_spike: bool = Field(
        default=False,
        description="Whether there was a volume spike"
    )
    ...
    def calculate_movement_metrics(self) -> None:
        ...
        if prev_american is not None and curr_american is not None:
            ...
            if prev_american != 0:
                self.movement_magnitude = abs(self.odds_change_american) / abs(prev_american)
        
        # Calculate line movement
        if (self.previous_odds.line_value is not None and 
            self.current_odds.line_value is not None):
            self.line_movement = self.current_odds.line_value - self.previous_odds.line_value
...
class MarketConsensus(ValidatedModel):
    ...
    # Consensus metrics
    average_odds_american: Optional[int] = Field(
        default=None,
        description="Average American odds across books"
    )
    
    median_odds_american: Optional[int] = Field(
        default=None,
        description="Median American odds across books"
    )
    
    odds_range_american: Optional[int] = Field(
        default=None,
        description="Range of American odds (max - min)"
    )
    
    average_line_value: Optional[float] = Field(
        default=None,
        description="Average line value across books"
    )
    
    line_range: Optional[float] = Field(
        default=None,
        description="Range of line values (max - min)"
    )
    
    # Market efficiency
    market_efficiency_score: Optional[float] = Field(
        default=None,
        description="Market efficiency score (0.0-1.0)",
        ge=0.0,
        le=1.0
    )
    ...
    arbitrage_opportunity: bool = Field(
        default=False,
        description="Whether arbitrage opportunity exists"
    )
    
    # Sharp action indicators
    sharp_action_detected: bool = Field(
        default=False,
        description="Whether sharp action is detected"
    )
    
    reverse_line_movement: bool = Field(
        default=False,
        description="Whether reverse line movement is present"
    )
    
    steam_move_count: int = Field(
        default=0,
        description="Number of steam moves detected",
        ge=0
    )
    ...
    participating_books: List[BookType] = Field(
        default_factory=list,
        description="Sportsbooks included in consensus"
    )
    
    book_count: int = Field(
        default=0,
        description="Number of books in consensus",
        ge=0
    )
    ...
    def add_odds_data(self, odds_data: List[OddsData]) -> None:
        ...
        market_odds = [odds for odds in odds_data if odds.market_type == self.market_type]
        ...
        if american_odds:
            ...
            self.median_odds_american = int(sorted(american_odds)[len(american_odds) // 2])
            self.odds_range_american = max(american_odds) - min(american_odds)
        
        # Line values
        line_values = [odds.line_value for odds in market_odds if odds.line_value is not None]
        if line_values:
        ...
        if self.odds_range_american is not None and self.average_odds_american is not None:
            if self.average_odds_american != 0:
                efficiency = 1.0 - (abs(self.odds_range_american) / abs(self.average_odds_american))
...
class BettingMarket(UnifiedEntity):
    ...
    current_odds: List[OddsData] = Field(
        default_factory=list,
        description="Current odds from all books"
    )
    ...
    line_movements: List[LineMovement] = Field(
        default_factory=list,
        description="All line movements for this market"
    )
    
    current_consensus: Optional[MarketConsensus] = Field(
        default=None,
        description="Current market consensus"
    )
    ...
    def get_best_odds(self, side: MarketSide) -> Optional[OddsData]:
        """
    ...
    def calculate_market_consensus(self) -> Optional[MarketConsensus]:
