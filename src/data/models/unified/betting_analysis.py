...
from .game import Team, GameStatus
from .odds import MarketType, BookType, OddsData
...
class BettingAnalysis(AnalysisEntity):
    ...
    market_type: MarketType = Field(
        ...,
        description="Market type being analyzed"
    )
    ...
    signals_detected: List[BettingSignalType] = Field(
        ...
        description="List of betting signals detected"
    )
    
    primary_signal: Optional[BettingSignalType] = Field(
        default=None,
        description="Primary/strongest signal detected"
    )
    ...
    recommended_bet: Optional[str] = Field(
        default=None,
        description="Specific bet recommendation",
        max_length=100
    )
    
    recommended_stake: Optional[float] = Field(
        default=None,
    ...
    odds_data: List[OddsData] = Field(
        default_factory=list,
        description="Odds data used in analysis"
    )
    
    feature_importance: Dict[str, float] = Field(
        default_factory=dict,
        description="Feature importance scores for the analysis"
    )
    
    model_features: Dict[str, Any] = Field(
        default_factory=dict,
        description="Model features and values used"
    )
    ...
    max_loss_potential: Optional[float] = Field(
        default=None,
        description="Maximum potential loss (0.0-1.0)",
        ge=0.0,
        le=1.0
    )
    ...
    optimal_bet_time: Optional[datetime] = Field(
        default=None,
        description="Optimal time to place bet (EST)"
    )
    
    expires_at: Optional[datetime] = Field(
        default=None,
        description="When this analysis expires (EST)"
    )
    ...
    @classmethod
    def validate_signals_detected(cls, v: List[BettingSignalType]) -> List[BettingSignalType]:
    ...
    @classmethod
    def validate_primary_signal(cls, v: Optional[BettingSignalType], info) -> Optional[BettingSignalType]:
...
class SharpAction(ValidatedModel):
    ...
    sharp_side: str = Field(
        ...,
        description="Side receiving sharp action",
        max_length=20
    )
    ...
    reverse_line_movement: bool = Field(
        default=False,
        description="Whether reverse line movement occurred"
    )
    
    steam_move: bool = Field(
        default=False,
        description="Whether steam move was detected"
    )
    
    coordinated_move: bool = Field(
        default=False,
        description="Whether coordinated move across books"
    )
    
    # Confidence metrics
    sharp_confidence: float = Field(
        ...,
        description="Confidence this is sharp action (0.0-1.0)",
        ge=0.0,
        le=1.0
    )
    ...
    signal_strength: SignalStrength = Field(
        default=SignalStrength.MODERATE,
        description="Strength of the sharp signal"
    )
...
class BettingSplit(ValidatedModel):
    ...
    timestamp: datetime = Field(
        ...,
        description="When split data was captured (EST)"
    )
    
    data_source: str = Field(
        ...,
        description="Source of split data",
        max_length=50
    )
    ...
    market_type: MarketType = Field(
        ...,
        description="Market type for this split"
    )
    
    # Public betting data
    public_bet_percentage: Optional[float] = Field(
        default=None,
        description="Percentage of public bets (0.0-1.0)",
        ge=0.0,
        le=1.0
    )
    
    public_money_percentage: Optional[float] = Field(
        default=None,
        description="Percentage of public money (0.0-1.0)",
        ge=0.0,
        le=1.0
    )
    
    # Sharp betting data
    sharp_bet_percentage: Optional[float] = Field(
        default=None,
        description="Percentage of sharp bets (0.0-1.0)",
        ge=0.0,
        le=1.0
    )
    
    sharp_money_percentage: Optional[float] = Field(
        default=None,
        description="Percentage of sharp money (0.0-1.0)",
        ge=0.0,
        le=1.0
    )
    
    # Side-specific splits
    home_bet_percentage: Optional[float] = Field(
        default=None,
    ...
    away_bet_percentage: Optional[float] = Field(
        default=None,
    ...
    over_bet_percentage: Optional[float] = Field(
        default=None,
        description="Percentage of bets on over (0.0-1.0)",
        ge=0.0,
        le=1.0
    )
    
    under_bet_percentage: Optional[float] = Field(
        default=None,
    ...
    # Volume data
    total_bet_count: Optional[int] = Field(
        default=None,
        description="Total number of bets",
        ge=0
    )
    
    total_money_amount: Optional[float] = Field(
        default=None,
    ...
    @computed_field
    @property
    def public_heavy_side(self) -> Optional[str]:
    ...
    @computed_field
    @property
    def sharp_heavy_side(self) -> Optional[str]:
