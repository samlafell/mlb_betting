...
class ActionNetworkTeam(IdentifiedModel, ValidatedModel):
    ...
    def validate_color_format(cls, v):
        ...
        if not v.isalnum():
            raise ValueError("Color must be a valid hex code (letters and numbers only)")
        return v.upper()
...
class ActionNetworkGame(IdentifiedModel, ValidatedModel):
    ...
    # Team information
    home_team: ActionNetworkTeam = Field(
        ...,
        description="Home team information"
    )
    
    away_team: ActionNetworkTeam = Field(
        ...,
    ...
    real_status: str = Field(
        default="scheduled",
        description="Real-time status of the game"
    )
    
    status_display: Optional[str] = Field(
        default=None,
    ...
    league_id: int = Field(
        ...,
        description="League ID (typically 8 for MLB)",
        ge=1
    )
    ...
    game_type: str = Field(
        default="reg",
        description="Game type (reg, playoff, etc.)",
        max_length=20
    )
    
    # Metadata
    history_url: Optional[str] = Field(
        default=None,
