...
class ValidatedModel(UnifiedBaseModel):
    ...
    validation_errors: Optional[Dict[str, str]] = Field(
        default=None,
        description="Validation errors encountered during processing"
    )
    
    @field_validator("data_quality")
    @classmethod
    def validate_data_quality(cls, v: Optional[str]) -> Optional[str]:
...
class SourcedModel(UnifiedBaseModel):
    ...
    source_url: Optional[str] = Field(
        default=None,
        description="URL where data was retrieved from",
        max_length=500
    )
    
    source_timestamp: Optional[datetime] = Field(
        default=None,
        description="When data was retrieved from source (EST)"
    )
