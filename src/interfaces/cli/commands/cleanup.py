...
def cleanup(
    ...
    console.print(f"📊 Current directory size: [bold]{format_size(initial_size)}[/bold]")
    ...
    console.print("   1. Run database schema setup: [cyan]psql -d mlb_betting -f sql/schemas/analysis_reports.sql[/cyan]")
    ...
    if files_by_type["reports"]:
        console.print(f"   4. [yellow]{len(files_by_type['reports'])} analysis reports[/yellow] can be migrated to database")
    ...
    if not dry_run:
        ...
        if archive_files:
            console.print(f"📋 {len(archive_files)} analysis files ready for database migration")
            console.print("   Use the enhanced pipeline to start using database storage")
    ...
    console.print("   • Use: [cyan]uv run -m src.interfaces.cli action-network pipeline[/cyan]")
    console.print("   • View opportunities: [cyan]uv run -m src.interfaces.cli action-network opportunities[/cyan]")
