...
async def extract_action_network_history(input_file: Path, 
    ...
    try:
        ...
        click.echo(f"📊 Found {len(games_with_history)} games with history URLs out of {total_games} total games")
        ...
        if dry_run:
            ...
            for i, game in enumerate(games_with_history, 1):
                click.echo(f"  {i}. {game['away_team']} @ {game['home_team']} (ID: {game['game_id']})")
        ...
        click.echo(f"📈 Extracting historical line movement data for {len(games_with_history)} games...")
        
        with click.progressbar(length=len(games_with_history), 
                             label='Extracting histories') as bar:
            ...
            for i in range(0, len(games_with_history), batch_size):
                ...
                # Extract batch
                batch_results = await data_service.collect_multiple_action_network_histories(batch)
...
def analyze_action_network_history(input_file: Path,
    ...
    try:
        ...
        for game in games_data:
            ...
            if line_summary:
                ...
                for market_type in ['moneyline', 'spread', 'total']:
                    ...
                    total_significant_movements += market_data.get('significant_moves', 0)
            ...
            click.echo(f"  🎯 Pregame: {game.get('pregame_entries', 0)}, Live: {game.get('live_entries', 0)}")
            
            if line_summary:
                click.echo(f"  📈 Line movements:")
                for market_type in ['moneyline', 'spread', 'total']:
                    market_data = line_summary.get(market_type, {})
                    movements = market_data.get('movements', 0)
                    significant = market_data.get('significant_moves', 0)
                    click.echo(f"    {market_type.title()}: {movements} moves ({significant} significant)")
        ...
        analysis_results.update({
            ...
            "significant_movement_rate": (
                ...
                if total_line_movements > 0 else 0
            )
        })
        ...
        click.echo(f"  📊 Significant rate: {analysis_results['significant_movement_rate']:.1f}%")
