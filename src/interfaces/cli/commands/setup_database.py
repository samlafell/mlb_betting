...
async def _setup_action_network_async(schema_file: Path, test_connection: bool, verbose: bool):
    """Async implementation of Action Network database setup."""
    if verbose:
        logger.info("Setting up Action Network database schema",
...
@click.option('--game-id', '-g',
              type=int,
              help='Specific game ID to check')
@click.option('--book-id', '-b',
              type=int,
              help='Specific book ID to check')
@click.option('--market-type', '-m',
              type=click.Choice(['moneyline', 'spread', 'total']),
              help='Specific market type to check')
@click.option('--verbose', '-v',
              is_flag=True,
              help='Enable verbose logging')
def check_data(game_id: Optional[int], book_id: Optional[int], 
               market_type: Optional[str], verbose: bool):
    """
...
async def _check_data_async(game_id: Optional[int], book_id: Optional[int], 
                           market_type: Optional[str], verbose: bool):
    ...
    try:
        ...
        if game_id:
            # Check specific game
            summary = await repository.get_line_movement_summary(game_id, book_id, market_type)
            ...
            if summary:
                ...
                for item in summary:
                    ...
                    click.echo(f"  📚 {item['sportsbook_display_name']} - {item['market_type'].title()} ({item['side']})")
                    
                    if item['opening_odds_american'] and item['closing_odds_american']:
                        click.echo(f"  📊 Opening: {item['opening_odds_american']:+d} → Closing: {item['closing_odds_american']:+d}")
                        movement = item['closing_odds_american'] - item['opening_odds_american']
                        if movement > 0:
                            click.echo(f"  📈 Movement: +{movement} (line moved against)")
                        elif movement < 0:
                            click.echo(f"  📉 Movement: {movement} (line moved toward)")
                        else:
                            click.echo(f"  ➡️  Movement: No change")
                    
                    if item['total_movements']:
                        click.echo(f"  🔄 Total movements: {item['total_movements']}")
                    
                    if item['reverse_line_movement_detected']:
                        click.echo(f"  🔄 Reverse line movement detected")
            ...
            if health_result["status"] == "healthy":
                ...
                click.echo(f"  • Line Movement Summary: {counts.get('line_movement_summary', 0):,}")
                ...
                if counts.get('betting_lines', 0) > 0:
                    ...
                    result = await connection.execute_async(
                        sample_query,
                        fetch="one",
                        table="action_network.betting_lines"
                    )
