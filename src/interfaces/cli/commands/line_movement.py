...
@click.option('--date', type=click.DateTime(formats=['%Y-%m-%d']), 
              default=None, help='Date to collect (default: today)')
@click.option('--backfill-days', type=int, default=1, 
              help='Number of days to backfill (default: 1)')
@click.option('--dry-run', is_flag=True, help='Show what would be collected without inserting')
def collect_historical(date: Optional[datetime], backfill_days: int, dry_run: bool):
    """
    ...
    click.echo(f"🎯 Collecting historical line movements")
...
@click.option('--bet-type', type=click.Choice(['moneyline', 'spread', 'total']), 
              help='Filter by bet type')
def analyze_movements(game_id: Optional[int], date: Optional[datetime], 
                     sportsbook: Optional[str], bet_type: Optional[str]):
    """
...
@line_movement.command()
@click.option('--date', type=click.DateTime(formats=['%Y-%m-%d']), 
              default=None, help='Date to check (default: today)')
def status(date: Optional[datetime]):
    """
...
async def _collect_historical_movements(target_date: datetime, backfill_days: int, dry_run: bool):
...
async def _analyze_line_movements(game_id: Optional[int], date: Optional[datetime], 
    ...
    try:
        ...
        for movement in movements:
            ...
            click.echo(f"   {movement['sportsbook_name']} | {movement['bet_type']} {movement['side']}")
            click.echo(f"   Opening: {movement['opening_odds']:+d} → Closing: {movement['closing_odds']:+d} (Change: {movement['odds_movement']:+d})")
    ...
    try:
        ...
        if sportsbook_coverage:
            ...
            for book in sportsbook_coverage:
                click.echo(f"   {book['display_name']}: {book['games_covered']} games, {book['total_movements']} movements")
