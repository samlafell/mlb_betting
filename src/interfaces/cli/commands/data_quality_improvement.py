...
async def _run_setup(connection, execute: bool, phase: str):
    ...
    try:
        for sql_config in sql_files:
            ...
            try:
                # Read SQL file
                with open(sql_config['file'], 'r') as f:
                    sql_content = f.read()
...
def update_sharp_action(date: Optional[datetime], force_update: bool):
...
def backfill_sharp_action(start_date: Optional[datetime], 
