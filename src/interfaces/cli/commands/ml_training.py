...
@click.option('--targets', multiple=True, help='Prediction targets to train (default: all)')
...
def train_models(
    ...
    async def _train():
        try:
            ...
            if targets:
                available_targets = ['moneyline_home_win', 'total_over_under', 'run_total_regression']
            ...
            click.echo(f"📈 Data samples: {results['training_results']['data_samples']}")
            click.echo(f"⏱️  Training time: {results['training_results']['training_time_seconds']:.1f}s")
            
            click.echo(f"\n🎯 Model Performance:")
            for model_name in results['models_trained']:
                model_results = results['training_results']['training_results'].get(model_name, {})
...
@click.option('--force', is_flag=True, help='Force retraining regardless of performance')
@click.option('--min-samples', default=100, help='Minimum samples for retraining (default: 100)')
def retrain_models(
    model: Optional[str],
    window_days: int,
    force: bool,
        ...
        try:
            ...
            if model:
                ...
                if detailed:
                    ...
                    click.echo(f"  Experiments logged: {service_stats['experiments_logged']}")
                    click.echo(f"  Avg training time: {service_stats['avg_training_time_seconds']:.1f}s")
...
@click.option('--schedule-type', type=click.Choice(['daily', 'weekly']), default='daily', help='Schedule type')
