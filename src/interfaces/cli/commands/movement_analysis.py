...
@movement.command()
@click.option('--input-file', '-i', required=True, help='Path to historical data JSON file')
...
@click.option('--min-movements', type=int, default=50, help='Minimum movements to analyze game')
...
async def analyze(
    ...
    console.print(f"[bold blue]🔍 Enhanced Movement Analysis[/bold blue]")
        ...
        for i, game_data in enumerate(games_to_analyze):
            ...
            try:
                ...
                if rlm_only and not analysis.rlm_indicators:
                    continue
                if steam_only and not any(c.steam_move_detected for c in analysis.cross_book_movements):
                    continue
                
                game_analyses.append(analysis)
                
            except Exception as e:
                console.print(f"[red]❌ Error analyzing game {game_data.get('game_id', 'unknown')}: {e}[/red]")
...
@movement.command()
@click.option('--input-file', '-i', required=True, help='Path to historical data JSON file')
@click.option('--min-rlm-strength', type=click.Choice(['weak', 'moderate', 'strong']), default='moderate', help='Minimum RLM strength to display')
@click.option('--market-type', type=click.Choice(['moneyline', 'spread', 'total']), help='Filter by market type')
...
async def rlm(
    ...
    console.print(f"[bold red]🔄 Reverse Line Movement Analysis[/bold red]")
    ...
    if not rlm_opportunities:
        console.print("[yellow]⚠️  No RLM opportunities found with the specified criteria[/yellow]")
        return
...
@movement.command()
@click.option('--input-file', '-i', required=True, help='Path to historical data JSON file')
@click.option('--min-books', type=int, default=3, help='Minimum number of books for steam move')
@click.option('--market-type', type=click.Choice(['moneyline', 'spread', 'total']), help='Filter by market type')
async def steam(
    input_file: str,
    min_books: int,
    ...
    console.print(f"[bold green]🚂 Steam Move Detection[/bold green]")
    ...
    if not steam_moves:
        console.print("[yellow]⚠️  No steam moves found with the specified criteria[/yellow]")
        return
...
async def _display_analysis_results(report: MovementAnalysisReport, show_details: bool):
    ...
    if report.games_with_rlm > 0:
        ...
        for game in rlm_games:
            ...
            moderate_rlm = [r for r in game.rlm_indicators if r.rlm_strength == "moderate"]
            
            console.print(f"  [bold]{game.away_team} @ {game.home_team}[/bold]")
            console.print(f"    Strong RLM: {len(strong_rlm)}, Moderate RLM: {len(moderate_rlm)}")
            
            if show_details:
                for rlm in game.rlm_indicators[:3]:  # Show top 3
                    book_name = _get_book_name(rlm.sportsbook_id)
                    console.print(f"    - {book_name} {rlm.market_type.value}: {rlm.rlm_strength} RLM")
    
    # Steam moves
    if report.games_with_steam_moves > 0:
        console.print("\n[bold green]🚂 Steam Move Alerts[/bold green]")
        
        steam_games = [g for g in report.game_analyses if any(c.steam_move_detected for c in g.cross_book_movements)]
        
        for game in steam_games[:5]:
            steam_moves = [c for c in game.cross_book_movements if c.steam_move_detected]
            ...
            if show_details:
                for steam in steam_moves[:2]:  # Show top 2
                    console.print(f"    - {steam.market_type.value}: {len(steam.participating_books)} books, {steam.consensus_strength} consensus")
...
async def _display_steam_moves(steam_moves, analyzer):
    ...
    for steam_data in steam_moves:
        ...
        game_name = f"{game.away_team} @ {game.home_team}"
        book_names = [analyzer.sportsbook_names.get(book_id, book_id) for book_id in steam.participating_books]
