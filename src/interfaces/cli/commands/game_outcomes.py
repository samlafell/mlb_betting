...
def check(start_date: Optional[str], end_date: Optional[str], days: int, force: bool, verbose: bool):
    ...
    async def run_check():
        with Progress(
            ...
            try:
                results = await check_game_outcomes(date_range=date_range, force_update=force)
...
@outcomes.command()
@click.option('--game-id', '-g', type=int, required=True, help='Game ID from core_betting.games')
@click.option('--force', '-f', is_flag=True, help='Force update even if outcome exists')
        ...
        try:
            ...
            async with get_connection() as conn:
                async with conn.cursor() as cursor:
                    ...
                    if not row:
                        console.print(f"❌ [bold red]Game not found:[/bold red] ID {game_id}")
                        return
            ...
            if not force:
                async with get_connection() as conn:
                    async with conn.cursor() as cursor:
                        ...
                        if existing:
                            console.print(f"ℹ️  [yellow]Outcome already exists:[/yellow] {existing[1]}-{existing[0]} (Use --force to update)")
                            return
            ...
            with Progress(
                ...
                async with MLBStatsAPIClient() as mlb_client:
                    outcome = await game_outcome_service._check_game_outcome(mlb_client, game_info)
                    ...
                    if outcome:
                        ...
                        console.print(f"📊 Final Score: {outcome.away_team} {outcome.away_score} - {outcome.home_score} {outcome.home_team}")
                        console.print(f"🏆 Winner: {outcome.home_team if outcome.home_win else outcome.away_team}")
                        
                        if outcome.over is not None:
                            console.print(f"📈 Total: {outcome.over} (line: {outcome.total_line})")
                        if outcome.home_cover_spread is not None:
                            console.print(f"📊 Spread: {outcome.home_team} {'covered' if outcome.home_cover_spread else 'did not cover'} (line: {outcome.home_spread_line})")
                    else:
...
"""
    ...
    console.print(Panel(summary_text.strip(), title="📋 Results Summary", border_style="blue"))
