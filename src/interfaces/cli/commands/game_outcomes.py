...
@click.option(
    "--show-missing", "-m", is_flag=True, help="Show games missing outcomes"
)
    ...
    async def run_resolution():
        try:
            with Progress(
                ...
                # Get stats first
                stats_task = progress.add_task("Getting resolution statistics...", total=None)
                
                from src.services.game_id_resolution_service import game_id_resolution_service
                ...
                try:
                    ...
                    # Run resolution
                    resolution_task = progress.add_task("Resolving missing game IDs...", total=None)
...
def missing(days: int):
    ...
    async def find_missing():
        try:
            ...
            console.print(f"📋 Found {len(missing_data['missing_games'])} games missing outcomes:")
            ...
            for game in missing_data["missing_games"]:
                ...
                table.add_row(
                    str(game["id"]),
                    game["game_date"].strftime("%Y-%m-%d") if game["game_date"] else "N/A",
...
async def _get_action_network_coverage(start_date, end_date):
    ...
    async with get_connection() as conn:
        async with conn.cursor() as cursor:
            ...
            coverage_percentage = (games_with_outcomes / total_games * 100) if total_games > 0 else 0
...
async def _get_all_sources_coverage(start_date, end_date):
    ...
    async with get_connection() as conn:
        async with conn.cursor() as cursor:
            ...
            total_games, games_with_outcomes, games_with_mlb_id, action_network_games, sbr_games = row
            coverage_percentage = (games_with_outcomes / total_games * 100) if total_games > 0 else 0
...
async def _find_missing_outcomes(start_date, end_date):
    ...
    async with get_connection() as conn:
        async with conn.cursor() as cursor:
            ...
            coverage_percentage = ((total_games - len(missing_games)) / total_games * 100) if total_games > 0 else 0
...
def _display_resolution_stats(stats: dict, days: int):
    ...
    if improvement > 0:
        ...
        console.print(f"  • Potential coverage: {stats.get('potential_coverage_percentage', 0):.1f}%")
    else:
...
"""
    ...
    if not dry_run and results["resolved_games"] > 0:
        ...
        console.print(f"  • Run outcome checking to update game results")
        console.print(f"  • Command: uv run -m src.interfaces.cli outcomes check --days 30")
    elif dry_run and results["resolved_games"] > 0:
        console.print(f"\n🎯 [bold]To Apply Changes:[/bold]")
        console.print(f"  • Run without --dry-run flag to update database")
        console.print(f"  • Command: uv run -m src.interfaces.cli outcomes resolve-ids --days 30")
