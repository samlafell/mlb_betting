...
def parse_date(date_str: str) -> datetime:
    ...
    try:
        ...
        try:
            ...
            raise click.BadParameter(f"Invalid date format: {date_str}. Use YYYY-MM-DD or YYYY-MM-DD HH:MM:SS")
...
@click.option(
    ...
    type=int,
    default=10,
    help="Progress update interval (games)"
)
@click.option(
    "--checkpoint-interval",
    type=int,
...
def collect_range(
    ...
    async def _collect_range():
        try:
            ...
            if dry_run:
                console.print("[yellow]DRY RUN MODE - No actual data collection will occur[/yellow]")
                return
            ...
            with Progress(
                ...
                try:
                    ...
                    if result.games_failed > 0:
                        ...
                        console.print(f"uv run -m src.interfaces.cli batch-collection retry-failed --batch-id {result.batch_id}")
...
@click.option(
    ...
    type=str,
    required=True,
    help="Batch ID to retry failed games for"
)
@click.option(
    "--output-dir",
    type=click.Path(path_type=Path),
...
def retry_failed(batch_id: str, output_dir: Path):
    ...
    async def _retry_failed():
        try:
            ...
            console.print("[yellow]Retry functionality requires checkpoint loading implementation[/yellow]")
            console.print(f"[cyan]Would retry failed games from batch: {batch_id}[/cyan]")
...
def collect_season(
    ...
    if dry_run:
        console.print("[yellow]DRY RUN MODE - This would collect the full season[/yellow]")
        console.print(f"[cyan]Estimated date range: {start_date} to {end_date}[/cyan]")
        console.print("[cyan]Estimated games: ~2,430 games (162 games × 15 teams)[/cyan]")
        console.print("[cyan]Estimated duration: 8-12 hours depending on success rate[/cyan]")
        return
