    ...
    async def send_personal_message(
        ...
        try:
            ...
            handled_error = handle_exception(
                e, component="websocket_manager", operation="send_personal_message"
            )
            logger.error(
    ...
    async def broadcast(self, message: WebSocketMessage):
        ...
        for connection in self.active_connections:
            try:
                ...
                logger.error(
                    "WebSocket error broadcasting message",
                    error=e,
                    correlation_id=e.correlation_id,
                )
                ...
                handled_error = handle_exception(
                    e, component="websocket_manager", operation="broadcast_message"
                )
                logger.error(
...
async def get_system_health():
    ...
    try:
        ...
        return SystemHealthResponse(
            ...
            data_freshness_score=monitoring_report.business_metrics.data_freshness_score
            if monitoring_report.business_metrics
            else 0,
            system_load={
                "cpu_usage": monitoring_report.system_metrics.cpu_usage
                if monitoring_report.system_metrics
                else 0,
                "memory_usage": monitoring_report.system_metrics.memory_usage
                if monitoring_report.system_metrics
                else 0,
                "disk_usage": monitoring_report.system_metrics.disk_usage
                if monitoring_report.system_metrics
                else 0,
            },
            active_pipelines=len(pipeline_orchestration_service.get_active_pipelines()),
            recent_success_rate=orchestration_metrics.get("combined_insights", {}).get(
        ...
        logger.error(
            "Database error getting system health",
            error=e,
            correlation_id=e.correlation_id,
        )
        raise HTTPException(
        ...
        handled_error = handle_exception(
            e, component="monitoring_dashboard", operation="get_system_health"
        )
        logger.error(
...
async def get_all_metrics():
    ...
    try:
        ...
        return MetricsResponse(
            ...
            business_metrics={
                "opportunities_detected": monitoring_report.business_metrics.opportunities_found
                if monitoring_report.business_metrics
                else 0,
                "recommendations_made": monitoring_report.business_metrics.recommendations_made
                if monitoring_report.business_metrics
                else 0,
                "total_value_identified": monitoring_report.business_metrics.total_value_identified
                if monitoring_report.business_metrics
                else 0,
                "data_freshness_score": monitoring_report.business_metrics.data_freshness_score
                if monitoring_report.business_metrics
            ...
            system_metrics={
                ...
                "database_connections": monitoring_report.system_metrics.database_connections
                if monitoring_report.system_metrics
                else 0,
            },
...
async def execute_pipeline_manual(
    ...
    try:
        ...
        metrics_service.record_break_glass_activation(
            "manual_pipeline_execution", f"Manual execution of {pipeline_type} pipeline"
        )

        # Execute pipeline
        result = await pipeline_orchestration_service.execute_smart_pipeline(
            pipeline_type=pipeline_type, force_execution=force_execution
        )
        ...
        logger.error(
            "Manual pipeline execution failed",
            error=handled_error,
            correlation_id=handled_error.correlation_id,
        )
        raise HTTPException(
...
async def websocket_endpoint(websocket: WebSocket):
    ...
    try:
        ...
        await manager.send_personal_message(
            WebSocketMessage(type="initial_state", data=health_response.model_dump()),
            websocket,
        )
        ...
        while True:
            try:
                ...
                if client_message.get("type") == "request_update":
                    ...
                    await manager.send_personal_message(
                        WebSocketMessage(
                            type="metrics_update", data=metrics_response.model_dump()
                        ),
                        websocket,
                    )
    ...
    while True:
        try:
            if manager.active_connections:
                ...
                await manager.broadcast(
                    WebSocketMessage(
                        type="system_health_update", data=health_response.model_dump()
                    )
                )
                ...
                if active_pipelines:
                    await manager.broadcast(
                        WebSocketMessage(
                            ...
                            data={
                                "active_pipelines": [
                                    p.model_dump() for p in active_pipelines
                                ]
                            },
                        )
                    )
            ...
            logger.error(
                "Monitoring service error during broadcast",
                error=e,
                correlation_id=e.correlation_id,
            )
            ...
            logger.error(
                "Error in broadcast system updates",
                error=handled_error,
                correlation_id=handled_error.correlation_id,
            )
            await asyncio.sleep(settings.dashboard.error_recovery_delay)
