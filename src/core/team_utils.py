...
def get_team_mapping_summary() -> dict[str, str]:
    ...
    """
    return {
        "al_east": {name: abbrev for name, abbrev in MLB_TEAM_MAPPINGS.items()
                   if abbrev in ["NYY", "BOS", "TOR", "TB", "BAL"]},
        "al_central": {name: abbrev for name, abbrev in MLB_TEAM_MAPPINGS.items()
                      if abbrev in ["CWS", "CLE", "DET", "KC", "MIN"]},
        "al_west": {name: abbrev for name, abbrev in MLB_TEAM_MAPPINGS.items()
                   if abbrev in ["HOU", "LAA", "OAK", "SEA", "TEX"]},
        "nl_east": {name: abbrev for name, abbrev in MLB_TEAM_MAPPINGS.items()
                   if abbrev in ["ATL", "MIA", "NYM", "PHI", "WSH"]},
        "nl_central": {name: abbrev for name, abbrev in MLB_TEAM_MAPPINGS.items()
                      if abbrev in ["CHC", "CIN", "MIL", "PIT", "STL"]},
        "nl_west": {name: abbrev for name, abbrev in MLB_TEAM_MAPPINGS.items()
                   if abbrev in ["ARI", "COL", "LAD", "SD", "SF"]},
    }


def create_external_source_id(source: str, game_id: str, book_id: str, bet_type: str) -> str:
    """
    ...
    if len(external_id) > 50:
        ...
        max_game_id_len = 50 - len(source_abbrev) - len(book_id) - len(bet_type) - 3  # 3 underscores
