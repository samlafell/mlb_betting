...
class RateLimiter:
    ...
    def __init__(self, use_redis: bool = False, redis_client=None):
        ...
        if use_redis and redis_client is None:
            logger.warning(
                "Redis requested but no client provided, falling back to in-memory"
            )
        ...
        if settings.security.enable_redis_rate_limiting and settings.security.redis_url:
            try:
                ...
                logger.info(
                    ...
                    redis_url=settings.security.redis_url,
                )
            except ImportError:
                logger.warning(
                    "Redis not available, install redis package for production rate limiting"
                )
            except Exception as e:
                logger.error(
                    "Failed to connect to Redis, falling back to in-memory", error=e
                )

        rate_limiter = RateLimiter(
            use_redis=settings.security.enable_redis_rate_limiting
    ...
    try:
        ...
        for allowed_ip in settings.security.break_glass_ip_whitelist:
            try:
                ...
                logger.warning(
                    "Invalid IP in whitelist configuration", invalid_ip=allowed_ip
                )
                continue
...
async def verify_api_key(
    ...
    correlation_id = request.headers.get(
        "X-Correlation-ID", f"auth_{int(time.time() * 1000000)}"
    )
    client_ip = request.client.host if request.client else "unknown"
