...
class DatabaseSettings(BaseSettings):
    ...
    duckdb_path: Optional[str] = Field(
        default=None,
        description="Legacy DuckDB file path",
        env="DUCKDB_PATH"
    )
...
class SchemaSettings(BaseSettings):
    ...
    raw_data_schema: str = Field(
        default="raw_data",
        description="Schema for raw external data"
    )
    
    # Core betting data schemas  
    core_betting_schema: str = Field(
        default="core_betting",
        description="Schema for core betting data"
    )
    
    # Analytics schemas
    analytics_schema: str = Field(
        default="analytics",
        description="Schema for analytics and derived data"
    )
    
    # Operational schemas
    operational_schema: str = Field(
        default="operational",
        description="Schema for operational data"
    )
    
    # Legacy schema mappings
    legacy_splits_schema: str = Field(
        default="splits",
        description="Legacy splits schema"
    )
    
    legacy_mlb_betting_schema: str = Field(
        default="mlb_betting", 
        description="Legacy MLB betting schema"
    )
...
class DataSourceSettings(BaseSettings):
    ...
    sbd_identifier: str = Field(
        default="SBD",
        description="SportsBettingDime identifier"
    )
    
    vsin_identifier: str = Field(
        default="VSIN",
        description="VSIN identifier"
    )
    
    action_network_identifier: str = Field(
        default="ACTION_NETWORK",
        description="Action Network identifier"
    )
    
    mlb_stats_api_identifier: str = Field(
        default="MLB_STATS_API",
        description="MLB Stats API identifier"
    )
    
    sportsbookreview_identifier: str = Field(
        default="SPORTSBOOKREVIEW",
        description="SportsbookReview identifier"
    )
...
class APISettings(BaseSettings):
    ...
    action_network_api_key: Optional[str] = Field(
        default=None,
...
class ScrapingSettings(BaseSettings):
    ...
    vsin_sharp_splits_path: str = Field(
        ...
        description="VSIN sharp splits path"
    )
    ...
    enable_stealth: bool = Field(
        default=True,
        description="Enable stealth mode for scraping"
    )
...
class LoggingSettings(BaseSettings):
    ...
    sql_log_file: Optional[Path] = Field(
        default=None,
        description="SQL operations log file",
        env="SQL_LOG_FILE"
    )
    
    log_sql_operations: bool = Field(
        default=True,
        description="Enable SQL operations logging"
    )
    ...
    performance_threshold_ms: int = Field(
        ...
        ge=1,
        le=60000,
        description="Performance logging threshold in ms"
    )
...
class BettingSettings(BaseSettings):
    ...
    enable_juice_filter: bool = Field(
        default=True,
        description="Enable juice filtering",
        env="ENABLE_JUICE_FILTER"
    )
    ...
    max_daily_recommendations: int = Field(
        ...
        ge=1,
        le=50,
        description="Maximum daily recommendations"
    )
    ...
    break_even_win_rate: float = Field(
        ...
        ge=0.5,
        le=0.7,
        description="Break-even win rate threshold"
    )
...
class NotificationSettings(BaseSettings):
    ...
    alert_retention_days: int = Field(
        ...
        ge=1,
        le=365,
        description="Alert retention period in days"
    )
...
class FeatureFlags(BaseSettings):
    ...
    enable_consensus_analysis: bool = Field(
        default=True,
        description="Enable consensus analysis",
        env="FEATURE_CONSENSUS"
    )
...
class UnifiedSettings(BaseSettings):
    ...
    # Application info
    app_name: str = Field(
        default="MLB Sharp Betting System",
    ...
    def load_legacy_config(self, config_path: Optional[Path] = None) -> Dict[str, Any]:
        """
