...
from ...services.scheduling.scheduler_engine_service import SchedulerEngineService
...
class SystemStateAnalysis:
    ...
    backtesting_age_hours: Optional[float] = None
    analysis_age_hours: Optional[float] = None
...
class PipelineOrchestrationService:
    ...
    async def analyze_system_state(self) -> SystemStateAnalysis:
        ...
        try:
            ...
            self.logger.info("System state analysis completed",
    ...
    async def execute_smart_pipeline(self, pipeline_type: str = "full",
                                   force_execution: bool = False,
                                   detection_minutes: int = 60) -> PipelineExecutionResult:
        """
        ...
        try:
            ...
            if not stages_to_execute:
                ...
                self.logger.info("Pipeline completed - no stages needed", pipeline_id=pipeline_id)
                return result
            ...
            successful_stages = sum(1 for stage_result in result.stages.values() 
            ...
            self.logger.info("Smart pipeline execution completed",
                           pipeline_id=pipeline_id,
            ...
            self.logger.error("Smart pipeline execution failed",
                            pipeline_id=pipeline_id,
                            error=str(e))
            ...
            if pipeline_id in self.active_pipelines:
                ...
                if len(self.completed_pipelines) > self.pipeline_history_limit:
                    self.completed_pipelines = self.completed_pipelines[-self.pipeline_history_limit:]
        ...
        try:
            ...
            if analysis.data_quality_score < self.config.min_data_quality_score:
                recommendations['optimization_suggestions'].append({
                    ...
                    'actions': ['validate_data_sources', 'enhance_collection_logic']
                })
            
            return recommendations
            
        except Exception as e:
            self.logger.error("Failed to generate pipeline recommendations", error=str(e))
            return {
                'error': str(e),
                'system_health': 'unknown',
                'immediate_actions': [{'action': 'investigate_error', 'priority': 'high'}]
            }
                ...
                async with conn.cursor() as cursor:
                    ...
                    if row:
                        ...
                        if row[0]:  # latest_update
                            ...
                            analysis.needs_data_collection = analysis.data_age_hours > self.config.data_freshness_threshold
                        else:
                    ...
                    if analysis.data_age_hours and analysis.data_age_hours < self.config.data_freshness_threshold:
                        quality_factors.append(0.3)
    ...
    async def _analyze_backtesting_state(self, analysis: SystemStateAnalysis):
        ...
        try:
            ...
            analysis.needs_backtesting = analysis.backtesting_age_hours > self.config.backtesting_freshness_threshold
    ...
    async def _analyze_analysis_state(self, analysis: SystemStateAnalysis):
        ...
        try:
            ...
            analysis.needs_analysis = analysis.analysis_age_hours > self.config.analysis_freshness_threshold
    ...
    def _determine_stages_to_execute(self, pipeline_type: str, system_state: Dict[str, Any],
                                   force_execution: bool) -> List[PipelineStage]:
    ...
    async def _execute_stages_sequential(self, result: PipelineExecutionResult,
    ...
    async def _execute_stages_parallel(self, result: PipelineExecutionResult,
                                     stages: List[PipelineStage]):
    ...
    async def _execute_single_stage(self, stage: PipelineStage,
                                   result: PipelineExecutionResult) -> PipelineStageResult:
    ...
    async def _execute_data_collection_stage(self, stage_result: PipelineStageResult):
        ...
        try:
            ...
            stage_result.records_processed = collection_result.get('total_records_collected', 0)
            stage_result.success_count = collection_result.get('total_records_stored', 0)
...
async def execute_smart_pipeline(pipeline_type: str = "full",
                                force_execution: bool = False) -> PipelineExecutionResult:
