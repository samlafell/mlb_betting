...
class WorkflowConfig:
    ...
    data_collection_command: List[str] = field(default_factory=lambda: [
        "uv", "run", "python", "-m", "src.interfaces.cli.main", "collect", "all"
    ])
    analysis_command: List[str] = field(default_factory=lambda: [
        "uv", "run", "python", "-m", "src.interfaces.cli.main", "analyze", "opportunities"
    ])
...
class PreGameWorkflowService:
    ...
    def __init__(self, config: Optional[WorkflowConfig] = None):
        ...
        self.logger.info("PreGameWorkflowService initialized",
                        max_concurrent_workflows=self.config.max_concurrent_workflows,
                        max_retries=self.config.max_retries,
                        notifications_enabled=self.config.enable_notifications)
    
    async def execute_pre_game_workflow(self, game_id: str, context: Optional[Dict[str, Any]] = None,
                                      minutes_before: int = 5) -> WorkflowResult:
        """
        ...
        game_description = f"{game_data.get('away_team', 'TBD')} @ {game_data.get('home_team', 'TBD')}"
        ...
        try:
            ...
            self.logger.error("Pre-game workflow failed",
                            workflow_id=workflow_id,
                            game_id=game_id,
                            error=str(e))
            ...
            if workflow_id in self.active_workflows:
                ...
                if len(self.completed_workflows) > self.workflow_history_limit:
                    self.completed_workflows = self.completed_workflows[-self.workflow_history_limit:]
    
    async def _execute_workflow_with_semaphore(self, workflow_result: WorkflowResult) -> WorkflowResult:
    ...
    async def _execute_workflow_stages(self, workflow_result: WorkflowResult) -> WorkflowResult:
    ...
    async def _execute_stage_with_retry(self, stage: WorkflowStage, 
                                      handler: Callable, 
        ...
        for attempt in range(self.config.max_retries + 1):
            try:
                ...
                error_msg = f"Stage timeout after {self.config.stage_timeout_seconds} seconds"
                stage_result.error_message = error_msg
                
                if attempt == self.config.max_retries:
                    stage_result.mark_completed(StageStatus.TIMEOUT, error_msg)
                    self.logger.error("Stage timed out after all retries",
                                    stage=stage.value,
                                    attempts=attempt + 1)
                    return stage_result
                ...
                if attempt == self.config.max_retries:
                    ...
                    self.logger.error("Stage failed after all retries",
                                    stage=stage.value,
                                    attempts=attempt + 1,
                                    error=error_msg)
                    return stage_result
                
                self.logger.warning("Stage attempt failed",
                                  stage=stage.value,
                                  attempt=attempt,
                                  error=error_msg)
    ...
    async def _execute_data_collection_stage(self, stage_result: StageResult, 
                                           workflow_result: WorkflowResult):
        ...
        try:
            ...
            if result['return_code'] != 0:
                raise WorkflowError(f"Data collection failed with code {result['return_code']}")
            
            # Store metadata
            stage_result.metadata['command'] = ' '.join(self.config.data_collection_command)
    ...
    async def _execute_betting_analysis_stage(self, stage_result: StageResult,
                                            workflow_result: WorkflowResult):
        """Execute betting analysis stage."""
        self.logger.info("Executing betting analysis stage",
                        workflow_id=workflow_result.workflow_id)
        
        try:
            # Run analysis command
            result = await self._run_subprocess(
                self.config.analysis_command,
                self.config.project_root
            )
            ...
            if result['return_code'] != 0:
                raise WorkflowError(f"Analysis failed with code {result['return_code']}")
    ...
    async def _execute_validation_stage(self, stage_result: StageResult,
                                      workflow_result: WorkflowResult):
        ...
        try:
            ...
            if data_stage and data_stage.status != StageStatus.SUCCESS:
                validation_errors.append("Data collection stage did not complete successfully")
            
            # Check analysis results
            analysis_stage = workflow_result.get_stage_result(WorkflowStage.BETTING_ANALYSIS)
            if analysis_stage and analysis_stage.status != StageStatus.SUCCESS:
                validation_errors.append("Betting analysis stage did not complete successfully")
    ...
    async def _execute_notification_stage(self, stage_result: StageResult,
                                        workflow_result: WorkflowResult):
        ...
        self.logger.info("Executing notification stage",
                        workflow_id=workflow_result.workflow_id)
        
        try:
            # Determine notification type based on workflow status
            if workflow_result.is_stage_successful(WorkflowStage.DATA_COLLECTION) and \
               workflow_result.is_stage_successful(WorkflowStage.BETTING_ANALYSIS):
    ...
    async def _send_notification(self, workflow_result: WorkflowResult, 
...
async def execute_workflow_for_game(game_id: str, context: Optional[Dict[str, Any]] = None,
                                  minutes_before: int = 5) -> WorkflowResult:
