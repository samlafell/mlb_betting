...
class GameResolutionResult:
    ...
    success: bool
    staging_game_id: Optional[int] = None
    mapping: Optional[GameMapping] = None
...
class CrossSiteGameResolutionService:
    ...
    async def resolve_game_by_mlb_stats_api(self, mlb_game_id: str) -> GameResolutionResult:
        ...
        try:
            ...
            logger.error("Error resolving game by MLB Stats API ID", 
                        mlb_game_id=mlb_game_id, error=str(e))
            return GameResolutionResult(
                success=False,
                error_message=str(e)
            )
    
    async def resolve_game_by_action_network(self, action_network_game_id: int, 
                                           home_team: str = None, away_team: str = None,
                                           game_datetime: datetime = None) -> GameResolutionResult:
        ...
        try:
            ...
            if home_team and away_team and game_datetime:
                ...
                existing_by_teams = await conn.fetchrow("""
                    ...
                    LIMIT 1
                """, home_normalized, away_normalized, game_datetime)
                ...
                if existing_by_teams:
                    ...
                    await conn.execute("""
                        ...
                        WHERE id = $2
                    """, action_network_game_id, existing_by_teams['id'])
            ...
            logger.error("Error resolving game by Action Network ID", 
                        action_network_game_id=action_network_game_id, error=str(e))
            return GameResolutionResult(
                success=False,
                error_message=str(e)
            )
    
    async def resolve_game_by_teams_and_date(self, home_team: str, away_team: str, 
                                           game_datetime: datetime, source_id: str = None,
                                           source_type: str = None) -> GameResolutionResult:
        ...
        try:
            ...
            if existing:
                ...
                if source_id and source_type:
                    await self._update_source_id(conn, existing['id'], source_type, source_id)
            ...
            # Create new game
            source_kwargs = {source_type: source_id} if source_id and source_type else {}
            new_game_id = await self._create_staging_game(
                conn,
                home_team=home_normalized,
                away_team=away_normalized,
            ...
            logger.error("Error resolving game by teams and date", 
                        home_team=home_team, away_team=away_team, error=str(e))
            return GameResolutionResult(
                success=False,
                error_message=str(e)
            )
    ...
    async def update_game_mapping(self, staging_game_id: int, **source_ids) -> bool:
        ...
        try:
            ...
            if success:
                logger.info("Updated game mapping", staging_game_id=staging_game_id, 
                           source_ids=source_ids)
            
            return success
            
        except Exception as e:
            logger.error("Error updating game mapping", 
                        staging_game_id=staging_game_id, error=str(e))
            return False
    ...
    async def _create_staging_game(self, conn: asyncpg.Connection, home_team: str, 
                                 away_team: str, game_datetime: datetime,
                                 **source_ids) -> int:
        """Create a new staging game entry."""
        # Build dynamic insert query
        columns = ["home_team", "away_team", "game_datetime", "created_at", "updated_at"]
        ...
        logger.info("Created new staging game", game_id=game_id, 
                   home_team=home_team, away_team=away_team)
        
        return game_id
    
    async def _update_source_id(self, conn: asyncpg.Connection, staging_game_id: int,
