...
class OutcomeMetric(BaseModel):
    ...
    strategy_name: str | None = Field(None, description="Associated strategy name")
    sportsbook_name: str | None = Field(None, description="Associated sportsbook")
    bet_type: str | None = Field(
        None, description="Type of bet (moneyline, spread, total)"
    )
    ...
    data_quality_score: float | None = Field(
        None, description="Associated data quality score"
    )
    sharp_action_detected: bool | None = Field(
        None, description="Whether sharp action was detected"
    )
    line_movement_magnitude: Decimal | None = Field(
        None, description="Line movement magnitude"
    )

    # Metadata
    game_date: datetime = Field(..., description="Game date")
    metric_calculated_at: datetime = Field(
        default_factory=lambda: datetime.now(timezone.utc)
    )
    outcome_confirmed_at: datetime | None = Field(
        None, description="When outcome was confirmed"
    )
...
class OutcomeMetricsService:
    ...
    async def record_strategy_performance_metric(
        ...
        try:
            ...
            data_quality_score = await self._get_game_data_quality_score(
                game_external_id
            )

            # Check for sharp action
            sharp_action_detected = await self._check_sharp_action(
                game_external_id, bet_type
            )

            # Get line movement data
            line_movement = await self._get_line_movement_magnitude(
                game_external_id, bet_type
            )
            ...
            logger.info(
                "Recorded strategy performance metric",
                metric_id=metric_id,
    ...
    async def record_betting_line_accuracy_metric(
        ...
        try:
            ...
            logger.info(
                "Recorded betting line accuracy metric",
                metric_id=metric_id,
    ...
    async def get_strategy_performance_summary(
        self, strategy_name: str, days_back: int = 30
    ) -> dict[str, Any]:
        """
        ...
        try:
            ...
            async with get_connection() as conn:
                ...
                result = await conn.fetchrow(
                    performance_query, strategy_name, start_date
                )
                ...
                quality_result = await conn.fetchrow(
                    quality_query, strategy_name, start_date
                )
                ...
                return {
                    ...
                    "high_quality_accuracy": float(
                        quality_result["high_quality_accuracy"] or 0.0
                    ),
                    "low_quality_accuracy": float(
                        quality_result["low_quality_accuracy"] or 0.0
                    ),
                    "sharp_action_accuracy": float(
                        quality_result["sharp_action_accuracy"] or 0.0
                    ),
                    "no_sharp_accuracy": float(
                        quality_result["no_sharp_accuracy"] or 0.0
                    ),
                    "quality_impact": (
    ...
    async def get_data_quality_impact_analysis(
        ...
        try:
            ...
            async with get_connection() as conn:
                ...
                correlation_coeff = await self._calculate_quality_accuracy_correlation(
                    start_date
                )
    ...
    async def _check_sharp_action(
        self, game_external_id: str, bet_type: str
    ) -> bool | None:
    ...
    async def _get_line_movement_magnitude(
        self, game_external_id: str, bet_type: str
    ) -> Decimal | None:
        ...
        try:
            async with get_connection() as conn:
                ...
                result = await conn.fetchrow(query, game_external_id, bet_type)
                return (
                    Decimal(str(result["movement_magnitude"]))
                    if result and result["movement_magnitude"]
                    else None
                )
    ...
    async def _calculate_quality_accuracy_correlation(
        ...
        try:
            async with get_connection() as conn:
                ...
                denominator = (
                    (n * sum_q2 - sum_q * sum_q) * (n * sum_a2 - sum_a * sum_a)
                ) ** 0.5
