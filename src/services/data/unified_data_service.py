...
class UnifiedDataService:
    ...
    async def collect_action_network_history(
        ...
        try:
            ...
            self.logger.info(
                "Collecting Action Network history for single game",
            ...
            missing_fields = [
                field for field in required_fields if field not in game_data
            ]
            ...
            if result.success and result.data:
                historical_data = result.data[
                    0
                ]  # First item is the ActionNetworkHistoricalData
                ...
                self.logger.info(
                    "Successfully collected Action Network history",
                    game_id=game_data["game_id"],
                ...
                self.logger.error(
                    "Failed to collect Action Network history",
                    game_id=game_data.get("game_id"),
            ...
            self.stats["failed_operations"] += 1
            self.logger.error(
                "Exception during Action Network history collection",
                game_id=game_data.get("game_id"),
                error=str(e),
            )
            return None

    async def collect_multiple_action_network_histories(
        ...
        try:
            ...
            self.logger.info(
                "Collecting Action Network histories for multiple games",
            ...
            for result in results:
                if result.success and result.data:
                    ...
                    failed_count += 1
                    self.logger.warning(
                        "Failed to collect history for game",
                        errors=result.errors,
            ...
            self.logger.info(
                "Completed multiple Action Network history collection",
            ...
            self.stats["failed_operations"] += 1
            self.logger.error(
                "Exception during multiple Action Network history collection",
                error=str(e),
            )
            return []

    async def extract_histories_from_json_file(
        ...
        try:
            self.logger.info(
                "Extracting histories from JSON file", file_path=json_file_path
            )
            ...
            return await self.collect_multiple_action_network_histories(
                games_with_history
            )

        except Exception as e:
            self.logger.error(
                "Failed to extract histories from JSON file",
                file_path=json_file_path,
                error=str(e),
            )
            return []

    async def save_historical_data_to_json(
        self,
        historical_data_list: list[ActionNetworkHistoricalData],
        ...
        try:
            self.logger.info(
                "Saving historical data to JSON file",
            ...
            self.logger.info(
                "Successfully saved historical data to JSON file",
                output_path=output_file_path,
            ...
            self.logger.error(
                "Failed to save historical data to JSON file",
                output_path=output_file_path,
                error=str(e),
            )
            return False
    ...
    async def _save_to_database(
        ...
        try:
            ...
            last_extraction_time = await repository.get_last_extraction_time(
            ...
            if last_extraction_time:
                ...
                if time_diff.total_seconds() < 300:  # 5 minutes
                    self.logger.info(
                        "Skipping database save - recent extraction exists",
            ...
            if save_result["success"]:
                ...
                self.logger.info(
                    "Saved Action Network data to database",
                    game_id=historical_data.game_id,
                ...
                self.logger.error(
                    "Failed to save Action Network data to database",
                    game_id=historical_data.game_id,
            ...
            self.stats["database_save_errors"] += 1
            self.logger.error(
                "Exception during database save",
                game_id=historical_data.game_id,
                error=str(e),
            )

    async def get_new_lines_since_last_extraction(
        self, game_id: int, book_id: int, market_type: str
    ) -> list[dict[str, Any]]:
        """
        ...
        try:
            ...
            self.logger.error(
                "Failed to get new lines since last extraction",
                game_id=game_id,
                book_id=book_id,
    ...
    async def get_line_movement_summary(
        self,
        game_id: int,
        ...
        try:
            repository = await self._get_database_repository()
            return await repository.get_line_movement_summary(
                game_id, book_id, market_type
            )
        except Exception as e:
            self.logger.error(
                "Failed to get line movement summary", game_id=game_id, error=str(e)
            )
            return []

    async def check_for_new_movements(
        self, games_data: list[dict[str, Any]]
    ) -> dict[str, Any]:
        """
        ...
        try:
            ...
            new_movements_summary = {
                ...
                "total_new_lines": 0,
                "by_market": {"moneyline": 0, "spread": 0, "total": 0},
                "by_book": {},
            ...
            for game_data in games_data:
                ...
                for book_id in book_ids:
                    for market_type in market_types:
                        new_lines = (
                            await repository.get_new_lines_since_last_extraction(
                                game_id, book_id, market_type
                            )
                        )
                        ...
                        if new_lines:
                            ...
                            new_movements_summary["by_market"][market_type] += len(
                                new_lines
                            )
                            ...
                            new_movements_summary["by_book"][book_name] += len(
                                new_lines
                            )
            ...
            self.logger.info(
                "Completed new movements check",
                total_games=new_movements_summary["total_games_checked"],
                games_with_new_movements=new_movements_summary[
                    "games_with_new_movements"
                ],
