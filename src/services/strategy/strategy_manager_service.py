...
from ...core.exceptions import StrategyError, ValidationError, DatabaseError
...
class StrategyConfiguration:
    ...
    def calculate_performance_trend(self, historical_performance: List[float]) -> PerformanceTrend:
        ...
        older_avg = sum(historical_performance[:-3]) / len(historical_performance[:-3]) if len(historical_performance) > 3 else recent_avg
...
class StrategyManagerService:
    ...
    def __init__(self, config: Optional[StrategyManagerConfig] = None):
        ...
        self.logger.info("StrategyManagerService initialized",
    ...
    async def get_active_strategies(self, strategy_type: Optional[StrategyType] = None) -> List[StrategyConfiguration]:
        """
        ...
        self.logger.debug("Retrieved active strategies",
                        total_count=len(active_strategies),
                        strategy_type=strategy_type.value if strategy_type else "all")
        
        return active_strategies
    
    async def get_strategy_by_id(self, strategy_id: str) -> Optional[StrategyConfiguration]:
        """
    ...
    async def get_strategy_by_name(self, strategy_name: str) -> Optional[StrategyConfiguration]:
        """
    ...
    async def create_strategy(self, strategy_config: Dict[str, Any]) -> StrategyConfiguration:
        """
    ...
    async def update_strategy_performance(self, strategy_id: str, performance_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if not strategy:
                self.logger.warning("Strategy not found for performance update", 
            ...
            if len(self.performance_history[strategy_id]) > 20:
                self.performance_history[strategy_id] = self.performance_history[strategy_id][-20:]
            ...
            self.logger.info("Strategy performance updated",
                           strategy_id=strategy_id,
                           win_rate=win_rate,
                           roi=roi,
            ...
            self.logger.error("Failed to update strategy performance",
                            strategy_id=strategy_id,
                            error=str(e))
            return False
    ...
    async def enable_strategy(self, strategy_id: str) -> bool:
        ...
        try:
            ...
            if self.config.enable_validation:
                validation_result = await self._validate_strategy_for_activation(strategy)
                if not validation_result['valid']:
                    self.logger.warning("Strategy validation failed for enablement",
                                      strategy_id=strategy_id,
            ...
            self.logger.info("Strategy enabled successfully",
                           strategy_id=strategy_id,
                           strategy_name=strategy.strategy_name)
            ...
            self.logger.error("Failed to enable strategy",
                            strategy_id=strategy_id,
                            error=str(e))
            return False
    ...
    async def disable_strategy(self, strategy_id: str, reason: str = "manual") -> bool:
        ...
        try:
            ...
            self.logger.info("Strategy disabled",
                           strategy_id=strategy_id,
                           strategy_name=strategy.strategy_name,
                           reason=reason)
            ...
            self.logger.error("Failed to disable strategy",
                            strategy_id=strategy_id,
                            error=str(e))
            return False
    ...
    async def _refresh_cache_if_needed(self):
        ...
        if (now - self.cache_last_updated).total_seconds() > (self.config.cache_refresh_interval_minutes * 60):
    ...
    async def _store_strategy(self, strategy: StrategyConfiguration):
        ...
        try:
            ...
            # For now, we'll use a placeholder
            self.logger.debug("Storing strategy to database", strategy_id=strategy.strategy_id)
            
            # Placeholder implementation
            await asyncio.sleep(0.01)  # Simulate database write
            
        except Exception as e:
            self.logger.error("Failed to store strategy", 
                            strategy_id=strategy.strategy_id, 
                            error=str(e))
            raise
    
    async def _validate_strategy_for_activation(self, strategy: StrategyConfiguration) -> Dict[str, Any]:
    ...
    async def _check_auto_disable_conditions(self, strategy: StrategyConfiguration) -> bool:
        """Check if strategy should be auto-disabled."""
        ...
        if strategy.roi_percentage < self.config.auto_disable_roi_threshold:
            ...
            self.logger.warning("Strategy auto-disabled due to poor ROI",
                              strategy_id=strategy.strategy_id,
        ...
        if (strategy.win_rate < 0.50 and strategy.roi_percentage < 0.0) or \
           (strategy.win_rate < 0.55 and strategy.roi_percentage < -5.0):
            await self.disable_strategy(strategy.strategy_id, "auto")
            self.logger.warning("Strategy auto-disabled due to poor performance",
                              strategy_id=strategy.strategy_id,
                              win_rate=strategy.win_rate,
                              roi=strategy.roi_percentage)
            return True
        ...
        if strategy.consecutive_losses >= self.config.max_consecutive_losses:
            await self.disable_strategy(strategy.strategy_id, "auto")
            self.logger.warning("Strategy auto-disabled due to consecutive losses",
                              strategy_id=strategy.strategy_id,
...
async def get_active_strategies(strategy_type: Optional[StrategyType] = None) -> List[StrategyConfiguration]:
