...
class SchedulerMetrics:
    ...
    last_game_alert: datetime | None = None
...
class SchedulerEngineService:
    ...
    def __init__(self, config: SchedulerConfig | None = None):
        ...
        self.logger.info(
            "SchedulerEngineService initialized",
    ...
    async def start(self, mode: str = "full") -> None:
        ...
        try:
            ...
            self.logger.info(
                "SchedulerEngineService started successfully",
                mode=mode,
    ...
    async def _setup_core_jobs(self):
        ...
        daily_job = ScheduledJob(
            ...
            trigger=CronTrigger(
                hour=self.config.daily_setup_hour, minute=0, timezone=self.timezone
            ),
    ...
    async def _setup_backtesting_jobs(self):
        ...
        weekly_backtest_job = ScheduledJob(
            ...
            trigger=CronTrigger(
                day_of_week="mon", hour=6, minute=0, timezone=self.timezone
            ),
    ...
    async def _job_wrapper(
        self, handler: Callable, args: list[Any], kwargs: dict[str, Any], job_id: str
    ):
        ...
        try:
            self.logger.debug(
                "Executing job", job_id=job_id, job_type=job.job_type.value
            )
    ...
    async def _send_error_notification(self, job_id: str, error_message: str):
        ...
        self.logger.warning(
            "JOB ERROR NOTIFICATION", job_id=job_id, error=error_message
        )
    ...
    def _display_startup_summary(self, mode: str):
        ...
        print(
            f"⏰ Game Alerts: {self.config.alert_minutes_before_game} minutes before each game"
        )
        print("🕐 Hourly Collection: Every hour at :00")
        print(
            f"🌅 Daily Setup: {self.config.daily_setup_hour:02d}:00 {self.config.timezone}"
        )
        print(
            f"📧 Notifications: {'✅ Enabled' if self.config.notifications_enabled else '❌ Disabled'}"
        )
...
async def start_scheduler(
    mode: str = "full", config: SchedulerConfig | None = None
