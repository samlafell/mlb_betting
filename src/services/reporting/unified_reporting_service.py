...
class UnifiedReportingService:
    ...
    async def generate_report(self, config: ReportConfig) -> ReportResult:
        ...
        try:
            ...
            if config.format == ReportFormat.CONSOLE:
                result.content = await self._generate_console_report(report_data, config)
                ...
                raise ReportGenerationException(f"Unsupported report format: {config.format}")
            ...
            logger.info(f"Report generated successfully in {result.generation_time:.2f}s")
    ...
    async def generate_opportunities_report(
        ...
        config = ReportConfig(
            report_type=ReportType.OPPORTUNITIES,
            format=format,
            output_path=output_path
        )
    ...
    async def _collect_report_data(self, config: ReportConfig) -> ReportData:
        ...
        try:
            if config.report_type == ReportType.DAILY:
                ...
                report_data = await self._collect_performance_report_data(report_data, config)
            elif config.report_type == ReportType.OPPORTUNITIES:
                report_data = await self._collect_opportunities_report_data(report_data, config)
            elif config.report_type == ReportType.SYSTEM_HEALTH:
                report_data = await self._collect_system_health_report_data(report_data, config)
            else:
    ...
    async def _collect_daily_report_data(self, report_data: ReportData, config: ReportConfig) -> ReportData:
        """Collect data for daily report."""
        # Use legacy service if available
        if hasattr(self, 'legacy_daily_report_service'):
            try:
                legacy_data = await self.legacy_daily_report_service.generate_daily_report(
    ...
    async def _collect_performance_report_data(self, report_data: ReportData, config: ReportConfig) -> ReportData:
    ...
    async def _collect_opportunities_report_data(self, report_data: ReportData, config: ReportConfig) -> ReportData:
    ...
    async def _collect_system_health_report_data(self, report_data: ReportData, config: ReportConfig) -> ReportData:
    ...
    async def _generate_console_report(self, data: ReportData, config: ReportConfig) -> str:
        ...
        # Footer
        lines.append("=" * 80)
        lines.append(f"Generated: {data.generated_at.strftime('%Y-%m-%d %H:%M:%S EST')}")
        lines.append("=" * 80)
        
        return "\n".join(lines)
    
    async def _generate_json_report(self, data: ReportData, config: ReportConfig) -> str:
    ...
    async def _generate_csv_report(self, data: ReportData, config: ReportConfig) -> str:
        ...
        if data.recommendations:
            writer = csv.writer(output)
            writer.writerow(['Game', 'Recommendation', 'Confidence', 'Strategy', 'Expected Value'])
    ...
    async def _generate_html_report(self, data: ReportData, config: ReportConfig) -> str:
    ...
    async def _send_email_report(self, data: ReportData, config: ReportConfig) -> ReportResult:
