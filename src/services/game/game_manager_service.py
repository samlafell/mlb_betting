...
class GameManagerService:
        ...
        try:
            ...
            async with get_connection() as conn:
                async with conn.cursor() as cursor:
                    ...
                    existing_game = await self._find_existing_game(
                        cursor, normalized_data
                    )
            ...
            self.logger.error(
                "Failed to create/update game", error=str(e), game_data=game_data
            )
    ...
    async def get_game_by_id(
        self, game_id: str | int, id_type: str = "any"
    ) -> dict[str, Any] | None:
        """
        ...
        try:
            async with get_connection() as conn:
                async with conn.cursor() as cursor:
                    ...
                    self.logger.debug(
                        "Retrieved recent games", count=len(games), days=days
                    )
                    return games

        except Exception as e:
            self.logger.error(
                "Failed to retrieve recent games", days=days, error=str(e)
            )
    ...
    async def get_games_by_date_range(
        ...
        try:
            ...
            raise DatabaseError(
                f"Failed to retrieve games by date range: {str(e)}"
            ) from e
    ...
    async def _validate_and_normalize_game_data(
        self, game_data: dict[str, Any]
    ) -> dict[str, Any]:
        """Validate and normalize game data."""
    ...
    async def _find_existing_game(
        self, cursor, game_data: dict[str, Any]
    ) -> dict[str, Any] | None:
    ...
    async def _update_existing_game(
        self, cursor, game_id: int, game_data: dict[str, Any]
    ) -> GameCreationResult:
...
async def get_game_by_id(
    game_id: str | int, id_type: str = "any"
) -> dict[str, Any] | None:
