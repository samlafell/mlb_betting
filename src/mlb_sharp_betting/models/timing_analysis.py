...
from mlb_sharp_betting.models.base import BaseModel, TimestampedModel, IdentifiedModel
...
class TimingPerformanceMetrics(BaseModel):
    ...
    total_bets: int = Field(
        ..., 
        description="Total number of bets in this bucket",
        ge=0
    )
    ...
    pushes: int = Field(
        default=0,
        description="Number of pushed bets",
        ge=0
    )
    
    # Financial metrics  
    total_units_wagered: Decimal = Field(
        ...,
        description="Total units wagered",
        ge=0
    )
    ...
    # Average odds
    avg_odds_at_recommendation: Optional[Decimal] = Field(
        default=None,
        description="Average odds when recommendation was made"
    )
    
    avg_closing_odds: Optional[Decimal] = Field(
        default=None,
        description="Average closing odds at game time"
    )
    ...
    def validate_bet_counts(cls, v: int, info: ValidationInfo) -> int:
        ...
        if info.data and "total_bets" in info.data:
            ...
            if total_decided > info.data["total_bets"]:
                raise ValueError("Sum of wins, losses, and pushes cannot exceed total bets")
    ...
    @property
    def odds_movement(self) -> Optional[float]:
...
class TimingBucketAnalysis(IdentifiedModel):
    ...
    source: Optional[DataSource] = Field(
        default=None,
        description="Data source filter (None for all sources)"
    )
    
    book: Optional[BookType] = Field(
        default=None, 
        description="Sportsbook filter (None for all books)"
    )
    
    split_type: Optional[SplitType] = Field(
        default=None,
        description="Bet type filter (None for all types)"
    )
    ...
    analysis_end_date: datetime = Field(
        ...,
...
class ComprehensiveTimingAnalysis(TimestampedModel):
    ...
    analysis_name: str = Field(
        ...,
        description="Name/description of this analysis",
        max_length=200
    )
    
    total_games_analyzed: int = Field(
        ...,
        description="Total number of games included in analysis",
        ge=0
    )
    
    total_recommendations: int = Field(
        ..., 
        description="Total number of recommendations analyzed",
        ge=0
    )
    ...
    best_bucket: Optional[TimingBucket] = Field(
        default=None,
        description="Best performing timing bucket"
    )
    
    best_source: Optional[DataSource] = Field(
        default=None,
        description="Best performing data source"
    )
    ...
    # Trend analysis
    trends: Dict[str, Union[str, float, int]] = Field(
        default_factory=dict,
        description="Trend analysis results and insights"
    )
    ...
    def validate_analysis_consistency(self) -> 'ComprehensiveTimingAnalysis':
        ...
        if self.bucket_analyses:
            bucket_total = sum(analysis.metrics.total_bets for analysis in self.bucket_analyses)
            # Allow some variance for filtered vs unfiltered data
            if abs(bucket_total - self.total_recommendations) > (self.total_recommendations * 0.1):
    ...
    def optimal_timing_recommendation(self) -> str:
        ...
        for analysis in self.bucket_analyses:
            ...
            roi_score = min(analysis.metrics.roi_percentage / 20, 1.0)  # Cap at 20% ROI = max score
            win_rate_score = max(0, (analysis.metrics.win_rate - 50) / 20)  # 50% = 0, 70% = 1.0
            confidence_score = analysis.metrics.confidence_level.value.count("HIGH") * 0.25
...
class RealtimeTimingLookup(BaseModel):
    ...
    hours_until_game: float = Field(
        ...,
        description="Hours until game start",
        ge=0
    )
    
    source: Optional[DataSource] = Field(
        default=None,
        description="Data source for the recommendation"
    )
    
    book: Optional[BookType] = Field(
        default=None,
        description="Sportsbook for the recommendation" 
    )
    
    split_type: SplitType = Field(
        ...,
        description="Type of bet being considered"
    )
...
class TimingRecommendation(BaseModel):
    ...
    # Input context
    lookup: RealtimeTimingLookup = Field(
        ...,
        description="The lookup parameters used"
    )
    
    # Historical performance
    historical_metrics: Optional[TimingPerformanceMetrics] = Field(
        default=None,
        description="Historical performance for this timing/context"
    )
    
    # Recommendation
    recommendation: str = Field(
        ...,
        description="Betting recommendation based on timing",
        max_length=500
    )
    
    confidence: str = Field(
        ...,
        description="Confidence level for this recommendation"
    )
    ...
    expected_roi: Optional[float] = Field(
        default=None,
        description="Expected ROI percentage based on historical data"
    )
    
    # Risk assessment
    risk_factors: List[str] = Field(
        default_factory=list,
        description="Risk factors to consider for this timing"
    )
    
    sample_size_warning: bool = Field(
        default=False,
        description="Whether to warn about insufficient sample size"
    )
    
    @property
    def is_recommended(self) -> bool:
        """Whether this timing is recommended for betting."""
        return self.confidence in ["HIGH_CONFIDENCE", "MODERATE_CONFIDENCE"] and not self.sample_size_warning
