...
class ActionNetworkPrice(ValidatedModel):
    ...
    decimal: Optional[float] = Field(
        default=None,
        description="Decimal odds format",
        gt=0
    )
    
    american: Optional[int] = Field(
        default=None,
...
class ActionNetworkMarketData(ValidatedModel):
    ...
    home: Optional[ActionNetworkPrice] = Field(
        default=None,
        description="Home team or over pricing"
    )
    
    away: Optional[ActionNetworkPrice] = Field(
        default=None,
        description="Away team or under pricing"
    )
    ...
    timestamp: Optional[datetime] = Field(
        default=None,
        description="Timestamp when this market data was recorded"
    )
...
class ActionNetworkHistoricalEntry(ValidatedModel):
    ...
    timestamp: Optional[datetime] = Field(
        default=None,
        description="Timestamp of this historical entry"
    )
    ...
    # Market data
    moneyline: Optional[ActionNetworkMarketData] = Field(
        default=None,
        description="Moneyline market data"
    )
    
    spread: Optional[ActionNetworkMarketData] = Field(
        default=None,
        description="Spread market data"
    )
    
    total: Optional[ActionNetworkMarketData] = Field(
        default=None,
        description="Total market data"
    )
...
class ActionNetworkHistoricalData(IdentifiedModel, ValidatedModel):
    ...
    game_id: int = Field(
        ...,
        description="Action Network game ID",
        ge=1
    )
    ...
    historical_entries: List[ActionNetworkHistoricalEntry] = Field(
        ...
        description="List of historical line movement entries"
    )
    ...
    pregame_entries: int = Field(
        default=0,
        description="Number of pregame entries",
        ge=0
    )
    
    live_entries: int = Field(
        default=0,
        description="Number of live entries",
        ge=0
    )
                ...
                for i in range(1, len(timestamps)):
                    if timestamps[i] < timestamps[i-1]:
                        ...
                        if time_diff > 60:  # More than 1 minute difference
                            raise ValueError("Historical entries should be in chronological order")
        return v
            ...
            for entry in self.historical_entries:
                ...
                if current_market and prev_entry:
                    ...
                    if prev_market and current_market:
                        ...
                        if self._has_line_movement(prev_market, current_market, market_type):
                            movements += 1
                            if self._is_significant_movement(prev_market, current_market, market_type):
                                significant_moves += 1
    ...
    def _has_line_movement(self, prev_market: ActionNetworkMarketData, 
        ...
        if market_type in ["spread", "total"]:
            return (prev_market.line != current_market.line or
                   (prev_market.home and current_market.home and 
                    prev_market.home.american != current_market.home.american))
        else:  # moneyline
            return ((prev_market.home and current_market.home and 
                    prev_market.home.american != current_market.home.american) or
                   (prev_market.away and current_market.away and 
                    prev_market.away.american != current_market.away.american))
    
    def _is_significant_movement(self, prev_market: ActionNetworkMarketData, 
                               current_market: ActionNetworkMarketData, 
                               market_type: str) -> bool:
        ...
        if market_type in ["spread", "total"]:
            ...
            if (prev_market.home and current_market.home and 
                prev_market.home.american and current_market.home.american):
                return abs(prev_market.home.american - current_market.home.american) >= 10
        return False
...
class TeamDimension(IdentifiedModel, ValidatedModel):
    ...
    team_id: int = Field(
        ...,
        description="Action Network team ID",
        ge=1
    )
    ...
    display_name: str = Field(
        ...,
        description="Display name (e.g., 'Yankees')",
        min_length=1,
        max_length=50
    )
    ...
    short_name: str = Field(
        ...,
        description="Short name (e.g., 'Yankees')",
        min_length=1,
        max_length=50
    )
    ...
    conference_type: str = Field(
        ...,
        description="Conference/League (AL or NL)",
        pattern="^(AL|NL)$"
    )
    ...
    url_slug: str = Field(
        ...,
        description="URL slug for team pages",
        min_length=1,
        max_length=100
    )
...
class GameFact(IdentifiedModel, ValidatedModel):
    ...
    id_mlbstatsapi: Optional[int] = Field(
        default=None,
    ...
    dim_home_team_actionid: int = Field(
        ...,
        description="Foreign key to home team in dim_teams",
        ge=1
    )
    
    dim_away_team_actionid: int = Field(
        ...,
        description="Foreign key to away team in dim_teams",
        ge=1
    )
    ...
    dim_date: date = Field(
        ...,
        description="Game date (local time)"
    )
    
    dim_time: Optional[time] = Field(
        default=None,
        description="Game start time (local time)"
    )
    
    dim_datetime: Optional[datetime] = Field(
        default=None,
        description="Game start timestamp with timezone"
    )
    ...
    venue_name: Optional[str] = Field(
        default=None,
        description="Stadium/venue where game is played",
        max_length=200
    )
