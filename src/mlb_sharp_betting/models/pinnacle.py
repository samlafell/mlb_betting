...
class PinnacleMarket(IdentifiedModel, ValidatedModel):
    ...
    matchup_id: int = Field(..., description="Pinnacle matchup ID", gt=0)

    market_type: PinnacleMarketType = Field(..., description="Type of betting market")

    key: str = Field(
        ..., description="Market key identifier", min_length=1, max_length=50
    )
    ...
    # Market details
    period: int = Field(
        ..., description="Period/quarter for the market (0 = full game)", ge=0
    )

    status: MarketStatus = Field(..., description="Current market status")

    cutoff_at: datetime = Field(..., description="When betting closes for this market")
    ...
    limits: list[PinnacleLimit] = Field(
        ..., description="List of betting limits for this market", min_items=1
    )

    # Metadata
    version: int = Field(
        ..., description="Market version number for tracking changes", ge=0
    )

    is_alternate: bool = Field(
        default=False, description="Whether this is an alternate line market"
    )
    ...
    @validator("prices")
    def validate_prices_consistency(
        cls, v: list[PinnaclePrice], values: dict[str, Any]
    ) -> list[PinnaclePrice]:
        """
        ...
        if market_type == PinnacleMarketType.MONEYLINE:
            ...
            if not designations.issuperset(required_designations):
                raise ValueError(
                    f"Moneyline market must have home and away prices, got: {designations}"
                )
            ...
            if not designations.issuperset(required_designations):
                raise ValueError(
                    f"Spread market must have home and away prices, got: {designations}"
                )
            ...
            if not designations.issuperset(required_designations):
                raise ValueError(
                    f"Total market must have over and under prices, got: {designations}"
                )

        return v

    @validator("line_value")
    def validate_line_value_required(
        ...
        market_type = values.get("market_type")
        if (
            market_type in {PinnacleMarketType.SPREAD, PinnacleMarketType.TOTAL}
            and v is None
        ):
    ...
    def get_price_by_designation(
        self, designation: PriceDesignation
    ) -> PinnaclePrice | None:
        """
    ...
    def market_display_name(self) -> str:
        ...
        if self.market_type == PinnacleMarketType.MONEYLINE:
            ...
            return (
                f"Team Total ({self.line_value})" if self.line_value else "Team Total"
            )
        else:
    ...
    class Config:
        json_schema_extra = {
            "example": {
                ...
                "limits": [{"amount": "2000.00", "type": "maxRiskStake"}],
                "version": 3149186706,
                "is_alternate": False,
            }
        }
...
class PinnacleOddsSnapshot(IdentifiedModel, ValidatedModel):
    ...
    markets: list[PinnacleMarket] = Field(
        ..., description="List of markets captured in this snapshot", min_items=1
    )
    ...
    def get_markets_by_type(
        self, market_type: PinnacleMarketType
    ) -> list[PinnacleMarket]:
        """
    ...
    def get_market_by_key(self, key: str) -> PinnacleMarket | None:
        """
        Get market by key.
...
class PinnacleSport(ValidatedModel):
    ...
    primary_market_type: str = Field(
        ..., description="Primary market type for this sport", alias="primaryMarketType"
    )
    is_featured: bool = Field(
        default=False, description="Whether sport is featured", alias="isFeatured"
    )
    is_hidden: bool = Field(
        default=False, description="Whether sport is hidden", alias="isHidden"
    )
    is_sticky: bool = Field(
        default=False, description="Whether sport is sticky", alias="isSticky"
    )
    matchup_count: int = Field(
        default=0, description="Number of matchups", alias="matchupCount"
    )
    matchup_count_se: int = Field(
        default=0, description="Number of SE matchups", alias="matchupCountSE"
    )
    feature_order: int = Field(
        default=0, description="Feature order", alias="featureOrder"
    )
...
class PinnacleLeague(ValidatedModel):
    ...
    is_featured: bool = Field(
        default=False, description="Whether league is featured", alias="isFeatured"
    )
    is_hidden: bool = Field(
        default=False, description="Whether league is hidden", alias="isHidden"
    )
    is_promoted: bool = Field(
        default=False, description="Whether league is promoted", alias="isPromoted"
    )
    is_sticky: bool = Field(
        default=False, description="Whether league is sticky", alias="isSticky"
    )
    matchup_count: int = Field(
        default=0, description="Number of matchups", alias="matchupCount"
    )
...
class PinnacleParticipant(ValidatedModel):
    ...
    id: int | None = Field(default=None, description="Participant ID")
    name: str = Field(..., description="Participant name")
    alignment: PinnacleAlignment = Field(..., description="Participant alignment")
    order: int = Field(default=0, description="Display order")
    rotation: int | None = Field(default=None, description="Rotation number")
    score: int | None = Field(default=None, description="Current score")
...
class PinnacleParentInfo(ValidatedModel):
    ...
    participants: list[PinnacleParticipant] = Field(
        ..., description="Parent participants"
    )
    start_time: datetime = Field(
        ..., description="Parent start time", alias="startTime"
    )
...
class PinnacleMatchup(ValidatedModel):
    ...
    participants: list[PinnacleParticipant] = Field(
        ..., description="Matchup participants"
    )
    start_time: datetime = Field(
        ..., description="Matchup start time", alias="startTime"
    )
    ...
    is_live: bool = Field(
        default=False, description="Whether matchup is live", alias="isLive"
    )
    is_featured: bool = Field(
        default=False, description="Whether matchup is featured", alias="isFeatured"
    )
    is_highlighted: bool = Field(
        default=False,
        description="Whether matchup is highlighted",
        alias="isHighlighted",
    )
    is_promoted: bool = Field(
        default=False, description="Whether matchup is promoted", alias="isPromoted"
    )
    has_live: bool = Field(
        default=False, description="Whether matchup has live betting", alias="hasLive"
    )
    has_markets: bool = Field(
        default=False, description="Whether matchup has markets", alias="hasMarkets"
    )

    # Optional fields for special bets
    special: PinnacleSpecial | None = Field(
        default=None, description="Special bet information"
    )
    parent: PinnacleParentInfo | None = Field(
        default=None, description="Parent matchup information"
    )
    type: str | None = Field(default=None, description="Matchup type")
    units: str | None = Field(default=None, description="Units for special bets")
