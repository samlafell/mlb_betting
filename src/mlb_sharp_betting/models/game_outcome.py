...
class GameOutcome(BaseModel):
    ...
    home_cover_spread: bool | None = Field(
        None, description="Whether home team covered spread (if available)"
    )

    # Additional context
    total_line: float | None = Field(
        None, description="Total line used for over/under calculation"
    )
    home_spread_line: float | None = Field(
        None,
        description="Spread line for home team (positive = home gets points, negative = home gives points)",
    )
    game_date: datetime | None = Field(None, description="Game date/time")
    ...
    def validate_home_win(cls, v: bool, info: ValidationInfo) -> bool:
        ...
        if info.data and "home_score" in info.data and "away_score" in info.data:
            ...
            if v != expected:
                raise ValueError(
                    f"home_win ({v}) doesn't match scores: home {info.data['home_score']}, away {info.data['away_score']}"
                )
        return v
    ...
    def validate_over_calculation(cls, v: bool, info: ValidationInfo) -> bool:
        ...
        if (
            info.data
            and "total_line" in info.data
            and info.data["total_line"] is not None
        ):
            if "home_score" in info.data and "away_score" in info.data:
                ...
                if v != expected:
                    raise ValueError(
                        f"over ({v}) doesn't match calculation: {total_score} vs {info.data['total_line']}"
                    )
        return v
    ...
    def validate_spread_cover(cls, v: bool | None, info: ValidationInfo) -> bool | None:
        ...
        if (
            v is not None
            and info.data
            and "home_spread_line" in info.data
            and info.data["home_spread_line"] is not None
        ):
            if "home_score" in info.data and "away_score" in info.data:
                home_spread_result = (
                    info.data["home_score"] + info.data["home_spread_line"]
                )
                expected = home_spread_result > info.data["away_score"]
                if v != expected:
                    raise ValueError(
                        f"home_cover_spread ({v}) doesn't match calculation"
                    )
        return v

    model_config = ConfigDict(
        json_encoders={datetime: lambda v: v.isoformat(), Team: lambda v: v.value},
