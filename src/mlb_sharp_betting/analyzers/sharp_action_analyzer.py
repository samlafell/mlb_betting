...
class SharpActionAnalyzer:
    ...
    def extract_sharp_features(self) -> pd.DataFrame:
                ...
                *,
                CASE 
                    WHEN split_type = 'moneyline' AND split_value LIKE '{%}' THEN
                        json_extract_string(split_value, '$.home')::FLOAT
                ...
                CASE 
                    WHEN split_type = 'moneyline' AND split_value LIKE '{%}' THEN
                        json_extract_string(split_value, '$.away')::FLOAT
                    WHEN split_type IN ('spread', 'total') THEN
                        TRY_CAST(split_value AS FLOAT)
                    ELSE NULL
                END as away_line,
        ...
        JOIN mlb_betting.main.game_outcomes go ON gf.game_id = go.game_id
    ...
    def calculate_sharp_scores(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        numeric_cols = ['avg_sharp_differential', 'max_line_movement', 'pct_reverse_movement', 
                       'pct_sharp_money', 'ml_sharp_differential', 'spread_sharp_differential']
        ...
        features_to_scale = ['avg_sharp_differential', 'max_line_movement', 'pct_reverse_movement']
    ...
    def analyze_predictive_power(self, df: pd.DataFrame) -> Dict:
        ...
        feature_cols = [
            'avg_sharp_differential', 'max_sharp_differential', 'min_sharp_differential',
            'avg_line_movement', 'max_line_movement', 'avg_line_direction',
            'pct_reverse_movement', 'pct_consensus_fade', 'pct_sharp_money',
            'avg_line_changes', 'num_books', 'sharp_action_score',
            'ml_sharp_differential', 'spread_sharp_differential', 'total_sharp_differential'
        ]
        ...
        for target_name, y in targets.items():
            ...
            if len(X_clean) < 50:  # Need minimum samples
                print(f"Skipping {target_name} - insufficient data ({len(X_clean)} samples)")
                continue
    ...
    def create_visualizations(self, df: pd.DataFrame) -> None:
        ...
        try:
            ...
            fig1 = px.histogram(
                df, x='sharp_action_score', color='sharp_strength',
                title='Distribution of Sharp Action Scores',
                labels={'sharp_action_score': 'Sharp Action Score', 'count': 'Number of Games'},
                nbins=30
            )
            fig1.show()
            
            # 2. Sharp Action vs Win Rate Analysis
            win_rate_analysis = df.groupby('sharp_strength').agg({
                'home_win': 'mean',
            ...
            win_rate_analysis.columns = ['Home Win Rate', 'Spread Cover Rate', 'Over Rate', 'Games']
            ...
            for outcome in ['Home Win Rate', 'Spread Cover Rate', 'Over Rate']:
                fig2.add_trace(go.Bar(
                    ...
                    text=[f'{rate:.1%}<br>n={games}' for rate, games in 
                          zip(win_rate_analysis[outcome], win_rate_analysis['Games'])],
                    textposition='auto'
                ))
            ...
            feature_cols = ['avg_sharp_differential', 'max_line_movement', 'pct_reverse_movement', 
                           'pct_sharp_money', 'sharp_action_score']
            outcome_cols = ['home_win']
            
            # Only include columns that exist
            available_cols = [col for col in feature_cols + outcome_cols if col in df.columns]
    ...
    def generate_insights(self, df: pd.DataFrame, model_results: Dict) -> Dict[str, str]:
        ...
        try:
            ...
            if 'sharp_strength' in df.columns:
                sharp_analysis = df.groupby('sharp_strength').agg({
                    'home_win': ['mean', 'count'],
                    'home_cover_spread': lambda x: x.fillna(False).mean(),
                    'over': 'mean'
                }).round(3)
...
- Games with reverse line movement: {(df['pct_reverse_movement'] > 0.5).sum()} ({(df['pct_reverse_movement'] > 0.5).mean()*100:.1f}%)
    ...
    def generate_betting_strategy(self, df: pd.DataFrame) -> Dict[str, str]:
        ...
        try:
            ...
            sharp_games = df[df['pct_sharp_money'] > 0.7]  # Games with strong sharp money
