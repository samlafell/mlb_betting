...
class PublicFadeProcessor(BaseStrategyProcessor):
    ...
    def validate_strategy_data(self, raw_data: list[dict]) -> bool:
        ...
        has_percentages = any(
            row.get("home_or_over_stake_percentage") is not None
            and row.get("home_or_over_bets_percentage") is not None
            for row in raw_data
        )
    ...
    async def process(
        ...
        public_data = await self.repository.get_public_betting_data(
            start_time, end_time
        )
        ...
        if not fade_opportunities:
            ...
            self.logger.debug(
                f"Analyzed {len(public_data)} public betting records, found 0 opportunities meeting threshold"
            )
            return []
        ...
        for fade_data in fade_opportunities:
            ...
            if (
                consensus_strength < 58.0
            ):  # Minimum 58% public consensus (LOWERED from 65% based on analysis)
                self.logger.debug(
                    f"Public consensus {consensus_strength:.1f}% below threshold 58.0%"
                )
                continue
            ...
            matching_strategies = self._find_matching_strategies(
                profitable_strategies, fade_data
            )
            ...
            confidence_data = self._calculate_confidence_for_public_fade(
                fade_data, matching_strategy
            )

            # Create the signal
            signal = self._create_betting_signal(
                fade_data, matching_strategy, confidence_data
            )
            signals.append(signal)

        self._log_processing_summary(
            len(signals), len(profitable_strategies), len(fade_opportunities)
        )
        return signals
    ...
    def _is_valid_fade_data(
        self, fade_data: dict[str, Any], now_est, minutes_ahead: int
    ) -> bool:
    ...
    def _calculate_confidence_for_public_fade(
        self, fade_data: dict[str, Any], strategy: ProfitableStrategy
    ) -> dict[str, Any]:
        """
        ...
        confidence_level_multiplier = {"HIGH": 1.3, "MODERATE": 1.1, "LOW": 0.9}.get(
            fade_confidence, 1.0
        )
