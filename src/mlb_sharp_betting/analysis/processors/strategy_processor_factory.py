...
from mlb_sharp_betting.analysis.processors.base_strategy_processor import (
...
from mlb_sharp_betting.services.dynamic_threshold_manager import (
...
class StrategyProcessorFactory:
    ...
    def __init__(
        ...
        self.logger.info(
            "Loading implemented processors during factory initialization..."
        )
        self.logger.info(
            "🎯 DYNAMIC THRESHOLDS ENABLED: Using progressive threshold optimization"
        )
        self.logger.info(
            f"📋 Available Strategies: {len(self.PROCESSOR_MAPPING)} total"
        )

        # Show which processors we expect to load
        implemented_strategies = [
            name
            for name, info in self.PROCESSOR_MAPPING.items()
            if info["status"] == "IMPLEMENTED"
        ]
        self.logger.info(
            f"🔄 Attempting to load {len(implemented_strategies)} implemented processors:"
        )
        ...
        try:
            ...
            self.logger.info(
                f"✅ Factory initialized with {len(self._processor_registry)} processors:"
            )
            ...
            if len(loaded_names) != len(implemented_strategies):
                ...
                self.logger.warning(
                    f"⚠️  Failed to load some processors: {', '.join(missing)}"
                )

        except Exception as e:
            self.logger.warning(
                f"⚠️  Some processors failed to load during initialization: {e}"
            )
    ...
    def _check_processor_status_safely(self, strategy_name: str) -> str:
        ...
        if self._loop_detection["last_strategy_checked"] != strategy_name:
            if (
                self._loop_detection["check_count"]
                > self._loop_detection["reset_threshold"]
            ):
    ...
    def create_all_processors(self) -> dict[str, BaseStrategyProcessor]:
        ...
        for strategy_name, info in self.PROCESSOR_MAPPING.items():
            ...
            try:
                ...
                self.logger.warning(
                    f"⚠️  Failed to create {strategy_name} processor: {e}"
                )
                failed_processors.append(strategy_name)
        ...
        if planned_processors:
            self.logger.info(
                f"📋 PLANNED (need implementation): {', '.join(planned_processors)}"
            )
    ...
    def create_processor(self, strategy_name: str) -> BaseStrategyProcessor | None:
        """
        ...
        try:
            ...
            if strategy_name == "sharp_action":
                ...
                processor = SharpActionProcessor(
                ...
                processor = OpposingMarketsProcessor(
                    self.repository, self.validator, self.config
                )
                processor.threshold_manager = self.threshold_manager
                return processor
                ...
                processor = BookConflictProcessor(
                    self.repository, self.validator, self.config
                )
                processor.threshold_manager = self.threshold_manager
                return processor
                ...
                processor = UnderdogValueProcessor(
                    self.repository, self.validator, self.config
                )
                processor.threshold_manager = self.threshold_manager
                return processor
                ...
                processor = LineMovementProcessor(
                    self.repository, self.validator, self.config
                )
                processor.threshold_manager = self.threshold_manager
                return processor
                ...
                processor = TimingBasedProcessor(
                    self.repository, self.validator, self.config
                )
                processor.threshold_manager = self.threshold_manager
                return processor
                ...
                processor = HybridSharpProcessor(
                    self.repository, self.validator, self.config
                )
                processor.threshold_manager = self.threshold_manager
                return processor
                ...
                if not processor_class:
                    self.logger.warning(
                        f"Processor class {processor_class_name} not found - needs implementation"
                    )
                    return None
                ...
                if not self._validate_processor(processor):
                    self.logger.error(
                        f"Processor {processor_class_name} failed validation"
                    )
                    return None
            ...
            self.logger.error(
                f"Error creating {processor_class_name} for {strategy_name}: {e}"
            )
            import traceback
    ...
    def _get_processor_class(
        ...
        try:
            ...
            try:
                ...
                if inspect.isclass(processor_class) and issubclass(
                    processor_class, BaseStrategyProcessor
                ):
                ...
                self.logger.debug(
                    f"Processor {processor_class_name} not yet implemented: {e}"
                )
                return None

        except Exception as e:
            self.logger.error(
                f"Error loading processor class {processor_class_name}: {e}"
            )
            return None
    ...
    def get_processors_by_type(self, signal_type: str) -> list[BaseStrategyProcessor]:
        ...
        for strategy_name, processor in self._processor_registry.items():
            try:
                if self._processor_matches_signal_type(strategy_name, signal_type):
                    ...
                    self.logger.debug(
                        f"✅ Found matching processor: {strategy_name} for {signal_type}"
                    )
            except Exception as e:
                self.logger.warning(
                    f"Error checking processor {strategy_name} for signal type {signal_type}: {e}"
                )
                continue
        ...
        self.logger.info(
            f"🎯 Found {len(unique_processors)} unique processors for {signal_type} (filtered from {len(matching_processors)} total matches)"
        )

        if not unique_processors:
            self.logger.warning(
                f"⚠️  No processors found for signal type: {signal_type}"
            )
            self.logger.info(
                f"📊 Available signal types: {list(set(self._processor_matches_signal_type(name, '') for name in self._processor_registry.keys()))}"
            )

        return unique_processors

    def _processor_matches_signal_type(
        self, processor_name: str, signal_type: str
    ) -> bool:
        """
        ...
        return (
            signal_type_lower in processor_name_lower
            or processor_name_lower in signal_type_lower
        )
