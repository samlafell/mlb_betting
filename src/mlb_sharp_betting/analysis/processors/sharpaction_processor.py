...
class SharpActionProcessor(BaseStrategyProcessor):
    ...
    async def process(
        ...
        if not raw_signals:
            self.logger.info(
                f"🔍 No sharp action data available, skipping processor (time window: {start_time} to {end_time})"
            )
            return []
        ...
        if not book_strategies:
            self.logger.warning(
                "No profitable book-specific sharp action strategies found"
            )
            return []
        ...
        self.logger.info(
            f"🔥 Processing {len(raw_signals)} sharp action signals with {len(book_strategies)} strategies (e.g., {strategy_names}...)"
        )
        ...
        for row in raw_signals:
            ...
            if not self._is_valid_signal_data(row, now_est, minutes_ahead):
                ...
                self.logger.debug(
                    f"Signal filtered: basic validation failed for {row.get('away_team')} @ {row.get('home_team')} ({row.get('differential')}%)"
                )
                continue
            ...
            if not matching_strategy:
                ...
                if filter_stats["no_strategy_match"] <= 3:
                    self.logger.info(
                        f"🔍 No strategy match for key '{book_strategy_key}' | Available: {list(book_strategies.keys())[:5]} | Signal: {row.get('away_team')} @ {row.get('home_team')} ({row.get('differential')}%)"
                    )
            ...
            if self._should_apply_juice_filter(row):
                ...
                self.logger.debug(
                    f"Signal filtered: juice filter failed for {row.get('away_team')} @ {row.get('home_team')}"
                )
                continue
            ...
            signal = self._create_book_specific_signal(
                row, matching_strategy, confidence_data, book_strategy_key
            )
        ...
        if filter_stats["total_raw"] == 0:
            ...
            self.logger.warning(
                f"⚠️  All {filter_stats['total_raw']} signals filtered out"
            )
        else:
            self.logger.info(
                f"✅ Generated {len(signals)} signals from {filter_stats['total_raw']} raw signals"
            )

        self._log_book_specific_summary(signals, book_strategies, len(raw_signals))
        return signals

    def _get_book_specific_strategies(
        self, profitable_strategies: list[ProfitableStrategy]
    ) -> dict[str, ProfitableStrategy]:
        """
        ...
        self.logger.info(
            f"Found {len(book_strategies)} book-specific sharp action strategies"
        )
        if book_strategies:
            self.logger.info(
                f"📊 Available book strategies: {list(book_strategies.keys())}"
            )
    ...
    async def _find_book_specific_strategy(
        self,
        book_strategy_key: str,
        row: dict[str, Any],
        ...
        if book_strategy_key in book_strategies:
            ...
            threshold_used = await self._get_actual_threshold_for_debug(
                strategy, abs_diff, source, split_type
            )
            ...
            self.logger.info(
                f"  - Threshold check: {abs_diff}% >= {threshold_used}% = {abs_diff >= threshold_used}"
            )

            threshold_result = await self._meets_strategy_threshold(
                strategy, abs_diff, source, split_type
            )
            if threshold_result:
                self.logger.info(
                    f"✅ Threshold passed for '{book_strategy_key}': {abs_diff}% >= {threshold_used}%"
                )
                return strategy
            else:
                self.logger.warning(
                    f"❌ Threshold failed for exact match '{book_strategy_key}': {abs_diff}% < {threshold_used}% (win_rate={strategy.win_rate}%)"
                )
        ...
        if source_key in book_strategies:
            ...
            if await self._meets_strategy_threshold(
                strategy, abs_diff, source, split_type
            ):
                return strategy
    ...
    async def _get_actual_threshold_for_debug(
        self,
        ...
        if hasattr(self, "threshold_manager") and self.threshold_manager:
            try:
                threshold_config = await self.threshold_manager.get_dynamic_threshold(
                    strategy_type="sharp_action", source=source, split_type=split_type
                )
                ...
                self.logger.debug(
                    f"🎯 Dynamic threshold debug for {source}/{split_type}: {threshold:.1f}% "
                ...
                self.logger.warning(
                    f"Failed to get dynamic threshold for debug, falling back to static: {e}"
                )
        ...
        self.logger.debug(
            f"📊 Static threshold fallback for debug: {threshold:.1f}% for win_rate {strategy.win_rate:.1%}"
        )
        return threshold

    def _extract_book_variants(
        ...
        # 🚀 SMART GROUPING: Only create variants for truly general strategies
            ...
            source_book in ["general", "all", "any", "unknown"]
            or "sharp_action" in strategy_name
        ):
        ...
        if len(book_variants) > 3:
            self.logger.debug(
                f"⚠️  Created {len(book_variants)} variants for strategy '{strategy.strategy_name}' - consider consolidation"
            )

        return book_variants

    def _create_book_variant(
        self, base_strategy: ProfitableStrategy, source: str, book: str
    ) -> ProfitableStrategy:
    ...
    def _is_sharp_action_strategy(self, strategy: ProfitableStrategy) -> bool:
        ...
        return (
            any(keyword in strategy_name for keyword in sharp_keywords)
            or any(pattern in source_book for pattern in book_specific_patterns)
            or any(pattern in strategy_name for pattern in book_specific_patterns)
        )

    async def _meets_strategy_threshold(
        self,
        strategy: ProfitableStrategy,
        abs_diff: float,
        source: str = "default",
        ...
        if hasattr(self, "threshold_manager") and self.threshold_manager:
            try:
                ...
                self.logger.debug(
                    f"🎯 Dynamic threshold for {source}/{split_type}: {threshold:.1f}% "
                    f"(phase: {threshold_config.phase.value}, sample_size: {threshold_config.sample_size})"
                )
                ...
                self.logger.warning(
                    f"Failed to get dynamic threshold, falling back to static: {e}"
                )
        ...
        self.logger.debug(
            f"📊 Static threshold fallback: {threshold:.1f}% for win_rate {strategy.win_rate:.1%}"
        )
        return abs_diff >= threshold

    def _calculate_book_specific_confidence(
        self,
        row: dict[str, Any],
        ...
        adjusted_confidence = max(
            0.1, min(0.95, adjusted_confidence)
        )  # Keep in valid range
    ...
    def _create_book_specific_signal(
        self,
        row: dict[str, Any],
        matching_strategy: ProfitableStrategy,
    ...
    def _log_book_specific_summary(
        self,
        ...
        for signal in signals:
            book_key = (
                signal.metadata.get("book_strategy_key", "unknown")
                if signal.metadata
                else "unknown"
            )
    ...
    def _is_valid_signal_data(
        ...
        try:
            ...
            if not (-1440 <= time_diff_minutes <= minutes_ahead):
                self.logger.warning(
                    f"🕒 Time window check failed: {time_diff_minutes} minutes (need -1440-{minutes_ahead}) for {row.get('away_team')} @ {row.get('home_team')}"
                )
                return False
            ...
            if missing_fields:
                self.logger.warning(
                    f"📋 Missing required fields {missing_fields} for {row.get('away_team')} @ {row.get('home_team')}"
                )
                return False
            ...
            if abs_diff < self.config.minimum_differential:
                self.logger.warning(
                    f"📉 Differential too weak: {abs_diff} < {self.config.minimum_differential} for {row.get('away_team')} @ {row.get('home_team')}"
                )
                return False

            self.logger.info(
                f"✅ Signal passed validation: {row.get('away_team')} @ {row.get('home_team')} ({abs_diff}% diff, {time_diff_minutes} min)"
            )
            return True
