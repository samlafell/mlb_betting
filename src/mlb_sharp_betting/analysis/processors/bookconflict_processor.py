...
class BookConflictProcessor(BaseStrategyProcessor):
    ...
    async def process(
        ...
        multi_book_data = await self.repository.get_multi_book_data(
            start_time, end_time
        )
        ...
        if not book_conflicts:
            ...
            self.logger.debug(
                f"Analyzed {len(multi_book_data)} multi-book records, found 0 conflicts meeting threshold"
            )
            return []
        ...
        for conflict_data in book_conflicts:
            ...
            if hasattr(self, "threshold_manager") and self.threshold_manager:
                try:
                    threshold_config = (
                        await self.threshold_manager.get_dynamic_threshold(
                    ...
                    self.logger.debug(
                        f"🎯 Dynamic book conflict threshold: {min_threshold:.1f}% "
                        f"(phase: {threshold_config.phase.value})"
                    )
                except Exception as e:
                    self.logger.warning(
                        f"Failed to get dynamic threshold for book conflicts: {e}"
                    )

            if conflict_strength < min_threshold:
                self.logger.debug(
                    f"Conflict strength {conflict_strength:.2f}% below threshold {min_threshold:.1f}%"
                )
                continue
            ...
            matching_strategies = self._find_matching_strategies(
                fixed_strategies, conflict_data
            )
            ...
            confidence_data = self._calculate_confidence_for_book_conflict(
                conflict_data, matching_strategy
            )

            # Create the signal
            signal = self._create_betting_signal(
                conflict_data, matching_strategy, confidence_data
            )
            signals.append(signal)

        self._log_processing_summary(
            len(signals), len(fixed_strategies), len(book_conflicts)
        )
        return signals
    ...
    def _analyze_book_conflict(self, book_data: list[dict]) -> dict[str, Any]:
        ...
        home_signals = len(
            [r for r in enhanced_data if r["weighted_sharp_signal"] > 10]
        )
        away_signals = len(
            [r for r in enhanced_data if r["weighted_sharp_signal"] < -10]
        )
        ...
        conflict_analysis = {
            ...
            # Average metrics
            "avg_credibility": sum(r["book_credibility"] for r in enhanced_data)
            / len(enhanced_data),
    ...
    def _is_significant_conflict(self, conflict_analysis: dict[str, Any]) -> bool:
        ...
        if (
            conflict_analysis["books_favor_home"] == 0
            or conflict_analysis["books_favor_away"] == 0
        ):
            return False
    ...
    def _classify_conflict_timing(self, record: dict[str, Any]) -> str:
        ...
        if isinstance(game_datetime, str):
            game_datetime = datetime.fromisoformat(
                game_datetime.replace("T", " ").replace("Z", "+00:00")
            )
        if isinstance(last_updated, str):
            last_updated = datetime.fromisoformat(
                last_updated.replace("T", " ").replace("Z", "+00:00")
            )
    ...
    def _is_valid_conflict_data(
        self, conflict_data: dict[str, Any], now_est, minutes_ahead: int
    ) -> bool:
    ...
    def _calculate_confidence_for_book_conflict(
        self, conflict_data: dict[str, Any], strategy: ProfitableStrategy
    ) -> dict[str, Any]:
        """
    ...
    def _extract_strategy_components(self, strategy_name: str) -> tuple[str, str]:
        ...
        - "book_conflicts_strategy_BASIC_CONFLICT" -> ("VSIN-unknown", "moneyline")
        """
        ...
        # Handle book conflicts strategy format
        source_book = "VSIN-unknown"  # Default for book conflicts
        split_type = "book_conflicts"  # Default
        ...
        # Extract split type information for book conflicts
        if (
            ...
            split_type = "total"
        elif "conflict" in strategy_name_lower or "book" in strategy_name_lower:
            split_type = "book_conflicts"
