...
class LateFlipProcessor(BaseStrategyProcessor):
    ...
    def validate_strategy_data(self, raw_data: list[dict]) -> bool:
        ...
        has_timeline_data = any(
            row.get("last_updated") is not None
            and row.get("differential") is not None
            and abs(row.get("differential", 0)) >= 8  # Minimum sharp threshold
            for row in raw_data
        )
    ...
    async def process(
        ...
        if not flip_opportunities:
            ...
            self.logger.debug(
                f"Analyzed {len(steam_data)} steam records, found 0 flips meeting threshold"
            )
            return []
        ...
        for flip_data in flip_opportunities:
            ...
            if (
                flip_strength < 8.5
            ):  # Minimum 8.5% flip strength (LOWERED from 12% based on analysis)
                self.logger.debug(
                    f"Flip strength {flip_strength:.2f}% below threshold 8.5%"
                )
                continue
            ...
            matching_strategies = self._find_matching_strategies(
                profitable_strategies, flip_data
            )
            ...
            confidence_data = self._calculate_confidence_for_late_flip(
                flip_data, matching_strategy
            )

            # Create the signal
            signal = self._create_betting_signal(
                flip_data, matching_strategy, confidence_data
            )
            signals.append(signal)

        self._log_processing_summary(
            len(signals), len(profitable_strategies), len(flip_opportunities)
        )
        return signals
    ...
    def _analyze_timeline_for_flip(
        ...
        late_readings = [
            r for r in enhanced_timeline if r["time_period"] in ["LATE", "VERY_LATE"]
        ]
        ...
        for reading in early_readings:
            if (
                abs(reading.get("differential", 0)) >= 6
            ):  # Minimum sharp threshold (LOWERED from 8%)
        ...
        flip_type = None
        if (early_diff > 0 and late_diff < 0) and (
            abs(early_diff) >= 10 and abs(late_diff) >= 10
        ):
            flip_type = "HOME_TO_AWAY"
        elif (early_diff < 0 and late_diff > 0) and (
            abs(early_diff) >= 10 and abs(late_diff) >= 10
        ):
        ...
        hours_between_readings = abs(
            early_reading["hours_before_game"] - late_reading["hours_before_game"]
        )
    ...
    def _is_valid_flip_data(
        self, flip_data: dict[str, Any], now_est, minutes_ahead: int
    ) -> bool:
    ...
    def _calculate_confidence_for_late_flip(
        self, flip_data: dict[str, Any], strategy: ProfitableStrategy
    ) -> dict[str, Any]:
        """
        ...
        time_development_multiplier = min(
            1.1, max(0.9, 1.0 + (hours_between - 6) / 20.0)
        )
