...
class BaseStrategyProcessor(ABC):
    ...
    async def process_with_error_handling(
        ...
        try:
            ...
            if signals:
                self.logger.info(
                    f"✅ {self.__class__.__name__}: {len(signals)} signals generated"
                )
            else:
            ...
            self.logger.error(
                f"❌ {self.__class__.__name__} processing failed: {e}", exc_info=True
            )
            return []
    ...
    def _is_valid_signal_data(
        self, row: dict[str, Any], now_est, minutes_ahead: int
    ) -> bool:
        """
            ...
            for modifier_type, modifier_value in modifiers.items():
                if modifier_type == ConfidenceModifierType.BOOK_RELIABILITY:
                    book_mod = self._calculate_book_modifier(
                        row.get("source"), row.get("book"), matching_strategy
                    )
    ...
    def _calculate_book_modifier(
        self, source: str, book: str, strategy: ProfitableStrategy = None
    ) -> float:
        """
    ...
    def _calculate_confidence(
        ...
        try:
            ...
            self.logger.warning(
                f"Failed to convert differential to float: {differential} ({type(differential)}), using 0.0"
            )
            abs_diff = 0.0
    ...
    def _log_processing_summary(
        self, total_signals: int, total_strategies: int, total_opportunities: int
    ):
        """
        ...
        if total_opportunities > 0:
            self.logger.info(
                f"✅ {processor_name}: Found {total_opportunities} betting opportunities"
            )
        else:
            self.logger.info(
                f"ℹ️  {processor_name}: No opportunities found in current data"
            )
        ...
        if total_signals > 0:
            ...
            self.logger.debug(
                f"📈 {processor_name} Efficiency: {efficiency:.1f}% ({total_opportunities}/{total_signals})"
            )

        # Log strategy utilization
        if total_strategies > 0:
            self.logger.debug(
                f"🎯 {processor_name} Strategy Utilization: {total_strategies} profitable strategies found"
            )
    ...
    def _create_betting_signal(
        ...
        return BettingSignal(
            ...
            signal_strength=abs(
                differential
            ),  # Use absolute differential as signal strength
