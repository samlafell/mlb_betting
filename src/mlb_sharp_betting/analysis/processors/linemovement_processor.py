...
class LineMovementProcessor(BaseStrategyProcessor):
    ...
    async def process(self, minutes_ahead: int, 
        ...
        movement_strategies = self._get_movement_strategies(profitable_strategies)
    ...
    def _get_movement_strategies(self, profitable_strategies: List[ProfitableStrategy]) -> List[ProfitableStrategy]:
        ...
        for strategy in profitable_strategies:
            ...
            if any(keyword in strategy_name for keyword in [
                'line_movement', 'movement', 'steam', 'reverse', 'late_movement'
            ]):
    ...
    def _group_by_game(self, raw_data: List[Dict[str, Any]]) -> Dict[str, List[Dict[str, Any]]]:
    ...
    def _analyze_line_movements(self, game_movements: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """
    ...
    def _detect_movement_pattern(self, previous: Dict[str, Any], current: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if significant_money_move and significant_bet_move:
                ...
                if (money_movement > 0 and bet_movement < 0) or (money_movement < 0 and bet_movement > 0):
    ...
    def _find_movement_strategy(self, movement: Dict[str, Any], 
                              movement_strategies: List[ProfitableStrategy]) -> ProfitableStrategy:
    ...
    def _meets_movement_threshold(self, strategy: ProfitableStrategy, movement_magnitude: float) -> bool:
    ...
    def _calculate_movement_confidence(self, movement: Dict[str, Any], 
    ...
    def _create_movement_signal(self, movement: Dict[str, Any], 
        ...
        team = movement.get('home_team') if movement_direction == 'HOME' else movement.get('away_team')
        recommendation = f"BET {team} {movement.get('split_type', '').upper()} - {movement_type.replace('_', ' ').title()}"
        
        signal = self._create_betting_signal(movement, matching_strategy, confidence_data)
    ...
    def _is_valid_movement_data(self, movement: Dict[str, Any], current_time: datetime, 
        ...
        try:
            ...
            required_fields = ['home_team', 'away_team', 'movement_type', 'movement_magnitude']
    ...
    def _should_apply_juice_filter(self, movement: Dict[str, Any]) -> bool:
        ...
        recommended_team = (movement.get('home_team') if movement_direction == 'HOME' 
                          else movement.get('away_team'))
        
        return self._should_filter_juice(
            'moneyline', movement.get('current_data', {}).get('split_value'),
            recommended_team, 
            movement.get('home_team'), movement.get('away_team')
        )
    
    def _log_movement_summary(self, signals: List[BettingSignal], 
        ...
        for signal in signals:
            if signal.metadata:
                ...
                movement_direction = signal.metadata.get('movement_direction', 'unknown')
