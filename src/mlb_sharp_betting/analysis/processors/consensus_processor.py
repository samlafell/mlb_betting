...
class ConsensusProcessor(BaseStrategyProcessor):
    ...
    def _get_consensus_strategies(self, profitable_strategies: List[ProfitableStrategy]) -> List[ProfitableStrategy]:
        ...
        for strategy in profitable_strategies:
            ...
            if any(keyword in strategy_name for keyword in [
                'consensus', 'heavy_consensus', 'mixed_consensus', 'follow', 'fade'
            ]):
    ...
    def _analyze_consensus_patterns(self, row: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if (money_pct >= 90 and bet_pct >= 90) or (money_pct <= 10 and bet_pct <= 10):
                return {
                    'consensus_type': 'CONSENSUS_HEAVY_HOME' if money_pct >= 90 else 'CONSENSUS_HEAVY_AWAY',
                    'recommended_side': row.get('home_team') if money_pct >= 90 else row.get('away_team'),
            ...
            elif ((money_pct >= 80 and bet_pct >= 60) or (money_pct <= 20 and bet_pct <= 40)):
                return {
                    'consensus_type': 'MIXED_CONSENSUS_HOME' if money_pct >= 80 else 'MIXED_CONSENSUS_AWAY',
                    'recommended_side': row.get('home_team') if money_pct >= 80 else row.get('away_team'),
    ...
    def _find_consensus_strategy(self, consensus_analysis: Dict[str, Any], 
                               consensus_strategies: List[ProfitableStrategy]) -> ProfitableStrategy:
    ...
    def _meets_consensus_threshold(self, strategy: ProfitableStrategy, consensus_strength: float) -> bool:
    ...
    def _calculate_consensus_confidence(self, row: Dict[str, Any], 
    ...
    def _get_consensus_confidence_modifier(self, consensus_analysis: Dict[str, Any]) -> float:
        ...
        alignment_modifier = 1.0 + (20 - min(20, consensus_alignment)) / 100.0  # Up to 1.2
    ...
    def _create_consensus_signal(self, row: Dict[str, Any], 
                               consensus_analysis: Dict[str, Any],
        ...
        if consensus_analysis['strategy_approach'] == 'FADE':
            ...
            recommended_side = (row.get('away_team') if consensus_analysis['recommended_side'] == row.get('home_team') 
    ...
    def _is_valid_consensus_data(self, row: Dict[str, Any], current_time: datetime, 
    ...
    def _log_consensus_summary(self, signals: List[BettingSignal], 
        ...
        for signal in signals:
            if signal.metadata:
                ...
                consensus_type_counts[consensus_type] = consensus_type_counts.get(consensus_type, 0) + 1
