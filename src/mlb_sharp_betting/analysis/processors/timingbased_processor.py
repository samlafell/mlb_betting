...
class TimingBasedProcessor(BaseStrategyProcessor):
    ...
    def _calculate_timing_metrics(
        self, row: dict[str, Any], current_time: datetime
    ) -> dict[str, Any]:
        """
        ...
        try:
            ...
            timing_credibility = (
                base_credibility * timing_multiplier * context_multiplier
            )
            ...
            return {
                ...
                "is_closing_window": timing_category
                in ["ULTRA_LATE", "CLOSING_HOUR", "CLOSING_2H"],
            }
    ...
    def _classify_game_context(
        self, row: dict[str, Any], game_datetime: datetime
    ) -> str:
    ...
    def _meets_timing_thresholds(self, timing_metrics: dict[str, Any]) -> bool:
        ...
        if timing_category in ["ULTRA_LATE", "CLOSING_HOUR"]:
            ...
            if timing_metrics["volume_reliability"] == "INSUFFICIENT_VOLUME":
                return (
                    timing_metrics["abs_differential"] >= 20.0
                )  # High differential can overcome low volume
        else:
    ...
    def _get_timing_strategies(
        ...
        for strategy in profitable_strategies:
            ...
            if any(
                ...
                for keyword in ["timing", "late", "closing", "ultra", "hour", "early"]
            ):
    ...
    def _find_timing_strategy(
        ...
        for strategy in timing_strategies:
            ...
            if timing_category.lower().replace("_", "") in strategy_name.replace(
                "_", ""
            ):
                return strategy
            ...
            if "timing" in strategy_name:
                ...
                # Earlier strategies need better performance
                    ...
                    not timing_metrics["is_closing_window"] and strategy.win_rate >= 55
                ):
                    return strategy
    ...
    def _create_timing_signal(
        ...
        signal.strategy_name = (
            f"timing_{timing_metrics['timing_category'].lower()}_{signal.strategy_name}"
        )

        return signal

    def _is_valid_timing_data(
    ...
    def _log_timing_summary(
        ...
        for signal in signals:
            timing_cat = (
                signal.metadata.get("timing_category", "unknown")
                if signal.metadata
                else "unknown"
            )
        ...
        closing_count = sum(
            timing_counts.get(cat, 0)
            for cat in ["ULTRA_LATE", "CLOSING_HOUR", "CLOSING_2H"]
        )
