...
class UnderdogValueProcessor(BaseStrategyProcessor):
    ...
    def _get_underdog_strategies(
        ...
        for strategy in profitable_strategies:
            ...
            if any(
                ...
                for keyword in ["underdog", "dog", "value", "underdog_ml", "ml_value"]
            ):
    ...
    def _analyze_underdog_value(self, row: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "underdog_side": row.get("away_team")
                if underdog_team == "AWAY_UNDERDOG"
    ...
    def _find_underdog_strategy(
        ...
        for strategy in underdog_strategies:
            ...
            if "value_away_dog" in value_signal.lower() and "away" in strategy_name:
                ...
                "moderate_value" in value_signal.lower() and "moderate" in strategy_name
            ):
                if self._meets_underdog_threshold(strategy, underdog_odds):
                    return strategy
            ...
            # General underdog matches
                ...
                if self._meets_underdog_threshold(strategy, underdog_odds):
                    ...
                    if sharp_confirmation != "NO_SHARP_CONFIRMATION":
                        ...
                        strategy.win_rate >= 55
                    ):  # Only take general without sharp if good win rate
                        return strategy

        return None

    def _meets_underdog_threshold(
        self, strategy: ProfitableStrategy, underdog_odds: int
    ) -> bool:
    ...
    def _calculate_underdog_confidence(
        self,
        row: dict[str, Any],
        underdog_analysis: dict[str, Any],
        ...
        if base_score < 0.5:  # Weak signals (like 2% differential)
            ...
            adjusted_confidence = base_score * min(
                1.1, underdog_modifier
            )  # Max 10% boost
            adjusted_confidence = max(
                0.1, min(0.55, adjusted_confidence)
            )  # Cap at 55% for weak signals
        elif base_score < 0.7:  # Moderate signals
            adjusted_confidence = base_score * min(
                1.2, underdog_modifier
            )  # Max 20% boost
            adjusted_confidence = max(
                0.1, min(0.80, adjusted_confidence)
            )  # Cap at 80% for moderate signals
        else:  # Strong signals
            adjusted_confidence = base_score * underdog_modifier
            adjusted_confidence = max(
                0.1, min(0.92, adjusted_confidence)
            )  # Cap at 92% max (not 95%)
    ...
    def _get_underdog_confidence_modifier(
        self, underdog_analysis: dict[str, Any]
    ) -> float:
        ...
        signal.metadata.update(
            {
                ...
                "expected_roi": self._calculate_expected_roi(
                    underdog_odds, matching_strategy.win_rate
                ),
            }
        )
    ...
    def _should_apply_juice_filter(
        self, row: dict[str, Any], underdog_analysis: dict[str, Any]
    ) -> bool:
    ...
    def _log_underdog_summary(
        ...
        for signal in signals:
            if signal.metadata:
                ...
                sharp_confirmation = signal.metadata.get(
                    "sharp_confirmation", "NO_SHARP_CONFIRMATION"
                )

                value_signal_counts[value_signal] = (
                    value_signal_counts.get(value_signal, 0) + 1
                )
