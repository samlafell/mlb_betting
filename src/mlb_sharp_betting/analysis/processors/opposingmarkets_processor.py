...
class OpposingMarketsProcessor(BaseStrategyProcessor):
    ...
    async def process(self, minutes_ahead: int, 
        ...
        for market_data in opposing_markets:
            ...
            confidence_data = self._calculate_confidence_for_opposing_market(market_data, matching_strategy)
    ...
    def _find_opposing_markets(self, ml_data: List[Dict], spread_data: List[Dict]) -> List[Dict]:
        ...
        for game_key in common_games:
            ...
            ml_recommended_team = self._get_recommended_team_from_differential(
                ml_record['differential'], ml_record['home_team'], ml_record['away_team']
            )
            
            spread_recommended_team = self._get_recommended_team_from_differential(
                spread_record['differential'], spread_record['home_team'], spread_record['away_team']
            )
            ...
            if ml_recommended_team != spread_recommended_team:
                ...
                opposition_strength = abs(ml_record['differential'] - spread_record['differential'])
                
                # Determine dominant market
                dominant_market = 'ML_STRONGER' if ml_strength > spread_strength else 'SPREAD_STRONGER'
                ...
                opposing_market = {
                    ...
                    'last_updated': max(ml_record['last_updated'], spread_record['last_updated']),
    ...
    def _group_data_by_game(self, data: List[Dict]) -> Dict[tuple, Dict]:
        ...
        for record in data:
            ...
            if key not in grouped or record['last_updated'] > grouped[key]['last_updated']:
                grouped[key] = record
        
        return grouped
    
    def _get_recommended_team_from_differential(self, differential: float, 
    ...
    def _calculate_confidence_for_opposing_market(self, market_data: Dict[str, Any], 
        ...
        if opposition_strength > 20.0:
            confidence_data['confidence_score'] = min(1.0, confidence_data['confidence_score'] * 1.1)
            confidence_data['confidence_explanation'] += " Strong market opposition detected."
    ...
    def _create_betting_signal(self, raw_data: Dict[str, Any], strategy: ProfitableStrategy,
        ...
        return BettingSignal(
            ...
            differential=float(raw_data['opposition_strength']),  # Use opposition strength
