...
from ..models.sharp import ConfidenceLevel, SharpAction
...
class BaseRepository(ABC):
    ...
    def _get_table_columns(self, table_name: str) -> list[str]:
        ...
        try:
            ...
            result = coordinator.execute_read(
                columns_query, (schema_name, table_name_only)
            )
            ...
            self.logger.error(
                "Failed to get table columns", table=table_name, error=str(e)
            )
    ...
    def _row_to_model(self, row: tuple, columns: list[str]) -> ModelType:
        ...
        try:
            ...
            self.logger.error(
                "Failed to convert row to model", error=str(e), row=row, columns=columns
            )
    ...
    def exists(self, **filters: Any) -> bool:
        ...
        try:
            ...
            self.logger.error(
                "Error checking entity existence", filters=filters, error=str(e)
            )
    ...
    def count(self, **filters: Any) -> int:
        ...
        try:
            ...
            self.logger.error("Error counting entities", filters=filters, error=str(e))
    ...
    def get_by_id(self, entity_id: str) -> ModelType | None:
        ...
        try:
            ...
            self.logger.error(
                "Error getting entity by ID", entity_id=entity_id, error=str(e)
            )
    ...
    def find_one(self, **filters: Any) -> ModelType | None:
        """
    ...
    def delete(self, entity_id: str) -> bool:
        ...
        try:
            ...
            self.logger.error(
                "Error deleting entity", entity_id=entity_id, error=str(e)
            )
            raise DatabaseError(f"Failed to delete entity: {e}")

    def bulk_create(
        self, models: list[ModelType], batch_size: int = 1000
    ) -> list[ModelType]:
        """
        ...
        try:
            ...
            for i in range(0, len(models), batch_size):
                ...
                if result:
                    table_columns = self._get_table_columns(self.table_name)
                    batch_created = [
                        self._row_to_model(row, table_columns) for row in result
                    ]
                    created_models.extend(batch_created)

                self.logger.info(
                    "Created batch of entities",
...
class GameRepository(BaseRepository):
    ...
    def find_by_date_range(
        ...
        try:
            ...
            query = (
                f"SELECT * FROM {self.table_name} {where_clause} ORDER BY game_datetime"
            )
            ...
            self.logger.error(
                "Failed to find games by date range",
    ...
    def find_completed_games(self, limit: int | None = None) -> list[Game]:
        """
    ...
    def find_upcoming_games(self, days_ahead: int = 7) -> list[Game]:
        ...
        return self.find_by_date_range(
            start_date, end_date, status=GameStatus.SCHEDULED
        )
...
class BettingSplitRepository(BaseRepository):
    ...
    def find_recent_splits(
        self, hours: int = 24, source: DataSource | None = None
    ) -> list[BettingSplit]:
        """
        ...
        try:
            ...
            self.logger.error(
                "Failed to find recent splits", hours=hours, source=source, error=str(e)
            )
...
class SharpActionRepository(BaseRepository):
    ...
    def find_actionable_bets(self, min_signals: int = 2) -> list[SharpAction]:
        ...
        try:
            ...
            self.logger.error(
                "Failed to find actionable bets", min_signals=min_signals, error=str(e)
            )
...
def get_sharp_action_repository(
    db_manager: DatabaseManager | None = None,
...
def get_game_outcome_repository(
    db_manager: DatabaseManager | None = None,
) -> GameOutcomeRepository:
