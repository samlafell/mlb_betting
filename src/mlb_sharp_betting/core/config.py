...
class PostgreSQLSettings(BaseSettings):
    ...
    port: int = Field(default=5432, description="PostgreSQL port", env="POSTGRES_PORT")

    database: str = Field(
        default="mlb_betting", description="PostgreSQL database name", env="POSTGRES_DB"
    )
    ...
    min_connections: int = Field(
        default=2, description="Minimum connections in pool", env="POSTGRES_MIN_CONN"
    )

    max_connections: int = Field(
        default=20, description="Maximum connections in pool", env="POSTGRES_MAX_CONN"
    )
...
class SchemaSettings(BaseSettings):
    ...
    name: str = Field(default="splits", description="Database schema name")
    ...
    legacy_splits_table: str = Field(
        default="splits", description="Legacy table name for backward compatibility"
    )
...
class DataSourceSettings(BaseSettings):
    ...
    sbd_identifier: str = Field(
        default="SBD", description="SportsBettingDime source identifier"
    )

    vsin_identifier: str = Field(default="VSIN", description="VSIN source identifier")
...
class VSINSettings(BaseSettings):
    ...
    base_url: str = Field(default="https://www.vsin.com", description="VSIN base URL")
...
class LoggingSettings(BaseSettings):
    ...
    structured: bool = Field(
        default=True, description="Whether to use structured JSON logging"
    )

    log_file: Path | None = Field(
        default=None, description="Optional log file path", env="MLB_LOG_FILE"
    )
...
class Settings(BaseSettings):
    ...
    debug: bool = Field(
        default=False, description="Enable debug mode", env="MLB_BETTING_DEBUG"
    )
