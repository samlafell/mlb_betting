...
class ConfidenceScorer:
    ...
    def calculate_confidence(
        ...
        source_score, source_win_rate, source_roi = (
            self._calculate_source_reliability_score(source, book, split_type)
        )

        strategy_score, strategy_win_rate, strategy_roi = (
            self._calculate_strategy_performance_score(
    ...
    def _calculate_signal_strength_score(self, differential: float) -> float:
        ...
        if abs_diff >= 25:
            return min(
                100, 90 + (abs_diff - 25) * 0.8
            )  # 90-100 points (LOWERED from 30%)
    ...
    def _generate_explanation(
        self, score: float, components: ConfidenceComponents
    ) -> str:
        ...
        if components.signal_strength_score >= 85:
            explanations.append(
                f"Very strong signal ({components.signal_differential:+.1f}% differential)"
            )
        elif components.signal_strength_score >= 70:
            explanations.append(
                f"Strong signal ({components.signal_differential:+.1f}% differential)"
            )
        elif components.signal_strength_score >= 55:
            explanations.append(
                f"Moderate signal ({components.signal_differential:+.1f}% differential)"
            )
        else:
            explanations.append(
                f"Weak signal ({components.signal_differential:+.1f}% differential)"
            )
        ...
        if components.source_win_rate:
            if components.source_reliability_score >= 85:
                explanations.append(
                    f"Highly reliable source ({components.source_win_rate:.1f}% win rate)"
                )
            elif components.source_reliability_score >= 70:
                explanations.append(
                    f"Reliable source ({components.source_win_rate:.1f}% win rate)"
                )
            else:
                explanations.append(
                    f"Average source reliability ({components.source_win_rate:.1f}% win rate)"
                )
        ...
        if components.strategy_roi:
            if components.strategy_performance_score >= 85:
                explanations.append(
                    f"Excellent strategy performance ({components.strategy_roi:+.1f}% ROI)"
                )
            elif components.strategy_performance_score >= 70:
                explanations.append(
                    f"Good strategy performance ({components.strategy_roi:+.1f}% ROI)"
                )
            else:
                explanations.append(
                    f"Average strategy performance ({components.strategy_roi:+.1f}% ROI)"
                )
        ...
        if components.data_age_minutes <= 30:
            ...
            explanations.append(
                f"Older data ({components.data_age_minutes:.0f} min old)"
            )

        return " • ".join(explanations)

    def _get_source_reliability_score(
        self, source_book_type: str, split_type: str
    ) -> tuple[float, str]:
        ...
        try:
            ...
            result = self.coordinator.execute_read(
                query, (source_book_type, split_type)
            )
            ...
            self.logger.error(
                "Failed to calculate source reliability score", error=str(e)
            )
            return (25.0, "Error calculating reliability")

    def _get_strategy_performance_score(
        self, strategy_name: str, source_book_type: str, split_type: str
    ) -> tuple[float, str]:
        ...
        try:
            ...
            result = self.coordinator.execute_read(
                query, (strategy_pattern, source_book_type, split_type)
            )
            ...
            self.logger.error(
                "Failed to calculate strategy performance score", error=str(e)
            )
