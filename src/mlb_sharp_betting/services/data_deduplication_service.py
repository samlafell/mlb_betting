...
class DataDeduplicationService:
    ...
    def _ensure_deduplication_tables(self):
        ...
        try:
            ...
            self.db_manager.execute_write(
                "CREATE SCHEMA IF NOT EXISTS mlb_betting.clean", []
            )
    ...
    def _get_dynamic_threshold(self, market_type: MarketType) -> float:
        ...
        try:
            results = self.db_manager.execute_read(
                performance_query, [market_type.value]
            )
    ...
    def process_raw_data_for_deduplication(self, lookback_days: int = 30) -> dict:
        ...
        """
        self.logger.info(
            "Processing raw data for deduplication", lookback_days=lookback_days
        )
    ...
    def _process_game_market_for_deduplication(
        self,
        game_id: str,
        home_team: str,
        away_team: str,
    ...
    def _deduplicate_by_value_changes(
        self,
        market_data: list,
        game_id: str,
        home_team: str,
        ...
        for row in market_data:
            ...
            if source_book_key in source_book_tracking:
                last_differential = source_book_tracking[source_book_key][
                    "differential"
                ]
                ...
                time_diff = (
                    (last_updated - last_timestamp).total_seconds()
                    if isinstance(last_updated, datetime)
                    else 0
                )

                if (
                    value_diff < 2.0 and time_diff < 1800
                ):  # 30 minutes for identical values
            ...
            if recommendation:
                ...
                self._track_signal_evolution(
                    ...
                    last_updated,
                    row[8],  # minutes_before_game
                )

        return recommendations_created

    def _check_cross_source_consensus(
        self,
        game_id: str,
        market_type: MarketType,
        recommended_side: str,
        ...
        try:
            agreeing_sources = self.db_manager.execute_read(
                consensus_query,
                [game_id, market_type.value, timestamp, recommended_side],
            )[0][0]
    ...
    def _create_clean_recommendation(
        self,
        row: tuple,
        game_id: str,
        home_team: str,
        ...
        try:
            ...
            self.logger.error(
                "Failed to create clean recommendation", game_id=game_id, error=str(e)
            )
            return None
    ...
    def track_signal_evolution(
        self, game_id: str, market_type: MarketType
    ) -> list[dict]:
        ...
        rows = self.db_manager.execute_read(
            evolution_query, [game_id, market_type.value]
        )
        ...
        for row in rows:
            ...
            evolution[key].append(
                {"differential": row[2], "timestamp": row[3], "minutes_before": row[4]}
            )
    ...
    def cleanup_historical_duplicates(self, keep_latest_only: bool = True) -> dict:
        ...
        count_before_sql = (
            "SELECT COUNT(*) FROM mlb_betting.splits.raw_mlb_betting_splits"
        )
        ...
        result = {
            ...
            "cleanup_type": "latest_only"
            if keep_latest_only
            else "same_timestamp_only",
        }
    ...
    def generate_data_quality_report(self) -> dict:
        ...
        clean_total, clean_games, clean_markets, clean_avg_conf, clean_avg_consensus = (
            clean_metrics
        )

        duplication_ratio = (
            (raw_total - clean_total) / raw_total if raw_total > 0 else 0
        )
        ...
        report = {
            "raw_data": {
                ...
                "avg_records_per_market": raw_total / raw_markets
                if raw_markets > 0
                else 0,
            },
            "clean_data": {
            ...
            "quality_metrics": {
                ...
                "data_reduction": f"{(1 - clean_total / raw_total):.1%}"
                if raw_total > 0
                else "0%",
...
async def main():
    ...
    print(
        f"   ✅ Processed {results['processed_games']} games, created {results['recommendations_created']} recommendations"
    )
    ...
    print(
        f"      • Eliminated {report['quality_metrics']['duplication_elimination']} duplicate data"
    )
    print(f"      • Reduced dataset by {report['quality_metrics']['data_reduction']}")
    print(
        f"      • Average consensus boost: {report['clean_data']['avg_consensus_boost']:.2f}"
    )
    ...
    for rec in clean_recs[:5]:  # Show top 5
        consensus_indicator = (
            f" (+{rec['consensus_boost']:.1f}% consensus)"
            if rec["consensus_boost"] > 0
            else ""
        )
        print(
            f"   • {rec['game']} | {rec['market_type']} | {rec['source']} | "
