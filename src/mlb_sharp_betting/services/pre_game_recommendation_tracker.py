...
class PreGameRecommendation:
    ...
    profit_loss: float | None = None  # Assuming $100 unit bets at -110
...
class PreGameRecommendationTracker:
    ...
    def parse_pre_game_email_content(
        self, email_content: str, game_info: dict[str, Any]
            ...
            for line_num, line in enumerate(lines):
                ...
                for pattern in recommendation_patterns:
                    ...
                    for match in matches:
                        ...
                        if (
                            len(recommendation_text) < 4
                            or "BET" not in recommendation_text
                        ):
                            continue

                        # Determine bet type
                        if (
                            "OVER" in recommendation_text
                            or "UNDER" in recommendation_text
                        ):
                            bet_type = "total"
                        elif any(
                            x in recommendation_text
                            for x in ["+", "-1.5", "-2.5", "+1.5", "+2.5"]
                        ):
                            bet_type = "spread"
                        else:
                            bet_type = "moneyline"
    ...
    async def log_pre_game_recommendations(
        self, recommendations: list[PreGameRecommendation]
    ) -> None:
        ...
        try:
            with self.db_manager.get_connection() as conn:
                ...
                self.logger.info(
                    "Logged pre-game recommendations", count=len(recommendations)
                )
    ...
    async def update_recommendation_outcomes(self, lookback_days: int = 7) -> None:
        ...
        try:
            with self.db_manager.get_connection() as conn:
                ...
                self.logger.info(
                    "Updated recommendation outcomes", updated_count=updated_count
                )
    ...
    def _evaluate_bet_outcome(
        ...
        try:
            ...
            self.logger.error(
                "Failed to evaluate bet outcome",
        ...
        try:
            with self.db_manager.get_connection() as conn:
                with conn.cursor() as cursor:
                    ...
                    win_rate = (
                        len(wins) / completed_count if completed_count > 0 else 0.0
                    )
                    ...
                    roi_per_100 = (
                        (total_pnl / (completed_count * 110) * 100)
                        if completed_count > 0
                        else 0.0
                    )
                    avg_bet_profit = (
                        total_pnl / completed_count if completed_count > 0 else 0.0
                    )

                    # Breakdowns
                    by_bet_type = self._calculate_breakdown(completed, "bet_type")
                    by_signal_source = self._calculate_breakdown(
                        completed, "signal_source"
                    )
                    by_confidence = self._calculate_breakdown(
                        completed, "confidence_level"
                    )

                    # Recent trends
                    last_7_days = self._calculate_recent_performance(recommendations, 7)
                    last_30_days = self._calculate_recent_performance(
                        recommendations, 30
                    )
    ...
    def _calculate_breakdown(
        self, recommendations: list[dict], breakdown_field: str
    ...
    def _calculate_recent_performance(
        self, recommendations: list[dict], days: int
    ) -> dict[str, Any]:
...
*General Balls*"""
                ...
                if by_signal_source:
                    ...
                    for source, data in by_signal_source.items():
                        if data["total"] > 0:
                            report_lines.append(
                                f"**{source}:** {data['win_rate']:.1%} ({data['wins']}W-{data['losses']}L) | ${data['total_pnl']:+.2f}"
                            )
                    report_lines.append("")
                ...
                if by_confidence:
                    ...
                    for confidence, data in by_confidence.items():
                        if data["total"] > 0:
                            report_lines.append(
                                f"**{confidence}:** {data['win_rate']:.1%} ({data['wins']}W-{data['losses']}L) | ${data['total_pnl']:+.2f}"
                            )
                    report_lines.append("")
                ...
                if by_bet_type:
                    ...
                    for bet_type, data in by_bet_type.items():
                        if data["total"] > 0:
                            report_lines.append(
                                f"**{bet_type.title()}:** {data['win_rate']:.1%} ({data['wins']}W-{data['losses']}L) | ${data['total_pnl']:+.2f}"
                            )
                    report_lines.append("")
