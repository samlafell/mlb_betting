...
class DynamicThresholdManager:
    ...
    async def get_dynamic_threshold(self, strategy_type: str, source: str = "default", 
        ...
        # Check cache first
        if strategy_key in self._threshold_cache and datetime.now(timezone.utc) < self._cache_expiry:
            return self._threshold_cache[strategy_key]
        
        # Load or create threshold configuration
        threshold_config = await self._load_or_create_threshold_config(strategy_key, strategy_type)
    ...
    async def _create_bootstrap_config(self, strategy_key: str, strategy_type: str) -> ThresholdConfig:
        """Create new threshold configuration with bootstrap values"""
        
        # Get bootstrap thresholds for this strategy type
        bootstrap = self.bootstrap_thresholds.get(strategy_type, self.bootstrap_thresholds['default'])
        ...
        self.logger.info(f"🆕 Created bootstrap threshold config for {strategy_key}: "
    ...
    def _refresh_cache_expiry(self) -> None:
        ...
        self._cache_expiry = datetime.now(timezone.utc) + timedelta(minutes=self._cache_duration_minutes)
    
    async def _load_or_create_threshold_config(self, strategy_key: str, strategy_type: str) -> ThresholdConfig:
        ...
        try:
            ...
            with self.db_manager.get_cursor() as cursor:
                ...
                if isinstance(row, dict):
                    ...
                    columns = ['strategy_key', 'phase', 'sample_size', 'minimum_threshold',
                             'moderate_threshold', 'high_threshold', 'current_roi',
                             'current_win_rate', 'confidence_score', 'threshold_history',
                             'last_optimization', 'next_optimization', 'created_at', 'updated_at']
                ...
                return ThresholdConfig(
                    ...
                    sample_size=int(data['sample_size']) if data['sample_size'] is not None else 0,
                    minimum_threshold=float(data['minimum_threshold']) if data['minimum_threshold'] is not None else 3.0,
                    moderate_threshold=float(data['moderate_threshold']) if data['moderate_threshold'] is not None else 5.0,
                    high_threshold=float(data['high_threshold']) if data['high_threshold'] is not None else 8.0,
                    current_roi=float(data['current_roi']) if data['current_roi'] is not None else 0.0,
                    current_win_rate=float(data['current_win_rate']) if data['current_win_rate'] is not None else 0.5,
                    confidence_score=float(data['confidence_score']) if data['confidence_score'] is not None else 0.1,
                    threshold_history=json.loads(data['threshold_history']) if data['threshold_history'] else [],
            ...
            self.logger.warning(f"Failed to load threshold config for {strategy_key}: {e}")
            return None
    ...
    async def _should_optimize_thresholds(self, config: ThresholdConfig) -> bool:
        ...
        if config.next_optimization and datetime.now(timezone.utc) >= config.next_optimization:
            return True
    ...
    async def _update_performance_metrics(self, config: ThresholdConfig) -> ThresholdConfig:
        ...
        try:
            ...
            self.logger.warning(f"Failed to update performance metrics for {config.strategy_key}: {e}")
        
        return config
    
    async def _apply_phase_progression(self, config: ThresholdConfig, phase: ThresholdPhase) -> ThresholdConfig:
        ...
        self.logger.info(f"📈 Applied {phase.value} progression to {config.strategy_key}: "
    ...
    async def _apply_roi_optimization(self, config: ThresholdConfig) -> ThresholdConfig:
        ...
        try:
            ...
            historical_data = await self._get_historical_signal_data(config.strategy_key)
            
            if not historical_data or len(historical_data) < 50:
                self.logger.warning(f"Insufficient data for ROI optimization of {config.strategy_key}")
                return config
            ...
            if optimization_result:
                ...
                # Schedule next optimization
                config.next_optimization = datetime.now(timezone.utc) + timedelta(days=14)
                ...
                self.logger.info(f"🎯 Applied ROI optimization to {config.strategy_key}: "
            ...
            self.logger.error(f"Failed to apply ROI optimization for {config.strategy_key}: {e}")
        
        return config
    
    async def _run_roi_optimization(self, historical_data: List[Dict[str, Any]]) -> Optional[ROIOptimizationResult]:
        """Run ROI optimization to find optimal threshold"""
        
        try:
            # Test different threshold values
            test_thresholds = np.arange(2.0, 25.0, 0.5)  # Test from 2% to 25% in 0.5% increments
            ...
            for threshold in test_thresholds:
                ...
                filtered_signals = [
                    signal for signal in historical_data 
            ...
            with self.db_manager.get_cursor() as cursor:
                ...
                for row in rows:
                    if isinstance(row, dict):
                        ...
                        columns = ['differential', 'game_id', 'split_type', 'source', 'book',
                                 'home_win', 'over', 'home_score', 'away_score']
                        data = dict(zip(columns, row))
    ...
    def _calculate_confidence_score(self, sample_size: int, roi: float, win_rate: float) -> float:
    ...
    async def _save_threshold_config(self, config: ThresholdConfig) -> None:
        ...
        try:
            ...
            self.logger.error(f"Failed to save threshold config for {config.strategy_key}: {e}")
            ...
            with self.db_manager.get_cursor() as cursor:
                ...
                for row in rows:
                    if isinstance(row, dict):
                        ...
                        columns = ['phase', 'count', 'avg_min_threshold', 'avg_roi', 'avg_sample_size']
                        data = dict(zip(columns, row))
                    ...
                    summary['overall_stats']['avg_roi_by_phase'][phase] = float(data['avg_roi'])
                    summary['overall_stats']['avg_threshold_by_phase'][phase] = float(data['avg_min_threshold'])
                    summary['overall_stats']['sample_size_by_phase'][phase] = float(data['avg_sample_size'])
