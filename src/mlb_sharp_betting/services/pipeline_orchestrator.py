...
class PipelineOrchestrator:
            ...
            with self.db_manager.get_cursor() as cursor:
                ...
                if data_result and data_result["latest_update"]:
                    ...
                    if data_result["unique_games"] < 5:
                        system_state["data_quality_issues"].append(
                            "Insufficient game coverage"
                        )
                    ...
                    system_state["recommendations"].append(
                        "No betting data found - collection required"
                    )
                ...
                if backtest_result and backtest_result["latest_backtest"]:
                    ...
                    system_state["needs_backtesting"] = (
                        ...
                        ]  # Always re-backtest after fresh data
                        or backtest_result["total_results"]
                        < 5  # Insufficient backtest results
                    )

                    if backtest_result["unique_strategies"] < 3:
                        system_state["data_quality_issues"].append(
                            "Limited strategy coverage in backtesting"
                        )
                    ...
                    system_state["recommendations"].append(
                        "No backtesting results found - analysis required"
                    )
                ...
                if not outcomes_result or outcomes_result["total_outcomes"] < 10:
                    system_state["data_quality_issues"].append(
                        "Insufficient game outcomes for backtesting"
                    )
                ...
                if system_state["needs_data_collection"]:
                    system_state["recommendations"].append(
                        "Fresh data collection recommended"
                    )
                if system_state["needs_backtesting"]:
                    system_state["recommendations"].append(
                        "Strategy backtesting recommended"
                    )
                if (
                    not system_state["needs_data_collection"]
                    and not system_state["needs_backtesting"]
                ):
                    system_state["recommendations"].append(
                        "System is up-to-date, ready for detection"
                    )
            ...
            system_state.update(
                {
                    ...
                    "data_quality_issues": [
                        "Analysis failure - defaulting to full pipeline"
                    ],
                }
            )
    ...
    async def execute_smart_pipeline(
        ...
        if force_fresh_data:
            system_state["needs_data_collection"] = True
            system_state["needs_backtesting"] = (
                True  # Always backtest after forced data collection
            )
        ...
        try:
            ...
            if system_state["needs_data_collection"]:
                ...
                try:
                    data_pipeline = DataPipeline(
                        sport="mlb", sportsbook="circa", dry_run=False
                    )
            ...
            if system_state["needs_backtesting"]:
                ...
                try:
                    ...
                    backtest_results = (
                        await self.backtesting_engine.run_daily_pipeline()
                    )
                    ...
                    execution_results["errors"].append(error_msg)
                    execution_results["warnings"].append(
                        "Detection will proceed without fresh backtesting"
                    )
            ...
            try:
                ...
                from mlb_sharp_betting.services.adaptive_detector import (
                    AdaptiveBettingDetector,
                )
                ...
                detection_results = await detector.analyze_opportunities(
                    detection_minutes
                )
                ...
                (
                    cross_market_flips,
                    flip_summary,
                ) = await flip_detector.detect_todays_flips_with_summary(
                    min_confidence=60.0
                )
