...
class QuotaTracker:
    ...
    def _check_window_reset(self) -> None:
        ...
        if self.time_window == "monthly":
            should_reset = (
                now.year != self.window_start.year
                or now.month != self.window_start.month
            )
        ...
        if should_reset:
            logger.info(
                f"Resetting quota for {self.service_name} - new {self.time_window} window"
            )
            self.current_usage = 0
    ...
    def record_usage(
        self, cost: int = 1, endpoint: str = "", metadata: dict[str, Any] = None
    ) -> None:
        """Record API usage."""
        ...
        logger.info(
            f"{self.service_name} quota usage: {cost} credits used, "
...
class CooldownTracker:
    ...
    def can_perform_operation(self, operation_type: str) -> RateLimitResult:
        ...
        return RateLimitResult(
            status=RateLimitStatus.ALLOWED, allowed=True, reason="Cooldown check passed"
        )
...
class UnifiedRateLimiter:
    ...
    def __init__(self, data_dir: Path | None = None):
        ...
        self.logger.info(
            "UnifiedRateLimiter initialized with consolidated rate limiting"
        )
    ...
    def _initialize_default_limiters(self) -> None:
        ...
        self.cooldown_trackers["alert_service"] = CooldownTracker(
            service_name="alert_service", cooldown_minutes=15
        )
    ...
    async def wait_for_rate_limit(
        ...
        while True:
            result = await self.check_rate_limit(
                service_name, operation_type, estimated_cost
            )
            ...
            self.logger.info(
                f"Rate limited, waiting {wait_time:.1f}s for {service_name}",
        ...
        with self._lock:
            ...
            if service_name in self.rate_trackers:
                ...
                status["rate_limits"] = {
                    ...
                    "last_request": rate_tracker.last_request_time.isoformat()
                    if rate_tracker.last_request_time
                    else None,
                }
