...
class MLBStatsAPIService:
        ...
        try:
            ...
            for game_data in schedule:
                try:
                    ...
                    self.logger.warning(
                        "Failed to parse game", game_data=game_data, error=str(e)
                    )
                    continue

            self.logger.info(
                "Successfully fetched MLB games", date=date_str, games_count=len(games)
            )
            ...
            self.logger.error(
                "Failed to fetch MLB games", date=target_date, error=str(e)
            )
            return []

    def find_game_by_teams(
        self, home_team: str, away_team: str, target_date: date | None = None
    ) -> MLBGameInfo | None:
        """
        ...
        search_dates = [
            ...
            target_date
            - timedelta(days=1),  # Yesterday (last resort for completed games)
        ]
        ...
        for search_date in search_dates:
            ...
            for game in games:
                ...
                if (
                    ...
                    and game.away_team == away_team_norm
                ):
                    match_found = True
                ...
                if not match_found:
                    home_match = (
                        home_team_norm.lower() in game.home_team.lower()
                        or game.home_team.lower() in home_team_norm.lower()
                        or home_team.lower() in game.home_team.lower()
                        or game.home_team.lower() in home_team.lower()
                    )

                    away_match = (
                        away_team_norm.lower() in game.away_team.lower()
                        or game.away_team.lower() in away_team_norm.lower()
                        or away_team.lower() in game.away_team.lower()
                        or game.away_team.lower() in away_team.lower()
                    )
                ...
                if not match_found and game.home_team_abbr and game.away_team_abbr:
                    home_match = (
                        home_team.upper() == game.home_team_abbr.upper()
                        or home_team_norm.upper() == game.home_team_abbr.upper()
                    )
                    away_match = (
                        away_team.upper() == game.away_team_abbr.upper()
                        or away_team_norm.upper() == game.away_team_abbr.upper()
                    )
        ...
        if all_matches:
            ...
            def sort_key(game):
                ...
                if status_rank < 10:  # Upcoming game
                    date_rank = abs(
                        (game.game_date.date() - date.today()).days
                    )  # 0 for today, 1 for tomorrow, etc.
                else:  # Completed game
                    date_rank = abs(
                        (date.today() - game.game_date.date()).days
                    )  # Positive for past
    ...
    def get_official_game_id(
        self, home_team: str, away_team: str, game_datetime: datetime | None = None
    ) -> str | None:
        """
        ...
        try:
            ...
            self.logger.error(
                "Failed to get official game ID",
                home_team=home_team,
                away_team=away_team,
                error=str(e),
            )
    ...
    def batch_get_game_ids(
        self, team_pairs: list[tuple[str, str, datetime | None]]
        ...
        for target_date, teams in date_groups.items():
            ...
            for home_team, away_team in teams:
                ...
                results[(home_team, away_team)] = (
                    game_info.game_id if game_info else None
                )

        self.logger.info(
            "Batch game ID lookup completed",
    ...
    def get_game_data(
        ...
        home_team: str,
        away_team: str,
        estimated_datetime: datetime | None = None,
    ) -> dict[str, Any] | None:
        """
