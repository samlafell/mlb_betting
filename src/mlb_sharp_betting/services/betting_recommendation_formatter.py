...
from ..models.betting_analysis import BettingSignal, ConfidenceLevel
...
class RecommendationConflictDetector:
    ...
    def _check_for_conflicts(signals: List[BettingSignal]) -> List[BettingSignal]:
        ...
        for i, ml_signal1 in enumerate(moneyline_recs):
            for ml_signal2 in moneyline_recs[i+1:]:
                if RecommendationConflictDetector._is_conflicting_teams(ml_signal1, ml_signal2):
        ...
        for i, spread_signal1 in enumerate(spread_recs):
            for spread_signal2 in spread_recs[i+1:]:
                if RecommendationConflictDetector._is_conflicting_teams(spread_signal1, spread_signal2):
        ...
        for i, total_signal1 in enumerate(total_recs):
            for total_signal2 in total_recs[i+1:]:
                if RecommendationConflictDetector._is_conflicting_totals(total_signal1, total_signal2):
    ...
    def _is_conflicting_teams(signal1: BettingSignal, signal2: BettingSignal) -> bool:
        ...
        rec1_team = RecommendationConflictDetector._extract_team_from_recommendation(signal1)
        rec2_team = RecommendationConflictDetector._extract_team_from_recommendation(signal2)
        
        # If recommending different teams, it's a conflict
        return rec1_team != rec2_team and rec1_team is not None and rec2_team is not None
    ...
    def resolve_conflicts(conflicts: Dict[str, List[BettingSignal]]) -> List[BettingSignal]:
        ...
        for game_key, conflicting_signals in conflicts.items():
            ...
            comparison_details = RecommendationConflictDetector._format_conflict_comparison(best_signal, other_signals)
    ...
    @staticmethod
    def _select_best_performing_signal(conflicting_signals: List[BettingSignal]) -> BettingSignal:
        ...
        for signal in conflicting_signals:
            performance_score = RecommendationConflictDetector._calculate_performance_score(signal)
    ...
    def _calculate_performance_score(signal: BettingSignal) -> float:
        ...
        win_rate_score = signal.win_rate if signal.win_rate else 50.0  # Default to 50% if missing
        
        # ROI score (0-50 points, capped)
        roi_score = min(max(signal.roi or 0, -20), 30) + 20  # Normalize ROI to 0-50 range
        
        # Sample size reliability score (0-25 points)
        if signal.total_bets and signal.total_bets > 0:
            sample_size_score = min(signal.total_bets / 2, 25)  # 1 bet = 0.5 points, cap at 25
        else:
            sample_size_score = 0
        
        # Differential strength score (0-25 points)
        differential_score = min(abs(signal.differential or 0) / 2, 25)  # 2% diff = 1 point, cap at 25
        
        # Confidence bonus (0-10 points)
        confidence_score = (signal.confidence_score or 0) * 10 if signal.confidence_score <= 1 else (signal.confidence_score or 0) / 10
        confidence_bonus = min(confidence_score, 10)
        
        total_score = win_rate_score + roi_score + sample_size_score + differential_score + confidence_bonus
        
        return total_score
    
    @staticmethod  
    def _format_conflict_comparison(best_signal: BettingSignal, other_signals: List[BettingSignal]) -> str:
        ...
        if abs(best_win_rate - other_win_rate) >= 5:
            comparison_parts.append(f"Win Rate: {best_win_rate:.1f}% vs {other_win_rate:.1f}%")
...
class BettingRecommendationFormatter:
    ...
    def format_console_recommendations(self, signals: List[BettingSignal], min_confidence: float) -> str:
        ...
        if conflicts:
            self.logger.warning(f"🚨 CONFLICTS DETECTED: {len(conflicts)} games have conflicting recommendations")
                ...
                if winning_signal:
                    ...
                    for signal in conflicting_signals:
                        if signal != winning_signal:
                            losing_signals.add(id(signal))  # Use id() to avoid issues with object comparison
            
            # Step 3: Remove only the losing signals, keep everything else
            signals = [s for s in signals if id(s) not in losing_signals]
            
            self.logger.info(f"✅ Conflict resolution complete: {len(losing_signals)} losing signals removed, {len(resolved_signals)} conflicts resolved")
        
        # Group by confidence level  
        high_confidence = [s for s in signals if self._get_confidence_percentage(s.confidence_score) >= 85]
        moderate_confidence = [s for s in signals if 70 <= self._get_confidence_percentage(s.confidence_score) < 85]
        low_confidence = [s for s in signals if 50 <= self._get_confidence_percentage(s.confidence_score) < 70]
        ...
        if high_confidence:
            output_lines.append(f"🔥 HIGH CONFIDENCE RECOMMENDATIONS ({len(high_confidence)}):")
            output_lines.append("=" * 60)
            
            for signal in high_confidence:
                rec_lines = self._format_enhanced_recommendation(signal, is_high_confidence=True)
        ...
        if moderate_confidence:
            ...
            for signal in moderate_confidence:
                rec_lines = self._format_enhanced_recommendation(signal, is_high_confidence=False)
                output_lines.extend(rec_lines)
        ...
        if low_confidence:
            ...
            for signal in low_confidence:
                rec_lines = self._format_enhanced_recommendation(signal, is_high_confidence=False, is_low_confidence=True)
                output_lines.extend(rec_lines)
    ...
    def format_json_recommendations(self, signals: List[BettingSignal], min_confidence: float) -> Dict[str, Any]:
        ...
        for signal in signals:
            ...
            recommendations.append({
                ...
                "game_time": signal.game_time.isoformat() if signal.game_time else None,
                "minutes_to_game": signal.minutes_to_game,
                "bet_recommendation": bet_details["bet_line"],
                "confidence_score": signal.confidence_score,
                "confidence_level": signal.confidence_level.value if signal.confidence_level else "UNKNOWN",
                "strategy_name": signal.strategy_name,
                "signal_strength": signal.signal_strength,
                "differential": signal.differential,
                "source": signal.source,
                "book": signal.book,
                "suggested_stake": self._calculate_stake_size(signal.confidence_score),
        ...
        return {
            ...
            "high_confidence_count": len([r for r in recommendations if r["confidence_score"] >= 0.80]),
            "moderate_confidence_count": len([r for r in recommendations if 0.70 <= r["confidence_score"] < 0.80]),
...
"""
    ...
    def _format_single_recommendation(self, signal: BettingSignal, is_high_confidence: bool) -> List[str]:
        ...
        lines.append(f"   🎯 Strategy: {self._format_strategy_name(signal.strategy_name)}")
        lines.append(f"   💵 Suggested Stake: {self._calculate_stake_size(signal.confidence_score)}")
    ...
    def _format_moneyline_bet(self, signal: BettingSignal, split_data: Dict) -> tuple[str, str]:
        ...
        if "home" in split_data and "away" in split_data:
            ...
            if signal.recommendation and signal.home_team.upper() in signal.recommendation.upper():
                bet_line = f"{signal.home_team} ML ({home_odds:+d})"
            elif signal.recommendation and signal.away_team.upper() in signal.recommendation.upper():
    ...
    def _format_total_bet(self, signal: BettingSignal, split_data: Dict) -> tuple[str, str]:
    ...
    def _format_spread_bet(self, signal: BettingSignal, split_data: Dict) -> tuple[str, str]:
    ...
    def _format_enhanced_recommendation(self, signal: BettingSignal, is_high_confidence: bool, is_low_confidence: bool = False) -> List[str]:
        ...
        lines.append(f"   💰 Sharp Edge: {signal.differential:.1f}% money/bet differential")
        ...
        if is_low_confidence:
            lines.append(f"   ⚠️  WARNING: Low confidence - consider avoiding this bet")
    ...
    def _parse_enhanced_recommendation_details(self, signal: BettingSignal) -> Dict[str, str]:
        ...
        try:
            ...
            if signal.split_type == "moneyline":
                clear_bet_line, line_info = self._format_enhanced_moneyline_bet(signal, split_data)
                
            elif signal.split_type == "total":
                clear_bet_line, line_info = self._format_enhanced_total_bet(signal, split_data)
            
            elif signal.split_type == "spread":
                clear_bet_line, line_info = self._format_enhanced_spread_bet(signal, split_data)
            ...
            return {
                "clear_bet_line": clear_bet_line,
                "line_info": line_info
            }
            
        except Exception as e:
            self.logger.warning("Failed to parse enhanced recommendation details", error=str(e))
            return {
                "clear_bet_line": signal.recommendation or "BET RECOMMENDATION",
                "line_info": f"Source: {signal.source}"
            }
    
    def _format_enhanced_moneyline_bet(self, signal: BettingSignal, split_data: Dict) -> tuple[str, str]:
    ...
    def _format_enhanced_total_bet(self, signal: BettingSignal, split_data: Dict) -> tuple[str, str]:
    ...
    def _format_enhanced_spread_bet(self, signal: BettingSignal, split_data: Dict) -> tuple[str, str]:
