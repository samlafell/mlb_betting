...
from ..db.table_registry import DatabaseType, get_table_registry
...
class AnalysisScriptRunner:
    ...
    def run_single_script(self, script_name: str) -> dict[str, Any]:
        ...
        try:
            ...
            if "preprocessing_stats" in result:
                ...
                if full_path.exists():
                    original_sql = full_path.read_text()
                    processed_sql = self.sql_preprocessor.process_sql_string(
                        original_sql
                    )

                    result["debug_info"] = {
                        "original_sql_preview": original_sql[:1000] + "..."
                        if len(original_sql) > 1000
                        else original_sql,
                        "processed_sql_preview": processed_sql[:1000] + "..."
                        if len(processed_sql) > 1000
                        else processed_sql,
                        "validation_issues": preprocessing_summary.get(
                            "validation_issues", []
                        ),
                    }
    ...
    def run_all_scripts(self) -> dict[str, Any]:
        ...
        self.logger.info(
            "Starting analysis script execution",
            total_scripts=len(self.analysis_scripts),
        )
        ...
        for script_name, script_path in self.analysis_scripts.items():
            try:
                ...
                results["preprocessing_stats"][script_name] = script_result[
                    "preprocessing_stats"
                ]

            except Exception as e:
                self.logger.error(
                    "Script processing failed", script=script_name, error=str(e)
                )
                results["failed_scripts"].append(script_name)
        ...
        results["execution_summary"] = {
            ...
            "success_rate": len(results["successful_scripts"])
            / len(self.analysis_scripts)
            * 100
            if self.analysis_scripts
            else 0,
        ...
        self.logger.info(
            "Analysis script execution completed",
    ...
    def get_script_status_report(self, results: dict[str, Any]) -> str:
        ...
        report_lines.extend(["", "## ❌ Failed Scripts"])
        ...
        if results["failed_scripts"]:
            for script_name in results["failed_scripts"]:
                ...
                report_lines.extend(
                    [f"### {script_name}", f"- **Error:** {script_result['error']}", ""]
                )

                # Show preprocessing issues if available
                if (
                    "preprocessing_stats" in script_result
                    and script_result["preprocessing_stats"]
                ):
                    preprocessing = script_result["preprocessing_stats"]
    ...
    async def test_single_script(self, script_name: str) -> dict[str, Any]:
        ...
        if not result["success"] and "preprocessing_stats" in result:
            ...
            result["debug_info"] = {
                "original_sql_preview": original_sql[:500] + "..."
                if len(original_sql) > 500
                else original_sql,
                "processed_sql_preview": processed_sql[:500] + "..."
                if len(processed_sql) > 500
                else processed_sql,
                "validation_issues_detail": preprocessing_stats.get(
                    "validation_issues", []
                ),
            }
...
def main():
    ...
    if single_result["success"]:
        ...
        print(
            f"🔄 Table replacements: {single_result['preprocessing_stats']['table_replacements_applied']}"
        )
        print(
            f"🔄 Syntax transformations: {single_result['preprocessing_stats']['syntax_transformations_applied']}"
        )
    ...
    if all_results["successful_scripts"]:
        ...
        for script in all_results["successful_scripts"]:
            ...
            print(
                f"   • {script}: {len(result['data'])} rows in {result['execution_time_seconds']:.2f}s"
            )
