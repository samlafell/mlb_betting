...
from .mlb_api_service import MLBGameInfo, MLBStatsAPIService
...
class PreGameWorkflowService:
    ...
    def __init__(
        ...
        workflow_queue_timeout: float = 300.0,
    ):  # 5 minutes max wait for workflow queue
        """
        ...
        self.emails_sent_today: dict[
            str, tuple[datetime, EmailType]
        ] = {}  # game_pk -> (timestamp, email_type)
        ...
        if not self.email_config.is_configured():
            self.logger.warning(
                "Email not configured - notifications will be logged only",
    ...
    def _should_send_email(self, game: MLBGameInfo, email_type: EmailType) -> bool:
        ...
        self.emails_sent_today = {
            k: v
            for k, v in self.emails_sent_today.items()
        ...
        if game_key in self.emails_sent_today:
            ...
            if last_sent_time.date() == today:
                if last_email_type == email_type:
                    ...
                    self.logger.info(
                        "Email of same type already sent for this game today",
    ...
    async def execute_pre_game_workflow(
        ...
        try:
            ...
            await asyncio.wait_for(
                self._workflow_semaphore.acquire(), timeout=self.workflow_queue_timeout
            )

            self.logger.info(
                "Acquired workflow queue access",
                workflow_id=workflow_id,
                game=game_desc,
                context=context or "default",
            )
            ...
            self.logger.error(
                "Failed to acquire workflow queue",
                workflow_id=workflow_id,
                game=game_desc,
        ...
        try:
            ...
            return await self._execute_workflow_with_semaphore(
                game, workflow_id, context, minutes_before, send_email
            )
            ...
            self.logger.info(
                "Released workflow queue access",
                workflow_id=workflow_id,
                game=game_desc,
                context=context or "default",
            )
    ...
    async def _execute_workflow_with_semaphore(
        ...
        self.logger.info(
            "Starting pre-game workflow execution",
            workflow_id=workflow_id,
            game=game_desc,
            context=context or "default",
        ...
        workflow_result = WorkflowResult(
            game=game, workflow_id=workflow_id, start_time=datetime.now(timezone.utc)
        )
        ...
        try:
            ...
            if send_email:
                ...
                try:
                    await self._execute_email_notification(
                        email_result, workflow_result
                    )
                ...
                self.logger.info(
                    "Email notification skipped by request",
                    context=context or "default",
                    minutes_before=minutes_before,
                )
            ...
            if (
                data_result.status == StageStatus.SUCCESS
                and workflow_result.stages[WorkflowStage.BETTING_ANALYSIS].status
                == StageStatus.SUCCESS
            ):
            ...
            workflow_result.stages[
                WorkflowStage.EMAIL_NOTIFICATION
            ] = await self._send_error_notification(workflow_result, str(e))
            self.metrics["workflows_failed"] += 1
            ...
            self.logger.info(
                "Pre-game workflow execution completed",
                workflow_id=workflow_id,
        ...
        while retry_count <= self.max_retries:
            try:
                if retry_count > 0:
                    ...
                    self.logger.info(
                        f"Retrying {stage.value} in {delay} seconds",
                ...
                error_msg = (
                    f"Stage {stage.value} timed out after {self.stage_timeout} seconds"
                )
            ...
            if retry_count > self.max_retries:
                self.logger.error(
                    f"Stage {stage.value} failed after {self.max_retries} retries",
                    game=game_desc,
                )
    ...
    async def _execute_data_collection(
        ...
        try:
            ...
            entrypoint_path = (
                self.project_root / "src" / "mlb_sharp_betting" / "entrypoint.py"
            )
            cmd = [
                "uv",
                "run",
            ...
            if result["returncode"] == 0:
                ...
                stage_result.error_message = (
                    f"Entrypoint failed with exit code {result['returncode']}"
                )
                self.logger.error(
    ...
    async def _execute_betting_analysis(
        ...
        try:
            ...
            if result["returncode"] == 0:
                ...
                await self._log_betting_recommendations(
                    stage_result.stdout, workflow_result.game
                )

                self.logger.info(
                    "Betting analysis completed successfully",
                    execution_time=stage_result.execution_time_seconds,
                ...
                stage_result.status = StageStatus.FAILED
                stage_result.error_message = (
                    f"Master detector failed with exit code {result['returncode']}"
                )
                self.logger.error(
                    "Betting analysis failed",
                    return_code=result["returncode"],
                    stderr=result["stderr"],
                )
    ...
    async def _execute_email_notification(
        ...
        try:
            ...
            if not self._should_send_email(workflow_result.game, email_type):
                stage_result.status = StageStatus.SUCCESS
                stage_result.error_message = (
                    f"Email skipped - {email_type.value} email already sent today"
                )
                self.logger.info(
                    "Email notification skipped - same type already sent for this game today",
                    email_type=email_type.value,
                )
                return
            ...
            self.logger.info(
                "Email notification sent successfully", email_type=email_type.value
            )
    ...
    def _generate_email_content(
        ...
        if analysis_stage and analysis_stage.stdout:
            ...
            has_betting_opportunities = (
                "VALIDATED BETTING SIGNALS" in analysis_stage.stdout
            )
...
General Balls"""
        ...
        html_content = self._generate_html_email(
            workflow_result, game_desc, game_time_str, analysis_output
        )
...
"""
    ...
    async def _log_betting_recommendations(
        self, analysis_output: str, game: MLBGameInfo
    ) -> None:
        ...
        try:
            ...
            recommendations = tracker.parse_pre_game_email_content(
                analysis_output, game_info
            )
            ...
            if recommendations:
                ...
                self.logger.info(
                    "Logged betting recommendations for tracking",
                    game=f"{game.away_team} @ {game.home_team}",
                ...
                self.recommendation_metrics["recommendations_logged"] += len(
                    recommendations
                )
                self.recommendation_metrics["games_with_recommendations"] += 1
            else:
                self.logger.info(
                    "No betting recommendations found to log",
                    game=f"{game.away_team} @ {game.home_team}",
                )

        except Exception as e:
            self.logger.warning(
                "Failed to log betting recommendations",
                game=f"{game.away_team} @ {game.home_team}",
                error=str(e),
            )

    def _get_stage_status_text(self, stage_result: StageResult | None) -> str:
        ...
        base_text = status_map.get(
            stage_result.status, f"❓ {stage_result.status.value}"
        )
    ...
    async def _send_error_notification(
        self, workflow_result: WorkflowResult, error_message: str
    ) -> StageResult:
        ...
        try:
            ...
            game_desc = (
                f"{workflow_result.game.away_team} @ {workflow_result.game.home_team}"
            )
...
<body style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
    ...
    def get_recent_workflows(self, limit: int = 10) -> list[WorkflowResult]:
        ...
        return sorted(self.workflow_history, key=lambda w: w.start_time, reverse=True)[
            :limit
        ]
    ...
    def get_metrics(self) -> dict[str, Any]:
        ...
        metrics = {
            **self.metrics,
            ...
            "workflow_queue_available": self._workflow_semaphore._value
            > 0,  # True if queue available
