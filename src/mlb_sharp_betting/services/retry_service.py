...
from typing import Callable, Dict, Any, Optional, Union, Awaitable, TypeVar, Generic
...
class RetryService:
    ...
    def __init__(self):
        ...
        self.circuit_breakers: Dict[OperationType, CircuitBreaker] = {
            OperationType.DATABASE: CircuitBreaker(failure_threshold=3, recovery_timeout=30),
            OperationType.API_CALL: CircuitBreaker(failure_threshold=5, recovery_timeout=60),
            OperationType.SCRAPING: CircuitBreaker(failure_threshold=3, recovery_timeout=45),
            OperationType.NOTIFICATION: CircuitBreaker(failure_threshold=2, recovery_timeout=120),
            OperationType.FILE_IO: CircuitBreaker(failure_threshold=2, recovery_timeout=15),
            OperationType.NETWORK: CircuitBreaker(failure_threshold=4, recovery_timeout=60),
            OperationType.BETTING_ANALYSIS: CircuitBreaker(failure_threshold=2, recovery_timeout=30)
        }
    ...
    async def execute_with_retry(self,
                               ...
                               base_delay: Optional[float] = None) -> T:
        """
        ...
        if not circuit_breaker.can_proceed():
            ...
            raise Exception(f"Circuit breaker open for operation type: {operation_type.value}")
        ...
        raise last_exception or Exception(f"Operation failed after {max_attempts} attempts: {operation_name}")
    ...
    def _get_default_retry_config(self, operation_type: OperationType) -> Dict[str, Any]:
    ...
    def _record_successful_attempt(self, operation_type: OperationType, attempt: RetryAttempt) -> None:
    ...
    def _record_failed_attempt(self, operation_type: OperationType, attempt: RetryAttempt) -> None:
    ...
    def _record_circuit_breaker_trip(self, operation_type: OperationType) -> None:
        ...
        self.logger.warning(f"Circuit breaker tripped for operation type: {operation_type.value}")
    ...
    def reset_circuit_breaker(self, operation_type: OperationType) -> None:
        ...
        self.logger.info(f"Circuit breaker reset for operation type: {operation_type.value}")
...
def retry_operation(operation_type: OperationType = OperationType.NETWORK,
                   service_name: Optional[str] = None,
                   max_retries: Optional[int] = None,
