...
class BettingAnalysisFormatter:
    ...
    def format_analysis(
        self,
        analysis_result: BettingAnalysisResult,
        juice_filter_summary: dict[str, Any] = None,
    ) -> str:
        ...
        # Strategy status
        output.append(
            self._format_strategy_status(analysis_result.strategy_performance)
        )
    ...
    def _format_header(
        ...
        if juice_filter_summary:
            if juice_filter_summary.get("enabled"):
                lines.append(
                    f"🚫 SMART JUICE FILTER: Won't bet favorites worse than {juice_filter_summary['max_juice_threshold']}"
                )
            else:
    ...
    def _format_strategy_status(
        ...
        if not strategy_performance:
            ...
            weighted_win_rate = (
                sum(s.win_rate * s.total_bets for s in strategy_performance)
                / total_bets
                if total_bets > 0
                else 0
            )
            weighted_roi = (
                sum(s.roi * s.total_bets for s in strategy_performance) / total_bets
                if total_bets > 0
                else 0
            )
            ...
            top_strategies = sorted(
                strategy_performance, key=lambda x: x.roi, reverse=True
            )[:3]
            lines.append("   🏆 Top Strategies:")
            for i, strategy in enumerate(top_strategies, 1):
                name_truncated = strategy.strategy_name[:25]
                lines.append(
                    f"      {i}. {name_truncated:<25} | {strategy.win_rate:5.1f}% WR | {strategy.roi:+6.1f}% ROI | {strategy.total_bets:3d} bets"
                )
    ...
    def _format_game_analysis(self, games: dict[tuple, GameAnalysis]) -> str:
        ...
        lines = [f"\n🎯 {total_opportunities} BETTING OPPORTUNITIES FOUND", "=" * 60]

        for game_key, game_analysis in sorted(
            games.items(), key=lambda x: x[1].game_time
        ):
    ...
    def _signal_to_recommendation(self, signal: BettingSignal) -> dict[str, Any]:
        ...
        type_display = (
            f"{type_emoji_map.get(signal.signal_type, '🎯')} {signal.strategy_name}"
        )
    ...
    def _format_recommendation(self, index: int, rec: dict[str, Any]) -> list[str]:
        ...
        lines.append(
            f"     {confidence_emoji} Confidence: {rec['confidence_score']:.0f}/100 ({rec['confidence_level']})"
        )
    ...
    def _parse_clear_bet_details(self, rec: dict[str, Any]) -> dict[str, str]:
        ...
        try:
            ...
            if (
                "moneyline" in strategy_name
                or "_ml_" in strategy_name
                or "ML" in bet_recommendation
            ):
                bet_type = "MONEYLINE"
            elif (
                "spread" in strategy_name
                or "_sprd_" in strategy_name
                or "SPREAD" in bet_recommendation
            ):
                bet_type = "SPREAD"
            elif (
                "total" in strategy_name
                or "_tot_" in strategy_name
                or "TOTAL" in bet_recommendation
                or "OVER" in bet_recommendation
                or "UNDER" in bet_recommendation
            ):
                bet_type = "TOTAL"
            else:
                bet_type = "UNKNOWN"
            ...
            if (
                "draftkings" in strategy_name
                or "dk" in strategy_name
                or "draftkings" in source_details
            ):
                book = "DraftKings"
            ...
            if original_bet:
                ...
                if (
                    bet_type == "MONEYLINE"
                    and "ML" not in clean_instruction
                    and "MONEYLINE" not in clean_instruction.upper()
                ):
                    ...
                    bet_type == "TOTAL"
                    and "TOTAL" not in clean_instruction.upper()
                    and "OVER" not in clean_instruction.upper()
                    and "UNDER" not in clean_instruction.upper()
                ):
    ...
    def _format_summary(self, analysis_result: BettingAnalysisResult) -> str:
        ...
        lines = [
            ...
            "   🤖 All recommendations use AI-validated strategies",
        ]

        return "\n".join(lines)

    def _format_no_opportunities(
        self,
        analysis_result: BettingAnalysisResult,
        juice_filter_summary: dict[str, Any] = None,
    ) -> str:
        ...
        lines = [
            ...
            "   ✅ All games were analyzed using validated strategies",
            "   ✅ Current betting data does not meet proven thresholds",
            "   ✅ This protects you from low-probability bets",
            "\n💡 RECOMMENDATION:",
            "   🛑 NO BETS RECOMMENDED AT THIS TIME",
            "   ⏰ Check again closer to other game times",
            "   📧 You'll receive email alerts when opportunities are found",
            "=" * 70,
        ]
    ...
    def format_debug_info(
        self, database_stats: dict[str, int], actionable_games: int
    ) -> str:
