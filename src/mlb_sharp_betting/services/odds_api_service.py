...
class OddsAPIUsageTracker:
    ...
    def record_usage(self, cost: int, endpoint: str, markets: list[str]) -> None:
        ...
        self._save_usage_data(data)
        logger.info(
            f"Odds API usage: {cost} credits used, {data['used']}/{self.max_monthly_usage} total this month"
        )
...
class OddsAPIService:
    ...
    def get_mlb_odds(
        ...
        if market_config not in self.MARKET_CONFIGS:
            raise ValueError(
                f"Invalid market_config. Choose from: {list(self.MARKET_CONFIGS.keys())}"
            )
        ...
        if not self.usage_tracker.can_make_call(estimated_cost):
            ...
            logger.warning(
                f"Cannot make Odds API call. Would exceed monthly limit. "
        ...
        logger.info(
            f"Fetching MLB odds with {market_config} markets (estimated cost: {estimated_cost})"
        )
        return self._make_request(f"sports/{self.SPORT_KEY}/odds", params)

    def get_specific_game_odds(
        self,
        event_id: str,
        market_config: str = "standard",
        regions: str = "us",
        ...
        if not self.usage_tracker.can_make_call(estimated_cost):
            usage_status = self.usage_tracker.get_usage_status()
            logger.warning(
                f"Cannot make Odds API call for game {event_id}. "
                f"Current usage: {usage_status['used']}/{usage_status['remaining']}"
            )
            return None
    ...
    def get_today_games(self) -> list[dict] | None:
        """Get today's MLB games (free endpoint - no usage cost)."""
        return self._make_request(
            f"sports/{self.SPORT_KEY}/events", {"dateFormat": "iso"}
        )
    ...
    def optimize_for_budget(self, games_needed: int) -> str:
        ...
        if remaining >= games_needed * 30:  # Standard config
            ...
            logger.warning(
                f"Budget allows for only {max_games} games with essential markets"
            )
            return "essential"
...
class OddsData(BaseModel):
    ...
    def get_moneyline_odds(self, bookmaker_key: str = None) -> dict | None:
    ...
    def get_spread_odds(self, bookmaker_key: str = None) -> dict | None:
    ...
    def get_total_odds(self, bookmaker_key: str = None) -> dict | None:
