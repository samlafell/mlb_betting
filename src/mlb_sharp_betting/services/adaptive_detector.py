...
from ..models.betting_analysis import BettingAnalysisResult, BettingSignal, GameAnalysis
...
class AdaptiveBettingDetector:
    ...
    async def initialize(self):
        ...
        try:
            ...
            self.logger.info(
                "Adaptive betting detector initialized with strategy manager"
            )
    ...
    async def analyze_opportunities(
        self, minutes_ahead: int = 60, debug_mode: bool = False
        ...
        try:
            ...
            signals = await self.orchestrator.execute_live_strategy_detection(
                minutes_ahead
            )
            ...
            return BettingAnalysisResult(
                games={}, strategy_performance=[], analysis_metadata={"error": str(e)}
            )
    ...
    async def _display_strategy_configuration(self, strategy_state):
        ...
        print(
            f"🕐 Last Updated: {strategy_state.last_updated.strftime('%Y-%m-%d %H:%M:%S UTC')}"
        )
        ...
        for i, config in enumerate(strategy_state.enabled_strategies, 1):
            roi_color = (
                "🔥"
                if config.recent_roi > 15
                else "⭐"
                if config.recent_roi > 10
                else "✅"
            )
            trend_icon = (
                "📈"
                if config.performance_trend == "IMPROVING"
                else "📉"
                if config.performance_trend == "DECLINING"
                else "➡️"
            )

            print(f"  {i}. {roi_color} {config.strategy_name}")
            print(
                f"     📊 ROI: {config.recent_roi:+.1f}% | WR: {config.recent_win_rate:.1f}% | Sample: {config.sample_size}"
            )
            print(
                f"     ⚙️  Confidence: {config.confidence_multiplier:.2f}x | Weight: {config.weight_in_ensemble:.2f}"
            )
            print(
                f"     🎚️  Threshold: {config.min_differential_threshold:.1f}% (+{config.threshold_adjustment:+.1f})"
            )
        ...
        if strategy_state.disabled_strategies:
            ...
            for config in strategy_state.disabled_strategies:
                print(
                    f"  • {config.strategy_name}: ROI {config.recent_roi:+.1f}% | WR {config.recent_win_rate:.1f}%"
                )
        ...
        print(
            f"  • Average Win Rate (Enabled): {perf.get('avg_win_rate_enabled', 0):.1f}%"
        )
        print(f"  • Total Sample Size: {perf.get('total_sample_size', 0)}")

        trends = perf.get("performance_trends", {})
        print(
            f"  • Trends: {trends.get('IMPROVING', 0)} improving, {trends.get('STABLE', 0)} stable, {trends.get('DECLINING', 0)} declining"
        )

    async def _apply_juice_filtering(
        self, signals: list[BettingSignal]
    ) -> list[BettingSignal]:
            ...
            if signal.split_type == "moneyline":
                ...
                if "BET" in signal.recommendation:
                    ...
                    if self.juice_filter.should_filter_bet(
                        ...
                        self.logger.debug(
                            f"Filtered heavy juice bet: {signal.recommendation} at {signal.split_value}"
                        )
                        continue
    ...
    def _group_signals_by_game(
        self, signals: list[BettingSignal]
    ) -> dict[tuple, GameAnalysis]:
    ...
    async def get_strategy_performance_summary(self) -> dict[str, Any]:
        ...
        try:
            ...
            all_configs = (
                strategy_state.enabled_strategies + strategy_state.disabled_strategies
            )
    ...
    async def _display_debug_analysis(self, result: BettingAnalysisResult):
        ...
        for (away, home, game_time), game_analysis in result.games.items():
            ...
            for signal in game_analysis.all_signals:
                ...
                print(
                    f"  {confidence_emoji} {signal.signal_type.value.upper()}: {bet_details['clear_bet_instruction']}"
                )
                print(
                    f"    🎰 Bet Type: {bet_details['bet_type']} | 💰 Odds: {bet_details['odds_display']} | 🏪 Book: {bet_details['recommended_book']}"
                )
                print(
                    f"    💪 Strength: {signal.signal_strength:.1f} | Confidence: {signal.confidence_score:.2f}"
                )
                print(
                    f"    📊 Strategy: {signal.strategy_name} | ROI: {signal.roi:+.1f}%"
                )
                if hasattr(signal, "metadata") and signal.metadata:
                    orchestrator_config = signal.metadata.get("orchestrator_config", {})
                    if orchestrator_config:
                        print(
                            f"    ⚙️  Weight: {orchestrator_config.get('strategy_weight', 0):.2f} | Trend: {orchestrator_config.get('performance_trend', 'UNKNOWN')}"
                        )
    ...
    async def _display_standard_analysis(self, result: BettingAnalysisResult):
        ...
        metadata = result.analysis_metadata
        print(
            f"⚙️  Using {metadata.get('enabled_strategies', 0)} orchestrator-configured strategies"
        )
        ...
        for (away, home, game_time), game_analysis in result.games.items():
            print(f"\n🏟️  {away} @ {home}")
            print(
                f"⏰ Starts in {game_analysis.minutes_to_game} minutes ({game_time.strftime('%H:%M')})"
            )
            print("-" * 50)
            ...
            if highest_signal:
                confidence_emoji = self._get_confidence_emoji(
                    highest_signal.confidence_score
                )
                bet_details = self._parse_clear_bet_instructions(highest_signal)

                print(
                    f"{confidence_emoji} BEST OPPORTUNITY: {bet_details['clear_bet_instruction']}"
                )
                print(f"  🎰 Bet Type: {bet_details['bet_type']}")
                print(f"  💰 Odds: {bet_details['odds_display']}")
                print(f"  🏪 Book: {bet_details['recommended_book']}")
                print(
                    f"  📊 Strategy: {highest_signal.strategy_name} (Recent ROI: {highest_signal.roi:+.1f}%)"
                )
                ...
                if hasattr(highest_signal, "metadata") and highest_signal.metadata:
                    orchestrator_config = highest_signal.metadata.get(
                        "orchestrator_config", {}
                    )
                    if orchestrator_config:
            ...
            other_signals = [
                s for s in game_analysis.all_signals if s != highest_signal
            ]
            if other_signals:
    ...
    def _parse_clear_bet_instructions(self, signal: BettingSignal) -> dict[str, str]:
        ...
        try:
            ...
            if signal.recommendation:
                ...
                if (
                    bet_type == "MONEYLINE"
                    and "ML" not in clean_rec
                    and "MONEYLINE" not in clean_rec.upper()
                ):
