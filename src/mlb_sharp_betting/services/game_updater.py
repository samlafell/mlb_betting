...
from ..db.repositories import get_game_outcome_repository, get_betting_split_repository
from ..db.connection import get_db_manager
from ..utils.mlb_api_client import MLBStatsAPIClient, MLBAPIError
...
class GameUpdater:
    ...
    async def update_game_outcomes_for_date(self, 
                                          target_date: date,
                                          use_betting_lines: bool = True) -> List[GameOutcome]:
        """
        ...
        try:
            ...
            async with MLBStatsAPIClient() as api_client:
                ...
                if not completed_games:
                    logger.info("No completed games found for date", date=target_date.isoformat())
                    return outcomes
                
                logger.info("Found completed games", count=len(completed_games), date=target_date.isoformat())
                
                # Process each completed game
                for game_data in completed_games:
                    try:
                        outcome = await self._process_completed_game(game_data, use_betting_lines)
                        if outcome:
                ...
                logger.info("Game outcomes update completed", 
            ...
            logger.error("Failed to update game outcomes", date=target_date.isoformat(), error=str(e))
            raise GameUpdateError(f"Failed to update game outcomes: {str(e)}")
    
    async def update_recent_game_outcomes(self, 
                                        days_back: int = 3,
                                        use_betting_lines: bool = True) -> List[GameOutcome]:
        """
        ...
        try:
            ...
            logger.info("Recent game outcomes update completed", 
    ...
    async def update_game_status(self, game_id: str) -> GameOutcome:
        ...
        try:
            ...
            async with MLBStatsAPIClient() as api_client:
                ...
                if status_code != "F":
                    raise GameUpdateError(f"Game {game_id} is not completed (status: {status_code})")
                
                # Process the completed game
                outcome = await self._process_completed_game(game_data, use_betting_lines=True)
    ...
    async def _process_completed_game(self, 
                                    game_data: Dict[str, Any],
                                    use_betting_lines: bool = True) -> Optional[GameOutcome]:
        """
        ...
        try:
            async with MLBStatsAPIClient() as api_client:
                ...
                if not result_data:
                    logger.warning("Could not extract game result data", 
                                 game_id=game_data.get("gamePk"))
                    return None
                ...
                existing_outcome = await self.outcome_repo.get_outcome_by_game_id(game_id)
            ...
            logger.error("Failed to process completed game", 
                       game_id=game_data.get("gamePk"), 
                       error=str(e))
            return None
    
    async def _get_betting_lines(self, 
                               home_team: Team, 
                               away_team: Team,
                               game_date: Optional[datetime]) -> Tuple[Optional[float], Optional[float]]:
        """
        ...
        try:
            ...
            splits = self.split_repo.find_all(home_team=home_team.value, away_team=away_team.value)
            
            if not splits:
                logger.debug("No betting splits found for matchup", 
                           home_team=home_team.value, 
                           away_team=away_team.value)
                return None, None
                ...
                if split.split_value is not None:
                    ...
                    if split.split_type == "total":
                        try:
                            ...
                            elif isinstance(split.split_value, str) and split.split_value.strip():
                                total_line = float(split.split_value)
                        except (ValueError, TypeError):
                            logger.warning("Invalid total value", 
                        ...
                        try:
                            ...
                            logger.warning("Invalid spread value", 
                                         split_value=split.split_value, 
                                         game_id=split.game_id)
                            continue
            ...
            if home_spread_line is not None and moneyline_data is not None:
                ...
                logger.debug("Interpreting spread with moneyline data",
                           home_team=home_team.value,
                           away_team=away_team.value,
                ...
                if home_odds < away_odds:  # Home team is favorite (more negative = stronger favorite)
                ...
                logger.debug("Final spread assignment",
                           home_team=home_team.value,
                           away_team=away_team.value,
                           home_spread_line=home_spread_line,
                           interpretation="favorite gives points" if home_spread_line < 0 else "underdog gets points")
                ...
                logger.debug("No moneyline data, using fallback spread logic",
                           home_team=home_team.value,
                           away_team=away_team.value,
                           raw_spread=home_spread_line)
            ...
            logger.debug("Final betting lines", 
                       home_team=home_team.value,
                       away_team=away_team.value,
                       total_line=total_line,
                       home_spread_line=home_spread_line)
            ...
            logger.error("Failed to get betting lines", 
                       home_team=home_team.value,
                       away_team=away_team.value,
                       error=str(e))
            return None, None
