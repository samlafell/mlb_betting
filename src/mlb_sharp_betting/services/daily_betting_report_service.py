...
class BettingOpportunity:
    ...
    actual_result: str | None = None  # 'WIN', 'LOSS', 'PUSH', 'VOID'
...
class BetSizingCalculator:
    ...
    def calculate_bet_sizing(
        ...
        try:
            ...
            logger.warning(
                "Failed to calculate bet sizing", line_value=line_value, error=str(e)
            )
            # Return default -110 sizing
            return Decimal("110"), Decimal("100"), Decimal("210")

    @staticmethod
    def _calculate_moneyline_sizing(
        line_value: str,
    ) -> tuple[Decimal, Decimal, Decimal]:
    ...
    @staticmethod
    def _calculate_spread_total_sizing(
        line_value: str,
    ) -> tuple[Decimal, Decimal, Decimal]:
...
class DailyBettingReportService:
    ...
    def __init__(self, db_manager: DatabaseManager | None = None):
        ...
        self.logger.info(
            "🚀 DailyBettingReportService initialized with table registry support"
        )
    ...
    async def generate_daily_report(
        self, target_date: date | None = None, output_format: str = "console"
    ) -> DailyBettingReport:
        """
        ...
        self.logger.info(
            "Starting daily betting report generation",
        ...
        try:
            ...
            if target_date <= today:
                self.logger.info(
                    "Updating game outcomes for completed games",
                    target_date=target_date.isoformat(),
                )
                try:
                    updated_outcomes = (
                ...
                self.logger.info(
                    "Target date is in future, skipping game outcome updates",
                    target_date=target_date.isoformat(),
                )
            ...
            for opp in raw_opportunities:
                ...
                if opp.bet_type == "moneyline" and is_moneyline_too_juiced(
                    opp.line_value, opp.recommended_side, opp.home_team, opp.away_team
                ):
                    self.logger.info(
                        "Filtered juiced moneyline bet",
            ...
            if not validated_opportunities:
                self.logger.warning(
                    "No validated betting opportunities found",
                    target_date=target_date.isoformat(),
                )
                # Return empty report
            ...
            selected_opportunities = validated_opportunities[
                : self.config["max_opportunities_per_day"]
            ]

            # Step 4: Add actual results for completed games
            opportunities_with_results = await self._add_actual_results(
                selected_opportunities, target_date
            )

            # Step 5: Calculate summary statistics
            summary_stats = self._calculate_summary_statistics(
                opportunities_with_results
            )

            # Step 6: Calculate quality metrics
            quality_metrics = await self._calculate_quality_metrics_validated(
                validated_opportunities, target_date
            )
            ...
            report = DailyBettingReport(
                ...
                detection_time_distribution=quality_metrics[
                    "detection_time_distribution"
                ],
            ...
            self.logger.info(
                "Daily betting report completed successfully",
                target_date=target_date.isoformat(),
            ...
            self.logger.error(
                "Daily betting report generation failed",
                target_date=target_date.isoformat(),
                error=str(e),
            )
            raise

    async def _extract_betting_opportunities(
        ...
        try:
            with self.db_manager.get_cursor() as cursor:
                ...
                cursor.execute(
                    ...
                    [
                        ...
                        self.config["max_hours_before_game"]
                        * 60,  # Convert hours to minutes
                        self.config["detection_cutoff_minutes"],
                        self.config["max_opportunities_per_day"]
                        * 2,  # Get extra for filtering, will limit later
                    ],
                )
                ...
                for row in results:
                    (
                        ...
                        expected_roi,
                        signal_detected_at,
                        minutes_before_game_at_detection,
                    ) = row
                    ...
                    stake_amount, win_amount, total_return = (
                        self.bet_calculator.calculate_bet_sizing(str(odds), bet_type)
                    )

                    # Calculate Kelly Criterion recommendation
                    kelly_fraction = min(
                        0.25, max(0.01, float(expected_roi) / 100.0 * 0.1)
                    ...
                    opportunity = BettingOpportunity(
                        ...
                        if hasattr(signal_detected_at, "tzinfo")
                        else signal_detected_at.replace(tzinfo=timezone.utc),
                        minutes_before_game=int(minutes_before_game_at_detection)
                        if minutes_before_game_at_detection
                        else 0,
                        detection_priority=self._calculate_priority(
                            expected_roi, confidence_score
                        ),
                        strategy_source=f"{strategy_variant}_{source.lower()}",
                        signal_strength=confidence,
                        last_updated=signal_detected_at
                        if hasattr(signal_detected_at, "tzinfo")
                        else signal_detected_at.replace(tzinfo=timezone.utc),
                    )
                ...
                self.logger.info(
                    "Extracted validated betting opportunities",
            ...
            self.logger.error(
                "Failed to extract validated betting opportunities", error=str(e)
            )
            return []
    ...
    async def _score_opportunities(
        self, raw_opportunities: list[dict[str, Any]]
    ) -> list[BettingOpportunity]:
        ...
        for raw_opp in raw_opportunities:
            try:
                ...
                stake_amount, win_amount, total_return = (
                    self.bet_calculator.calculate_bet_sizing(
                        raw_opp["split_value"] or "-110", raw_opp["split_type"]
                    )
                )
    ...
    async def _select_top_opportunities(
        self, scored_opportunities: list[BettingOpportunity]
    ) -> list[BettingOpportunity]:
    ...
    async def _add_actual_results(
        self, opportunities: list[BettingOpportunity], target_date: date
    ) -> list[BettingOpportunity]:
        ...
        try:
            ...
            self.logger.error(
                "Failed to fetch game outcomes",
                target_date=target_date.isoformat(),
                error=str(e),
            )
            return opportunities

    def _apply_game_outcome(
        self, opportunity: BettingOpportunity, outcome: dict[str, Any]
    ) -> BettingOpportunity:
        ...
        try:
            ...
            if opportunity.bet_type == "moneyline":
                ...
                if opportunity.recommended_side == "home":
                    ...
                    bet_result = (
                        "WIN" if not outcome.get("home_cover_spread") else "LOSS"
                    )
    ...
    def _calculate_summary_statistics(
        ...
        expected_value = sum(
            float(opp.stake_amount) * (float(opp.roi_estimate) / 100.0)
            for opp in opportunities
        )

        # Check for actual results
        opportunities_with_results = [
            opp for opp in opportunities if opp.actual_result is not None
        ]
        ...
        if opportunities_with_results:
            ...
            pushes = len(
                [
                    opp
                    for opp in opportunities_with_results
                    if opp.actual_result == "PUSH"
                ]
            )

            actual_profit_loss = sum(
                opp.actual_profit_loss
                for opp in opportunities_with_results
            ...
            actual_roi = (
                float(actual_profit_loss / completed_risk * 100)
                if completed_risk > 0
                else 0.0
            )
    ...
    async def _calculate_quality_metrics_validated(
        self, validated_opportunities: list[BettingOpportunity], target_date: date
    ) -> dict[str, Any]:
        ...
        avg_confidence = sum(
            opp.confidence_score for opp in validated_opportunities
    ...
    async def _store_daily_report(self, report: DailyBettingReport) -> None:
        ...
        try:
            # Store as JSON file
            report_file = (
                self.reports_dir / f"daily_report_{report.report_date.isoformat()}.json"
            )
    ...
    def format_console_report(self, report: DailyBettingReport) -> str:
        ...
        if not report.selected_opportunities:
            ...
            for i, opp in enumerate(report.selected_opportunities, 1):
                ...
                if opp.actual_result is not None:
                    if opp.actual_result == "WIN":
                        result_text = (
                            f" | ✅ {opp.actual_result} (+${opp.actual_profit_loss})"
                        )
        ...
        if report.actual_results_available and report.completed_bets > 0:
            win_rate = (
                (report.wins / report.completed_bets * 100)
                if report.completed_bets > 0
                else 0
            )
            lines.extend(
                [
