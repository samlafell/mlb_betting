...
class TimingAnalysisService:
    ...
    async def analyze_timing_performance(
        ...
        with self.db_manager.get_cursor() as cursor:
            ...
            if overall_result and overall_result["total_bets"] > 0:
                overall_metrics = TimingPerformanceMetrics(
                    ...
                    total_units_wagered=Decimal(
                        str(overall_result["total_units_wagered"])
                    ),
                    total_profit_loss=Decimal(str(overall_result["total_profit_loss"])),
                    avg_odds_at_recommendation=Decimal(
                        str(overall_result["avg_odds_at_recommendation"])
                    )
                    if overall_result["avg_odds_at_recommendation"]
                    else None,
                    avg_closing_odds=Decimal(str(overall_result["avg_closing_odds"]))
                    if overall_result["avg_closing_odds"]
                    else None,
                )
            ...
            recommendation_history_table = self.table_registry.get_table(
            ...
            """,
                (
                    ...
                    source.value if source else None,
                    book.value if book else None,
                    book.value if book else None,
                    split_type.value if split_type else None,
                    split_type.value if split_type else None,
                    strategy_name,
                    strategy_name,
                ),
            ...
            for bucket in TimingBucket:
                ...
                if bucket_result and bucket_result["total_bets"] >= minimum_sample_size:
                    bucket_metrics = TimingPerformanceMetrics(
                        ...
                        total_units_wagered=Decimal(
                            str(bucket_result["total_units_wagered"])
                        ),
                        total_profit_loss=Decimal(
                            str(bucket_result["total_profit_loss"])
                        ),
                        avg_odds_at_recommendation=Decimal(
                            str(bucket_result["avg_odds_at_recommendation"])
                        )
                        if bucket_result["avg_odds_at_recommendation"]
                        else None,
                        avg_closing_odds=Decimal(str(bucket_result["avg_closing_odds"]))
                        if bucket_result["avg_closing_odds"]
                        else None,
                    )
        ...
        if bucket_analyses:
            ...
            best_bucket_analysis = max(
                bucket_analyses, key=lambda x: x.metrics.roi_percentage
            )
            best_bucket = best_bucket_analysis.timing_bucket

            # If not filtered, find best source and strategy
            if source is None:
                best_source = await self._find_best_source(
                    start_date, end_date, book, split_type, strategy_name
                )
            if strategy_name is None:
                best_strategy = await self._find_best_strategy(
                    start_date, end_date, source, book, split_type
                )
        ...
        self.logger.info(
            ...
            best_bucket=best_bucket.value
            if hasattr(best_bucket, "value") and best_bucket
            else (best_bucket if best_bucket else None),
        )

        return analysis

    async def get_realtime_timing_recommendation(
        self, lookup: RealtimeTimingLookup, use_cache: bool = True
    ) -> TimingRecommendation:
        """
    ...
    async def get_timing_performance_summary(
        self, days_back: int = 30, minimum_sample_size: int = 10
    ) -> list[dict]:
        """
        ...
        with self.db_manager.get_cursor() as cursor:
            # Get table name from registry
            current_timing_performance_table = self.table_registry.get_table(
    ...
    def _build_analysis_query(
        ...
        bucket_filter: TimingBucket | None,
    ) -> tuple[str, tuple]:
        ...
        if split_type:
            ...
            params.append(
                split_type.value if hasattr(split_type, "value") else split_type
            )
        ...
        if bucket_filter:
            ...
            params.append(
                bucket_filter.value
                if hasattr(bucket_filter, "value")
                else bucket_filter
            )
    ...
    async def _find_best_source(
        ...
        with self.db_manager.get_cursor() as cursor:
            # Get table name from registry
            timing_bucket_performance_table = self.table_registry.get_table(
    ...
    async def _analyze_trends(
        ...
        trends["optimal_timing"] = (
            best_bucket.timing_bucket.value
            if hasattr(best_bucket.timing_bucket, "value")
            else best_bucket.timing_bucket
        )
        ...
        bucket_performance = {
            (
                ba.timing_bucket.value
                if hasattr(ba.timing_bucket, "value")
                else ba.timing_bucket
            ): ba.metrics.roi_percentage
            for ba in bucket_analyses
        }

        # Check if performance decreases with time to game
        bucket_order = ["0-2h", "2-6h", "6-24h", "24h+"]
        performance_by_order = [
            bucket_performance.get(bucket, 0)
        ...
        if len(performance_by_order) >= 2:
            is_decreasing = all(
                performance_by_order[i] >= performance_by_order[i + 1]
                for i in range(len(performance_by_order) - 1)
            )
            is_increasing = all(
                performance_by_order[i] <= performance_by_order[i + 1]
                for i in range(len(performance_by_order) - 1)
        ...
        trends["avg_odds_movement"] = (
            total_movement / len(bucket_analyses) if bucket_analyses else 0
        )

        return trends

    async def _store_analysis_results(
        self, analysis: ComprehensiveTimingAnalysis
    ) -> None:
        """Store comprehensive analysis results in database."""
        try:
            with self.db_manager.get_cursor() as cursor:
                # Get table names from registry
                comprehensive_analyses_table = self.table_registry.get_table(
                ...
                """,
                    (
                        ...
                        float(analysis.overall_metrics.avg_odds_at_recommendation)
                        if analysis.overall_metrics.avg_odds_at_recommendation
                        else None,
                        float(analysis.overall_metrics.avg_closing_odds)
                        if analysis.overall_metrics.avg_closing_odds
                        else None,
                        analysis.best_bucket.value
                        if hasattr(analysis.best_bucket, "value")
                        and analysis.best_bucket
                        else (analysis.best_bucket if analysis.best_bucket else None),
                        analysis.best_source.value
                        if hasattr(analysis.best_source, "value")
                        and analysis.best_source
                        else (analysis.best_source if analysis.best_source else None),
                ...
                for bucket_analysis in analysis.bucket_analyses:
                    ...
                    """,
                        (
                            bucket_analysis.timing_bucket.value
                            if hasattr(bucket_analysis.timing_bucket, "value")
                            else bucket_analysis.timing_bucket,
                            bucket_analysis.source.value
                            if hasattr(bucket_analysis.source, "value")
                            and bucket_analysis.source
                            else (
                                bucket_analysis.source
                                if bucket_analysis.source
                                else None
                            ),
                            bucket_analysis.book.value
                            if hasattr(bucket_analysis.book, "value")
                            and bucket_analysis.book
                            else (
                                bucket_analysis.book if bucket_analysis.book else None
                            ),
                            bucket_analysis.split_type.value
                            if hasattr(bucket_analysis.split_type, "value")
                            and bucket_analysis.split_type
                            else (
                                bucket_analysis.split_type
                                if bucket_analysis.split_type
                                else None
                            ),
                            ...
                            float(bucket_analysis.metrics.avg_odds_at_recommendation)
                            if bucket_analysis.metrics.avg_odds_at_recommendation
                            else None,
                            float(bucket_analysis.metrics.avg_closing_odds)
                            if bucket_analysis.metrics.avg_closing_odds
                            else None,
                        ),
                    )
    ...
    async def _get_cached_recommendation(
        self, lookup: RealtimeTimingLookup
    ) -> TimingRecommendation | None:
        """Get cached timing recommendation if available and not expired."""
        try:
            with self.db_manager.get_cursor() as cursor:
                # Get table name from registry
                timing_recommendations_cache_table = self.table_registry.get_table(
                ...
                """,
                    (
                        ...
                        lookup.source.value
                        if hasattr(lookup.source, "value") and lookup.source
                        else (lookup.source if lookup.source else None),
                        lookup.book.value
                        if hasattr(lookup.book, "value") and lookup.book
                        else (lookup.book if lookup.book else None),
                        lookup.split_type.value
                        if hasattr(lookup.split_type, "value")
                        else lookup.split_type,
                ...
                if result:
                    ...
                    if result["historical_total_bets"]:
                        ...
                        historical_roi = (
                            float(result["historical_roi"])
                            if result["historical_roi"]
                            else 0.0
                        )
                        historical_total_bets = float(result["historical_total_bets"])
                        historical_win_rate = (
                            float(result["historical_win_rate"])
                            if result["historical_win_rate"]
                            else 0.0
                        )
                        ...
                        historical_metrics = TimingPerformanceMetrics(
                            ...
                            wins=int(
                                ...
                                * float(historical_total_bets)
                                / 100
                            )
                            if result["historical_win_rate"]
                            else 0,
                            losses=result["historical_total_bets"]
                            - int(
                                float(historical_win_rate)
                                * float(historical_total_bets)
                                / 100
                            ...
                            if result["historical_win_rate"]
                            else result["historical_total_bets"],
                            pushes=0,
                            total_units_wagered=Decimal(
                                str(result["historical_total_bets"])
                            ),
                            total_profit_loss=Decimal(
                                str(
                                    float(historical_roi)
                                    * float(historical_total_bets)
                                    / 100
                                )
                            )
                    ...
                    return TimingRecommendation(
                        ...
                        expected_win_rate=float(result["expected_win_rate"])
                        if result["expected_win_rate"]
                        else None,
                        expected_roi=float(result["expected_roi"])
                        if result["expected_roi"]
                        else None,
    ...
    async def _get_historical_performance(
        ...
        with self.db_manager.get_cursor() as cursor:
            ...
            if lookup.source:
                ...
                params.append(
                    lookup.source.value
                    if hasattr(lookup.source, "value")
                    else lookup.source
                )
            ...
            if lookup.book:
                query += " AND book = %s"
                params.append(
                    lookup.book.value if hasattr(lookup.book, "value") else lookup.book
                )
            ...
            if result and result["total_bets"] > 0:
                return TimingPerformanceMetrics(
                    ...
                    avg_odds_at_recommendation=Decimal(
                        str(result["avg_odds_at_recommendation"])
                    )
                    if result["avg_odds_at_recommendation"]
                    else None,
                    avg_closing_odds=Decimal(str(result["avg_closing_odds"]))
                    if result["avg_closing_odds"]
                    else None,
                )
    ...
    def _generate_recommendation(
        ...
        historical_metrics: TimingPerformanceMetrics | None,
    ) -> TimingRecommendation:
        ...
        if sample_size_warning:
            risk_factors.append(
                f"Small sample size ({historical_metrics.total_bets} bets)"
            )
    ...
    async def _cache_recommendation(
        self, lookup: RealtimeTimingLookup, recommendation: TimingRecommendation
    ) -> None:
            ...
            with self.db_manager.get_cursor() as cursor:
                ...
                """,
                    (
                        ...
                        recommendation.historical_metrics.total_bets
                        if recommendation.historical_metrics
                        else None,
                        float(recommendation.historical_metrics.win_rate_percentage)
                        if recommendation.historical_metrics
                        else None,
                        float(recommendation.historical_metrics.roi_percentage)
                        if recommendation.historical_metrics
                        else None,
