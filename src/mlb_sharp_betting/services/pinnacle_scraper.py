...
class PinnacleScraper:
    ...
    def __init__(self, config: PinnacleScraperConfig | None = None):
            ...
            try:
                ...
                async with session.get(
                    ...
                    if response.status == 200:
                        ...
                        self.logger.warning(
                            "Failed to scrape data", url=url, status=response.status
                        )
                ...
                self.logger.error(
                    "Scraping error", url=url, error=str(e), attempt=attempt + 1
                )
                if attempt == self.config.max_retries - 1:
                    return None
                await asyncio.sleep(self.config.retry_delay * (2**attempt))
    ...
    async def scrape_active_matchups(self) -> list[int]:
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                ...
                self.logger.info(
                    "Successfully scraped matchup IDs", count=len(matchup_ids)
                )
                return matchup_ids
        ...
        try:
            async with aiohttp.ClientSession() as session:
                ...
                for market_data in data:
                    ...
                    essential_info = self._extract_essential_market_info(
                        market_data, matchup_id
                    )
                    if essential_info:
                ...
                self.logger.debug(
                    ...
                    markets_found=len(essential_markets),
                )
                return essential_markets

        except Exception as e:
            self.logger.error(
                "Failed to scrape markets", matchup_id=matchup_id, error=str(e)
            )
            return []

    def _extract_essential_market_info(
        self, raw_data: dict[str, Any], matchup_id: int
    ) -> dict[str, Any] | None:
    ...
    def _extract_team_info(self, raw_data: dict[str, Any]) -> dict[str, str] | None:
    ...
    def _extract_price_info(
        ...
        for price_data in prices_data:
            ...
            if price_value is not None and designation in [
                "home",
                "away",
                "over",
                "under",
            ]:
                prices.append({"designation": designation, "price": int(price_value)})
    ...
    def _extract_start_time(self, raw_data: dict[str, Any]) -> str | None:
        """Extract start time from various possible locations."""
        # Try different locations for start time
        start_time = (
            raw_data.get("startTime")
            or raw_data.get("start_time")
            or (raw_data.get("parent", {}).get("startTime"))
        )
        ...
        try:
            ...
            for i, result in enumerate(results):
                if isinstance(result, Exception):
                    self.logger.warning(
                        "Failed to scrape matchup markets",
            ...
            self.logger.info(
                "Successfully completed MLB data scrape",
    ...
    async def scrape_team_matchup(
        ...
        try:
            ...
            for market in all_data.get("markets", []):
                ...
                if (
                    teams.get("home") == home_team.value
                    and teams.get("away") == away_team.value
                ):
                    matching_markets.append(market)

            self.logger.info(
                "Found matching markets for teams",
            ...
            self.logger.error(
                "Failed to scrape team matchup data",
                home_team=home_team,
                away_team=away_team,
                error=str(e),
            )
