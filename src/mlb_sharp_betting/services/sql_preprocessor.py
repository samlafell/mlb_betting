...
class SQLPreprocessor:
    ...
    def _fix_sql_comments(self, sql: str) -> str:
        ...
        patterns = [
            (r'\bwith\s+sharp\s+action\s+detection\b', '-- with sharp action detection'),
            (r'\bwith\s+line\s+movement\s+analysis\b', '-- with line movement analysis'),
            (r'\bwith\s+enhanced\s+timing\s+patterns\b', '-- with enhanced timing patterns'),
        ]
    ...
    def _fix_round_function(self, sql: str) -> str:
        ...
        while True:
            ...
            closing_paren_pos = find_matching_paren(result_sql, round_pos + 5)  # +5 for 'round'
            ...
            result_sql = result_sql[:round_pos] + replacement + result_sql[closing_paren_pos + 1:]
    ...
    def _fix_postgresql_types(self, sql: str) -> str:
        ...
        for pattern in boolean_field_patterns:
            def replace_bool_agg(match):
                ...
                if any(time_word in field.lower() for time_word in ['date', 'time', 'datetime', 'timestamp']):
    ...
    def _fix_boolean_comparisons(self, sql: str) -> str:
        ...
        patterns = [
            ...
            (r'\b([a-zA-Z_][a-zA-Z0-9_]*\.)?(home_win|home_cover_spread|over|under|has_reliable_volume|has_reverse_movement)\s*=\s*1\b', r'\1\2 = true'),
            (r'\b([a-zA-Z_][a-zA-Z0-9_]*\.)?(home_win|home_cover_spread|over|under|has_reliable_volume|has_reverse_movement)\s*=\s*0\b', r'\1\2 = false'),
            
            # Handle generic boolean patterns - but exclude row numbers and ranks
            (r'\b(?!rn\b|latest_rank\b|rank\b)([a-zA-Z_][a-zA-Z0-9_]*)\s*=\s*1\b(?=\s*(?:AND|OR|THEN|$))', r'\1 = true'),
            (r'\b(?!rn\b|latest_rank\b|rank\b)([a-zA-Z_][a-zA-Z0-9_]*)\s*=\s*0\b(?=\s*(?:AND|OR|THEN|$))', r'\1 = false'),
            ...
            (r'\b(direction_conflicts|fade_successful|consensus_follow_correct|consensus_fade_correct|has_reliable_volume|has_reverse_movement)\s*=\s*true\b', r'\1 = 1'),
            (r'\b(direction_conflicts|fade_successful|consensus_follow_correct|consensus_fade_correct|has_reliable_volume|has_reverse_movement)\s*=\s*false\b', r'\1 = 0'),
        ]
    ...
    def _handle_missing_columns(self, sql: str) -> str:
        ...
        patterns = [
            ...
            (r'\bWHERE\s+COALESCE\s*\(\s*line_movement[^)]*\)\s*[><=]+\s*\d+\s+AND', 'WHERE'),
            
            # Handle specific line_movement patterns in CASE statements - replace with simpler conditions
            (r'ABS\s*\(\s*COALESCE\s*\(\s*line_movement,\s*0\s*\)\s*\)\s*<\s*[\d.]+', '1 = 1'),
            (r'ABS\s*\(\s*COALESCE\s*\(\s*line_movement,\s*0\s*\)\s*\)\s*>\s*[\d.]+', '1 = 0'),
    ...
    def _fix_strategy_performance_cte(self, sql: str) -> str:
        ...
        if 'strategy_performance AS' in sql:
            ...
            sql = re.sub(cte_pattern, fix_cte_columns, sql, flags=re.IGNORECASE | re.DOTALL)
