...
from .pre_game_recommendation_tracker import (
...
class EnhancedPreGameNotificationService:
    ...
    def _load_email_configuration(self) -> dict[str, Any]:
        ...
        return {
            ...
            "to_addresses": getattr(
                self.settings, "email_notification_recipients", ""
            ).split(",")
            if getattr(self.settings, "email_notification_recipients", "")
            else [],
            "enabled": bool(
                getattr(self.settings, "email_smtp_server", None)
                and getattr(self.settings, "email_notification_recipients", None)
            ),
        }
    ...
    async def schedule_daily_notifications(
        ...
        self.logger.info(
            "Scheduling daily pre-game notifications",
        ...
        try:
            ...
            self.logger.info(
                "Daily notification scheduling completed",
                games_scheduled=scheduled_count,
    ...
    async def _create_notification_schedule(
        self, game: dict[str, Any]
    ) -> GameNotificationSchedule | None:
        """Create a notification schedule for a single game."""

        try:
            game_pk = game["game_pk"]
            game_start_time = datetime.fromisoformat(game["game_datetime_utc"]).replace(
                tzinfo=timezone.utc
            )
            ...
            notification_time = game_start_time - timedelta(
                minutes=self.notification_window_minutes
            )

            # Add processing buffer
            processing_start_time = notification_time - timedelta(
                seconds=self.processing_buffer_seconds
            )

            # Skip if notification time has already passed
            if processing_start_time <= datetime.now(timezone.utc):
                self.logger.warning(
                    "Game notification time has passed, skipping",
                    game_pk=game_pk,
            ...
            self.logger.error(
                "Failed to create notification schedule", game=game, error=str(e)
            )
            return None
    ...
    async def _monitor_notification_schedule(self):
        ...
        while self.scheduled_notifications:
            ...
            for game_pk, schedule in self.scheduled_notifications.items():
                if (
                    not schedule.notification_sent
                    and not schedule.scheduled
                    and current_time >= schedule.notification_time
                ):
            ...
            for schedule in notifications_to_process:
                try:
                    ...
                    if schedule.retry_count < schedule.max_retries:
                        ...
                        schedule.notification_time = current_time + timedelta(
                            minutes=1
                        )  # Retry in 1 minute
                        self.logger.info(
    ...
    async def _process_game_notification(self, schedule: GameNotificationSchedule):
        ...
        try:
            ...
            if workflow_result and getattr(workflow_result, "recommendations", None):
                ...
                self.notification_metrics.recommendations_generated += len(
                ...
                self.logger.warning(
                    "No recommendations generated from workflow",
                    game_pk=schedule.game_pk,
                )
                schedule.recommendations = []
            ...
            if self.email_config["enabled"] and schedule.email_addresses:
                ...
                self.logger.warning(
                    "Email not configured or no recipients",
                    game_pk=schedule.game_pk,
            ...
            if schedule.recommendations:
                await self.recommendation_tracker.log_pre_game_recommendations(
                    schedule.recommendations
                )
                schedule.database_logged = True
                self.notification_metrics.database_records_created += len(
                    schedule.recommendations
                )

                self.logger.info(
                    "Recommendations logged to database",
                    game_pk=schedule.game_pk,
            ...
            self.notification_metrics.average_processing_time_seconds = (
                self.notification_metrics.average_processing_time_seconds
                * self.notification_metrics.notifications_sent
                + processing_time
            ) / (self.notification_metrics.notifications_sent + 1)
            ...
            timing_difference = abs(
                (actual_notification_time - intended_notification_time).total_seconds()
            )
            ...
            self.logger.info(
                "Game notification processing completed",
                game_pk=schedule.game_pk,
            ...
            self.logger.error(
                "Game notification processing failed",
                game_pk=schedule.game_pk,
                error=str(e),
            )
            schedule.last_error = str(e)
    ...
    async def _send_enhanced_email_notification(
        self, schedule: GameNotificationSchedule
    ):
        ...
        try:
            ...
            self.logger.error(
                "Failed to send enhanced email notification",
                game_pk=schedule.game_pk,
                error=str(e),
            )
            raise
    ...
    def _generate_enhanced_email_html(self, schedule: GameNotificationSchedule) -> str:
        ...
        game_time_est = schedule.game_start_time.astimezone(
...
Notification generated at {datetime.now(timezone.utc).isoformat()} UTC
    ...
    async def get_notification_status(self) -> dict[str, Any]:
        ...
        status_counts = {"scheduled": 0, "sent": 0, "failed": 0, "pending": 0}
                ...
                upcoming_notifications.append(
                    {
                        ...
                        "teams": f"{schedule.away_team} @ {schedule.home_team}",
                        "notification_time": schedule.notification_time.isoformat(),
                        "minutes_until_notification": int(
                            (schedule.notification_time - current_time).total_seconds()
                            / 60
                        ),
                    }
                )
        ...
        return {
            ...
            "upcoming_notifications": sorted(
                upcoming_notifications, key=lambda x: x["notification_time"]
            ),
    ...
    async def send_test_notification(
        ...
        test_recipients = (
            [test_email] if test_email else self.email_config["to_addresses"]
        )
