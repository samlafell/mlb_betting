...
class GameOutcomeService:
    ...
    async def update_daily_games(self, use_betting_lines: bool = True) -> GameUpdateSummary:
        ...
        try:
            ...
            if yesterday_outcomes:
                ...
                logger.info("Found completed games from yesterday", 
                          count=len(yesterday_outcomes), date=yesterday)
            ...
            if today_outcomes:
                ...
                logger.info("Found completed games from today", 
                          count=len(today_outcomes), date=today)
            ...
            logger.info("Daily game update completed", 
    ...
    async def update_specific_date(self, target_date: date, use_betting_lines: bool = True) -> GameUpdateSummary:
        """Update games for a specific date"""
        start_time = datetime.now()
        
        logger.info("Starting specific date update", date=target_date, use_betting_lines=use_betting_lines)
        ...
        try:
            ...
            logger.info("Specific date update completed", 
                       date=target_date,
    ...
    async def refresh_full_season(self, 
                                 season_year: Optional[int] = None,
                                 use_betting_lines: bool = False,
                                 batch_size: int = 7) -> SeasonRefreshResult:
        ...
        logger.info("Starting full season refresh", 
        ...
        try:
            ...
            while current_date <= season_end:
                batch_end = min(current_date + timedelta(days=batch_size - 1), season_end)
                ...
                if batch_outcomes:
                    batch_summary = GameUpdateSummary(
                        ...
                        betting_statistics=self._calculate_betting_statistics(batch_outcomes),
                        database_status=await self._get_database_status(),
            ...
            logger.info("Full season refresh completed", 
                       season_year=season_year,
                       total_games=len(all_outcomes),
                       dates_processed=dates_processed,
    ...
    def _calculate_betting_statistics(self, outcomes: List[GameOutcome]) -> Dict[str, Any]:
