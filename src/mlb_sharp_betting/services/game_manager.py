...
class GameManager:
        ...
        try:
            ...
            with self.db_manager.get_cursor() as cursor:
                ...
                if existing_game:
                    ...
                    if (
                        "game_date" not in mapped_fields
                        and "game_datetime" in mapped_fields
                    ):
                    ...
                    self.logger.debug(
                        f"Created new game {game_identifier} with ID {game_id}"
                    )
                    return game_id

        except Exception as e:
            self.logger.error(
                f"Failed to create/update game {game_data.get('game_id', 'unknown')}: {e}"
            )
    ...
    def process_games_from_betting_splits(
        ...
        try:
            ...
            for split in splits_data:
                try:
                    ...
                    self.logger.warning(
                        f"Failed to process split for game extraction: {e}"
                    )
                    errors += 1
                    continue
            ...
            for game_data in unique_games.values():
                try:
                    ...
                    self.logger.error(
                        f"Failed to process game {game_data.get('game_id', 'unknown')}: {e}"
                    )
                    errors += 1
                    continue
            ...
            stats = {
                ...
                "errors": errors,
                "success_rate": (games_processed / len(unique_games)) * 100
                if unique_games
                else 0,
            }
