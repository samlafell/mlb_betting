...
class StrategyValidation:
    ...
    def __init__(self, 
                 profitable_strategies: Optional[List[ProfitableStrategy]] = None,
    ...
    async def initialize(self):
        ...
        try:
            ...
            self.logger.error(f"❌ Failed to initialize Strategy Validation Service: {e}")
            raise
    ...
    async def check_strategy_gate(self, strategy_name: str, context: str = "unknown") -> ValidationGateResponse:
        ...
        try:
            ...
            if record.validation_status not in [ValidationStatus.VALIDATED, ValidationStatus.APPROVED]:
                return ValidationGateResponse(
                    result=ValidationGateResult.BLOCKED_UNVALIDATED,
                    strategy_name=strategy_name,
                    allowed=False,
    ...
    async def register_strategy(self, strategy_name: str, 
                              validation_status: ValidationStatus = ValidationStatus.UNVALIDATED) -> bool:
        ...
        try:
            ...
            self.logger.info(f"Strategy registered: {strategy_name} with status {validation_status.value}")
            return True
    ...
    async def trigger_circuit_breaker(self, strategy_name: str, reason: str):
        ...
        if record:
            ...
            self.logger.warning(f"⚡ Circuit breaker triggered for {strategy_name}: {reason}")
    ...
    async def _get_recent_performance(self, strategy_name: str) -> Optional[Dict[str, float]]:
    ...
    def get_registry_status(self) -> Dict[str, Any]:
        ...
        validated_strategies = sum(
            1 for r in self.validation_records.values() 
            if r.validation_status in [ValidationStatus.VALIDATED, ValidationStatus.APPROVED]
        )
        ...
        return {
            ...
            'emergency_suspension_active': self.emergency_controls['emergency_suspension_active'],
            'strategies_with_circuit_breakers': sum(1 for r in self.validation_records.values() if r.circuit_breaker_open),
...
_strategy_validation_instance: Optional[StrategyValidation] = None
