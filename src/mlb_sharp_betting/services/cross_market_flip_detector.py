...
from ..models.splits import SplitType, DataSource, BookType
...
class FlipType(Enum):
    ...
    CROSS_MARKET_CONTRADICTION = "cross_market_contradiction"  # Spread early home, ML late away  
    WEAK_LATE_CONTRADICTION = "weak_late_contradiction"  # Strong early signal, weak late opposite
...
class CrossMarketFlipDetector:
    ...
    def __init__(self, db_manager: DatabaseManager):
        ...
        self.early_cutoff_hours = 4.0   # Increased from 3.0 - require truly early signals  
        self.late_cutoff_hours = 2.0    # Keep at 2.0 for late signals
        self.min_confidence_threshold = 75.0  # Increased from 60.0 - require high confidence
        self.min_signal_strength_diff = 10.0  # New: minimum differential between early/late signals
    ...
    async def detect_flips_for_game(
        ...
        try:
            ...
            if len(signals) < 2:
                logger.debug("Insufficient signals for flip detection", 
                           game_id=game_id, signal_count=len(signals))
                return []
            
            # Group signals by timing
            early_signals = [s for s in signals if s.hours_before_game >= self.early_cutoff_hours]
            late_signals = [s for s in signals if s.hours_before_game <= self.late_cutoff_hours]
            
            if not early_signals or not late_signals:
                logger.debug("Missing early or late signals", 
                           game_id=game_id, 
            ...
            strong_early_signals = [s for s in early_signals if abs(s.differential) >= 15.0]
            for i, signal1 in enumerate(strong_early_signals):
                for signal2 in strong_early_signals[i+1:]:
                    try:
                        contradiction = self._analyze_same_timing_contradiction(signal1, signal2, game_id, "early")
                        if contradiction and contradiction.confidence_score >= self.min_confidence_threshold:
                            same_timing_contradictions.append(contradiction)
                    except Exception as e:
                        logger.debug("Failed to analyze same timing contradiction", error=str(e))
            
            # Check late signals against each other (only if strong signals)
            strong_late_signals = [s for s in late_signals if abs(s.differential) >= 15.0]
            for i, signal1 in enumerate(strong_late_signals):
                for signal2 in strong_late_signals[i+1:]:
                    try:
                        contradiction = self._analyze_same_timing_contradiction(signal1, signal2, game_id, "late")
                        if contradiction and contradiction.confidence_score >= self.min_confidence_threshold:
                            same_timing_contradictions.append(contradiction)
                    except Exception as e:
                        logger.debug("Failed to analyze same timing contradiction", error=str(e))
            ...
            logger.info("Detected cross-market flips",
                       game_id=game_id,
            ...
            logger.error("Failed to detect flips for game",
    ...
    async def detect_recent_flips(
        ...
        try:
            ...
            results = self.db_manager.execute_query(
                query, 
                (cutoff_time, self.min_sharp_threshold),
                fetch=True
            )
            ...
            for row in results or []:
                ...
                all_flips.extend([f for f in flips if f.confidence_score >= min_confidence])
            
            # Sort by confidence score
            all_flips.sort(key=lambda x: x.confidence_score, reverse=True)
            
            logger.info("Detected recent cross-market flips",
    ...
    async def detect_todays_flips_with_summary(
        ...
        try:
            ...
            tomorrow_end = (now_est + timedelta(days=1)).replace(hour=23, minute=59, second=59)
            ...
            results = self.db_manager.execute_query(
                query, 
                (today, tomorrow, self.min_sharp_threshold),
                fetch=True
            )
            ...
            for row in results or []:
                game_id = row[0]
                flips = await self.detect_flips_for_game(game_id)
                quality_flips = [f for f in flips if f.confidence_score >= min_confidence]
            ...
            summary = {
                ...
                "avg_confidence": sum(f.confidence_score for f in all_flips) / len(all_flips) if all_flips else 0,
                "flip_types": {},
                "recommended_bets": len([f for f in all_flips if f.strategy_recommendation != "NO BET"])
            }
            
            # Count flip types
            for flip in all_flips:
                flip_type = flip.flip_type.value
                summary["flip_types"][flip_type] = summary["flip_types"].get(flip_type, 0) + 1
            
            if games_evaluated == 0:
                logger.info("No games found for today's cross-market flip analysis")
            elif len(all_flips) == 0:
                logger.info(f"Evaluated {games_evaluated} games today - no cross-market flips detected above {min_confidence}% confidence threshold")
            else:
    ...
    def _create_market_signal(self, row: tuple) -> Optional[MarketSignal]:
        """Create a MarketSignal from database row."""
        try:
            (game_id, home_team, away_team, game_datetime,
             split_type_str, source_str, book_str,
             stake_pct, bet_pct, differential, last_updated, hours_before) = row
    ...
    def _is_total_flip_banned(self, early: MarketSignal, late: MarketSignal) -> bool:
        ...
        source_book_combo = f"{early.source.value}-{early.book.value if early.book else 'NULL'}"
        
        if source_book_combo in self.TOTAL_FLIP_BANNED_COMBINATIONS:
            logger.debug("Total flip banned due to poor backtest performance", 
    ...
    def _analyze_signal_pair(
        ...
        if (abs(early.differential) < self.min_sharp_threshold or 
            abs(late.differential) < self.min_sharp_threshold):
            return None
        ...
        strategy_rec, reasoning = self._generate_strategy_recommendation(early, late, flip_type)
    ...
    def _classify_flip_type(self, early: MarketSignal, late: MarketSignal) -> Optional[FlipType]:
        ...
        # Cross-market contradiction (different bet types, opposite teams)
            ...
            if (early.recommended_team != late.recommended_team and 
                early.split_type != SplitType.TOTAL and 
                late.split_type != SplitType.TOTAL):
                return FlipType.CROSS_MARKET_CONTRADICTION
        
        # Weak late contradiction (strong early, weak late opposite)
        if (early.strength in [SignalStrength.STRONG, SignalStrength.VERY_STRONG] and
            late.strength == SignalStrength.WEAK and
            early.recommended_team != late.recommended_team):
            return FlipType.WEAK_LATE_CONTRADICTION
        
        return None
    
    def _calculate_confidence_conservative(self, early: MarketSignal, late: MarketSignal) -> float:
        ...
        diff_factor = min(10, abs(early.differential - late.differential) * 0.3)  # Reduced
        
        # Calculate total confidence
        confidence = (base_confidence + early_strength_factor + late_strength_factor + 
                     timing_factor + source_factor + book_factor + diff_factor)
    ...
    def _generate_strategy_recommendation(
        ...
        if flip_type == FlipType.CROSS_MARKET_CONTRADICTION:
            ...
            reasoning = (f"Early {early.split_type.value} signal on {early.recommended_team} "
            ...
            strategy = f"BET {early.recommended_team}"
            reasoning = (f"Same market flip: {early.split_type.value} moved from "
            ...
            strategy = f"BET {early.recommended_team}"
            reasoning = (f"Strong early {early.split_type.value} signal on {early.recommended_team} "
    ...
    def _identify_risk_factors(self, early: MarketSignal, late: MarketSignal) -> List[str]:
        ...
        if late.strength in [SignalStrength.STRONG, SignalStrength.VERY_STRONG]:
            risk_factors.append(f"Strong late signal ({abs(late.differential):.1f}%) may indicate legitimate sharp move")
        ...
        hours_gap = abs(early.hours_before_game - late.hours_before_game)
        if hours_gap < 2:
            risk_factors.append("Short time gap between signals may indicate normal market movement")
    ...
    def _deduplicate_flips_aggressively(self, flips: List[FlipDetection]) -> List[FlipDetection]:
        ...
        for game_id, game_flips in flips_by_game.items():
            ...
            if (best_flip.confidence_score >= self.min_confidence_threshold and
                abs(best_flip.early_signal.differential) >= self.min_sharp_threshold and
                abs(best_flip.late_signal.differential) >= self.min_sharp_threshold):
                
                # Ensure there's meaningful difference between early and late signals
                signal_diff = abs(abs(best_flip.early_signal.differential) - abs(best_flip.late_signal.differential))
                if signal_diff >= self.min_signal_strength_diff:
                    deduplicated.append(best_flip)
                else:
                    logger.debug("Filtered flip due to insufficient signal difference",
                               game_id=game_id,
                               signal_diff=signal_diff,
                               min_required=self.min_signal_strength_diff)
            else:
                logger.debug("Filtered flip due to quality checks",
                           game_id=game_id,
    ...
    def _analyze_same_timing_contradiction(
        ...
        try:
            ...
            contradiction_type = self._classify_same_timing_contradiction(signal1, signal2)
            ...
            adjusted_confidence = base_confidence * 0.8  # Reduce confidence for same-timing
            ...
            return FlipDetection(
                ...
                early_signal=signal1 if signal1.hours_before_game >= signal2.hours_before_game else signal2,
                late_signal=signal2 if signal1.hours_before_game >= signal2.hours_before_game else signal1,
    ...
    def _classify_same_timing_contradiction(self, signal1: MarketSignal, signal2: MarketSignal) -> Optional[FlipType]:
        """
        ...
        if (signal1.split_type in ['spread', 'moneyline'] and 
            signal2.split_type in ['spread', 'moneyline'] and
            team1 != team2):
            return FlipType.CROSS_MARKET_CONTRADICTION
    ...
    def _generate_same_timing_recommendation(
        ...
        stronger_signal = signal1 if signal1.strength.value > signal2.strength.value else signal2
        weaker_signal = signal2 if stronger_signal == signal1 else signal1
        
        recommendation = stronger_signal.recommended_team
        reasoning = (f"Same-timing contradiction detected: {stronger_signal.split_type} "
