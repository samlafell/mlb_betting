...
class Alert:
    ...
    source_book_type: str | None = None
...
class AlertService:
    ...
    def _load_config(self, config_path: Path | None) -> dict[str, Any]:
        ...
        if config_path and config_path.exists():
            try:
                ...
                self.logger.warning(
                    "Failed to load config, using defaults", error=str(e)
                )
    ...
    async def process_backtesting_results(
        self, results: BacktestingResults
    ) -> list[Alert]:
        ...
        try:
            ...
            if (
                results.game_outcome_freshness_hours
                > self.config["data_freshness_alert_hours"]
            ):
                alert = self._create_alert(
                    alert_type=AlertType.DATA_QUALITY,
        ...
        try:
            ...
            if cooldown_key in self.alert_cooldowns:
                ...
                if time_since_last.total_seconds() < cooldown_minutes * 60:
                    self.logger.debug(
                        "Alert rate limited", alert_type=alert.alert_type.value
                    )
                    return
            ...
            self.logger.error(
                "Failed to process alert", alert_id=alert.id, error=str(e)
            )
...
"""
    ...
    def _cleanup_old_alerts(self) -> None:
        ...
        self.alert_cooldowns = {
            k: v for k, v in self.alert_cooldowns.items() if v > cutoff_date
        }
    ...
    async def get_active_alerts(
        self, severity_filter: list[AlertSeverity] | None = None
    ) -> list[Alert]:
    ...
    async def generate_weekly_summary(self) -> str:
        ...
        week_alerts = [
            a for a in self.alert_history if start_date <= a.created_at <= end_date
        ]

        # Categorize alerts
        critical_alerts = [
            a for a in week_alerts if a.severity == AlertSeverity.CRITICAL
        ]
        high_alerts = [a for a in week_alerts if a.severity == AlertSeverity.HIGH]
        performance_alerts = [
            a for a in week_alerts if a.alert_type == AlertType.PERFORMANCE_DECLINE
        ]
...
"""
        ...
        if performance_alerts:
            ...
            for strategy, count in sorted(
                strategy_issues.items(), key=lambda x: x[1], reverse=True
            ):
