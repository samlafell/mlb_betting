...
class CollectionManager:
        ...
        for source_name, source_type, sportsbook in sources:
            ...
            try:
                ...
                if splits:
                    ...
                    self.logger.info(
                        "Successfully collected from source",
                        source=source_name,
                        splits_count=len(splits),
                    )
                else:
                    self.logger.warning(
                        "No data collected from source", source=source_name
                    )
                ...
                self.logger.error(
                    "Source collection failed", source=source_name, error=str(e)
                )

        return all_splits

    async def _collect_from_source(
        self, source: str, sport: str, sportsbook: str | None
    ) -> list[BettingSplit]:
    ...
    async def _collect_vsin_data(
        self, sport: str, sportsbook: str
    ) -> list[BettingSplit]:
        """Collect and parse VSIN data for a specific sportsbook."""
        try:
            scrape_result = await self.vsin_scraper.scrape(
            ...
            self.logger.error(
                "VSIN data collection failed", sportsbook=sportsbook, error=str(e)
            )
            return []

    async def run_automatic_flip_detection(self) -> dict[str, Any] | None:
...
class PersistenceManager:
    ...
    def __init__(self, connection_manager: ConnectionManager, stats: DataServiceStats):
        ...
        self.betting_split_repo = get_betting_split_repository(
            connection_manager.db_manager
        )
        self.game_repo = get_game_repository(connection_manager.db_manager)
        self.sharp_action_repo = get_sharp_action_repository(
            connection_manager.db_manager
        )
    ...
    def store_betting_splits(
        ...
        self.logger.info(
            "Starting betting splits storage",
        ...
        for i in range(0, len(valid_splits), batch_size):
            ...
            batch_stats = self._process_batch(
                batch, validate=validate, skip_duplicates=skip_duplicates
            )
    ...
    def _process_batch(
        self, batch: list[BettingSplit], validate: bool, skip_duplicates: bool
    ) -> dict[str, int]:
...
class DeduplicationManager:
    ...
    def _ensure_deduplication_tables(self):
        ...
        try:
            ...
            betting_recommendations_table = table_registry.get_table(
            ...
            self.logger.info(
                "Deduplication tables ensured in new consolidated schema",
    ...
    def process_raw_data_for_deduplication(self, lookback_days: int = 30) -> dict:
        ...
        try:
            self.logger.info(
                "Starting deduplication process", lookback_days=lookback_days
            )
    ...
    def _process_game_market_for_deduplication(
        ...
        try:
            ...
            market_data = self.connection.execute_read(
                query, [game_id, market_type.value]
            )
...
class DataService:
    ...
    async def collect_and_store(
        ...
        try:
            ...
            self.logger.info(
                "Data pipeline completed successfully", stats=pipeline_stats
            )
    ...
    async def collect_from_source(
        self, source: str, sport: str = "mlb", **kwargs: Any
    ) -> list[BettingSplit]:
        """Collect data from a specific source."""
        return await self.collector._collect_from_source(
            source, sport, kwargs.get("sportsbook")
        )
...
def get_data_service(db_manager: DatabaseManager | None = None) -> DataService:
