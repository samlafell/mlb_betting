...
class LifecycleThresholds:
    ...
    auto_disable_roi_threshold: float = (
        -10.0
    )  # Hard threshold: disable ANY strategy with ROI < -10%
...
class StrategyManager:
    ...
    async def get_threshold_config(self, source: str) -> ThresholdConfig:
        ...
        # Ultimate fallback
        self.logger.warning(
            "No thresholds found, using conservative defaults", source=source
        )
    ...
    async def get_best_strategies_by_type(
        self, split_type: str = None
    ) -> list[StrategyConfig]:
        ...
        if split_type:
            active_strategies = [
                s for s in active_strategies if s.split_type == split_type
            ]

        return active_strategies[:3]

    async def is_strategy_enabled(
        self, strategy_name: str, min_win_rate: float = 0.50
    ) -> bool:
        ...
        should_disable = self._should_disable_strategy(
            config.win_rate, config.roi_per_100, config.total_bets
        )
    ...
    async def get_live_strategy_configuration(
    ...
    async def execute_live_strategy_detection(
        ...
        for signal_type, strategy_configs in strategies_by_signal_type.items():
            try:
                ...
                processors = self.processor_factory.get_processors_by_type(
                    signal_type.value
                )

                if not processors:
                    self.logger.warning(
                        f"No processor found for signal type: {signal_type}"
                    )
                    continue
                ...
                for strategy_config in strategy_configs:
                    try:
                        adjusted_signals = self._apply_strategy_adjustments(
                            base_signals, strategy_config
                        )
                        all_signals.extend(adjusted_signals)
                    except Exception as e:
                        self.logger.error(
                            f"Failed to apply strategy adjustments for {strategy_config.strategy_name}: {e}"
                        )

            except Exception as e:
                self.logger.error(
                    f"Failed to execute processor for signal type {signal_type}: {e}"
                )

        # Apply ensemble logic and conflict resolution
        if len(all_signals) > 1:
            all_signals = self._apply_ensemble_logic(
                all_signals, live_config.enabled_strategies
            )
            all_signals = self._resolve_signal_conflicts(
                all_signals, live_config.enabled_strategies
            )
    ...
    async def identify_high_roi_strategies(
        self, lookback_days: int = 30
    ) -> list[HighROIStrategy]:
        ...
        try:
            ...
            with self.db_manager.get_cursor() as cursor:
                cursor.execute(
                    query, (start_date, self.min_bet_count, self.min_roi_threshold)
                )
            ...
            for row in results:
                ...
                min_threshold, high_threshold = await self._calculate_thresholds(
                    ...
                    row["roi_per_100_unit"],
                    row["win_rate"],
                    row.get("source_book_type", "default"),
                    row.get("split_type", "default"),
                )
                ...
                strategy = HighROIStrategy(
                    ...
                    roi_per_100_unit=row[
                        "roi_per_100_unit"
                    ],  # Already cast to FLOAT in SQL
            ...
            self.logger.info(
                "High-ROI strategies identified",
                total_strategies=len(high_roi_strategies),
                contrarian_count=self.metrics["contrarian_strategies_found"],
                opposing_markets_count=self.metrics[
                    "opposing_markets_strategies_found"
                ],
            )
    ...
    async def auto_integrate_high_roi_strategies(
        self, lookback_days: int = 30
    ) -> list[dict[str, Any]]:
        ...
        for strategy in high_roi_strategies:
            try:
                ...
                config_updated = (
                    await self._update_strategy_configuration_for_integration(strategy)
                )
                ...
                self.logger.error(
                    f"Failed to integrate strategy {strategy.strategy_id}: {e}"
                )
                integration_results.append(
                    {
                        "strategy": strategy,
    ...
    async def _load_strategy_configs(self) -> dict[str, StrategyConfig]:
        ...
        try:
            # 🚀 PHASE 2B: Get table name from registry
            strategy_configurations_table = self.table_registry.get_table(
            ...
            with self.db_manager.get_cursor() as cursor:
                ...
                if not schema_exists:
                    self.logger.info(
                        "Backtesting schema doesn't exist yet - using fallback strategy configs"
                    )
                ...
                if not table_exists:
                    self.logger.info(
                        "backtesting.strategy_configurations table doesn't exist yet - using fallback strategy configs"
                    )
                    return self._get_fallback_strategy_configs()
                ...
                if not column_exists:
                    self.logger.info(
                        "source_book_type column doesn't exist yet - using fallback strategy configs"
                    )
                    return self._get_fallback_strategy_configs()
                ...
                for row in results:
                    ...
                    roi_per_100_float = (
                        row["roi_per_100"] if row["roi_per_100"] is not None else 1.0
                    )
                    win_rate_float = (
                        row["win_rate"] if row["win_rate"] is not None else 0.5
                    )
                ...
                self.logger.info(
                    f"Loaded {len(configs)} strategy configurations from database"
                )
    ...
    def _get_fallback_strategy_configs(self) -> dict[str, StrategyConfig]:
        ...
        self.logger.info(
            f"Using {len(fallback_configs)} fallback strategy configurations"
        )
    ...
    async def _load_threshold_configs(self) -> dict[str, ThresholdConfig]:
        ...
        try:
            # 🚀 PHASE 2B: Get table name from registry
            threshold_configurations_table = self.table_registry.get_table(
            ...
            with self.db_manager.get_cursor() as cursor:
                ...
                if not schema_exists:
                    self.logger.info(
                        "Backtesting schema doesn't exist yet - using fallback threshold configs"
                    )
                ...
                if not table_exists:
                    self.logger.info(
                        "backtesting.threshold_configurations table doesn't exist yet - using fallback threshold configs"
                    )
                    return self._get_fallback_threshold_configs()
                ...
                if not column_exists:
                    self.logger.info(
                        "source column doesn't exist yet - using fallback threshold configs"
                    )
                    return self._get_fallback_threshold_configs()
                ...
                self.logger.info(
                    f"Loaded {len(configs)} threshold configurations from database"
                )
    ...
    def _get_fallback_threshold_configs(self) -> dict[str, ThresholdConfig]:
        ...
        self.logger.info(
            f"Using {len(fallback_configs)} fallback threshold configurations"
        )
        return fallback_configs
    ...
    async def _check_performance_degradation(self) -> bool:
        ...
        try:
            ...
            for strategy_name, current_roi in current_performance.items():
                config = await self.get_strategy_performance(strategy_name)
                if config and current_roi < (
                    config.roi_per_100
                    + self.update_policy.performance_degradation_threshold
                ):
                    return True
    ...
    async def _create_strategy_configurations(
        ...
        for result in backtest_results:
            ...
            confidence_multiplier = min(
                1.2, 0.8 + (float(result["roi_per_100"]) / 100.0)
            )
            ...
            config = StrategyConfiguration(
                ...
                max_recommendations_per_day=min(
                    10, max(1, int(result["roi_per_100"] / 5))
                ),
                last_performance_update=result["created_at"],
                strategy_status=StrategyStatus.ACTIVE
                if is_enabled
                else StrategyStatus.PAUSED,
            )
    ...
    def _map_strategy_to_signal_type(self, strategy_name: str) -> SignalType | None:
    ...
    def _apply_strategy_adjustments(
        self, signals: list[BettingSignal], strategy_config: StrategyConfiguration
    ) -> list[BettingSignal]:
        ...
        for signal in signals:
            ...
            adjusted_confidence = (
                signal.confidence * strategy_config.confidence_multiplier
            )
            adjusted_confidence = min(
                1.0, max(0.0, adjusted_confidence)
            )  # Clamp to [0, 1]
        ...
        for (game_id, bet_type), signals in signal_groups.items():
            if len(signals) == 1:
                ...
                ensemble_signal = BettingSignal(
                    ...
                    metadata={
                        "ensemble_strategies": [
                            s.metadata.get("strategy_name", "unknown") for s in signals
                        ],
    ...
    def _generate_performance_summary(
        self, configurations: list[StrategyConfiguration]
    ) -> dict[str, Any]:
        ...
        return {
            ...
            "avg_win_rate": sum(c.recent_win_rate for c in enabled_configs)
            / len(enabled_configs)
            if enabled_configs
            else 0,
            "avg_roi": sum(c.recent_roi for c in enabled_configs) / len(enabled_configs)
            if enabled_configs
            else 0,
            "total_sample_size": sum(c.sample_size for c in configurations),
            "high_performers": len(
                [c for c in enabled_configs if c.recent_roi >= 15.0]
            ),
            "moderate_performers": len(
                [c for c in enabled_configs if 10.0 <= c.recent_roi < 15.0]
            ),
    ...
    async def _calculate_thresholds(
        self,
        strategy_variant: str,
        roi: float,
        win_rate: float,
        ...
        try:
            ...
            threshold_config = await threshold_manager.get_dynamic_threshold(
                strategy_type=strategy_variant, source=source, split_type=split_type
            )
            ...
            self.logger.debug(
                f"🎯 Using dynamic thresholds for {strategy_variant}: "
            ...
            self.logger.warning(
                f"Failed to get dynamic thresholds for {strategy_variant}, using fallback: {e}"
            )
        ...
        base_threshold = max(
            5.0, 50.0 / max(float(roi), 1.0)
        )  # More aggressive (was 10.0, 100.0)

        # Adjust based on win rate - ensure float conversion
        win_rate_adjustment = (
            60.0 - float(win_rate)
        ) / 15.0  # Less penalty (was /10.0)
        ...
        high_threshold = max(
            6.0, min(20.0, high_threshold)
        )  # 6-20% range (was 10.0 min)

        self.logger.debug(
            f"📊 Static threshold fallback for {strategy_variant}: "
            f"min={min_threshold:.1f}%, high={high_threshold:.1f}%"
        )

        return min_threshold, high_threshold

    async def _update_strategy_configuration_for_integration(
        self, strategy: HighROIStrategy
    ) -> bool:
        """Update strategy configuration for integration"""
        try:
            # Implementation would update database with new strategy configuration
            self.logger.info(
                f"Updated configuration for strategy {strategy.strategy_id}"
            )
            return True
        except Exception as e:
            self.logger.error(
                f"Failed to update configuration for {strategy.strategy_id}: {e}"
            )
            return False
    ...
    async def _register_active_strategy(self, strategy: HighROIStrategy):
        ...
        try:
            ...
            self.logger.error(
                f"Failed to register strategy {strategy.strategy_id}: {e}"
            )
            raise
    ...
    async def _log_strategy_integration(self, strategy: HighROIStrategy):
        ...
        try:
            ...
            self.logger.error(
                f"Failed to log integration for {strategy.strategy_id}: {e}"
            )

    def _should_disable_strategy(
        self, win_rate: float, roi: float, total_bets: int
    ) -> bool:
        """
