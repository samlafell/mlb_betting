...
class BettingFeatureFlags:
    ...
    def __init__(self, data_dir: Path | None = None):
        ...
        self.logger.info(
            "BettingFeatureFlags initialized with safe rollback capability"
        )
            ...
            if flag_file.exists():
                ...
                for flag_name, flag_data in data.get("flags", {}).items():
                    if flag_name in self.flags:
                        ...
                        flag.current_value = flag_data.get(
                            "current_value", flag.default_value
                        )
                ...
                self.logger.info(
                    f"Loaded feature flag state for {len(data.get('flags', {}))} flags"
                )
    ...
    def set_flag(self, flag_name: str, value: Any, reason: str = "") -> bool:
        ...
        with self._lock:
            ...
            if not self._validate_flag_value(flag, value):
                self.logger.error(
                    f"Invalid value type for flag {flag_name}: {type(value)}"
                )
                return False
            ...
            self.logger.info(
                f"Feature flag updated: {flag_name} = {value} (was {old_value}). Reason: {reason}"
            )
    ...
    def rollback_flag(self, flag_name: str, reason: str = "") -> bool:
        ...
        with self._lock:
            ...
            self.logger.warning(
                f"Feature flag rolled back: {flag_name} = {flag.default_value} (was {old_value}). Reason: {reason}"
            )
            return True
    ...
    def _check_flag_dependencies(self, flag_name: str, value: Any) -> bool:
        ...
        for dependency in flag.dependencies:
            if not self.is_enabled(dependency):
                self.logger.warning(
                    f"Dependency not met for {flag_name}: {dependency} is disabled"
                )
                return False
    ...
    async def check_automatic_rollback(self) -> None:
        ...
        try:
            accuracy_status = (
                self.backtesting_engine.accuracy_monitor.get_current_status()
            )
    ...
    async def _check_performance_based_rollback(
        self, accuracy_status: dict[str, Any]
    ) -> None:
        ...
        if win_rate_baseline > 0:
            win_rate_degradation = (
                win_rate_baseline - current_win_rate
            ) / win_rate_baseline
    ...
    async def _trigger_automatic_rollback(self, trigger_type: str, reason: str) -> None:
        ...
        self.logger.critical(
            f"Triggering automatic rollback: {trigger_type} - {reason}"
        )
    ...
    def get_flags_by_category(
        self, category: FlagCategory
    ...
    def create_flag_group_transaction(
        self, flag_changes: dict[str, Any], reason: str = ""
    ) -> bool:
        """
        ...
        with self._lock:
            ...
            for flag_name, new_value in flag_changes.items():
                ...
                if not self._validate_flag_value(flag, new_value):
                    self.logger.error(
                        f"Invalid value in transaction for {flag_name}: {new_value}"
                    )
                    return False
            ...
            try:
                ...
                self.logger.info(
                    f"Flag group transaction completed: {len(flag_changes)} flags updated. Reason: {reason}"
                )
                return True
...
class TemporaryFlagOverride:
    ...
    def __enter__(self):
        ...
        for flag_name, override_value in self.flag_overrides.items():
            self.original_values[flag_name] = self.feature_flags.get_flag_value(
                flag_name
            )
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        for flag_name, original_value in self.original_values.items():
            self.feature_flags.set_flag(
                flag_name, original_value, "Restore from temporary override"
            )
