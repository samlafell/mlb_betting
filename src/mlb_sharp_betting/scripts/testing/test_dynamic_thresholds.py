...
async def test_dynamic_thresholds():
        ...
        for source in sources[:2]:  # Test first 2 sources
            for split_type in split_types[:2]:  # Test first 2 split types
                try:
                    ...
                    print(
                        f"  {strategy_type:15} | {source:5} | {split_type:10} | "
    ...
    for strategy_type in strategy_types:
        ...
        if bootstrap:
            print(
                f"   • {strategy_type:15}: min={bootstrap['min']:4.1f}%, "
    ...
    print(
        "   ✅ STRATEGY SPECIFIC: Different strategies have different optimal thresholds"
    )
