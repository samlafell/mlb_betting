            ...
            for use_tomorrow in [False, True]:
                ...
                try:
                    ...
                    if extracted_date:
                        ...
                        print(f"   ⚠️  Could not extract date from HTML")
                    ...
                    if found_teams:
                        ...
                        print(f"   ❌ No team variations found")
                    ...
                    if mlb_team_links:
                        ...
                        yankee_links = [link for link in mlb_team_links 
                                      if any(term in link.get_text().lower() 
                                           for term in ['yankee', 'nyy', 'new york'])]
                        toronto_links = [link for link in mlb_team_links 
                                       if any(term in link.get_text().lower() 
                                            for term in ['toronto', 'blue jay', 'tor', 'jays'])]
                        
                        if yankee_links:
                            print(f"   🏟️  Yankees-related links: {[link.get_text() for link in yankee_links]}")
                        if toronto_links:
                            print(f"   🏟️  Toronto-related links: {[link.get_text() for link in toronto_links]}")
                    else:
                        print(f"   ❌ No MLB team links found")
                    
                    # Try to find the main content table
                    main_content = scraper._extract_main_content(soup)
                    if main_content:
                        print(f"   ✅ Found main betting table")
                        
                        # Parse the betting splits
                        splits = scraper._parse_betting_splits(main_content, 'mlb', sportsbook)
                        ...
                        for split in splits:
                            ...
                            is_yankee_game = any(term in away_team or term in home_team 
                                               for term in ['yankee', 'nyy', 'new york'])
                            is_toronto_game = any(term in away_team or term in home_team 
                                                for term in ['toronto', 'blue jay', 'tor', 'jays'])
                        ...
                        if yankee_toronto_games:
                            print(f"   🎯 Found {len(yankee_toronto_games)} Yankees vs Toronto games:")
                            for game in yankee_toronto_games:
                                ...
                                print(f"      Away Line: {game.get('Away Line', 'N/A')}")
                                print(f"      Home Line: {game.get('Home Line', 'N/A')}")
                        else:
                            print(f"   ❌ No Yankees vs Toronto games found in parsed splits")
                            
                            # Show all parsed games for reference
                            if splits:
                                print(f"   📋 All parsed games:")
                                for i, split in enumerate(splits[:5]):  # Show first 5
                                    game = split.get('Game', 'Unknown')
                                    print(f"      [{i+1}] {game}")
                    else:
                        print(f"   ❌ Could not find main betting table")
                    
                except Exception as e:
                    print(f"   ❌ Error scraping {view_type.lower()} view: {e}")
                    logger.error("Scraping error", sportsbook=sportsbook, use_tomorrow=use_tomorrow, error=str(e))
    ...
    try:
        ...
        with db_manager.get_cursor() as cursor:
            ...
            if results:
                ...
                print(f"{'Game ID':<10} {'Away Team':<20} {'Home Team':<20} {'Source':<8} {'Book':<10} {'Last Updated'}")
                print("-" * 90)
                ...
                for row in results:
                    ...
                    updated = str(row['last_updated'])[:16] if row['last_updated'] else 'None'
                    
                    print(f"{game_id:<10} {away:<20} {home:<20} {source:<8} {book:<10} {updated}")
                    
                    # Check if this is actually the Yankees vs Toronto game
                    if row['has_yankees'] == 'Yankees' and row['has_toronto'] == 'Toronto':
                        print(f"   🎯 This appears to be the Yankees vs Toronto game!")
            else:
                print(f"❌ No matches found for game ID 777203 or Yankees vs Toronto combinations")
...
async def test_data_collection_pipeline():
    ...
    try:
        ...
        print(f"🚀 Running data collection pipeline...")
        ...
        if storage_stats.get('timing_rejections', 0) > 0:
            ...
            print(f"   ⏰ Timing Rejections: {timing_rejections} ({rejection_rate:.1f}%)")
            
            if rejection_rate > 20:
                print(f"   ⚠️  HIGH REJECTION RATE - Many games may have already started")
