...
def demo_database_layer():
    ...
    try:
        ...
        with db_manager.get_cursor() as cursor:
            ...
            # Query test data
            cursor.execute("SELECT * FROM test_table WHERE name = ?", ("Phase 2 Test",))
    ...
    try:
        ...
        if parsing_result.has_data:
            for i, betting_split in enumerate(parsing_result.parsed_data):
                ...
                logger.info(
                    f"Validation result for split {i + 1}",
                ...
                if validation_result["errors"]:
                    logger.warning(
                        "Validation errors", errors=validation_result["errors"]
                    )

                if validation_result["warnings"]:
                    logger.info(
                        "Validation warnings", warnings=validation_result["warnings"]
                    )
    ...
    try:
        ...
        for i, split in enumerate(sample_splits):
            ...
            if validation["is_valid"]:
                ...
                logger.warning(
                    f"Split {i + 1} failed validation", errors=validation["errors"]
                )
