...
class RecommendationResult:
    ...
    # Outcome data
    actual_result: Optional[str]  # 'WIN', 'LOSS', 'PUSH', 'VOID'
    game_completed: bool
...
@click.option("--bet-type",
              type=click.Choice(["moneyline", "spread", "total"]),
              help="Filter by bet type")
@click.option("--debug", is_flag=True,
              help="Enable debug logging")
def generate_performance_report(date: Optional[str], format: str, output: Optional[Path],
                              include_unmatched: bool, min_confidence: Optional[str],
                              bet_type: Optional[str], debug: bool):
...
async def _generate_performance_report_async(
    ...
    if not recommendations:
        logger.warning("No betting recommendations found for date", date=target_date.isoformat())
        return _create_empty_metrics(target_date)
    
    # Filter recommendations if requested
    if min_confidence:
        recommendations = [r for r in recommendations if _confidence_meets_minimum(r.confidence_level, min_confidence)]
    
    if bet_type:
        recommendations = [r for r in recommendations if r.bet_type == bet_type]
    
    # Match recommendations with game outcomes
    matched_results = await _match_recommendations_with_outcomes(recommendations, target_date)
...
async def _get_recommendations_for_date(tracker: PreGameRecommendationTracker, target_date: date) -> List[RecommendationResult]:
        ...
        with tracker.db_manager.get_cursor() as cursor:
            ...
            for row in rows:
                ...
                if isinstance(recommended_at, str):
                    recommended_at = datetime.fromisoformat(recommended_at.replace('Z', '+00:00'))
...
def _apply_outcome_to_recommendation(rec: RecommendationResult, outcome: Dict[str, Any]) -> RecommendationResult:
...
def _determine_bet_result(rec: RecommendationResult, outcome: Dict[str, Any]) -> str:
    ...
    try:
        ...
        logger.warning("Failed to determine bet result", 
...
def _find_fuzzy_match(rec: RecommendationResult, outcome_map: Dict[str, Dict[str, Any]]) -> Optional[Dict[str, Any]]:
...
def _calculate_performance_metrics(results: List[RecommendationResult], target_date: date) -> BettingPerformanceMetrics:
    """Calculate comprehensive performance metrics."""
    
    # Filter to only matched results with outcomes
    matched_results = [r for r in results if r.match_quality in ['EXACT', 'FUZZY'] and r.actual_result]
    ...
    win_percentage = (total_wins / len(matched_results) * 100) if matched_results else 0.0
    ...
    spread_win_pct = (spread_wins / len(spread_results) * 100) if spread_results else 0.0
    ...
    # Timing analysis
    minutes_before = [r.minutes_before_game for r in results if r.minutes_before_game is not None]
...
def _save_csv_report(metrics: BettingPerformanceMetrics, output_path: Path) -> None:
    ...
    with open(output_path, 'w', newline='') as csvfile:
        ...
        # Write header
        writer.writerow([
            'Date', 'Total_Recommendations', 'Matched_Recommendations', 'Unmatched_Recommendations',
            'Total_Wins', 'Total_Losses', 'Total_Pushes', 'Win_Percentage',
            'Moneyline_Record', 'Moneyline_Win_Pct', 'Spread_Record', 'Spread_Win_Pct',
            'Total_Record', 'Total_Win_Pct', 'Total_Risk_Amount', 'Total_Profit_Loss',
            'ROI_Percentage', 'High_Conf_Record', 'Moderate_Conf_Record', 'Low_Conf_Record',
            'Avg_Minutes_Before_Game', 'Earliest_Recommendation', 'Latest_Recommendation'
        ])
