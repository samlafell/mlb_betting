...
def run_full_diagnostic(
    ...
    async def run_diagnostic():
        try:
            ...
            click.echo(
                "This will analyze the disconnect between backtesting and live detection.\n"
            )
            ...
            click.echo(
                f"⏱️  Duration: {summary['diagnostic_duration_seconds']:.2f} seconds"
            )
            click.echo()
            ...
            if strategy_diag:
                ...
                working_strategies = [
                    name
                    for name, diag in strategy_diag.items()
            ...
            if recommendations:
                ...
                for i, rec in enumerate(recommendations[:10], 1):
                    ...
                    click.echo(
                        f"{i}. {priority_emoji} [{rec['priority']}] {rec['action']}"
                    )
            ...
            if summary["failed_checkpoints"] > 0:
                ...
                click.echo(
                    "⚠️  WARNING: Address warning checkpoints for better performance"
                )
            ...
            if len(working_strategies) < len(strategy_diag) // 2:
                click.echo(
                    "🔧 STRATEGY ISSUE: Many strategies are not generating signals"
                )
...
@click.option(
    ...
    is_flag=True,
    help="Test signal generation with permissive settings",
)
    ...
    async def debug_single_strategy():
        try:
            ...
            if not strategy_diag:
                ...
                click.echo(
                    f"Available strategies: {', '.join(available_strategies[:10])}"
                )
                return False
            ...
            click.echo(
                f"📈 Data Quality Score: {strategy_diag['data_quality_score']:.2f}"
            )
            click.echo()
            ...
            if strategy_diag["processor_status"] == "FAIL":
                ...
                click.echo(
                    f"   Strategy is working and found {strategy_diag['raw_signals']} signals."
                )
...
def quick_health_check():
    ...
    async def quick_check():
        try:
            ...
            # Quick summary
            data_result = next(
                (r for r in results if r.checkpoint_name == "Data Availability"), None
            )
            ...
            working_count = len(
                [
                    d
                    for d in strategy_diagnostics.values()
            ...
            if data_result and data_result.status.value == "FAIL":
                ...
                click.echo(
                    "   source .env && uv run src/mlb_sharp_betting/cli.py data collect --force"
                )
            elif working_count == 0:
                click.echo("\n🚀 Quick Fix: Check processor configuration")
                click.echo("   Run full diagnostic for detailed analysis:")
                click.echo(
                    "   uv run src/mlb_sharp_betting/cli.py diagnostics run-full-diagnostic --verbose"
                )
...
@click.option(
    "--split-type",
    default="moneyline",
    help="Split type to test (moneyline, spread, total)",
)
...
def debug_thresholds(
    strategy: str, source: str, split_type: str, show_dynamic: bool, test_signals: bool
):
    """Debug threshold system to see what values are actually being used."""

    async def run_threshold_debug():
        from mlb_sharp_betting.analysis.processors.sharpaction_processor import (
            SharpActionProcessor,
        )
        from mlb_sharp_betting.db.connection import get_db_manager
        from mlb_sharp_betting.models.betting_analysis import SignalProcessorConfig
        from mlb_sharp_betting.services.betting_signal_repository import (
        ...
        try:
            ...
            if show_dynamic:
                ...
                threshold_config = await threshold_manager.get_dynamic_threshold(
                    strategy_type=strategy, source=source, split_type=split_type
                )

                click.echo(f"Phase: {threshold_config.phase.value}")
                click.echo(f"Sample Size: {threshold_config.sample_size}")
                click.echo(
                    f"Minimum Threshold: {threshold_config.minimum_threshold:.1f}%"
                )
                click.echo(
                    f"Moderate Threshold: {threshold_config.moderate_threshold:.1f}%"
                )
                ...
                if bootstrap:
                    ...
                    click.echo(
                        f"Min: {bootstrap.get('min', 'N/A')}%, Mod: {bootstrap.get('mod', 'N/A')}%, High: {bootstrap.get('high', 'N/A')}%"
                    )
            ...
            if test_signals:
                ...
                for diff in test_differentials:
                    ...
                    click.echo(
                        f"Signal {diff:4.1f}% vs Threshold {actual_threshold:4.1f}% = {status}"
                    )

            click.echo("\n🎯 DIAGNOSIS COMPLETE")
            click.echo(
                "If you see high threshold values (>8%), the dynamic system may not be working properly."
            )
            click.echo(
                "If you see low threshold values (3-8%) but signals still fail, check the integration."
            )
