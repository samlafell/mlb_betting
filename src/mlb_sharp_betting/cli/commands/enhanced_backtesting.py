...
@click.option("--detailed", is_flag=True, help="Show detailed backtesting results")
@click.option(
    "--include-diagnostics", is_flag=True, help="Include comprehensive diagnostics"
)
@click.option(
    "--include-alignment", is_flag=True, help="Validate live recommendation alignment"
)
def run_backtesting(
    lookback_days: int,
    ensure_fresh_data: bool,
    store_results: bool,
    max_data_age: int,
    detailed: bool,
    ...
    async def run_enhanced_backtesting():
        ...
        click.echo(
            f"🔍 Diagnostics: {'ENABLED' if include_diagnostics else 'DISABLED'}"
        )
        click.echo(
            f"🎯 Alignment Check: {'ENABLED' if include_alignment else 'DISABLED'}"
        )
        ...
        try:
            ...
            if ensure_fresh_data:
                ...
                if "data_collection_metrics" in results:
                    ...
                    click.echo(
                        f"   💾 Records Stored: {metrics.get('stored_records', 0)}"
                    )
                    click.echo(
            ...
            if "execution_time_seconds" in results:
                click.echo(
                    f"   ⏱️  Total Time: {results['execution_time_seconds']:.2f}s"
                )
            ...
            if "backtest_results" in results:
                ...
                if isinstance(backtest, dict):
                    ...
                    click.echo(
                        f"   💰 Profitable: {backtest.get('profitable_strategies', 0)}"
                    )
                    click.echo(f"   📊 Total Bets: {backtest.get('total_bets', 0)}")
                    click.echo(
                        f"   📈 Average ROI: {backtest.get('average_roi', 0):.1f}%"
                    )
            ...
            if include_alignment and "alignment_results" in results:
                ...
                if isinstance(alignment, dict):
                    ...
                    if alignment_score >= 80:
                        click.echo(
                            "   ✅ EXCELLENT alignment with live recommendations"
                        )
                    elif alignment_score >= 60:
                        click.echo("   👍 GOOD alignment with live recommendations")
                    else:
                        click.echo("   ⚠️  POOR alignment - review may be needed")
    ...
    async def run_quick_backtest():
        ...
        try:
            ...
            if "backtest_results" in results:
                ...
                if isinstance(backtest, dict):
                    click.echo(
                        f"   📊 Total Strategies: {backtest.get('total_strategies', 0)}"
                    )
                    click.echo(
                        f"   💰 Profitable: {backtest.get('profitable_strategies', 0)}"
                    )
...
def run_diagnostics():
    ...
    async def run_diagnostics_check():
        ...
        try:
            ...
            diagnostics_results = (
                await backtesting_engine.diagnostics.run_full_diagnostic()
            )
