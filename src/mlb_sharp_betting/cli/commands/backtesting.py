...
class AutomatedBacktestingCLI:
    ...
    def __init__(self, config_path: Path | None = None):
        ...
        self.config_path = (
            config_path or self.project_root / "config" / "backtesting_config.json"
        )
    ...
    async def run_scheduler_mode(
        self, debug: bool = False, no_alerts: bool = False
    ) -> None:
        ...
        try:
            ...
            self.backtesting_engine.start_automated_scheduling(
            ...
            # Keep running
            self.logger.info(
                "BacktestingEngine scheduler is running. Press Ctrl+C to stop."
            )
            while True:
    ...
    async def run_single_analysis(
        self, force: bool = False, detailed: bool = False
    ) -> None:
                ...
                try:
                    ...
                    backtest_results_obj = BacktestingResults(
                        backtest_date=datetime.strptime(
                            backtest_dict["end_date"], "%Y-%m-%d"
                        ),
                        total_strategies_analyzed=backtest_dict.get(
                            "total_strategies", 0
                        ),
                        strategies_with_adequate_data=backtest_dict.get(
                            "total_strategies", 0
                        ),
                        profitable_strategies=backtest_dict.get(
                            "profitable_strategies", 0
                        ),
                        declining_strategies=backtest_dict.get("total_strategies", 0)
                        - backtest_dict.get("profitable_strategies", 0),
                        stable_strategies=0,
                        ...
                        execution_time_seconds=results.get(
                            "execution_time_seconds", 0.0
                        ),
                        created_at=datetime.now(timezone.utc),
                    )

                    alerts = await self.alert_service.process_backtesting_results(
                        backtest_results_obj
                    )
                except Exception as e:
    ...
    async def show_status(self) -> None:
        ...
        try:
            ...
            # Run diagnostics
            diagnostics_results = (
                await self.backtesting_engine.diagnostics.run_full_diagnostic()
            )
    ...
    def _print_analysis_results(
        self, results: dict, alerts: list, execution_time: float, detailed: bool = False
    ) -> None:
        ...
        if "backtest_results" in results:
            ...
            if isinstance(backtest, dict):
                ...
                print(
                    f"💰 Profitable Strategies: {backtest.get('profitable_strategies', 0)}"
                )
        ...
        if "diagnostics_results" in results:
            ...
            if isinstance(diagnostics, dict):
                for checkpoint, result in diagnostics.items():
                    ...
                    print(
                        f"   {status_emoji} {checkpoint.replace('_', ' ').title()}: {status}"
                    )
    ...
    def _print_detailed_strategy_results(self, strategy_results):
        ...
        if isinstance(strategy_results, list):
            ...
            sorted_results = sorted(
                strategy_results, key=lambda x: x.get("roi_per_100", 0), reverse=True
            )
        else:
    ...
    def _print_system_status(
        self, status_info: dict, diagnostics_results: dict, active_alerts: list
    ) -> None:
        ...
        if status_info:
            ...
            print(
                f"   📦 Modules Loaded: {', '.join(modules_loaded) if modules_loaded else 'None'}"
            )
            print(
                f"   ⚡ Initialized: {'✅ Yes' if status_info.get('initialized', False) else '❌ No'}"
            )
    ...
    def _print_test_results(
        self, diagnostics_results: dict, backtest_results: dict
    ) -> None:
        ...
        if (
            ...
            and "backtest_results" in backtest_results
        ):
    ...
    def _print_daily_report(self, results: dict) -> None:
        ...
        if "backtest_results" in results:
            ...
            if isinstance(backtest, dict):
                print("\n📊 SUMMARY:")
                print(
                    f"   🎯 Strategies Analyzed: {backtest.get('total_strategies', 0)}"
                )
                print(
                    f"   💰 Profitable Strategies: {backtest.get('profitable_strategies', 0)}"
                )
...
@click.option("--force", is_flag=True, help="Bypass circuit breaker (use with caution)")
def run_backtesting(
    mode: str, config: Path | None, debug: bool, no_alerts: bool, force: bool
):
...
def show_backtesting_status(config: Path | None):
...
def test_backtesting_system(config: Path | None):
...
Examples:
  ...
  %(prog)s --mode test               # Test system components
    ...
    parser.add_argument(
        "--no-alerts", action="store_true", help="Disable alert notifications"
    )

    parser.add_argument(
        "--force", action="store_true", help="Bypass circuit breaker (use with caution)"
    )
