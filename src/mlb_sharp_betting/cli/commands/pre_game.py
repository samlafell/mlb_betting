...
def status(project_root: Path | None):
    ...
    try:
        ...
        click.echo("Mode: pregame")
        click.echo(
            f"Notifications: {'✅ Enabled' if scheduler_engine.notifications_enabled else '❌ Disabled'}"
        )
...
def test_workflow(game_pk: int | None, project_root: Path | None):
    ...
    async def run_test():
        ...
        try:
            ...
            click.echo(
                f"📅 Game Time: {game_time_est.strftime('%I:%M %p EST on %B %d, %Y')}"
            )
            ...
            click.echo("\n📋 Workflow Results:")
            click.echo(
                f"Status: {'✅ Success' if result.overall_status.value == 'success' else '❌ Failed'}"
            )
            ...
            for stage_name, stage_result in result.stages.items():
                ...
                click.echo(
                    f"  {status_emoji} {stage_display}: {stage_result.status.value}"
                )
...
def start_full_scheduler(project_root: Path | None, notifications: bool):
...
@click.option(
    "--to-emails",
    prompt="Recipient emails (comma-separated)",
...
"""
    ...
    click.echo("\n⚠️  Security Note:")
    click.echo(
        "Store these credentials securely and never commit them to version control!"
    )


@pregame_group.command("list-games")
@click.option(
    "--date", "-d", help="Date to list games for (YYYY-MM-DD, defaults to today)"
)
def list_games(date: str | None):
