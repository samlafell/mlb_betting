...
@click.option("--strategy", type=str, help="Filter by strategy name")
@click.option(
    "--min-sample-size",
    type=int,
    default=10,
...
def analyze(
    ...
    async def run_analysis():
        click.echo("🕐 TIMING ANALYSIS")
        click.echo(f"📅 Period: {start_date.date()} to {end_date.date()}")
        click.echo(
            f"🔍 Filters: Source={source or 'All'}, Book={book or 'All'}, Type={split_type or 'All'}, Strategy={strategy or 'All'}"
        )
...
@timing_group.command()
@click.option(
    "--hours-until-game", "-h", type=float, required=True, help="Hours until game start"
)
@click.option(
    "--source",
...
@click.option("--strategy", type=str, help="Strategy generating the recommendation")
@click.option(
    "--no-cache", is_flag=True, help="Skip cache and generate fresh recommendation"
)
def recommend(
    ...
    async def get_recommendation():
        click.echo("🎯 TIMING RECOMMENDATION")
        ...
        try:
            ...
            recommendation = await timing_service.get_realtime_timing_recommendation(
                lookup=lookup, use_cache=not no_cache
            )
...
@timing_group.command()
@click.option(
    "--days-back", "-d", type=int, default=30, help="Number of days to look back"
)
@click.option(
    "--min-sample-size", type=int, default=10, help="Minimum bets for inclusion"
)
def summary(days_back: int, min_sample_size: int):
    """Show timing performance summary."""

    async def show_summary():
        click.echo("📊 TIMING PERFORMANCE SUMMARY")
        ...
        try:
            ...
            summary_data = await timing_service.get_timing_performance_summary(
                days_back=days_back, minimum_sample_size=min_sample_size
            )
...
@click.option(
    "--source", required=True, type=click.Choice(["VSIN", "SBD"]), help="Data source"
)
@click.option(
    "--book",
    type=click.Choice(["circa", "draftkings", "fanduel", "betmgm"]),
...
def track(
    ...
    async def track_recommendation():
        click.echo("📝 Tracking recommendation...")
...
def _display_console_analysis(analysis):
    ...
    if analysis.bucket_analyses:
        ...
        for bucket_analysis in sorted(
            analysis.bucket_analyses,
            key=lambda x: bucket_order.get(
                x.timing_bucket.value
                if hasattr(x.timing_bucket, "value")
                else x.timing_bucket,
                999,
            ),
        ):
            metrics = bucket_analysis.metrics
            bucket_name = (
                bucket_analysis.timing_bucket.display_name
                if hasattr(bucket_analysis.timing_bucket, "display_name")
    ...
    if analysis.best_bucket:
        click.echo("\n🏆 BEST PERFORMERS")
        best_bucket_name = (
            analysis.best_bucket.display_name
            if hasattr(analysis.best_bucket, "display_name")
            else analysis.best_bucket
        )
...
def _display_recommendation(recommendation):
    ...
    bucket_name = (
        lookup.timing_bucket.display_name
        if hasattr(lookup.timing_bucket, "display_name")
        else lookup.timing_bucket
    )
    ...
    if recommendation.sample_size_warning:
        click.echo("⚠️  WARNING: Insufficient sample size for reliable prediction")
...
def _display_summary_table(summary_data):
    ...
    avg_win_rate = (
        sum(row["win_rate"] * row["total_bets"] for row in summary_data) / total_bets
        if total_bets > 0
        else 0
    )
    avg_roi = (
        sum(row["roi_percentage"] * row["total_bets"] for row in summary_data)
        / total_bets
        if total_bets > 0
        else 0
    )
    ...
    if summary_data:
        ...
        click.echo(
            f"   Best timing: {best_bucket['timing_bucket']} ({best_bucket['roi_percentage']:.1f}% ROI)"
        )


def _export_json_analysis(analysis, output_path: Path | None):
...
def _export_csv_analysis(analysis, output_path: Path | None):
        ...
        for bucket_analysis in analysis.bucket_analyses:
            ...
            writer.writerow(
                [
                    ...
                    else (bucket_analysis.source if bucket_analysis.source else "All"),
                    bucket_analysis.book.value
                    if hasattr(bucket_analysis.book, "value") and bucket_analysis.book
                    else (bucket_analysis.book if bucket_analysis.book else "All"),
