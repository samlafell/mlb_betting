...
class BettingAnalyzer:
    ...
    def print_strategy_summary(self, df: pd.DataFrame, strategy_name: str):
        ...
        if "win_rate" in df.columns and "total_bets" in df.columns:
            ...
            for idx, row in top_strategies.iterrows():
                ...
                strategy_id = row.get(
                    "source_book_type", row.get("strategy_name", "Unknown")
                )
                print(
                    f"      • {strategy_id}{bet_info}: {row['win_rate']:.1f}%{roi_info}"
                )
    ...
    def generate_roi_summary(self):
        ...
        for strategy_name, df in self.results.items():
            ...
            if "win_rate" in df.columns and "total_bets" in df.columns:
                profitable = df[
                    (df["win_rate"] >= 52.4) & (df["total_bets"] >= 5)
                ].copy()
        ...
        for idx, row in top_5.iterrows():
            strategy_id = row.get(
                "source_book_type", row.get("bet_recommendation", "Unknown")
            )
            ...
            print(
                f"    Win Rate: {win_rate:.1f}% | Sample: {total_bets} bets | {roi_text} | Confidence: {confidence}"
            )
            print()
    ...
    def save_results(self):
        ...
        try:
            ...
            for strategy_name, df in self.results.items():
                if not df.empty:
                    filename = (
                        f"{strategy_name.lower().replace(' ', '_')}_{timestamp}.csv"
                    )
