...
from ..models.splits import BettingSplit, SplitType, DataSource, BookType
...
class SBDParser(BaseParser):
    ...
    def _normalize_team_name(self, team_name: str, team_code: str = "") -> Optional[Team]:
        """
        ...
        try:
            ...
            logger.warning("Could not normalize team", 
                         team_name=team_name, 
                         team_code=team_code)
            return None
            
        except Exception as e:
            logger.error("Team normalization failed", 
                        team_name=team_name, 
                        team_code=team_code,
                        error=str(e))
            return None
    
    def _get_official_game_id(self, game_data: Dict[str, Any]) -> Optional[str]:
        """
        ...
        try:
            ...
            logger.error("Failed to get official game ID", 
                        game_data=game_data,
                        error=str(e))
            # Fallback to SBD game ID
            return game_data.get('game_id')
    
    def _parse_spread_split(self, game_data: Dict[str, Any], spread_data: Dict[str, Any]) -> Optional[BettingSplit]:
        """
        ...
        try:
            ...
            home_team = self._normalize_team_name(
                game_data['home_team'], 
                game_data.get('home_code', '')
            )
            away_team = self._normalize_team_name(
                game_data['away_team'], 
                game_data.get('away_code', '')
            )
            ...
            last_updated = self._parse_datetime(updated_str) if updated_str else datetime.now(pytz.UTC)
            ...
            sharp_action = "home" if sharp_action_detected and home_stake_pct > home_bets_pct else None
            ...
            logger.debug("Parsed spread split", 
            ...
            logger.error("Failed to parse spread split", 
                        game_id=game_data.get('game_id', 'unknown'),
                        error=str(e))
            return None
    
    def _parse_total_split(self, game_data: Dict[str, Any], total_data: Dict[str, Any]) -> Optional[BettingSplit]:
        """
        ...
        try:
            ...
            # Convert sharp action to string format
            sharp_action = "over" if sharp_action_detected and over_stake_pct > over_bets_pct else None
            if sharp_action_detected and not sharp_action:
                sharp_action = "under"
            ...
            logger.debug("Parsed total split", 
                        game_id=official_game_id,
            ...
            logger.error("Failed to parse total split", 
                        game_id=game_data.get('game_id', 'unknown'),
                        error=str(e))
            return None
    
    def _parse_moneyline_split(self, game_data: Dict[str, Any], ml_data: Dict[str, Any]) -> Optional[BettingSplit]:
        """
        ...
        try:
            ...
            logger.debug("Parsed moneyline split", 
                        game_id=official_game_id,
                        sharp_action=sharp_action)
            ...
            logger.error("Failed to parse moneyline split", 
                        game_id=game_data.get('game_id', 'unknown'),
                        error=str(e))
            return None
    ...
    async def parse_raw_data(self, raw_data: Dict[str, Any]) -> Optional[BettingSplit]:
        ...
        try:
            ...
            logger.warning("No valid splits found in game data", 
    ...
    def parse_all_splits(self, raw_data: List[Dict[str, Any]]) -> List[BettingSplit]:
        ...
        for game_data in raw_data:
            try:
                ...
                logger.error("Failed to prepare game for batch processing", 
                           game_id=game_data.get('game_id', 'unknown'),
                           error=str(e))
        ...
        for game_data in raw_data:
            try:
                ...
                if not official_game_id:
                    ...
                    logger.warning("Using SBD game ID as fallback", 
                ...
                for split_type in ['spread', 'total', 'moneyline']:
                    ...
                    parsed_split = None
                    if split_type == 'spread':
                        parsed_split = self._parse_spread_split_with_id(game_data_with_official_id, split_data)
                    elif split_type == 'total':
                        parsed_split = self._parse_total_split_with_id(game_data_with_official_id, split_data)
                    elif split_type == 'moneyline':
                        parsed_split = self._parse_moneyline_split_with_id(game_data_with_official_id, split_data)
                    
                    if parsed_split:
                        all_splits.append(parsed_split)
                        logger.debug("Successfully parsed split", 
                                   official_game_id=official_game_id,
        ...
        logger.info("SBD parsing completed", 
                   games_processed=len(raw_data),
                   splits_parsed=len(all_splits))
        
        return all_splits
    
    def _parse_spread_split_with_id(self, game_data: Dict[str, Any], spread_data: Dict[str, Any]) -> Optional[BettingSplit]:
        """
    ...
    def _parse_total_split_with_id(self, game_data: Dict[str, Any], total_data: Dict[str, Any]) -> Optional[BettingSplit]:
        """
    ...
    def _parse_moneyline_split_with_id(self, game_data: Dict[str, Any], ml_data: Dict[str, Any]) -> Optional[BettingSplit]:
        """
    ...
    def validate_structure(self, data: Any) -> bool:
        ...
        for item in data:
            ...
            required_fields = ['game_id', 'betting_splits', 'home_team', 'away_team', 'date']
