...
from .base import BaseParser, ValidationConfig, ValidationResult
...
class VSINParser(BaseParser):
    ...
    def __init__(self, validation_config: ValidationConfig | None = None) -> None:
        """
        ...
        """
        super().__init__(parser_name="VSIN", validation_config=validation_config)
        ...
        self.field_mappings = {
            ...
            "total": [
                ...
                "o/u",
                "Total",
            ],  # Updated to include 'Total' from scraper
            "moneyline": [
                "moneyline",
                "money line",
                "ml",
                ...
                "away line",
                "home line",
            ],  # Updated with capitalized fields
            ...
            "spread_home_bets_pct": [
                "home spread bets %",
                "Home Spread Bets %",
                "spread bets %",
            ],
            "spread_away_bets_pct": [
                "away spread bets %",
                "Away Spread Bets %",
                "spread bets away %",
            ],
            ...
            "spread_home_stake_pct": [
                "home spread handle %",
                "Home Spread Handle %",
                "spread handle %",
            ],
            "spread_away_stake_pct": [
                "away spread handle %",
                "Away Spread Handle %",
                "spread handle away %",
            ],
    ...
    async def parse_raw_data(self, raw_data: dict[str, Any]) -> BettingSplit | None:
        ...
        try:
            ...
            # Parse spread data
            spread_split = self._parse_spread_split(
                raw_data, game_info, source, book, last_updated
            )
            if spread_split:
                splits.append(spread_split)

            # Parse total data
            total_split = self._parse_total_split(
                raw_data, game_info, source, book, last_updated
            )
            if total_split:
            ...
            moneyline_split = self._parse_moneyline_split(
                raw_data, game_info, source, book, last_updated
            )
            if moneyline_split:
            ...
            self.logger.error(
                "Failed to parse VSIN data", error=str(e), raw_data=raw_data
            )
            return None

    async def parse_all_splits(
                    ...
                    try:
                        ...
                        if mlb_game_data:
                            ...
                            if official_datetime.tzinfo is not None:
                                ...
                                est_datetime = official_datetime.astimezone(
                                    est_tz
                                ).replace(tzinfo=None)
                            else:
                            ...
                            self.logger.debug(
                                ...
                                final_datetime=final_datetime.strftime(
                                    "%Y-%m-%d %H:%M:%S"
                                ),
                        ...
                        self.logger.warning(
                            "Failed to get game data", game_key=game_key, error=str(e)
                        )
        ...
        for raw_data in raw_data_list:
            try:
                ...
                if not official_data:
                    self.logger.warning(
                        "No official game data found", game_key=game_key
                    )
                    continue
                ...
                game_info_with_official = {
                    ...
                    "home_team": game_info["home_team"],
                    "away_team": game_info["away_team"],
                    "game_datetime": official_data[
                        "game_datetime"
                    ],  # Use official datetime
                    "matchup_text": game_info["matchup_text"],
                }

                # Parse splits for this game
                splits = self._parse_splits_with_id(
                    raw_data, official_data["game_id"], game_info_with_official
                )
    ...
    def _parse_splits_with_id(
        self, raw_data: dict[str, Any], official_game_id: str, game_info: dict[str, Any]
    ) -> list[BettingSplit]:
        """
        ...
        try:
            ...
            # Parse spread data
            spread_split = self._parse_spread_split(
                raw_data, complete_game_info, source, book, last_updated
            )
            if spread_split:
                splits.append(spread_split)
    ...
    def _parse_game_info(self, raw_data: dict[str, Any]) -> dict[str, Any] | None:
        """
        ...
        try:
            ...
            for field_key in raw_data:
                if any(
                    game_field in field_key.lower()
                    for game_field in self.field_mappings["game"]
                ):
    ...
    def _parse_teams_from_text(self, game_text: str) -> dict[str, Team] | None:
        """
        ...
        try:
            ...
            self.logger.debug(
                "Failed to parse teams from text", error=str(e), game_text=game_text
            )
            return None

    def _normalize_team_name_enhanced(self, name: str) -> str | None:
        """
    ...
    def _generate_game_id(
        self, away_team: Team, home_team: Team, game_datetime: datetime | None
    ) -> str:
        """
    ...
    def _parse_game_datetime(self, raw_data: dict[str, Any]) -> datetime | None:
        """
    ...
    def _parse_spread_split(
        ...
        try:
            ...
            if away_spread:
                ...
                spread_value = self._find_field_value(
                    raw_data, self.field_mappings["spread"]
                )
                spread_float = (
                    self._parse_numeric_value(spread_value) if spread_value else None
                )

            # Find betting percentages - use spread-specific fields for spread splits
            home_bets_pct = self._find_field_value(
                raw_data, self.field_mappings["spread_home_bets_pct"]
            ) or self._find_field_value(raw_data, self.field_mappings["home_bets_pct"])
            away_bets_pct = self._find_field_value(
                raw_data, self.field_mappings["spread_away_bets_pct"]
            ) or self._find_field_value(raw_data, self.field_mappings["away_bets_pct"])

            # Find stake percentages - use spread-specific fields
            home_stake_pct = self._find_field_value(
                raw_data, self.field_mappings["spread_home_stake_pct"]
            )
            away_stake_pct = self._find_field_value(
                raw_data, self.field_mappings["spread_away_stake_pct"]
            )
            ...
            # Find bet counts
            home_bets_count = self._find_field_value(
                raw_data, self.field_mappings["home_bets_count"]
            )
            away_bets_count = self._find_field_value(
                raw_data, self.field_mappings["away_bets_count"]
            )
    ...
    def _parse_total_split(
        ...
        try:
            ...
            total_float = (
                self._parse_numeric_value(total_value) if total_value else None
            )

            # Find betting percentages
            over_bets_pct = self._find_field_value(
                raw_data, self.field_mappings["over_bets_pct"]
            )
            under_bets_pct = self._find_field_value(
                raw_data, self.field_mappings["under_bets_pct"]
            )

            # Find stake percentages
            over_stake_pct = self._find_field_value(
                raw_data, self.field_mappings["over_stake_pct"]
            )
            under_stake_pct = self._find_field_value(
                raw_data, self.field_mappings["under_stake_pct"]
            )

            # Check if we have any total betting data
            if not any(
                [over_bets_pct, under_bets_pct, over_stake_pct, under_stake_pct]
            ):
                return None

            # Find bet counts
            over_bets_count = self._find_field_value(
                raw_data, self.field_mappings["over_bets_count"]
            )
            under_bets_count = self._find_field_value(
                raw_data, self.field_mappings["under_bets_count"]
            )
    ...
    def _parse_moneyline_split(
        ...
        try:
            # Find betting percentages
            home_bets_pct = self._find_field_value(
                raw_data, self.field_mappings["home_bets_pct"]
            )
            away_bets_pct = self._find_field_value(
                raw_data, self.field_mappings["away_bets_pct"]
            )
            ...
            home_stake_pct = self._find_field_value(
                raw_data, self.field_mappings["home_stake_pct"]
            )
            away_stake_pct = self._find_field_value(
                raw_data, self.field_mappings["away_stake_pct"]
            )
    ...
    def _find_field_value(
        self, raw_data: dict[str, Any], field_aliases: list[str]
    ) -> str | None:
        """
    ...
    def _parse_numeric_value(
        self, value: str | None, target_type: type = float
    ) -> float | None:
        """
    ...
    def _parse_moneyline_odds(self, odds_str: str) -> int | None:
        """
    ...
    async def _custom_validation(self, item: BettingSplit) -> ValidationResult:
        ...
        if (
            item.home_or_over_bets_percentage is not None
            and item.away_or_under_bets_percentage is not None
        ):
            total_pct = (
                item.home_or_over_bets_percentage + item.away_or_under_bets_percentage
            )
            if not (95 <= total_pct <= 105):  # Allow 5% tolerance
                warnings.append(f"Bet percentages don't sum to 100%: {total_pct}%")

        if (
            item.home_or_over_stake_percentage is not None
            and item.away_or_under_stake_percentage is not None
        ):
            total_pct = (
                item.home_or_over_stake_percentage + item.away_or_under_stake_percentage
            )
            if not (95 <= total_pct <= 105):  # Allow 5% tolerance
...
async def parse_vsin_data(
    raw_data: list[dict[str, Any]], validation_config: ValidationConfig | None = None
):
    """
