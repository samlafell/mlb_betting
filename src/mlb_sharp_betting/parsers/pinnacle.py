...
from ..models.pinnacle import PinnacleMarket, PinnacleMarketType
...
class PinnacleParser(BaseParser):
    ...
    def __init__(self, validation_config: ValidationConfig | None = None) -> None:
        ...
        """
        super().__init__(parser_name="Pinnacle", validation_config=validation_config)
    ...
    async def parse_raw_data(self, raw_data: dict[str, Any]) -> BettingSplit | None:
        ...
        try:
            ...
            if market.market_type == PinnacleMarketType.MONEYLINE:
                ...
                self.logger.debug(
                    "Unsupported market type", market_type=market.market_type
                )
                return None
    ...
    def _parse_moneyline_split(self, market: PinnacleMarket) -> BettingSplit | None:
        """
    ...
    def _parse_spread_split(self, market: PinnacleMarket) -> BettingSplit | None:
        """
    ...
    def _parse_total_split(self, market: PinnacleMarket) -> BettingSplit | None:
        """
    ...
    def _generate_game_id(self, market: PinnacleMarket) -> str:
        ...
        try:
            ...
            official_game_id = self.mlb_api_service.get_official_game_id(
                market.home_team.value, market.away_team.value, market.game_datetime
            )
            ...
            self.logger.debug(
                "Failed to generate official game ID, using fallback", error=str(e)
            )
            date_str = market.game_datetime.strftime("%Y%m%d")
            return f"{market.away_team.value}_{market.home_team.value}_{date_str}"

    async def parse_pinnacle_markets(
        self, markets: list[PinnacleMarket]
    ) -> list[BettingSplit]:
        """
        ...
        self.logger.info(
            "Successfully parsed Pinnacle markets",
    ...
    async def _custom_validation(self, item: BettingSplit) -> ValidationResult:
        ...
        if item.home_or_over_bets_percentage is not None:
            if (
                item.home_or_over_bets_percentage < 5
                or item.home_or_over_bets_percentage > 95
            ):
                warnings.append(
                    f"Unusual home/over percentage: {item.home_or_over_bets_percentage}"
                )

        if item.away_or_under_bets_percentage is not None:
            if (
                item.away_or_under_bets_percentage < 5
                or item.away_or_under_bets_percentage > 95
            ):
                warnings.append(
                    f"Unusual away/under percentage: {item.away_or_under_bets_percentage}"
                )
...
async def parse_pinnacle_data(
    markets: list[PinnacleMarket], validation_config: ValidationConfig | None = None
) -> list[BettingSplit]:
    """
