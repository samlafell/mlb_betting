...
class TableComparisonResult:
    ...
    last_legacy_record: datetime | None
    last_new_record: datetime | None
...
"""
    ...
    def _format_table_comparisons(self) -> str:
        ...
        for comp in self.table_comparisons:
            status_emoji = (
                "✅"
                if comp.data_integrity_status == "GOOD"
                else "⚠️"
                if comp.data_integrity_status == "WARNING"
                else "❌"
            )
            lines.append(
                f"  {status_emoji} {comp.legacy_table} → {comp.new_table}: {comp.percentage_migrated:.1f}% ({comp.new_count:,}/{comp.legacy_count:,})"
            )
            if comp.issues:
...
class MigrationMonitor:
    ...
    async def generate_migration_report(
        self, lookback_hours: int = 24, include_historical: bool = True
    ) -> MigrationReport:
        """
        ...
        try:
            ...
            freshness_status, migration_lag = await self._assess_data_freshness(
                lookback_hours
            )

            # Determine overall status
            overall_status = self._determine_overall_status(
                table_comparisons, overall_percentage
            )

            # Generate recommendations
            recommendations = self._generate_recommendations(
                table_comparisons, overall_percentage, migration_lag
            )
    ...
    async def _compare_tables(
        ...
        try:
            ...
            # Get record counts
            legacy_count = await self._get_table_count(
                legacy_table,
                timestamp_col,
                lookback_hours if not include_historical else None,
            )
            new_count = await self._get_table_count(
                new_table,
                timestamp_col,
                lookback_hours if not include_historical else None,
            )
            ...
            self.logger.error(
                "Failed to compare tables", table_name=table_name, error=str(e)
            )
            return TableComparisonResult(
                legacy_table=mapping["legacy"],
                new_table=mapping["new"],
    ...
    async def _get_table_count(
        self, table_name: str, timestamp_col: str, lookback_hours: int | None = None
    ) -> int:
        ...
        try:
            ...
            with self.db_manager.get_cursor() as cursor:
                ...
                if lookback_hours:
                    ...
                    query = (
                        f"SELECT COUNT(*) FROM {table_name} WHERE {timestamp_col} >= %s"
                    )
            ...
            self.logger.warning(
                "Failed to get table count", table=table_name, error=str(e)
            )
            return 0

    async def _get_latest_timestamp(
        self, table_name: str, timestamp_col: str
    ) -> datetime | None:
        ...
        try:
            ...
            self.logger.warning(
                "Failed to get latest timestamp", table=table_name, error=str(e)
            )
            return None

    async def _assess_table_integrity(
        self, legacy_table: str, new_table: str, timestamp_col: str, lookback_hours: int
    ) -> tuple[str, list[str]]:
        ...
        try:
            ...
            with self.db_manager.get_cursor() as cursor:
                ...
                recent_new_query = (
                    f"SELECT COUNT(*) FROM {new_table} WHERE {timestamp_col} >= %s"
                )
    ...
    def _determine_overall_status(
        self, comparisons: list[TableComparisonResult], overall_percentage: float
    ) -> str:
        """Determine overall migration status."""
        error_count = sum(
            1 for comp in comparisons if comp.data_integrity_status == "ERROR"
        )
        warning_count = sum(
            1 for comp in comparisons if comp.data_integrity_status == "WARNING"
        )
    ...
    def _generate_recommendations(
        ...
        error_tables = [
            comp for comp in comparisons if comp.data_integrity_status == "ERROR"
        ]
        if error_tables:
            recommendations.append(
                f"URGENT: Fix {len(error_tables)} tables with errors"
            )

        # Check migration progress
        if overall_percentage < 25:
            recommendations.append(
                "Migration just started - monitor closely for next few hours"
            )
        elif overall_percentage < 75:
            recommendations.append(
                "Migration in progress - verify application services are updated"
            )
        elif overall_percentage < 95:
            recommendations.append(
                "Migration nearly complete - prepare for legacy table deprecation"
            )

        # Check data lag
        if migration_lag > 120:  # More than 2 hours
            recommendations.append(
                "High data lag detected - check application service configuration"
            )
        elif migration_lag > 60:  # More than 1 hour
            recommendations.append(
                "Moderate data lag - monitor application performance"
            )
        ...
        zero_new = [
            ...
            if comp.new_count == 0 and comp.legacy_count > 0
        ]
        if zero_new:
            recommendations.append(
                "Some tables not receiving new data - verify service updates"
            )

        if not recommendations:
            recommendations.append(
                "Migration proceeding smoothly - continue monitoring"
            )
    ...
    async def _check_rollback_readiness(self) -> bool:
        ...
        try:
            ...
            for mapping in self.table_mappings.values():
                ...
                recent_count = await self._get_table_count(
                    legacy_table, timestamp_col, 24
                )
                if recent_count > 0:
    ...
    async def save_report(
        self, report: MigrationReport, output_dir: Path | None = None
    ) -> Path:
