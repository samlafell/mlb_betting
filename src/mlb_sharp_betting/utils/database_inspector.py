...
from ..db.repositories import get_betting_split_repository
...
async def check_splits():
    ...
    june_16_splits = [
        ...
        if "777484" in s.game_id
        or "777483" in s.game_id
        or "777486" in s.game_id
        or "777487" in s.game_id
        or "777488" in s.game_id
    ]

    print(f"June 16 games splits: {len(june_16_splits)}")
    for split in june_16_splits:
        print(
            f"   {split.game_id}: {split.split_type}={split.split_value} ({split.source}/{split.book})"
        )

    # Close connection handled automatically
