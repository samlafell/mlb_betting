...
class GameTimeValidator:
    ...
    def validate_split_timing(self, split: BettingSplit, current_time: Optional[datetime] = None) -> Tuple[ValidationResult, Dict[str, Any]]:
        ...
        try:
            ...
            if not split.game_datetime:
                ...
                self.logger.warning("Split missing game_datetime", 
                                  game_id=split.game_id,
                                  home_team=split.home_team,
                                  away_team=split.away_team)
                return ValidationResult.INVALID_TIME, {**metadata, "reason": "missing_game_datetime"}
            ...
            if time_since_start.total_seconds() < 0:
                ...
                self.logger.debug("Split validated - future game", 
                                game_id=split.game_id,
            ...
            if time_since_start <= self.grace_period:
                self.validation_stats["valid_splits"] += 1
                self.logger.debug("Split validated - within grace period", 
                                game_id=split.game_id,
                                minutes_since_start=time_since_start.total_seconds() / 60)
                return ValidationResult.VALID, {**metadata, "reason": "within_grace_period"}
            
            # Game started more than grace period ago
            self.validation_stats["expired_splits"] += 1
            self.logger.info("Split rejected - game started too long ago", 
                           game_id=split.game_id,
                           minutes_since_start=time_since_start.total_seconds() / 60,
                           grace_period_minutes=self.grace_period.total_seconds() / 60)
            
            return ValidationResult.EXPIRED, {**metadata, "reason": "exceeded_grace_period"}
            ...
            try:
                ...
                if game_status:
                    metadata.update({
                        ...
                        "scheduled_start_time": game_status.get("scheduled_start_time")
                    })
                    
                    # Handle postponed games
                    if game_status.get("status") in ["Postponed", "Cancelled", "Suspended"]:
                        self.validation_stats["postponed_games"] += 1
                        self.logger.info("Game postponed/cancelled", 
                                       game_id=split.game_id,
                                       status=game_status.get("status"))
                        return ValidationResult.POSTPONED, {**metadata, "reason": f"game_{game_status.get('status').lower()}"}
                    ...
                    if actual_start and actual_start != split.game_datetime:
                        ...
                        if time_since_actual_start <= self.grace_period:
                            ...
                            self.validation_stats["valid_splits"] += 1
                            self.logger.info("Split validated - delayed game within grace period", 
                                           game_id=split.game_id,
                                           original_start=split.game_datetime.isoformat(),
                                           actual_start=actual_start.isoformat(),
                                           minutes_since_actual_start=time_since_actual_start.total_seconds() / 60)
                            return ValidationResult.DELAYED, {**metadata, "reason": "delayed_game_valid"}
                        else:
                            self.validation_stats["expired_splits"] += 1
                            self.logger.info("Split rejected - delayed game exceeded grace period", 
                                           game_id=split.game_id,
                                           actual_start=actual_start.isoformat(),
                                           minutes_since_actual_start=time_since_actual_start.total_seconds() / 60)
                            return ValidationResult.EXPIRED, {**metadata, "reason": "delayed_game_expired"}
                
            except Exception as e:
                self.validation_stats["api_errors"] += 1
                self.logger.warning("MLB API check failed, using basic validation", 
                                  game_id=split.game_id,
                                  error=str(e))
        ...
        for split in splits:
            try:
                if use_mlb_api:
                    ...
                    result, metadata = asyncio.run(self.validate_with_mlb_api(split, current_time))
                else:
                ...
                self.logger.error("Error validating split in batch", 
    ...
    def get_validation_stats(self) -> Dict[str, Any]:
        ...
        stats = {
            **self.validation_stats,
            "rejection_rate": (self.validation_stats["expired_splits"] + 
                             self.validation_stats["postponed_games"] + 
                             self.validation_stats["invalid_times"]) / max(total_validations, 1),
            "api_error_rate": self.validation_stats["api_errors"] / max(total_validations, 1),
            "uptime_hours": (datetime.now(timezone.utc) - self.validation_stats["last_reset"]).total_seconds() / 3600
        }
...
_validator_instance: Optional[GameTimeValidator] = None
...
def get_game_time_validator(grace_period_minutes: int = 5) -> GameTimeValidator:
    ...
    if _validator_instance is None:
        ...
        logger.info("Created new GameTimeValidator instance", 
                   grace_period_minutes=grace_period_minutes)
    
    return _validator_instance


def validate_split_timing(split: BettingSplit, use_mlb_api: bool = False) -> Tuple[bool, Dict[str, Any]]:
    """
...
def validate_splits_batch(splits: List[BettingSplit], use_mlb_api: bool = False) -> Tuple[List[BettingSplit], List[Dict[str, Any]]]:
    """
