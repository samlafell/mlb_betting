...
from .base import HTMLScraper, RateLimitConfig, RetryConfig, ScrapingResult
...
class VSINScraper(HTMLScraper):
    ...
    def _get_target_date_est(self, target_date: datetime | None = None) -> datetime:
        """
    ...
    def build_url(
        self, sport: str, sportsbook: str | None = None, use_tomorrow: bool = False
    ) -> str:
        """
    ...
    def _extract_date_from_html(self, soup: BeautifulSoup) -> datetime | None:
        """
                ...
                for link in date_links:
                    ...
                    if date_match:
                        try:
                            ...
                            self.logger.debug(
                                "Failed to parse date", date_str=date_str, error=str(e)
                            )
                            continue
            ...
            for pattern in patterns:
                ...
                if match:
                    ...
                    if month_num:
                        try:
                            return datetime(
                                current_year, month_num, int(day_num)
                            ).date()
                        except ValueError:
                            continue
    ...
    def _validate_date_match(
        self, extracted_date: datetime, target_date: datetime, tolerance_hours: int = 6
    ) -> bool:
        """
    ...
    async def scrape_sport(
        ...
        return await self.scrape(
            sport=sport, sportsbook=sportsbook, target_date=target_date
        )
    ...
    async def scrape(self, **kwargs: Any) -> ScrapingResult:
        ...
        try:
            ...
            self.logger.info(
                "Starting VSIN scrape with date validation",
                sport=sport,
                sportsbook=sportsbook,
            ...
            for attempt_num, use_tomorrow in enumerate(attempts, 1):
                try:
                    ...
                    self.logger.info(
                        f"VSIN scrape attempt {attempt_num}",
                        use_tomorrow=use_tomorrow,
                        url=url,
                    )
                    ...
                    if extracted_date:
                        date_match = self._validate_date_match(
                            extracted_date, target_date_est
                        )
                        ...
                        if date_match:
                            ...
                            if main_content:
                                betting_splits = self._parse_betting_splits(
                                    main_content, sport, sportsbook
                                )
                                if betting_splits:
                                    successful_scrape = {
                                        "data": betting_splits,
                                ...
                                self.logger.warning(
                                    "No main content found despite date match"
                                )
                        else:
                            self.logger.warning(
                                "Date mismatch, trying next view",
                                extracted_date=extracted_date.strftime("%Y-%m-%d"),
                                target_date=target_date_est.strftime("%Y-%m-%d"),
                            )
                    else:
            ...
            if successful_scrape:
                ...
                self.logger.info(
                    "Successfully scraped VSIN data with date validation",
                    sport=sport,
                    sportsbook=sportsbook,
                    splits_count=len(data),
                    extracted_date=successful_scrape["extracted_date"].strftime(
                        "%Y-%m-%d"
                    ),
                ...
                self.logger.error(
                    "All VSIN scrape attempts failed",
                    sport=sport,
                    sportsbook=sportsbook,
                    target_date=target_date_est.strftime("%Y-%m-%d"),
            ...
            errors.append(error_msg)
            self.logger.error(
                "VSIN scraping failed", sport=sport, sportsbook=sportsbook, error=str(e)
            )
    ...
    def _find_betting_table_fallback(self, soup: BeautifulSoup) -> Tag | None:
        """
        ...
        for table in tables:
            ...
            if any(
                ...
                for keyword in ["%", "line", "odds", "bet", "handle"]
            ):
    ...
    def _parse_betting_splits(
        self, content: Tag, sport: str, sportsbook: str
    ) -> list[dict[str, Any]]:
        """
        ...
        try:
            ...
            for table in tables:
                ...
                freezetable_splits = self._parse_freezetable_structure(
                    table, sport, sportsbook
                )
            ...
            if not splits_data:
                splits_data = self._parse_alternative_structure(
                    content, sport, sportsbook
                )
    ...
    def _parse_tbody_structure(
        self, table: Tag, sport: str, sportsbook: str
    ) -> list[dict[str, Any]]:
        """
    ...
    def _parse_freezetable_structure(
        self, table: Tag, sport: str, sportsbook: str
    ) -> list[dict[str, Any]]:
        """
        ...
        try:
            ...
            for row in rows:
                ...
                mlb_team_links = [
                    link for link in team_links if "/mlb/teams/" in link.get("href", "")
                ]
            ...
            self.logger.info(
                "Found freezetable data row",
                team_links=max_team_links,
                expected_games=max_team_links // 2,
            )

            # Parse multiple games from this single row
            splits_data = self._parse_multiple_games_from_row(
                best_row, sport, sportsbook
            )
    ...
    def _parse_multiple_games_from_row(
        self, row: Tag, sport: str, sportsbook: str
    ) -> list[dict[str, Any]]:
        """
        ...
        try:
            ...
            self.logger.info(
                "Found game cells in freezetable row",
            ...
            for cell_index, cell, team_links in game_cells:
                try:
                    ...
                    self.logger.debug(
                        "Error parsing individual game from freezetable",
        ...
        try:
            ...
            if len(game_cells) > 1:
                ...
                if len(odds_links) >= 2:
                    ...
                    odds_divs = moneyline_cell.find_all(
                        "div", class_="scorebox_highlight"
                    )
                    if len(odds_divs) >= 2:
            ...
            if len(game_cells) > 4:
                ...
                if total_div:
                    ...
                    if total_match:
                        ...
                        self.logger.debug(
                            "Found total line in freezetable",
                            total=total_line,
                            cell_text=total_text,
                        )
            ...
            if len(game_cells) > 7:
                ...
                if len(spread_links) >= 2:
                    ...
                    spread_divs = spread_cell.find_all(
                        "div", class_="scorebox_highlight"
                    )
            ...
            if has_meaningful_data:
                self.logger.debug(
                    "Successfully extracted freezetable game data",
                    away_team=away_team,
                    home_team=home_team,
                    away_line=away_line,
                    home_line=home_line,
            ...
            self.logger.debug(
                "Error extracting game data from freezetable", error=str(e)
            )
            return None
    ...
    def _extract_headers(self, header_row: Tag | None) -> list[str]:
        """
    ...
    def _parse_betting_row(
        self, row: Tag, headers: list[str], sport: str, sportsbook: str
    ) -> dict[str, Any] | None:
        """
        ...
        try:
            ...
            if len(team_names) < 2:
                self.logger.debug(
                    "Could not find two team names", team_names=team_names
                )
                return None
            ...
            if total_cell:
                ...
                if total_div:
                    ...
                    if total_match:
                        total_line = total_match.group(1)
                        self.logger.debug(
                            "Found total line", total=total_line, cell_text=total_text
                        )
            ...
            if spread_handle_cell:
                ...
                if len(spread_handle_divs) >= 2:
                    ...
                    away_spread_handle_match = re.search(
                        r"(\d+)%", away_spread_handle_text
                    )
                    home_spread_handle_match = re.search(
                        r"(\d+)%", home_spread_handle_text
                    )
    ...
    async def validate_sport_sportsbook(self, sport: str, sportsbook: str) -> bool:
        ...
        """
        return (
            sport.lower() in self.SPORTS_URLS
            and sportsbook.lower() in self.SPORTSBOOK_VIEWS
        )


# Convenience function
async def scrape_vsin_mlb(
    sportsbook: str = "circa", target_date: datetime | None = None
) -> ScrapingResult:
    """
