...
class PinnacleScraper(JSONScraper):
    ...
    async def scrape(self, **kwargs) -> ScrapingResult:
        ...
        try:
            ...
            async def process_matchup(matchup_id: int) -> Tuple[Optional[Dict[str, Any]], List[str], int]:
            ...
            for i, result in enumerate(results):
                if isinstance(result, Exception):
                    error_msg = f"Failed to process matchup {matchup_ids[i]}: {str(result)}"
            ...
            self.logger.info("Pinnacle scraping completed",
    ...
    async def _process_single_matchup(self, matchup_id: int) -> Tuple[Optional[Dict[str, Any]], List[str], int]:
        ...
        try:
            ...
            self.logger.debug("Processed complete matchup", 
                            matchup_id=matchup_id,
    ...
    async def _get_matchup_details(self, matchup_id: int) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            self.logger.debug("Failed to get matchup details", matchup_id=matchup_id, error=str(e))
            return None
    ...
    async def _get_matchup_markets(self, matchup_id: int) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            self.logger.debug("Failed to get matchup markets", matchup_id=matchup_id, error=str(e))
            return []
    
    def _extract_essential_market_info(self, raw_market: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    async def scrape_team_matchup(self, home_team: Team, away_team: Team) -> ScrapingResult:
        ...
        try:
            ...
            for matchup in all_data_result.data:
                if (matchup.get("home_team") == home_team.value and 
                    matchup.get("away_team") == away_team.value):
                    matching_matchups.append(matchup)
            
            self.logger.info("Found matching matchups for teams",
            ...
            self.logger.error("Team matchup scraping error", 
                            home_team=home_team, 
                            away_team=away_team, 
                            error=error_msg)
