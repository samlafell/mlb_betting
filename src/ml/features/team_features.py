...
class TeamFeatureExtractor(BaseFeatureExtractor):
    ...
    async def extract_features(
        ...
        try:
            ...
            if not data_quality['is_valid']:
                logger.warning(f"Team data quality issues for game {game_id}: {data_quality['missing_columns']}")
    ...
    async def _extract_recent_form_features(
        ...
        try:
            cutoff_date = game_date - timedelta(days=30)  # Look back 30 days for recent games
            
            # Get recent games for both teams
            home_recent = df.filter(
            ...
            ).sort('game_datetime', descending=True).head(self.recent_form_window)
            
            away_recent = df.filter(
            ...
            return {
                'home_recent_form_weighted': Decimal(str(home_form)) if home_form is not None else None,
                'away_recent_form_weighted': Decimal(str(away_form)) if away_form is not None else None,
    ...
    async def _extract_h2h_features(
        ...
        try:
            ...
            h2h_games = df.filter(
                ...
                pl.col('home_score').is_not_null() &
                pl.col('away_score').is_not_null()
            ).sort('game_datetime', descending=True).head(self.h2h_lookback_games)
            ...
            for row in h2h_games.rows(named=True):
                ...
                if home_score > away_score:  # Home team won
                    ...
                    if game_away == home_team:
                        home_wins += 1
                        home_advantage_scores.append(0.5)  # Home team won on road (still good)
                    else:
                        away_wins += 1
                        home_advantage_scores.append(0.5)  # Away team won on road
            
            avg_total_runs = np.mean(total_runs) if total_runs else None
            home_advantage = np.mean(home_advantage_scores) if home_advantage_scores else None
            
            return {
                'h2h_home_wins_last_10': home_wins,
                'h2h_away_wins_last_10': away_wins,
                'h2h_total_games': h2h_games.height,
                'h2h_avg_total_runs': Decimal(str(avg_total_runs)) if avg_total_runs is not None else None,
                'h2h_home_advantage': Decimal(str(home_advantage)) if home_advantage is not None else None
            }
    ...
    async def _extract_season_features(
        ...
        try:
            ...
            return {
                ...
                'home_win_pct': Decimal(str(home_stats['win_pct'])) if home_stats.get('win_pct') is not None else None,
                'away_win_pct': Decimal(str(away_stats['win_pct'])) if away_stats.get('win_pct') is not None else None,
                'home_runs_per_game': Decimal(str(home_stats['runs_per_game'])) if home_stats.get('runs_per_game') is not None else None,
                'away_runs_per_game': Decimal(str(away_stats['runs_per_game'])) if away_stats.get('runs_per_game') is not None else None,
                'home_runs_allowed_per_game': Decimal(str(home_stats['runs_allowed_per_game'])) if home_stats.get('runs_allowed_per_game') is not None else None,
                'away_runs_allowed_per_game': Decimal(str(away_stats['runs_allowed_per_game'])) if away_stats.get('runs_allowed_per_game') is not None else None
            }
    ...
    async def _extract_pitcher_features(self, game_info: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if game_info.get('home_pitcher_era'):
                pitcher_features['home_pitcher_season_era'] = Decimal(str(game_info['home_pitcher_era']))
            
            if game_info.get('away_pitcher_era'):
                pitcher_features['away_pitcher_season_era'] = Decimal(str(game_info['away_pitcher_era']))
    ...
    async def _extract_venue_features(
        ...
        try:
            ...
            venue_stats = venue_games.select([
                ((pl.col('home_score') > pl.col('away_score')).sum() / pl.len()).alias('home_win_rate'),
                (pl.col('home_score') + pl.col('away_score')).mean().alias('avg_total_runs')
            ])
            ...
            if not venue_stats.is_empty():
                ...
                home_team_factor = self._calculate_team_venue_factor(home_team_games, home_team)
                away_team_factor = self._calculate_team_venue_factor(away_team_games, away_team)
                
                return {
                    'home_field_advantage_factor': Decimal(str(home_win_rate)),
                    'venue_total_factor': Decimal(str(avg_total / 9.0)),  # Normalize to league average
                    'venue_home_team_factor': Decimal(str(home_team_factor)) if home_team_factor is not None else None,
                    'venue_away_team_factor': Decimal(str(away_team_factor)) if away_team_factor is not None else None
                }
    ...
    async def _extract_weather_features(self, game_info: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                'temperature_impact_total': Decimal(str(temperature_impact)) if temperature_impact is not None else None,
                'wind_impact_total': Decimal(str(wind_impact)) if wind_impact is not None else None,
    ...
    async def _extract_rest_travel_features(
        ...
        try:
            ...
            home_last_game = df.filter(
                ((pl.col('home_team') == home_team) | (pl.col('away_team') == home_team)) &
                (pl.col('game_datetime') < game_date) &
                pl.col('home_score').is_not_null()
            ).sort('game_datetime', descending=True).head(1)
            
            away_last_game = df.filter(
                ((pl.col('home_team') == away_team) | (pl.col('away_team') == away_team)) &
                (pl.col('game_datetime') < game_date) &
                pl.col('away_score').is_not_null()
            ).sort('game_datetime', descending=True).head(1)
    ...
    async def _extract_situational_features(
        ...
        try:
            ...
            return {
                ...
                'game_importance_score': Decimal(str(season_progress)) if season_progress is not None else None
            }
    ...
    def _calculate_weighted_form(self, games_df: pl.DataFrame, team: str) -> Optional[float]:
    ...
    def _calculate_season_stats(self, games_df: pl.DataFrame, team: str) -> Dict[str, Any]:
        """Calculate season statistics for a team"""
        if games_df.is_empty():
            return {'record': None, 'win_pct': None, 'runs_per_game': None, 'runs_allowed_per_game': None}
    ...
    def _calculate_team_venue_factor(self, venue_games: pl.DataFrame, team: str) -> Optional[float]:
