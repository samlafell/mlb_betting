...
class FeaturePipeline:
    ...
    async def extract_features_for_game(
        ...
        try:
            ...
            if 'betting_splits_data' in data_sources:
                ...
                betting_splits_features = await self.betting_splits_extractor.extract_features(
            ...
            if include_interactions:
                interaction_features = self._compute_interaction_features(feature_components)
            ...
            logger.info(f"Successfully extracted features for game {game_id} in {processing_time:.1f}ms")
    ...
    async def extract_batch_features(
        ...
        async def extract_single_game(game_id: int) -> Tuple[int, Optional[FeatureVector]]:
            async with semaphore:
                feature_vector = await self.extract_features_for_game(game_id, cutoff_time)
        ...
        logger.info(f"Batch extraction complete: {successful_extractions}/{len(game_ids)} successful")
    ...
    async def _load_game_data(self, game_id: int, cutoff_time: datetime) -> Dict[str, pl.DataFrame]:
        ...
        try:
            ...
            if temporal_rows:
                data_sources['temporal_data'] = pl.DataFrame([dict(row) for row in temporal_rows])
            ...
            if market_rows:
                data_sources['market_data'] = pl.DataFrame([dict(row) for row in market_rows])
            ...
            if team_rows:
                data_sources['team_data'] = pl.DataFrame([dict(row) for row in team_rows])
            ...
            if splits_rows:
                data_sources['betting_splits_data'] = pl.DataFrame([dict(row) for row in splits_rows])
    ...
    def _compute_derived_features(self, feature_components: Dict[str, Any]) -> Dict[str, Any]:
        """Compute derived features from base feature components"""
        derived = {}
        
        try:
            # Sharp action intensity score (combines temporal and betting splits)
            if 'temporal_features' in feature_components and 'betting_splits_features' in feature_components:
                ...
                if temporal.sharp_action_intensity_60min:
                    sharp_intensity += float(temporal.sharp_action_intensity_60min) * 0.6
            ...
            if 'team_features' in feature_components:
                ...
                if team.home_win_pct and team.away_win_pct:
                    derived['team_strength_differential'] = float(team.home_win_pct) - float(team.away_win_pct)
                
                if team.home_recent_form_weighted and team.away_recent_form_weighted:
                    derived['recent_form_differential'] = float(team.home_recent_form_weighted) - float(team.away_recent_form_weighted)
    ...
    def _compute_interaction_features(self, feature_components: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if 'temporal_features' in feature_components and 'betting_splits_features' in feature_components:
                ...
                if temporal.money_vs_bet_divergence_home and splits.home_money_bet_divergence:
                    interactions['sharp_public_interaction_home'] = (
                        float(temporal.money_vs_bet_divergence_home) * 
                        float(splits.home_money_bet_divergence)
                    )
            
            # Market consensus vs team performance
            if 'market_features' in feature_components and 'team_features' in feature_components:
                market = feature_components['market_features']
                team = feature_components['team_features']
                
                if market.sportsbook_consensus_strength and team.home_win_pct:
                    interactions['consensus_team_strength'] = (
                        float(market.sportsbook_consensus_strength) * 
                        float(team.home_win_pct)
                    )
            
            # Weather impact on totals vs market movement
            if 'team_features' in feature_components and 'temporal_features' in feature_components:
                team = feature_components['team_features']
                temporal = feature_components['temporal_features']
                
                if team.temperature_impact_total and temporal.opening_to_current_total:
                    interactions['weather_market_interaction'] = (
                        float(team.temperature_impact_total) * 
    ...
    def _calculate_quality_metrics(
        ...
        source_flags = {
            'action_network_data': 'temporal_features' in feature_components or 'market_features' in feature_components,
    ...
    def _feature_vector_to_db_format(self, feature_vector: FeatureVector) -> Tuple:
        ...
        temporal_json = feature_vector.temporal_features.model_dump() if feature_vector.temporal_features else {}
        market_json = feature_vector.market_features.model_dump() if feature_vector.market_features else {}
        team_json = feature_vector.team_features.model_dump() if feature_vector.team_features else {}
        splits_json = feature_vector.betting_splits_features.model_dump() if feature_vector.betting_splits_features else {}
    ...
    def get_pipeline_stats(self) -> Dict[str, Any]:
        ...
        if stats['total_games_processed'] > 0:
            stats['success_rate'] = stats['successful_extractions'] / stats['total_games_processed']
        else:
