...
class TemporalFeatureExtractor(BaseFeatureExtractor):
    ...
    async def extract_features(
        ...
        try:
            ...
            if not data_quality['is_valid']:
                logger.warning(f"Data quality issues for game {game_id}: {data_quality['missing_columns']}")
            ...
            if game_data.is_empty():
                logger.warning(f"No data available for game {game_id} before cutoff {cutoff_time}")
            ...
            if minutes_before < 60:
                raise ValueError(f"Data leakage prevention: cutoff time must be >= 60 minutes before game (got {minutes_before})")
            
            # Extract line movement features
            line_movement_features = self._extract_line_movement_features(game_data, cutoff_time)
            
            # Extract sharp action features
            sharp_action_features = self._extract_sharp_action_features(game_data, cutoff_time)
    ...
    def _extract_line_movement_features(self, df: pl.DataFrame, cutoff_time: datetime) -> Dict[str, Any]:
        ...
        try:
            ...
            velocity_data = recent_data.group_by(['sportsbook_name', 'market_type']).agg([
                pl.col('home_ml_odds').diff().abs().sum().alias('ml_changes'),
                pl.col('home_spread_line').diff().abs().sum().alias('spread_changes'),
            ...
            avg_velocity = velocity_data.select([
                ((pl.col('ml_changes') + pl.col('spread_changes') + pl.col('total_changes')) / pl.col('data_points')).mean().alias('velocity')
            ]).item() if not velocity_data.is_empty() else 0.0
            ...
            if not opening_data.is_empty() and not current_data.is_empty():
                ...
                movements = {
                    'opening_to_current_ml_home': float(current.get('home_ml_odds', 0) - opening.get('home_ml_odds', 0)),
                    'opening_to_current_ml_away': float(current.get('away_ml_odds', 0) - opening.get('away_ml_odds', 0)),
                    'opening_to_current_spread_home': float(current.get('home_spread_line', 0) - opening.get('home_spread_line', 0)),
                    'opening_to_current_total': float(current.get('total_line', 0) - opening.get('total_line', 0))
                }
            
            return {
                'line_movement_velocity_60min': Decimal(str(avg_velocity)) if avg_velocity else None,
                **movements
            }
    ...
    def _extract_sharp_action_features(self, df: pl.DataFrame, cutoff_time: datetime) -> Dict[str, Any]:
        ...
        try:
            ...
            steam_moves = df.group_by('timestamp').agg([
                (pl.col('home_ml_odds').diff().abs() > 10).sum().alias('ml_moves'),
            ...
            if not recent_sharp.is_empty():
                ...
                time_weights = recent_sharp.with_columns([
                    ((cutoff_time.timestamp() - pl.col('timestamp').dt.timestamp()) / 3600).alias('hours_ago')
                ]).with_columns([
                    (1.0 / (1.0 + pl.col('hours_ago'))).alias('time_weight')
                ])
                
                intensity = time_weights.select('time_weight').sum().item() / max(recent_sharp.height, 1)
            
            return {
                'sharp_action_intensity_60min': Decimal(str(intensity)) if intensity > 0 else None,
        ...
        try:
            ...
            for row in divergence_data.rows(named=True):
                ...
                if market in ['moneyline', 'spread']:
                    ...
                    divergences['money_vs_bet_divergence_away'] = -row['home_divergence'] if row['home_divergence'] else None
                elif market == 'total':
                    divergences['money_vs_bet_divergence_over'] = row['over_divergence']  
                    divergences['money_vs_bet_divergence_under'] = -row['over_divergence'] if row['over_divergence'] else None
        ...
        try:
            ...
            if not avg_variances.is_empty():
                ...
                return {
                    ...
                    'sportsbook_variance_ml': Decimal(str(variances.get('avg_ml_var', 0) or 0)),
                    'sportsbook_variance_spread': Decimal(str(variances.get('avg_spread_var', 0) or 0)),
                    'sportsbook_variance_total': Decimal(str(variances.get('avg_total_var', 0) or 0)),
    ...
    def _extract_movement_patterns(self, df: pl.DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            ml_home_change = (last.get('home_ml_odds', 0) or 0) - (first.get('home_ml_odds', 0) or 0)
            ml_direction = 'stable'
            if abs(ml_home_change) > 5:  # Significant movement threshold
                ml_direction = 'toward_home' if ml_home_change < 0 else 'toward_away'  # Lower odds = better chance
            
            # Spread movement direction  
            spread_change = (last.get('home_spread_line', 0) or 0) - (first.get('home_spread_line', 0) or 0)
            ...
            total_change = (last.get('total_line', 0) or 0) - (first.get('total_line', 0) or 0)
            ...
            movement_data = df.group_by('sportsbook_name').agg([
                (pl.col('home_ml_odds').last() - pl.col('home_ml_odds').first()).alias('ml_change'),
                (pl.col('home_spread_line').last() - pl.col('home_spread_line').first()).alias('spread_change'),
                (pl.col('total_line').last() - pl.col('total_line').first()).alias('total_change')
            ])
