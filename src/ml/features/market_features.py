...
class MarketFeatureExtractor(BaseFeatureExtractor):
    ...
    async def extract_features(
        ...
        try:
            ...
            if not data_quality["is_valid"]:
                logger.warning(
                    f"Market data quality issues for game {game_id}: {data_quality['missing_columns']}"
                )
    ...
    def _extract_efficiency_features(self, df: pl.DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            movements = df.group_by(["sportsbook_name", "market_type"]).agg(
                [
                    (pl.col("home_ml_odds").diff().abs() > self.steam_move_threshold)
                    .sum()
                    .alias("ml_movements"),
                    (
                        pl.col("home_spread_odds").diff().abs()
                        > self.steam_move_threshold
                    )
                    .sum()
                    .alias("spread_movements"),
                    (pl.col("over_odds").diff().abs() > self.steam_move_threshold)
                    .sum()
                    .alias("total_movements"),
            ...
            if not movements.is_empty():
                total_movements = movements.select(
                    [
                        (
                            pl.col("ml_movements")
                            + pl.col("spread_movements")
                            + pl.col("total_movements")
                        )
                        .sum()
                        .alias("total")
                    ]
            ...
            data_frequency = df.height / max(
                (df.select("timestamp").unique().height), 1
            )
            liquidity_score = min(
                1.0, (unique_books / 10) * (data_frequency / 60)
            )  # Normalize to 0-1
    ...
    def _extract_steam_move_features(self, df: pl.DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            steam_data = (
                df.sort("timestamp")
                .group_by(["timestamp", "market_type"])
                .agg(
                    [
                        (
                            pl.col("home_ml_odds").diff().abs()
                            > self.steam_move_threshold
                        )
                        .sum()
                        .alias("ml_steam"),
                        ...
                        pl.col("home_spread_odds")
                        .diff()
                        .abs()
                        .max()
                        .alias("max_spread_move"),
            ...
            if not steam_moves.is_empty():
                magnitude_stats = steam_moves.select(
                    [
                        (
                            (
                                pl.col("max_ml_move")
                                + pl.col("max_spread_move")
                                + pl.col("max_total_move")
                            )
                            / 3
                        )
                        .mean()
                ...
                all_books = (
                    steam_moves.select("books_involved")
                    .to_series()
                    .explode()
                    .unique()
                    .to_list()
            ...
            return {
                ...
                "steam_move_magnitude": Decimal(str(avg_magnitude))
                if avg_magnitude > 0
                else None,
                "largest_steam_move": Decimal(str(largest_move))
                if largest_move > 0
                else None,
    ...
    def _extract_arbitrage_features(self, df: pl.DataFrame) -> Dict[str, Any]:
        ...
        try:
                ...
                arb_calcs = arbitrage_data.with_columns(
                    [
                        ...
                        (
                            1.0
                            - (
                                1.0 / pl.col("best_home_ml")
                                + 1.0 / pl.col("best_away_ml")
                            )
                        ).alias("ml_arb"),
                        # Spread arbitrage
                        (
                            1.0
                            - (
                                1.0 / pl.col("best_home_spread")
                                + 1.0 / pl.col("best_away_spread")
                            )
                        ).alias("spread_arb"),
                        # Total arbitrage
                        (
                            1.0
                            - (1.0 / pl.col("best_over") + 1.0 / pl.col("best_under"))
                        ).alias("total_arb"),
                    ]
                ).filter(
            ...
            return {
                "max_ml_arbitrage_opportunity": Decimal(str(max_ml_arb))
                if max_ml_arb > 0
                else None,
                "max_spread_arbitrage_opportunity": Decimal(str(max_spread_arb))
                if max_spread_arb > 0
                else None,
                "max_total_arbitrage_opportunity": Decimal(str(max_total_arb))
                if max_total_arb > 0
                else None,
                "arbitrage_duration_minutes": arb_duration
                if arb_duration > 0
                else None,
            }
    ...
    def _extract_consensus_features(self, df: pl.DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            participating_books = (
                df.select("sportsbook_name").unique().to_series().to_list()
            )
            participating_books = [
                book for book in participating_books if book is not None
            ]
            ...
            return {
                ...
                "sportsbook_consensus_strength": Decimal(str(consensus_strength))
    ...
    def _extract_market_depth_features(self, df: pl.DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            depth_data = df.group_by("timestamp").agg(
                [
                    (pl.col("home_ml_odds").max() - pl.col("home_ml_odds").min()).alias(
                        "ml_spread"
                    ),
                    (pl.col("over_odds").max() - pl.col("under_odds").min()).alias(
                        "total_spread"
                    ),
                ]
            )
            ...
            if best_ml_spread is not None and best_total_spread is not None:
                ...
                efficiency_score = 1.0 / (
                    1.0 + avg_spread / 100
                )  # Normalize spread impact

            return {
                "best_ml_spread": best_ml_spread,
                "best_total_spread": best_total_spread,
                "odds_efficiency_score": Decimal(str(efficiency_score))
    ...
    def _extract_line_movement_patterns(self, df: pl.DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            movements = (
                df.sort("timestamp")
                .group_by(["sportsbook_name", "market_type"])
                .agg(
                    [
                        (
                            pl.col("home_ml_odds").diff().abs()
            ...
            if not movements.is_empty():
                ...
                if not totals.is_empty():
                    ...
                    movement_frequency = (
                        total_movements / max(time_span, 1) if time_span > 0 else 0
                    )
            ...
            late_movement_indicator = (
                not late_movements.is_empty()
                and late_movements.height > df.height * 0.2
            )

            return {
                "total_line_movements": total_movements,
                "average_movement_magnitude": Decimal(str(avg_magnitude))
                if avg_magnitude > 0
                else None,
                "movement_frequency": Decimal(str(movement_frequency))
    ...
    def _extract_sharp_public_indicators(self, df: pl.DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            sharp_home = sharp_data.filter(
                pl.col("sharp_action_direction") == "home"
            ).height
            sharp_away = sharp_data.filter(
                pl.col("sharp_action_direction") == "away"
            ).height
            sharp_over = sharp_data.filter(
                pl.col("sharp_action_direction") == "over"
            ).height
            sharp_under = sharp_data.filter(
                pl.col("sharp_action_direction") == "under"
            ).height

            # Calculate public preferences (based on bet percentages)
            public_data = df.filter(pl.col("bet_percentage_home").is_not_null())
            ...
            if not public_data.is_empty():
                ...
                if not public_stats.is_empty():
                    ...
                    sharp_home_pct = (
                        sharp_home / max(sharp_home + sharp_away, 1)
                    ) * 100
                    sharp_over_pct = (
                        sharp_over / max(sharp_over + sharp_under, 1)
                    ) * 100
            ...
            return {
                "sharp_public_divergence_ml": Decimal(str(ml_divergence))
                if ml_divergence is not None
                else None,
                "sharp_public_divergence_spread": Decimal(str(spread_divergence))
                if spread_divergence is not None
                else None,
                "sharp_public_divergence_total": Decimal(str(total_divergence))
                if total_divergence is not None
                else None,
            }
        ...
        try:
            ...
            if not spread_data.is_empty():
                avg_spread = spread_data.select(
                    [
                        ((pl.col("ml_spread") + pl.col("total_spread")) / 2)
                        .mean()
                        .alias("avg_spread")
                    ]
                ).item()
            ...
            return {
                "bid_ask_spread_estimate": Decimal(str(bid_ask_estimate))
                if bid_ask_estimate is not None
                else None,
