...
class RedisFeatureStore:
    ...
    def __init__(self, redis_url: Optional[str] = None):
        ...
        self.redis_url = redis_url or getattr(self.settings, 'redis_url', 'redis://localhost:6379/0')
    ...
    async def cache_feature_vector(
        ...
        try:
            ...
            logger.debug(f"Cached feature vector for game {game_id} in {processing_time:.1f}ms")
            return True
    ...
    async def cache_batch_features(
        ...
        try:
            ...
            processing_time = (datetime.utcnow() - start_time).total_seconds() * 1000
            logger.info(f"Batch cached {successful_caches}/{len(feature_vectors)} feature vectors in {processing_time:.1f}ms")
    ...
    async def get_batch_features(
        ...
        try:
            ...
            for i in range(0, len(game_ids), self.max_batch_size):
                ...
                # Generate cache keys
                cache_keys = [self._generate_feature_key(game_id, feature_version) for game_id in batch_ids]
                ...
                for game_id, data in zip(batch_ids, cached_data):
                    if data is not None:
                        try:
                            ...
                            logger.warning(f"Error deserializing cached data for game {game_id}: {e}")
            ...
            logger.info(f"Batch retrieved {cache_hits}/{len(game_ids)} feature vectors in {processing_time:.1f}ms")
    ...
    async def invalidate_game_cache(self, game_id: int, feature_version: Optional[str] = None) -> bool:
        """
        ...
        try:
            ...
            if feature_version:
                ...
                logger.debug(f"Invalidated cache for game {game_id} version {feature_version}: {deleted} keys")
            else:
                ...
                if keys:
                    ...
                    logger.debug(f"Invalidated cache for game {game_id} all versions: {deleted} keys")
    ...
    async def cache_model_prediction(
        ...
        try:
            ...
            cache_key = f"{self.model_prediction_prefix}:game:{game_id}:model:{model_name}"
            ...
            logger.error(f"Error caching prediction for game {game_id} model {model_name}: {e}")
            return False
    ...
    async def get_model_prediction(
        ...
        try:
            ...
            logger.error(f"Error retrieving prediction for game {game_id} model {model_name}: {e}")
            return None
    ...
    def _update_stats(self, operation: str, processing_time_ms: float, success: bool):
        ...
        if operation == 'get':
            ...
            new_avg = ((current_avg * (total_gets - 1)) + processing_time_ms) / total_gets
            ...
            new_avg = ((current_avg * (total_sets - 1)) + processing_time_ms) / total_sets
