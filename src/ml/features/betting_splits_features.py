...
class BettingSplitsFeatureExtractor(BaseFeatureExtractor):
    ...
    def __init__(self, feature_version: str = "v2.1"):
        ...
        self.min_split_threshold = (
            1.0  # Minimum percentage difference to consider significant
        )
        self.sharp_action_sources = {
            "vsin",
            "action_network",
        }  # Sources with reliable sharp indicators
        self.consensus_sources = {
            "sbd",
            "action_network",
        }  # Sources for consensus analysis
    ...
    async def extract_features(
        ...
        try:
            ...
            if not data_quality["is_valid"]:
                logger.warning(
                    f"Betting splits data quality issues for game {game_id}: {data_quality['missing_columns']}"
                )
            ...
            if game_data.is_empty():
                logger.warning(
                    f"No betting splits data available for game {game_id} before cutoff"
                )
            ...
            logger.info(
                f"Successfully extracted betting splits features for game {game_id}"
            )
            return features

        except Exception as e:
            logger.error(
                f"Error extracting betting splits features for game {game_id}: {e}"
            )
            raise
    ...
    def _extract_source_attribution(self, df: pl.DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"data_sources": data_sources, "sportsbook_coverage": sportsbooks}
    ...
    def _extract_split_aggregates(self, df: pl.DataFrame) -> Dict[str, Any]:
        ...
        try:
                ...
                [
                    (
                        (
                            ...
                            - pl.col("collected_at").dt.timestamp()
                        )
                        / 3600
                    ).alias("hours_ago")
                ]
            ...
            aggregates = weighted_data.select(
                [
                    (pl.col("bet_percentage_home") * pl.col("time_weight")).sum()
                    / pl.col("time_weight").sum().alias("avg_bet_home"),
                    (pl.col("bet_percentage_away") * pl.col("time_weight")).sum()
                    / pl.col("time_weight").sum().alias("avg_bet_away"),
                    (pl.col("money_percentage_home") * pl.col("time_weight")).sum()
                    / pl.col("time_weight").sum().alias("avg_money_home"),
                    (pl.col("money_percentage_away") * pl.col("time_weight")).sum()
                    / pl.col("time_weight").sum().alias("avg_money_away"),
                    (pl.col("bet_percentage_over") * pl.col("time_weight")).sum()
                    / pl.col("time_weight").sum().alias("avg_bet_over"),
                    (pl.col("bet_percentage_under") * pl.col("time_weight")).sum()
                    / pl.col("time_weight").sum().alias("avg_bet_under"),
                    (pl.col("money_percentage_over") * pl.col("time_weight")).sum()
                    / pl.col("time_weight").sum().alias("avg_money_over"),
                    (pl.col("money_percentage_under") * pl.col("time_weight")).sum()
                    / pl.col("time_weight").sum().alias("avg_money_under"),
                ]
            )
            ...
            return {
                "avg_bet_percentage_home": Decimal(
                    str(agg_row.get("avg_bet_home", 0) or 0)
                ),
                "avg_bet_percentage_away": Decimal(
                    str(agg_row.get("avg_bet_away", 0) or 0)
                ),
                "avg_money_percentage_home": Decimal(
                    str(agg_row.get("avg_money_home", 0) or 0)
                ),
                "avg_money_percentage_away": Decimal(
                    str(agg_row.get("avg_money_away", 0) or 0)
                ),
                "avg_bet_percentage_over": Decimal(
                    str(agg_row.get("avg_bet_over", 0) or 0)
                ),
                "avg_bet_percentage_under": Decimal(
                    str(agg_row.get("avg_bet_under", 0) or 0)
                ),
                "avg_money_percentage_over": Decimal(
                    str(agg_row.get("avg_money_over", 0) or 0)
                ),
                "avg_money_percentage_under": Decimal(
                    str(agg_row.get("avg_money_under", 0) or 0)
                ),
            }
    ...
    def _extract_sharp_action_features(self, df: pl.DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            strength_counts = sharp_data.group_by("sharp_action_strength").agg(
                pl.len().alias("count")
            )
            ...
            if not strength_counts.is_empty():
                ...
                if not max_strength.is_empty():
                    dominant_strength = (
                        max_strength.select("sharp_action_strength").item() or "weak"
                    )
            ...
            return {
                ...
                "sharp_action_strength": dominant_strength
                if sharp_signals > 0
                else None,
        ...
        try:
            ...
            divergence_data = df.filter(
                ...
                [
                    ...
                    (pl.col("money_percentage_away") - pl.col("bet_percentage_away"))
                    .mean()
                    .alias("away_divergence"),
                    (pl.col("money_percentage_over") - pl.col("bet_percentage_over"))
                    .mean()
                    .alias("over_divergence"),
                    (pl.col("money_percentage_under") - pl.col("bet_percentage_under"))
                    .mean()
                    .alias("under_divergence"),
                ]
            )
            ...
            return {
                "home_money_bet_divergence": Decimal(
                    str(div_row.get("home_divergence", 0) or 0)
                ),
                "away_money_bet_divergence": Decimal(
                    str(div_row.get("away_divergence", 0) or 0)
                ),
                "over_money_bet_divergence": Decimal(
                    str(div_row.get("over_divergence", 0) or 0)
                ),
                "under_money_bet_divergence": Decimal(
                    str(div_row.get("under_divergence", 0) or 0)
                ),
            }
    ...
    def _extract_variance_features(self, df: pl.DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                "bet_percentage_variance_home": Decimal(
                    str(var_row.get("bet_home_var", 0) or 0)
                ),
                "money_percentage_variance_home": Decimal(
                    str(var_row.get("money_home_var", 0) or 0)
                ),
                "bet_percentage_variance_over": Decimal(
                    str(var_row.get("bet_over_var", 0) or 0)
                ),
                "money_percentage_variance_over": Decimal(
                    str(var_row.get("money_over_var", 0) or 0)
                ),
            }
    ...
    def _extract_weighted_features(self, df: pl.DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            weighted_data = df.with_columns(
                [
                    pl.col("sportsbook_name")
                    .str.to_lowercase()
                    .map_elements(
                        lambda x: sportsbook_weights.get(
                            x, 0.3
                        ),  # Default weight for unknown books
            ...
            weighted_stats = weighted_data.select(
                [
                    ...
                    / pl.col("book_weight").sum().alias("weighted_sharp_score"),
            ...
            return {
                "weighted_sharp_action_score": Decimal(
                    str(stats_row.get("weighted_sharp_score", 0) or 0)
                ),
                "weighted_public_sentiment": Decimal(
                    str(stats_row.get("weighted_public_sentiment", 50) or 50)
                ),
            }
    ...
    def _extract_source_specific_features(self, df: pl.DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            sbd_consensus_strength = min(
                1.0, sbd_book_count / 9.0
            )  # SBD covers 9+ books
            ...
            return {
                ...
                "sbd_consensus_strength": Decimal(str(sbd_consensus_strength))
                if sbd_consensus_strength > 0
                else None,
    ...
    def _create_empty_features(self) -> BettingSplitsFeatures:
        ...
        return BettingSplitsFeatures(
            feature_version=self.feature_version, last_updated=datetime.utcnow()
        )
