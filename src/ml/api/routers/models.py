...
@router.get(
    "/models/{model_name}/performance", response_model=List[ModelPerformanceResponse]
)
async def get_model_performance(
    ...
    days: Optional[int] = Query(
        30, ge=1, le=365, description="Performance period in days"
    ),
    ml_service: PredictionService = Depends(get_ml_service),
...
async def get_model_leaderboard(
    ...
    limit: Optional[int] = Query(
        10, ge=1, le=50, description="Number of models to return"
    ),
    ml_service: PredictionService = Depends(get_ml_service),
) -> List[Dict[str, Any]]:
    """
    ...
    try:
        ...
        leaderboard = await ml_service.get_model_leaderboard(
            metric=metric, prediction_type=prediction_type, days=days, limit=limit
        )
        ...
        raise HTTPException(
            status_code=500, detail="Failed to retrieve model leaderboard"
        )
...
async def get_model_recent_predictions(
    ...
    limit: Optional[int] = Query(
        50, ge=1, le=200, description="Maximum predictions to return"
    ),
    ml_service: PredictionService = Depends(get_ml_service),
) -> List[Dict[str, Any]]:
    """
    ...
    try:
        ...
        predictions = await ml_service.get_model_recent_predictions(
            model_name=model_name, model_version=model_version, days=days, limit=limit
        )
