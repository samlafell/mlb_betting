...
class BatchPredictionRequest(BaseModel):
    ...
    game_ids: List[str] = Field(
        ..., max_items=50, description="List of game IDs (max 50)"
    )
    model_name: Optional[str] = Field(None, description="Specific model to use")
...
async def predict_game(
    ...
    try:
        logger.info(
            f"Prediction request for game {request.game_id} by user {current_user.get('user_id', 'unknown')}"
        )
...
async def predict_games_batch(
    ...
    try:
        logger.info(
            f"Batch prediction request for {len(request.game_ids)} games by user {current_user.get('user_id', 'unknown')}"
        )

        if len(request.game_ids) > 50:
            raise HTTPException(
                status_code=400, detail="Maximum 50 games allowed per batch request"
            )
        ...
        raise HTTPException(
            status_code=500, detail="Failed to generate batch predictions"
        )
...
async def get_cached_prediction(
    ...
    try:
        ...
        prediction = await ml_service.get_cached_prediction(
            game_id=game_id, model_name=model_name
        )

        if not prediction:
            raise HTTPException(
                status_code=404, detail=f"No cached prediction found for game {game_id}"
            )
        ...
        raise HTTPException(
            status_code=500, detail="Failed to retrieve cached prediction"
        )
...
async def get_todays_predictions(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Failed to retrieve today's predictions"
        )
