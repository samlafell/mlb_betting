...
class LightGBMTrainer:
    ...
    async def train_models(
        ...
        try:
            logger.info(f"Starting LightGBM training pipeline: {start_date} to {end_date}")
            ...
            for target in prediction_targets:
                ...
                with mlflow.start_run(run_name=f"{target}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"):
                    ...
                    X, y, feature_names = await self._prepare_target_dataset(training_data, target)
                    
                    if len(X) < 50:
                        logger.warning(f"Insufficient data for target {target}: {len(X)} samples")
                        continue
    ...
    async def retrain_model(
        ...
        """
        try:
            logger.info(f"Retraining model {model_name} with {sliding_window_days}-day window")
            ...
            # Load recent data
            recent_data = await self._load_training_data(start_date, end_date, use_cached_features=True)
            
            if not recent_data or len(recent_data) < min_samples:
                logger.warning(f"Insufficient recent data for retraining: {len(recent_data) if recent_data else 0}")
                return {'status': 'skipped', 'reason': 'insufficient_data'}
            
            # Retrain model
            with mlflow.start_run(run_name=f"{model_name}_retrain_{datetime.now().strftime('%Y%m%d_%H%M%S')}"):
                X, y, feature_names = await self._prepare_target_dataset(recent_data, model_name)
                ...
                drift_detected = self._detect_feature_drift(retrain_results['feature_importance'], model_name)
                
                retrain_results['drift_detected'] = drift_detected
                retrain_results['sliding_window_days'] = sliding_window_days
                
                await self._log_model_results(retrain_results, model_name, feature_names, is_retrain=True)
                
                logger.info(f"Model {model_name} retrained successfully. Drift detected: {drift_detected}")
    ...
    async def evaluate_model_performance(
        ...
        """
        try:
            logger.info(f"Evaluating model {model_name}: {evaluation_start} to {evaluation_end}")
            
            # Load evaluation data
            eval_data = await self._load_training_data(evaluation_start, evaluation_end, use_cached_features=True)
            ...
            X, y_true, feature_names = await self._prepare_target_dataset(eval_data, model_name)
    ...
    async def _load_training_data(
        ...
        try:
            ...
            if not games:
                logger.warning(f"No games found in training period: {start_date} to {end_date}")
                return []
            ...
            for game in games:
                ...
                if feature_vector is None:
                    ...
                    if feature_vector and use_cached_features:
                        await self.redis_store.cache_feature_vector(game_id, feature_vector)
    ...
    async def _prepare_target_dataset(
        ...
        try:
            ...
            logger.info(f"Prepared dataset for {target}: {X.shape[0]} samples, {X.shape[1]} features")
    ...
    def _feature_vector_to_array(self, feature_vector: FeatureVector) -> Optional[np.ndarray]:
        ...
        try:
            ...
            if feature_vector.betting_splits_features:
                ...
                for key, value in splits_dict.items():
                    if key not in ['feature_version', 'last_updated', 'data_sources', 'sportsbook_coverage']:
                        if isinstance(value, Decimal):
            ...
            feature_dict['completeness_score'] = float(feature_vector.feature_completeness_score)
    ...
    def _get_feature_names(self) -> List[str]:
        ...
        base_features = [
            'temporal_minutes_before_game', 'temporal_sharp_action_intensity_60min',
            'temporal_opening_to_current_ml', 'temporal_money_vs_bet_divergence_home',
            'market_line_stability_score', 'market_steam_move_indicators',
            'market_arbitrage_opportunity', 'market_consensus_strength',
            'team_home_win_pct', 'team_away_win_pct', 'team_home_recent_form',
            'team_away_recent_form', 'team_h2h_home_advantage',
            'splits_avg_money_home', 'splits_sharp_action_signals',
            'splits_consensus_ml', 'splits_weighted_sharp_score',
            'derived_combined_sharp_intensity', 'derived_market_efficiency',
            'interaction_sharp_public_home', 'interaction_consensus_team_strength',
            'completeness_score', 'source_coverage', 'total_features'
        ]
    ...
    async def _train_target_model(
        ...
        try:
            ...
            cv_scores = await self._perform_cross_validation(X_train, y_train, model_config, cv_folds)
        ...
        try:
            ...
            for train_idx, val_idx in tscv.split(X):
                ...
                if model_config['objective'] == 'binary':
                    ...
                    fold_score = -np.sqrt(np.mean((y_cv_val - y_pred) ** 2))  # Negative RMSE
    ...
    async def _log_model_results(
        ...
        try:
            ...
            importance_dict = {f"importance_{k}": v for k, v in results['feature_importance'].items()}
    ...
    def _detect_feature_drift(
        ...
        try:
            ...
            logger.info(f"Feature drift analysis for {model_name}: avg_drift={avg_drift:.3f}, threshold={drift_threshold}")
    ...
    def _update_training_stats(self, training_results: Dict[str, Any], training_time: float):
        """Update training statistics"""
        ...
        new_avg = ((current_avg * (total_experiments - len(training_results))) + training_time) / total_experiments
        ...
        for target, results in training_results.items():
            ...
            for metric, value in results['test_metrics'].items():
                current_best = self.training_stats['best_model_scores'][target].get(metric, -float('inf'))
