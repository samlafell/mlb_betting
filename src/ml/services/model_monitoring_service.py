...
class ModelMonitoringService:
        ...
        try:
            ...
            for metrics in recent_metrics:
                model_alerts = await self._evaluate_model_performance(
                    metrics, baseline_metrics.get(f"{metrics.model_name}_{metrics.prediction_type}")
                )
            ...
            if alerts:
                ...
                logger.info(f"No performance issues detected for {len(recent_metrics)} models")
    ...
    def _check_relative_thresholds(
        ...
        if accuracy_drop > self.thresholds['accuracy_drop']:
            alerts.append(PerformanceAlert(
                ...
                threshold_value=baseline.accuracy - self.thresholds['accuracy_drop'],
        ...
        if roi_drop > self.thresholds['roi_drop']:
            alerts.append(PerformanceAlert(
                ...
                threshold_value=baseline.roi_percentage - self.thresholds['roi_drop'],
        ...
        if sharpe_drop > self.thresholds['sharpe_ratio_drop']:
            alerts.append(PerformanceAlert(
                ...
                threshold_value=baseline.sharpe_ratio - self.thresholds['sharpe_ratio_drop'],
    ...
    async def store_alerts(self, alerts: List[PerformanceAlert]) -> bool:
        ...
        try:
            async with get_db_connection() as conn:
                ...
                logger.info(f"Stored {len(alerts)} performance alerts with MLFlow context")
                return True
    ...
    async def _get_mlflow_context(self, model_name: str, prediction_type: str) -> Dict[str, Any]:
