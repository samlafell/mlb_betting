...
class FeatureDriftDetectionService:
    ...
    async def detect_feature_drift(
        self, model_name: str, model_version: str = None, lookback_days: int = 7
        ...
        try:
            ...
            missing_features = set(baseline_features.keys()) - set(
            ...
            logger.info(
                f"Drift detection completed for {model_name}: {len(drift_results)} features analyzed"
            )
    ...
    async def _analyze_feature_drift(
        ...
        try:
            ...
            drift_detected = (
                importance_drift_pct > self.thresholds["importance_drift"]
                or drift_score > self.thresholds["distribution_drift"]
            )
    ...
    async def store_drift_results(
        ...
        try:
            ...
            async with get_db_transaction() as conn:
                ...
                logger.info(
                    f"Stored {len(drift_results)} drift detection results for {model_name} with MLFlow context"
                )
                return True
        ...
        try:
            ...
            if experiments:
                ...
                return {
                    ...
                    "baseline_run_id": latest_model["run_id"]
                    if latest_model
                    else None,  # Use same run as baseline for now
                }
            ...
            logger.warning(
                f"Could not get MLFlow context for drift detection of {model_name}: {e}"
            )
            return {}
    ...
    async def run_drift_detection_cycle(self, model_name: str = None) -> Dict[str, Any]:
        ...
        try:
            ...
            drift_detected_count = len(
                [r for r in total_drift_results if r.drift_detected]
            )
