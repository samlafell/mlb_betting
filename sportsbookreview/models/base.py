...
class SportsBookReviewTimestampedModel(SportsbookReviewBaseModel, TimestampedModel):
    ...
    source_url: Optional[str] = Field(
        default=None,
        description="Source URL from SportsbookReview.com",
        max_length=500
    )
    
    scrape_timestamp: Optional[datetime] = Field(
        default=None,
        description="When this data was scraped from SportsbookReview"
    )
    
    @field_validator("scrape_timestamp", mode="before")
    def parse_scrape_timestamp(cls, v: Any) -> Optional[datetime]:
        """
        ...
        if odds_str.startswith(('+', '-')):
            try:
                ...
                if not (-50000 <= odds_value <= 50000):
                    raise ValueError(f"{field} odds value {odds_value} is outside reasonable bounds")
            except ValueError:
                raise ValueError(f"{field} must be a valid American odds format (+/-XXX)")
        else:
            # Decimal odds format
            try:
    ...
    def validate_spread_format(self, field: str) -> None:
        ...
        try:
            ...
            if not (-20.0 <= spread_value <= 20.0):
                raise ValueError(f"{field} spread value {spread_value} is outside reasonable bounds for MLB")
    ...
    def validate_total_format(self, field: str) -> None:
        ...
        try:
            ...
            if not (2.0 <= total_value <= 30.0):
                raise ValueError(f"{field} total value {total_value} is outside reasonable bounds for MLB")
        except (ValueError, TypeError):
