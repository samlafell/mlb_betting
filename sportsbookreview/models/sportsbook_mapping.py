...
class MarketAvailability(str, Enum):
    ...
    CONDITIONALLY_AVAILABLE = "conditionally_available"  # Offered under certain conditions
...
class SportsbookCapabilities(SportsbookReviewBaseModel):
    ...
    supports_live_betting: bool = Field(
        default=False,
        description="Whether sportsbook offers live betting"
    )
    
    supports_props: bool = Field(
        default=False,
        description="Whether sportsbook offers prop bets"
    )
    
    supports_parlays: bool = Field(
        default=False,
        description="Whether sportsbook offers parlay bets"
    )
    
    supports_early_cash_out: bool = Field(
        default=False,
        description="Whether sportsbook offers early cash out"
    )
    
    # Odds characteristics
    typical_juice_percentage: Optional[float] = Field(
        default=None,
    ...
    odds_update_frequency_minutes: Optional[int] = Field(
        default=None,
    ...
    minimum_bet_amount: Optional[float] = Field(
        default=None,
        description="Minimum bet amount in USD",
        ge=0.01
    )
    
    maximum_bet_amount: Optional[float] = Field(
        default=None,
    ...
    available_states: Optional[List[str]] = Field(
        default=None,
        description="US states where sportsbook is available"
    )
    
    available_countries: Optional[List[str]] = Field(
        default=None,
        description="Countries where sportsbook is available"
    )
...
class MarketMapping(SportsbookReviewBaseModel):
    ...
    display_name: Optional[str] = Field(
        default=None,
        description="How sportsbook displays this market",
        max_length=100
    )
    ...
    typical_lines: Optional[List[float]] = Field(
        default=None,
        description="Typical line values offered (e.g., [-1.5, -2.5] for spreads)"
    )
    
    line_increment: Optional[float] = Field(
        default=None,
    ...
    # Juice/vig patterns
    typical_juice_range: Optional[Dict[str, float]] = Field(
        default=None,
        description="Typical juice range for this market (min/max)"
    )
    
    # Market timing
    opens_hours_before_game: Optional[float] = Field(
        default=None,
        description="Hours before game this market typically opens",
        ge=0.0
    )
    
    closes_minutes_before_game: Optional[float] = Field(
        default=None,
    ...
    @classmethod
    def validate_juice_range(cls, v: Optional[Dict[str, float]]) -> Optional[Dict[str, float]]:
...
class SportsbookMapping(SportsBookReviewTimestampedModel):
    ...
    display_name: str = Field(
        ...,
        description="Human-readable sportsbook name",
        min_length=1,
        max_length=100
    )
    ...
    base_url: Optional[str] = Field(
        default=None,
        description="Base URL for sportsbook website",
        max_length=200
    )
    
    odds_page_pattern: Optional[str] = Field(
        default=None,
        description="URL pattern for odds pages",
        max_length=300
    )
    ...
    market_mappings: Dict[str, MarketMapping] = Field(
        ...
        description="Market mappings by bet type"
    )
    
    # Scraping and parsing metadata
    parsing_rules: Optional[Dict[str, Any]] = Field(
        default=None,
        description="Custom parsing rules for this sportsbook"
    )
    
    rate_limit_requests_per_minute: Optional[int] = Field(
        default=None,
        description="Rate limit for scraping requests",
        ge=1,
        le=1000
    )
    
    rate_limit_delay_seconds: Optional[float] = Field(
        default=None,
    ...
    is_active: bool = Field(
        default=True,
        description="Whether sportsbook is actively tracked"
    )
    ...
    consecutive_failures: int = Field(
        default=0,
        description="Number of consecutive scraping failures",
        ge=0
    )
    
    # Data quality tracking
    data_quality_metrics: Optional[Dict[str, Any]] = Field(
        default=None,
        description="Data quality metrics for this sportsbook"
    )
    ...
    @classmethod
    def validate_market_mappings(cls, v: Dict[str, MarketMapping]) -> Dict[str, MarketMapping]:
    ...
    def supported_bet_types(self) -> Set[BetType]:
        ...
        return {
            ...
            if mapping.availability in [MarketAvailability.ALWAYS_AVAILABLE, MarketAvailability.CONDITIONALLY_AVAILABLE]
        }
    
    @computed_field
    @property
    def average_juice_percentage(self) -> Optional[float]:
    ...
    def get_market_mapping(self, bet_type: BetType) -> Optional[MarketMapping]:
        """
