...
class OddsSnapshot(SportsbookReviewBaseModel):
    ...
    odds_value: str | int | float | None = Field(
        default=None, description="Odds value in sportsbook format"
    )

    # American odds equivalent (for standardization)
    american_odds: int | None = Field(
        default=None, description="Odds in American format (+/-XXX)"
    )

    # Decimal odds equivalent
    decimal_odds: float | None = Field(
        default=None, description="Odds in decimal format", ge=1.0
    )
    ...
    status: LineStatus = Field(
        default=LineStatus.ACTIVE, description="Status of the betting line"
    )

    # Timestamp when odds were observed
    timestamp: datetime = Field(
        default_factory=datetime.now, description="When these odds were observed"
    )
    ...
    @classmethod
    def validate_decimal_odds(cls, v: float | None) -> float | None:
...
class LineMovementData(SportsbookReviewBaseModel):
    ...
    line_value: float | None = Field(
        default=None, description="Line value (e.g., -1.5 for spread, 9.5 for total)"
    )

    # Odds snapshots over time
    odds_history: list[OddsSnapshot] = Field(
        default_factory=list, description="Historical odds values"
    )
    ...
    movement_direction: OddsMovement = Field(
        default=OddsMovement.STABLE, description="Overall movement direction"
    )

    total_movements: int = Field(
        default=0, description="Total number of line movements", ge=0
    )

    volatility_score: float | None = Field(
        default=None, description="Volatility score (0.0-1.0)", ge=0.0, le=1.0
    )

    @computed_field
    @property
    def odds_range(self) -> dict[str, int | float] | None:
        ...
        american_odds = [
            snap.american_odds
            for snap in self.odds_history
            if snap.american_odds is not None
        ]
    ...
    def _analyze_movement(self) -> None:
        ...
        for snapshot in self.odds_history:
            if previous_odds is not None and snapshot.american_odds is not None:
                if (
                    abs(snapshot.american_odds - previous_odds) >= 5
        ...
        if self.opening_odds and self.current_odds:
            if (
                self.opening_odds.american_odds is not None
                and self.current_odds.american_odds is not None
            ):
                ...
                if (
                    movements > len(self.odds_history) * 0.3
                ):  # 30% of snapshots had movements
        ...
        if len(self.odds_history) > 1:
            ...
            if len(odds_values) > 1:
                ...
                variance = sum((x - mean_odds) ** 2 for x in odds_values) / len(
                    odds_values
                )
...
class OddsData(SportsBookReviewTimestampedModel, SportsbookReviewValidatedModel):
    ...
    # Core identification
    game_id: str = Field(
        ..., description="Associated game identifier", min_length=1, max_length=100
    )

    # Game information (denormalized for easier querying)
    home_team: str = Field(
        ..., description="Home team abbreviation", min_length=2, max_length=5
    )

    away_team: str = Field(
        ..., description="Away team abbreviation", min_length=2, max_length=5
    )
    ...
    sportsbook: SportsbookName = Field(..., description="Sportsbook name")

    bet_type: BetType = Field(..., description="Type of bet")

    market_side: MarketSide = Field(..., description="Side/option being bet")
    ...
    odds_format: OddsFormat = Field(
        default=OddsFormat.AMERICAN, description="Primary odds format used"
    )

    market_open_time: datetime | None = Field(
        default=None, description="When market opened for betting"
    )

    market_close_time: datetime | None = Field(
        default=None, description="When market closed"
    )

    last_update_time: datetime = Field(
        default_factory=datetime.now, description="Last time odds were updated"
    )

    # Data quality and validation
    data_quality: DataQuality = Field(
        default=DataQuality.MEDIUM, description="Data quality assessment"
    )

    validation_errors: list[str] = Field(
        default_factory=list, description="List of validation errors encountered"
    )
    ...
    # Juice/vig analysis
    juice_percentage: float | None = Field(
        default=None, description="Estimated juice/vig percentage", ge=0.0, le=100.0
    )
    ...
    @computed_field
    @property
    def current_odds(self) -> OddsSnapshot | None:
    ...
    def add_odds_update(
        ...
        snapshot = OddsSnapshot(
            odds_value=odds_value, timestamp=timestamp, status=status
        )
        ...
        if isinstance(odds_value, str):
            ...
            try:
                ...
                self.validation_errors.append(
                    f"Unable to parse odds value: {odds_value}"
                )
                return
            ...
            for i in range(1, len(self.line_movement.odds_history)):
                ...
                if prev_odds is not None and curr_odds is not None:
                    ...
                    if change > 1000:  # Massive odds swing
                        issues.append(
                            f"Suspicious odds movement: {prev_odds} to {curr_odds}"
                        )
