...
def create_sample_enhanced_game() -> EnhancedGame:
    ...
    home_pitcher = PitcherInfo(
        player_id=592789, full_name="Logan Webb", throws="R", era=3.25, wins=8, losses=5
    )
    ...
    condition_str = (
        game.weather_data.condition.value
        if hasattr(game.weather_data.condition, "value")
        else str(game.weather_data.condition)
    )
    print(f"   Weather: {condition_str} ({game.weather_data.temperature}°F)")
    print(f"   Pitching: {game.pitcher_matchup.handedness_matchup}")
    quality_str = (
        game.data_quality.value
        if hasattr(game.data_quality, "value")
        else str(game.data_quality)
    )
...
def create_sample_odds_data(game_id: str) -> OddsData:
    ...
    bet_type_str = (
        ...
        if hasattr(odds_data.bet_type, "value")
        else str(odds_data.bet_type)
    )
    market_side_str = (
        odds_data.market_side.value
        if hasattr(odds_data.market_side, "value")
        else str(odds_data.market_side)
    )
    sportsbook_str = (
        odds_data.sportsbook.value
        if hasattr(odds_data.sportsbook, "value")
        else str(odds_data.sportsbook)
    )
    movement_str = (
        odds_data.line_movement.movement_direction.value
        if hasattr(odds_data.line_movement.movement_direction, "value")
        else str(odds_data.line_movement.movement_direction)
    )
...
def create_sample_sportsbook_mapping() -> SportsbookMapping:
    ...
    supported_types = [
        ...
        for bt in sportsbook_mapping.supported_bet_types
    ]
...
async def test_mlb_enrichment_service():
    ...
    try:
        ...
        correlation = MLBGameCorrelation(
            ...
            venue_info=VenueInfo(
                venue_id=22, venue_name="Dodger Stadium", city="Los Angeles", state="CA"
            ),
            weather_data=WeatherData(condition=WeatherCondition.CLEAR, temperature=72),
        )

        print("✅ Simulated enrichment correlation:")
        print(f"   MLB Game ID: {correlation.mlb_game_id}")
        print(f"   Confidence: {correlation.confidence_score}")
        print(
            f"   Venue: {correlation.venue_info.venue_name if correlation.venue_info else 'N/A'}"
        )
...
async def main():
    ...
    print(
        f"   Market available: {sportsbook_mapping.is_market_available(odds_data.bet_type)}"
    )
    print()
