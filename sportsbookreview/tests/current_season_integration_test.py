...
from ..services.mlb_api_service import MLBAPIService, MLBGameInfo
from ..models.game import EnhancedGame, Team, GameType, GameStatus
...
class CurrentSeasonIntegrationTest:
    ...
    async def _test_live_data_quality(self) -> Dict[str, Any]:
            ...
            for days_back in range(14):
                ...
                for game in games:
                    ...
                    game_info = {
                        ...
                        "datetime": game.game_datetime.isoformat() if game.game_datetime else None,
                    ...
                    if issues:
                        results["data_quality_issues"].extend([
                            f"{date_str} - Game {game.game_pk}: {issue}" for issue in issues
                        ])
        ...
        print(f"🏟️ Team coverage: {results['team_coverage']['unique_teams']}/30 teams ({results['team_coverage']['coverage_percentage']:.1%})")
        print(f"🏟️ Venue coverage: {results['venue_coverage']['unique_venues']} unique venues")
    ...
    async def _test_live_correlation(self) -> Dict[str, Any]:
        ...
        print(f"  Perfect matches (≥95%): {results['accuracy_stats']['perfect_matches']}/{total_tests}")
        print(f"  High confidence (≥80%): {results['accuracy_stats']['high_confidence']}/{total_tests}")
        print(f"  Medium confidence (≥50%): {results['accuracy_stats']['medium_confidence']}/{total_tests}")
        print(f"  Low confidence (>0%): {results['accuracy_stats']['low_confidence']}/{total_tests}")
    ...
    async def _test_double_header_handling(self) -> Dict[str, Any]:
        ...
        async with MLBAPIService() as mlb_service:
            ...
            for days_back in range(30):  # Check last month
                ...
                team_games = {}
                for game in games:
                    team_pair = tuple(sorted([game.home_team_name, game.away_team_name]))
                ...
                for team_pair, team_games_list in team_games.items():
                    if len(team_games_list) > 1:
                        ...
                        for game in team_games_list:
                            game_info = {
                                "game_pk": game.game_pk,
                                "time": game.game_datetime.strftime("%H:%M") if game.game_datetime else None,
                            ...
                            try:
                                ...
                                logger.warning(f"Correlation failed for double header game {game.game_pk}: {e}")
        ...
        if results["correlation_accuracy"]:
            avg_confidence = sum(r["confidence"] for r in results["correlation_accuracy"]) / len(results["correlation_accuracy"])
            print(f"📊 Average correlation confidence for double headers: {avg_confidence:.2f}")
    ...
    async def _test_live_performance(self) -> Dict[str, Any]:
        ...
        avg_api_time = sum(results["api_response_times"]) / len(results["api_response_times"])
        avg_correlation_time = sum(results["correlation_times"]) / len(results["correlation_times"]) if results["correlation_times"] else 0
        
        print(f"⏱️ Average API response time: {avg_api_time:.3f}s")
        print(f"🔗 Average correlation time: {avg_correlation_time:.3f}s")
        print(f"💾 Cache statistics: {results['cache_performance']['in_memory']['cache_size']} entries")
    ...
    async def _test_data_consistency(self) -> Dict[str, Any]:
        ...
        async with MLBAPIService() as mlb_service:
            ...
            if len(games1) == len(games2):
                ...
                for g1, g2 in zip(games1, games2):
                    if (g1.game_pk == g2.game_pk and 
                        g1.home_team_name == g2.home_team_name and
                        g1.away_team_name == g2.away_team_name):
                        game_matches += 1
        ...
        print(f"📊 Game status tracking: {len(results['game_status_changes'])} status records")
    ...
    async def _test_full_integration(self) -> Dict[str, Any]:
        ...
        success_count = sum(1 for test in results["integration_tests"] if test.get("success", False))
        print(f"🎯 Integration tests: {success_count}/{len(results['integration_tests'])} passed")
    ...
    async def _generate_report(self, results: Dict[str, Any]):
        ...
        with open(summary_file, 'w') as f:
            ...
            f.write(f"Live Data Quality:\n")
            f.write(f"  Total games found: {live_data['total_games_found']}\n")
            f.write(f"  Team coverage: {live_data['team_coverage']['unique_teams']}/30\n")
            f.write(f"  Data quality issues: {len(live_data['data_quality_issues'])}\n\n")
            
            # Correlation summary
            correlation = results['correlation_tests']
            total_correlation_tests = len(correlation['correlation_tests'])
            f.write(f"Correlation Accuracy:\n")
            f.write(f"  Total tests: {total_correlation_tests}\n")
            f.write(f"  Perfect matches: {correlation['accuracy_stats']['perfect_matches']}\n")
            f.write(f"  High confidence: {correlation['accuracy_stats']['high_confidence']}\n")
            f.write(f"  Failures: {correlation['accuracy_stats']['failures']}\n\n")
            
            # Double header summary
            double_headers = results['double_header_tests']
            f.write(f"Double Header Handling:\n")
            f.write(f"  Double headers found: {len(double_headers['double_headers_found'])}\n")
            f.write(f"  Days with double headers: {len(double_headers['double_header_days'])}\n\n")
            
            # Performance summary
            performance = results['performance_tests']
            if performance['api_response_times']:
                avg_api_time = sum(performance['api_response_times']) / len(performance['api_response_times'])
                f.write(f"Performance:\n")
                f.write(f"  Average API response time: {avg_api_time:.3f}s\n")
                f.write(f"  Cache entries: {performance['cache_performance']['in_memory']['cache_size']}\n\n")
            
            # Integration summary
            integration = results['integration_tests']
            success_count = sum(1 for test in integration['integration_tests'] if test.get('success', False))
            f.write(f"Integration Pipeline:\n")
            f.write(f"  Tests passed: {success_count}/{len(integration['integration_tests'])}\n")
...
async def run_current_season_test(output_dir: str = "current_season_test_output") -> Dict[str, Any]:
    """
