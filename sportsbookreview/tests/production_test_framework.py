...
from ..services.mlb_api_service import MLBAPIService, MLBGameInfo, CorrelationResult
...
class ProductionTestFramework:
            ...
            async with MLBAPIService() as mlb_service:
                ...
                try:
                    ...
                    suite.results.append(TestResult(
                        ...
                        details={"games_found": len(games), "test_date": str(today)},
                ...
                try:
                    ...
                    for days_back in range(7):  # Test last 7 days
                        ...
                        for game in games:
                            ...
                            if not game.game_pk:
                                quality_issues.append(f"Missing game_pk for {test_date}")
                            if not game.home_team_name or not game.away_team_name:
                                quality_issues.append(f"Missing team names for game {game.game_pk}")
                            if not game.game_datetime:
                                quality_issues.append(f"Missing datetime for game {game.game_pk}")
                            if game.home_team_name == game.away_team_name:
                                quality_issues.append(f"Same home/away team for game {game.game_pk}")
                    ...
                    if not quality_issues:
                        suite.results.append(TestResult(
                            ...
                            details={"total_games_checked": total_games, "days_checked": 7},
                            execution_time=execution_time
                        ))
                    else:
                        suite.results.append(TestResult(
                            ...
                            details={"issues": quality_issues[:10], "total_issues": len(quality_issues)},
                            execution_time=execution_time
                        ))
        ...
        try:
            async with MLBAPIService() as mlb_service:
                ...
                if games:
                    ...
                    bulk_correlation_time = (datetime.now() - start_time).total_seconds()
                    avg_time_per_correlation = bulk_correlation_time / correlation_count if correlation_count > 0 else 0
        ...
        try:
            async with MLBAPIService() as mlb_service:
                ...
                suite.results.append(TestResult(
                    ...
                    passed=team_coverage > 0.5,  # Should see at least half the teams
    ...
    async def generate_test_report(self, test_suites: Dict[str, TestSuite]):
        ...
        for suite_name, suite in test_suites.items():
            status = "✅ PASS" if suite.success_rate == 1.0 else "❌ FAIL" if suite.success_rate == 0.0 else "⚠️  PARTIAL"
            print(f"{status} {suite.name}: {suite.passed_tests}/{suite.total_tests} tests passed ({suite.success_rate:.1%})")
