...
from ..models.odds_data import LineMovementData, MarketSide, OddsData
from ..models.sportsbook_mapping import SportsbookName
from .base_parser import ParseError, SportsbookReviewBaseParser
...
class SportsbookReviewParser(SportsbookReviewBaseParser):
    ...
    def parse_page(self, html_content: str, source_url: str) -> list[dict[str, Any]]:
        ...
        try:
            ...
            return self.parse_bet_type_page(
                html_content, bet_type, game_date, source_url
            )
    ...
    def parse_bet_type_page(
        self, html_content: str, bet_type: str, game_date: date, source_url: str
    ) -> list[dict[str, Any]]:
        """
        ...
        try:
            ...
            json_games = self._extract_games_from_json(
                html_content, bet_type, game_date, source_url
            )
            if json_games:
                logger.info(
                    f"Successfully extracted {len(json_games)} games from JSON data"
                )
                return json_games
                        ...
                        if (
                            ...
                            if odds_tables and len(odds_tables) > 0:
                                ...
                                for game_row in game_rows:
                                    game_data_validator = self._process_json_game_row(
                                        game_row, bet_type, game_date, source_url
                                    )
    ...
    def _process_json_game_row(
        self, game_row: dict[str, Any], bet_type: str, game_date: date, source_url: str
        ...
        try:
            ...
            if not odds_views:
                logger.debug(
                    "Skipped game row with empty oddsViews – likely incomplete data block"
                )
                return None
            # ------------------------------------------------------------------
            ...
            home_team = (
                Team.normalize_team_name(home_team_raw) if home_team_raw else None
            )
            away_team = (
                Team.normalize_team_name(away_team_raw) if away_team_raw else None
            )

            if not home_team or not away_team:
                logger.warning(
                    f"Could not normalize team names for a game in {source_url}. Skipping."
                )
                return None
            ...
            game_datetime = self._create_game_datetime(
                game_date, game_view.get("startDate")
            )

            # Extract basic game information
            game_data = {
                "sbr_game_id": str(
                    game_view.get("gameId", "")
                ...
                "home_pitcher": self._format_pitcher_name(
                    game_view.get("homeStarter", {})
                ),
                "away_pitcher": self._format_pitcher_name(
                    game_view.get("awayStarter", {})
                ),
            ...
            if (
                game_view.get("homeTeamScore") is not None
                and game_view.get("awayTeamScore") is not None
            ):
            ...
            if not validated_game_data:
                logger.warning(
                    f"Game data failed validation for game {game_data.get('sbr_game_id')}"
                )
                return None
    ...
    def _parse_game_time(self, start_date_str: str) -> str | None:
    ...
    def _format_pitcher_name(self, pitcher_data: dict[str, Any]) -> str | None:
    ...
    def _format_odds_line(
        self, line_data: dict[str, Any], bet_type: str
    ) -> dict[str, Any]:
        ...
        if bet_type:
            ...
            logger.warning(
                "bet_type is None/empty in _format_odds_line - this may cause validation issues"
            )
    ...
    def _create_game_datetime(self, game_date: date, start_date_str: str) -> datetime:
        ...
        try:
            if start_date_str:
                ...
                est_tz = ZoneInfo("America/New_York")
                return datetime.combine(game_date, datetime.min.time()).replace(
                    tzinfo=est_tz
                )
        except Exception:
    ...
    def _parse_html_fallback(
        ...
        try:
            ...
            if not main_table:
                logger.warning(
                    f"Could not find main table for {bet_type} on {game_date}"
                )
                return []
            ...
            for container in game_containers:
                ...
                if game_info:
                    ...
                    current_game_data = {
                        ...
                        "bet_type": bet_type,
                        "odds_data": {},
                        "public_betting_percentage": game_info.get(
            ...
            logger.debug(
                f"Parsed {len(games_data)} games for {bet_type} on {game_date}"
            )
            return games_data

        except Exception as e:
            logger.error(
                f"Error in HTML fallback parsing for {bet_type} page for {game_date}: {e}"
            )
            return []

    def _extract_game_info_from_container(
        self, container: Tag
    ) -> dict[str, Any] | None:
        """
        ...
        try:
            ...
            line_history_link = container.find(
                "a", href=lambda x: x and "line-history" in x
            )
    ...
    def _extract_odds_data_from_container(
        self, container: Tag, bet_type: str
    ) -> dict[str, Any] | None:
        """
        ...
        try:
            ...
            for link in sportsbook_links:
                ...
                sportsbook_name = self._extract_sportsbook_name_from_tracker(
                    tracker_value
                )

                if sportsbook_name:
                    # Extract odds from this sportsbook section
                    odds_values = self._extract_odds_from_sportsbook_section(
                        link, bet_type
                    )
                    if odds_values:
    ...
    def _extract_odds_from_sportsbook_section(
        self, link: Tag, bet_type: str
    ) -> dict[str, Any] | None:
    ...
    def _parse_odds_span(self, span: Tag, bet_type: str) -> dict[str, Any] | None:
        ...
        try:
            ...
            if bet_type == "moneyline":
                ...
                if total_match:
                    return {"total": total_match.group(1), "odds": total_match.group(2)}
    ...
    def _extract_public_betting_percentage(
        self, container: Tag
    ) -> dict[str, str] | None:
    ...
    def parse_betting_data(
        self, html_content: str, game: EnhancedGame
    ) -> list[OddsData]:
        """
        ...
        try:
            ...
            logger.debug(
                f"Parsed {len(odds_data)} odds entries for game {game.sbr_game_id}"
            )
            return odds_data
    ...
    def _extract_teams(self, soup: BeautifulSoup) -> list[str] | None:
        ...
        if title:
            ...
            vs_match = re.search(
                r"([A-Za-z\s]+)\s+(?:vs|@|\-)\s+([A-Za-z\s]+)", title_text
            )
            if vs_match:
    ...
    def _extract_game_datetime(self, soup: BeautifulSoup) -> datetime | None:
    ...
    def _extract_venue_info(self, soup: BeautifulSoup) -> dict[str, Any] | None:
    ...
    def _parse_moneyline_odds(
        self, soup: BeautifulSoup, game: EnhancedGame
    ) -> list[OddsData]:
        ...
        for section in moneyline_sections:
            ...
            if len(odds_elements) >= 2:
                home_odds = self.parse_american_odds(
                    odds_elements[0].get_text(strip=True)
                )
                away_odds = self.parse_american_odds(
    ...
    def _parse_spread_odds(
        ...
        for section in spread_sections:
            ...
            if spread_elements and odds_elements:
                spread_value = self.parse_spread_value(
                    spread_elements[0].get_text(strip=True)
                )
                spread_odds = self.parse_american_odds(
                    odds_elements[0].get_text(strip=True)
                )
                ...
                if spread_value is not None and spread_odds:
                    ...
                    market_side = (
                        MarketSide.HOME_SPREAD
                        if spread_value < 0
                        else MarketSide.AWAY_SPREAD
                    )
    ...
    def _parse_total_odds(
        ...
        for section in total_sections:
            ...
            if total_elements and over_elements and under_elements:
                total_value = self.parse_total_value(
                    total_elements[0].get_text(strip=True)
                )
                over_odds = self.parse_american_odds(
                    over_elements[0].get_text(strip=True)
                )
                under_odds = self.parse_american_odds(
                    under_elements[0].get_text(strip=True)
                )
    ...
    def _extract_sportsbook_name(self, section: Tag) -> SportsbookName | None:
    ...
    def _generate_sbr_game_id(self, source_url: str) -> str:
        ...
        for part in url_parts:
            ...
            # Look for team patterns
                ...
                team in part.lower()
                for team in ["yankees", "dodgers", "giants", "mets"]
            ):
                team_part = part
    ...
    def parse_datetime(self, datetime_text: str) -> datetime | None:
        """Parse datetime string using the base class method."""
        return self.parse_game_datetime(datetime_text)

    def parse_spread_value(self, spread_text: str) -> float | None:
        """
    ...
    def parse_total_value(self, total_text: str) -> float | None:
        """
    ...
    def normalize_sportsbook_name(
        ...
        logger.debug(
            f"Unknown sportsbook '{sportsbook_text}', defaulting to DraftKings"
        )
        return SportsbookName.DRAFTKINGS
