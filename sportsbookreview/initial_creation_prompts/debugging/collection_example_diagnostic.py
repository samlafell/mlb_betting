...
class MultiSchemaCollectionDiagnostics:
        ...
        try:
            async with self.storage.pool.acquire() as conn:
                ...
                for table in mlb_tables:
                    ...
                    try:
                        row_count = await conn.fetchval(f"SELECT COUNT(*) FROM mlb_betting.{table_name}")
                    except:
                    ...
                    if bet_type and bet_type_row['new_count'] > 0:
                        ...
                        if sample:
                            ...
                            sample_records[bet_type] = {
                                ...
                                'sample_sportsbook': game_data.get('odds_data', [{}])[0].get('sportsbook') if game_data.get('odds_data') else None
                            }
    ...
    async def test_schema_routing(self) -> Dict[str, Any]:
        ...
        try:
            async with self.storage.pool.acquire() as conn:
                ...
                for table in mlb_tables:
                    try:
                        await conn.fetchval(f"SELECT 1 FROM mlb_betting.{table} LIMIT 1")
        ...
        try:
            async with self.storage.pool.acquire() as conn:
                ...
                for bet_type in bet_types:
                    ...
                    transformation_result = self.test_bet_type_transformation(game_data, bet_type)
    ...
    def test_bet_type_transformation(self, game_dict: Dict[str, Any], bet_type: str) -> Dict[str, Any]:
        ...
        try:
            ...
            for odds in odds_data:
                ...
                if bet_type == 'moneyline':
                    ...
                    if not record['home_ml'] or not record['away_ml']:
                        errors.append(f"Missing moneyline data for {odds.get('sportsbook')}")
                    ...
                    if not record['home_spread'] or not record['away_spread']:
                        errors.append(f"Missing spread data for {odds.get('sportsbook')}")
                    ...
                    if not record['total_line'] or not record['over_price'] or not record['under_price']:
                        errors.append(f"Missing totals data for {odds.get('sportsbook')}")
            ...
            return {
                ...
                'sample_transformed_record': transformed_records[0] if transformed_records else None,
                'transformation_errors': errors,
                'data_quality': {
                    'has_sportsbook_names': all(r.get('sportsbook') for r in transformed_records),
                    'has_timestamps': all(r.get('timestamp') for r in transformed_records),
                    'records_with_complete_data': len([r for r in transformed_records if self.is_record_complete(r, bet_type)])
                }
            }
    ...
    def is_record_complete(self, record: Dict[str, Any], bet_type: str) -> bool:
        ...
        if bet_type == 'moneyline':
            ...
            return bool(record.get('total_line') and record.get('over_price') and record.get('under_price'))
        return False
        ...
        try:
            ...
            async with self.storage.pool.acquire() as conn:
                for bet_type in bet_types:
                    ...
                    logger.info(f"Testing integration for {bet_type} - SBR ID: {game_data.get('sbr_game_id')}")
    ...
    async def verify_cross_schema_data(self) -> Dict[str, Any]:
        ...
        try:
            async with self.storage.pool.acquire() as conn:
                ...
                verification_results['sample_game_ids'] = [dict(game) for game in recent_games[:3]]
                ...
                for table in ['moneyline', 'spread', 'totals']:
                    try:
                        betting_records = await conn.fetch(f"""
                        ...
                        if betting_records and betting_records[0]['count'] > 0:
                            verification_results[f'{table}_records'] = dict(betting_records[0])
                        else:
    ...
    def generate_recommendations(self, diagnostic_results: Dict[str, Any]) -> List[str]:
        ...
        if schema_validation.get('status') == 'success':
            ...
            if not schema_info.get('public_games', {}).get('exists', False):
                recommendations.append("Create public.games table - games data has nowhere to go")
            elif schema_info.get('public_games', {}).get('row_count', 0) == 0:
                recommendations.append("public.games table is empty - verify game data is being inserted")
            
            # Check mlb_betting tables
            missing_tables = schema_info.get('missing_mlb_betting_tables', [])
            if missing_tables:
                recommendations.append(f"Create missing mlb_betting tables: {', '.join(missing_tables)}")
            ...
            if empty_betting_tables:
                recommendations.append(f"mlb_betting tables are empty: {', '.join(empty_betting_tables)}. Verify IntegrationService is routing betting data correctly.")
        ...
        if staging.get('status') == 'exists':
            ...
            if new_rows > 0:
                recommendations.append(f"Process {new_rows} pending records in staging table")
            
            if failed_rows > 0:
                recommendations.append(f"Investigate {failed_rows} failed records - check logs for IntegrationService errors")
            
            # Check bet type distribution
            bet_type_breakdown = staging.get('bet_type_breakdown', [])
            if not bet_type_breakdown:
                recommendations.append("No bet type data found in staging - verify scraping is working")
        ...
        if routing.get('status') == 'tested':
            ...
            if not routing_tests.get('cross_schema_transaction', True):
                recommendations.append("Cross-schema transactions are failing - this will prevent proper data insertion")
            ...
            if failed_access:
                recommendations.append(f"Cannot access tables: {', '.join(failed_access)}. Check permissions.")
        ...
        if integration_test.get('status') == 'tested':
            ...
            for bet_type, result in bet_type_results.items():
                if result.get('status') == 'failed' or (result.get('status') == 'tested' and not result.get('success', False)):
                    failed_integrations.append(bet_type)
            
            if failed_integrations:
                recommendations.append(f"IntegrationService failing for: {', '.join(failed_integrations)}. Check schema routing in IntegrationService.")
        ...
        if cross_schema.get('status') == 'verified':
            ...
            if orphaned_counts:
                recommendations.append(f"Found orphaned betting records (betting data without corresponding games): {', '.join(orphaned_counts)}")
            ...
            if games_count > 0 and not betting_tables_with_data:
                recommendations.append("Games exist but no betting data found - IntegrationService may not be writing to mlb_betting schema")
...
class EnhancedMultiSchemaOrchestrator:
    ...
    async def process_staging_with_schema_awareness(self, batch_size: int = 100, progress_callback=None):
            ...
            while True:
                ...
                for row in rows:
                    ...
                    try:
                        ...
                        logger.debug(f"Processing row {row_id} - Bet Type: {bet_type}, SBR ID: {sbr_id}")
                        
                        # Validate data structure
                        if not self.validate_game_data_structure(game_dict, bet_type):
                            logger.warning(f"Row {row_id} has invalid data structure for {bet_type}")
                            await conn.execute("UPDATE sbr_parsed_games SET status='failed' WHERE id=$1", row_id)
                            continue
                        
                        # Log what we're about to process
                        odds_count = len(game_dict.get('odds_data', []))
                        logger.debug(f"Row {row_id} has {odds_count} odds records for {bet_type}")
                        ...
                        async with conn.transaction():
                            try:
                                ...
                                logger.debug(f"Row {row_id} integration result: {inserted} records inserted")
                                
                                # Verify insertion actually happened in correct schemas
                                verification_result = await self.verify_post_integration(conn, game_dict, bet_type)
                                ...
                                if inserted > 0 and verification_result['success']:
                                    ...
                                    self.update_schema_stats(schema_stats, verification_result, bet_type)
                                    
                                    logger.info(f"✓ Row {row_id} ({bet_type}) successfully processed - Game: {verification_result.get('game_found')}, Betting: {verification_result.get('betting_found')}")
                                    
                                else:
                                    # Integration claimed success but verification failed
                                    if inserted > 0:
                                        logger.error(f"✗ Row {row_id} integration claimed success but verification failed: {verification_result}")
                                        schema_stats['cross_schema_failures'] += 1
                                    
                                    # Check for duplicates
                                    existing = await self.storage.get_existing_games([str(sbr_id)]) if sbr_id else []
                                    new_status = "duplicate" if existing else "failed"
                                    
                                    logger.warning(f"✗ Row {row_id} marked as {new_status} - Integration: {inserted}, Verification: {verification_result}")
                                ...
                                logger.error(f"✗ Integration failed for row {row_id}: {integration_error}")
                                await conn.execute("UPDATE sbr_parsed_games SET status='failed' WHERE id=$1", row_id)
                                schema_stats['cross_schema_failures'] += 1
                ...
                if progress_callback:
                    progress_callback(90, f"Processed {processed_rows} rows - Schema stats: {schema_stats}")

        logger.info(f"Multi-schema processing complete:")
        logger.info(f"  - Processed: {processed_rows}")
        logger.info(f"  - Public.games inserts: {schema_stats['games_inserted_to_public']}")
        logger.info(f"  - MLB betting inserts: ML={schema_stats['moneyline_inserted_to_mlb_betting']}, Spread={schema_stats['spread_inserted_to_mlb_betting']}, Totals={schema_stats['totals_inserted_to_mlb_betting']}")
        logger.info(f"  - Cross-schema failures: {schema_stats['cross_schema_failures']}")
        
        return schema_stats
    
    def validate_game_data_structure(self, game_dict: Dict[str, Any], bet_type: str) -> bool:
        ...
        for odds in odds_data:
            ...
            if bet_type == 'moneyline':
                if not (odds.get('moneyline_home') or odds.get('home_ml')) or not (odds.get('moneyline_away') or odds.get('away_ml')):
                    logger.debug(f"Missing moneyline data for {odds.get('sportsbook')}")
                    return False
            elif bet_type == 'spread':
                if not (odds.get('spread_home') or odds.get('home_spread')) or not (odds.get('spread_away') or odds.get('away_spread')):
                    logger.debug(f"Missing spread data for {odds.get('sportsbook')}")
                    return False
            elif bet_type in ('total', 'totals'):
                if not odds.get('total_line') or not (odds.get('total_over') or odds.get('over_price')) or not (odds.get('total_under') or odds.get('under_price')):
    ...
    async def verify_post_integration(self, conn, game_dict: Dict[str, Any], bet_type: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if target_table:
                ...
                betting_exists = await conn.fetchval(f"""
                    SELECT EXISTS(
            ...
            verification_result['success'] = verification_result['game_found'] and verification_result['betting_found']
    ...
    def update_schema_stats(self, schema_stats: Dict[str, int], verification_result: Dict[str, Any], bet_type: str) -> None:
...
async def run_multi_schema_diagnostics():
    ...
    try:
        ...
        if connectivity.get('status') == 'connected':
            print(f"✓ Database connected: {connectivity.get('database_info', {}).get('database')}")
            print(f"✓ Public schema: {'✓' if connectivity.get('public_schema_exists') else '✗'}")
            print(f"✓ MLB betting schema: {'✓' if connectivity.get('mlb_betting_schema_exists') else '✗'}")
        else:
