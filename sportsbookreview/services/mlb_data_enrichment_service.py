...
from mlb_sharp_betting.services.retry_service import RetryService, OperationType
from mlb_sharp_betting.utils.timezone_utils import ensure_est
...
class MLBDataEnrichmentService:
    ...
    def __init__(self):
        ...
        self.request_delay = self.config.get("rate_limits", {}).get("request_delay_seconds", 0.5)
        self.max_requests_per_minute = self.config.get("rate_limits", {}).get("max_requests_per_minute", 60)
        ...
        try:
            ...
            if not game.mlb_game_id:
                ...
                if not mlb_game_id:
                    ...
                    raise ValueError(f"Could not correlate game {game.sbr_game_id} with MLB Stats API")
            else:
            ...
            pitcher_matchup = results[2] if not isinstance(results[2], Exception) else None
            game_context = results[3] if not isinstance(results[3], Exception) else None
            context_metrics = results[4] if not isinstance(results[4], Exception) else None
    ...
    async def _correlate_game_id(self, game: EnhancedGame) -> Tuple[Optional[str], float]:
        """
        ...
        try:
            ...
            self.logger.error(
                "Game correlation failed",
                game_id=game.sbr_game_id,
                error=str(e)
            )
            return None, 0.0
    
    def _calculate_game_match_confidence(self, sbr_game: EnhancedGame, mlb_game: Dict[str, Any]) -> float:
        """
        ...
        try:
            ...
            if (sbr_game.home_team.value == mlb_home_team and 
                sbr_game.away_team.value == mlb_away_team):
                confidence += 0.7
            elif (sbr_game.home_team.value in mlb_home_team or mlb_home_team in sbr_game.home_team.value) and \
                 (sbr_game.away_team.value in mlb_away_team or mlb_away_team in sbr_game.away_team.value):
                confidence += 0.5  # Partial team match
            
            # Date matching (20% weight)
            mlb_game_date = datetime.fromisoformat(mlb_game["gameDate"].replace('Z', '+00:00')).date()
            ...
            mlb_game_time = datetime.fromisoformat(mlb_game["gameDate"].replace('Z', '+00:00'))
            time_diff = abs((mlb_game_time - sbr_game.game_date).total_seconds()) / 3600  # Hours
    ...
    async def _fetch_schedule_for_date(self, date: datetime.date) -> List[Dict[str, Any]]:
        """
    ...
    async def _fetch_venue_info(self, mlb_game_id: str) -> Optional[VenueInfo]:
        """
        ...
        try:
            ...
            self.logger.error(
                "Failed to fetch venue info",
                mlb_game_id=mlb_game_id,
                error=str(e)
            )
            return None
    
    async def _fetch_weather_data(self, mlb_game_id: str) -> Optional[WeatherData]:
        """
    ...
    async def _fetch_pitcher_matchup(self, mlb_game_id: str) -> Optional[PitcherMatchup]:
        """
        ...
        try:
            ...
            self.logger.error(
                "Failed to fetch pitcher matchup",
                mlb_game_id=mlb_game_id,
                error=str(e)
            )
            return None
    
    async def _fetch_game_context(self, mlb_game_id: str) -> Optional[GameContext]:
        """
        ...
        try:
            ...
            return GameContext(
                ...
                attendance=game_data.get("liveData", {}).get("boxscore", {}).get("info", [{}])[0].get("attendance")
            )
    ...
    async def _fetch_context_metrics(self, mlb_game_id: str) -> Optional[Dict[str, Any]]:
        """
        ...
        try:
            ...
            self.logger.error(
                "Failed to fetch context metrics",
                mlb_game_id=mlb_game_id,
                error=str(e)
            )
            return None
    
    async def _fetch_game_data(self, mlb_game_id: str) -> Optional[Dict[str, Any]]:
        """
    ...
    async def _make_api_call(self, url: str, params: Optional[Dict[str, Any]] = None) -> Optional[Dict[str, Any]]:
        """
...
_mlb_enrichment_service_instance: Optional[MLBDataEnrichmentService] = None
