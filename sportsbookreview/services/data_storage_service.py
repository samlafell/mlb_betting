...
try:
    ...
    from sportsbookreview.services.mlb_data_enrichment_service import MLBDataEnrichmentService
...
class DataStorageService:
    ...
    async def store_game_data(self, game_data: Dict[str, Any]) -> Optional[int]:
        """
        ...
        try:
            ...
            if correlation_result and correlation_result.confidence > 0.7:
                ...
                if mlb_game and mlb_game.game_pk:
                    ...
                    # Update data quality
                    game.data_quality = 'HIGH' if correlation_result.confidence > 0.9 else 'MEDIUM'
                    
                    self.stats.mlb_enrichments_applied += 1
                    
                    logger.debug(f"Applied MLB enrichment to game {game.sbr_game_id} (confidence: {correlation_result.confidence:.2f})")
    ...
    async def store_raw_html(self, url: str, html: str) -> Optional[int]:
        """
    ...
    async def store_parsed_data(self, raw_html_id: int, parsed_games: List[GameDataValidator]):
        """
            ...
            async with self.pool.acquire() as conn:
                async with conn.transaction():
                    for game_validator in parsed_games:
                        ...
                        self.stats.games_processed += 1
                        self.stats.games_inserted += 1
                    logger.info(f"Stored {len(parsed_games)} parsed games for raw_html_id {raw_html_id}")
            await self.update_raw_html_status(raw_html_id, 'processed')
        except Exception as e:
            logger.error(f"Failed to store parsed data for raw_html_id {raw_html_id}: {e}")
    ...
    async def insert_or_update_game(self, game: EnhancedGame) -> Optional[int]:
        """
        ...
        try:
            ...
            async with self.pool.acquire() as conn:
                ...
                games_table = TABLE_REGISTRY.get_table('games') if TABLE_REGISTRY else 'core_betting.games'
                
                # Check if game already exists
                check_query = f"SELECT id FROM {games_table} WHERE sportsbookreview_game_id = $1"
                ...
                if existing_id:
                    ...
                    result = await conn.fetchval(
                        ...
                        str(game.home_team),
                        str(game.away_team),
                        game.game_datetime.date() if hasattr(game.game_datetime, 'date') else game.game_datetime,
                        game.game_datetime,
    ...
    async def update_mlb_enrichment(self, conn: asyncpg.Connection, game_id: int, game: EnhancedGame):
        """
        ...
        try:
            ...
            await conn.execute(
                ...
                getattr(game, 'venue_name', None),
                getattr(game, 'venue_id', None),
                game.weather_data.get('condition') if hasattr(game, 'weather_data') and game.weather_data else None,
                game.weather_data.get('temperature') if hasattr(game, 'weather_data') and game.weather_data else None,
                game.weather_data.get('wind_speed') if hasattr(game, 'weather_data') and game.weather_data else None,
                game.weather_data.get('wind_direction') if hasattr(game, 'weather_data') and game.weather_data else None,
                game.weather_data.get('humidity') if hasattr(game, 'weather_data') and game.weather_data else None,
    ...
    async def store_betting_data(self, game_id: int, betting_data: List[Any]):
        ...
        """
        try:
            logger.debug(f"store_betting_data called with {len(betting_data)} records for game_id {game_id}")
            for bet_record in betting_data:
                ...
                if isinstance(bet_record, dict):
                    ...
                    logger.debug(f"Processing bet_record: bet_type='{bet_type}', sportsbook='{bet_record.get('sportsbook')}', record={bet_record}")
    ...
    async def store_moneyline_data(self, game_id: int, data: Dict[str, Any]):
        ...
        try:
            async with self.pool.acquire() as conn:
                # 🚀 PHASE 2A: Use new consolidated schema table
                table_name = TABLE_REGISTRY.get_table('moneyline') if TABLE_REGISTRY else 'core_betting.betting_lines_moneyline'
                ...
                logger.debug(f"Stored moneyline data to {table_name} for game {game_id}")
    ...
    async def store_spread_data(self, game_id: int, data: Dict[str, Any]):
        ...
        try:
            async with self.pool.acquire() as conn:
                # 🚀 PHASE 2A: Use new consolidated schema table
                table_name = TABLE_REGISTRY.get_table('spreads') if TABLE_REGISTRY else 'core_betting.betting_lines_spreads'
    ...
    async def store_total_data(self, game_id: int, data: Dict[str, Any]):
        ...
        try:
            async with self.pool.acquire() as conn:
                # 🚀 PHASE 2A: Use new consolidated schema table
                table_name = TABLE_REGISTRY.get_table('totals') if TABLE_REGISTRY else 'core_betting.betting_lines_totals'
    ...
    def get_storage_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'success_rate': (self.stats.games_inserted / max(self.stats.games_processed, 1)) * 100,
            'processing_time': self.stats.processing_time,
            'avg_processing_time': self.stats.processing_time / max(self.stats.games_processed, 1)
        }
