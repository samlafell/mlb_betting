...
class CollectionOrchestrator:
    ...
    def __init__(
        ...
        # Initialize services
        self.scraper: Optional[SportsbookReviewScraper] = None
        self.storage: Optional[DataStorageService] = None
    ...
    async def process_staging(self, batch_size: int = 100, progress_callback: Optional[Callable[[float, str], None]] = None):
        """Load rows from sbr_parsed_games(status='new' or 'parsed') into core tables via DataStorageService."""
        if not self.storage:
            logger.warning("Storage service not initialised – skipping staging processing.")
            return
        ...
        async with self.storage.pool.acquire() as conn:
            while True:
                rows = await conn.fetch("SELECT id, game_data FROM sbr_parsed_games WHERE status IN ('new', 'parsed') LIMIT $1", batch_size)
                if not rows:
                    break
                ...
                for row in rows:
                    try:
                        ...
                        if game_bet_type in bet_type_stats:
                            ...
                            logger.warning(f"Found game with null bet_type: {game_dict.get('sbr_game_id')}")
                        ...
                        for odds in game_dict.get('odds_data', []):
                            ...
                            if not record['bet_type']:
                                ...
                                if not record['bet_type']:
                                    logger.warning(f"Skipping odds record with null bet_type for game {game_dict.get('sbr_game_id')}")
                        ...
                        async with conn.transaction():
                            ...
                            if inserted > 0:
                                ...
                                if sbr_id and self.storage:
                                    try:
                                        existing = await self.storage.get_existing_games([str(sbr_id)])
                                    except Exception as dup_exc:
                                        logger.debug("Duplicate check failed for %s: %s", sbr_id, dup_exc)

                                new_status = "duplicate" if existing else "failed"

                            # Persist status
                            await conn.execute(
                                "UPDATE sbr_parsed_games SET status=$1 WHERE id=$2", new_status, row['id']
                            )

                        if inserted > 0:
                            processed_rows += 1
                        logger.debug(
                            "Staging row %s processed – inserted=%s, status=%s", row['id'], inserted, new_status
                        )
                    except Exception as exc:
                        logger.error(f"Failed to load staging row {row['id']}: {exc}")
                        await conn.execute("UPDATE sbr_parsed_games SET status='failed' WHERE id=$1", row['id'])
        ...
        if bet_type_stats['null_bet_type'] > 0:
            null_percentage = (bet_type_stats['null_bet_type'] / max(total_games, 1)) * 100
            logger.warning(f"⚠️  NULL BET_TYPE DETECTED: {bet_type_stats['null_bet_type']} games ({null_percentage:.1f}%) had null bet_type")
        
        if bet_type_stats['skipped_odds'] > 0:
            logger.warning(f"⚠️  SKIPPED ODDS: {bet_type_stats['skipped_odds']} odds records skipped due to null bet_type")
        ...
        while current_date <= end_date:
            try:
                ...
                if progress_callback:
                    progress = (processed_days / total_days) * 100 # Scraping is 100% of this phase now
                    progress_callback(progress, f"Scraped {current_date} ({processed_days}/{total_days} days)")
        ...
        for batch_idx in range(total_batches):
            ...
            try:
                ...
                if progress_callback:
                    ...
                    overall_progress = 50 + (batch_progress * 40)  # 40% for storage phase
                    progress_callback(
                        overall_progress,
                        f"Stored batch {batch_idx + 1}/{total_batches}"
                    )
                
                logger.info(f"Stored batch {batch_idx + 1}/{total_batches} ({len(stored_ids)} games)")
                
            except Exception as e:
                logger.error(f"Error storing batch {batch_idx + 1}: {e}")
                self.stats.errors_encountered.append(f"Storage batch {batch_idx + 1}: {e}")
        
        # Update storage stats
        self.stats.storage_success_rate = (self.stats.games_stored / max(self.stats.games_processed, 1)) * 100
        
        # Get storage service stats
        storage_stats = self.storage.get_storage_stats()
        self.stats.betting_records_stored = storage_stats.get('betting_records_inserted', 0)
        self.stats.mlb_enrichments_applied = storage_stats.get('mlb_enrichments_applied', 0)
        
        if self.stats.games_processed > 0:
            self.stats.enrichment_success_rate = (self.stats.mlb_enrichments_applied / self.stats.games_processed) * 100
    ...
    async def load_checkpoint(self):
        ...
        try:
            ...
            logger.info(f"Resumed from checkpoint with {len(self.processed_games)} processed games")
    ...
    async def finalize_collection(self):
        ...
        if self.storage:
            ...
            self.stats.betting_records_stored = s_stats.get("betting_records_inserted", 0)
    ...
    async def save_final_results(self):
        ...
        try:
            results_file = self.output_dir / f"collection_results_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    ...
    def get_collection_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'error_summary': {
                ...
                'error_rate_percent': round((len(self.stats.errors_encountered) / max(self.stats.pages_scraped, 1)) * 100, 2)
            }
        }
...
async def test_collection_system(output_dir: Optional[Path] = None) -> Dict[str, Any]:
    """
    ...
    async def main():
        ...
        try:
            ...
            try:
                ...
                results = await storage.pool.fetch(f'''
