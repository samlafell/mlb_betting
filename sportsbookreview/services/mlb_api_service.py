...
class CorrelationResult:
    ...
    confidence: float
    mlb_game: MLBGameInfo | None = None
...
class MLBAPIService:
    ...
    def _load_from_persistent_cache(self, date_str: str) -> list[MLBGameInfo] | None:
        ...
        try:
            ...
            for game_dict in cache_data["games"]:
                ...
                if isinstance(game_dict["game_datetime"], str):
                    game_dict["game_datetime"] = datetime.fromisoformat(
                        game_dict["game_datetime"]
                    )
                games.append(MLBGameInfo(**game_dict))

            logger.debug(
                f"Loaded {len(games)} games from persistent cache: {cache_file}"
            )
            return games
        ...
        if self.enable_persistent_cache and os.path.exists(self.CACHE_DIR):
            try:
                ...
                for file in cache_files:
                    try:
                        with open(os.path.join(self.CACHE_DIR, file)) as f:
                            data = json.load(f)
    ...
    def _parse_mlb_game(self, game_data: dict[str, Any]) -> MLBGameInfo | None:
        """
        ...
        try:
            ...
            game_datetime_est = game_datetime_utc.astimezone(est_timezone).replace(
                tzinfo=None
            )
            ...
            home_team = (
                home_team_data.get("team", {})
                if isinstance(home_team_data, dict)
                else {}
            )
            away_team = (
                away_team_data.get("team", {})
            ...
            home_team_abbr = MLB_ID_TO_ABBREVIATION.get(
                home_team_id, f"TEAM_{home_team_id}"
            )
            away_team_abbr = MLB_ID_TO_ABBREVIATION.get(
                away_team_id, f"TEAM_{away_team_id}"
            )
    ...
    async def correlate_game(
        ...
        if not home_team_std or not away_team_std:
            logger.warning(
                f"Could not standardize team names: {home_team}, {away_team}"
            )
            return CorrelationResult(
                confidence=0.0, match_reasons=["Unknown team names"]
            )
        ...
        if not mlb_games:
            return CorrelationResult(
                confidence=0.0, match_reasons=["No MLB games found for date"]
            )
        ...
        for mlb_game in mlb_games:
            confidence, reasons = self._calculate_match_confidence(
                home_team_std, away_team_std, game_datetime, mlb_game, tolerance_hours
            )
        ...
        return CorrelationResult(
            confidence=best_confidence, mlb_game=best_match, match_reasons=best_reasons
        )
    ...
    def _calculate_match_confidence(
        ...
        if mlb_game.home_team_name == sbr_home and mlb_game.away_team_name == sbr_away:
            confidence += 0.7
            reasons.append("Exact team match")
        elif (
            mlb_game.home_team_name == sbr_away and mlb_game.away_team_name == sbr_home
        ):
    ...
    def _standardize_team_name(self, team_name: str) -> str | None:
        """
    ...
    async def enrich_game_data(self, mlb_game_pk: str) -> dict[str, Any] | None:
        """
