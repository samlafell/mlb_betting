...
class ThresholdSignalAnalyzer:
    ...
    async def run_complete_analysis(self):
        ...
        try:
            ...
            recommendations = await self._generate_recommendations(
                signal_distributions, threshold_results, processor_analysis, confidence_analysis
            )
    ...
    async def _analyze_data_availability(self):
        ...
        for i, row in enumerate(rows[:10]):
            ...
            print(f"      {i+1:2d}. {row['source']}/{row['book']}/{row['split_type']}: "
        ...
        outcome_rows = await self.connection.fetch(outcome_query, self.start_date, self.end_date)
        games_with_outcomes = outcome_rows[0]['games_with_outcomes'] if outcome_rows else 0
    ...
    async def _analyze_signal_strength_distributions(self):
        ...
        for query_name, query in queries.items():
            ...
            if differentials:
                ...
                print(f"      Range: {np.min(differentials):.1f}% to {np.max(differentials):.1f}%")
    ...
    async def _extract_processor_thresholds(self, processor) -> Dict[str, Any]:
        ...
        try:
            ...
            if 'BookConflict' in class_name:
                thresholds['conflict_strength_min'] = 8.0  # From code: if conflict_strength < 8.0
                thresholds['type'] = 'conflict_variance'
                
            elif 'PublicFade' in class_name:
                thresholds['consensus_strength_min'] = 65.0  # From code: if consensus_strength < 65.0
                thresholds['type'] = 'public_consensus'
                
            elif 'LateFlip' in class_name:
                thresholds['flip_strength_min'] = 12.0  # From code: if flip_strength < 12.0
    ...
    async def _diagnose_processor_pipeline(self, proc_name: str, processor) -> Dict[str, Any]:
        ...
        try:
            ...
            if hasattr(processor, 'repository'):
                try:
                    ...
                    if hasattr(processor.repository, 'get_multi_book_data'):
                        raw_data = await processor.repository.get_multi_book_data(start_time, end_time)
                        issues['raw_data_count'] = len(raw_data) if raw_data else 0
                        
                    elif hasattr(processor.repository, 'get_public_betting_data'):
                        raw_data = await processor.repository.get_public_betting_data(start_time, end_time)
                        issues['raw_data_count'] = len(raw_data) if raw_data else 0
                        
                    elif hasattr(processor.repository, 'get_steam_move_data'):
                        raw_data = await processor.repository.get_steam_move_data(start_time, end_time)
                        issues['raw_data_count'] = len(raw_data) if raw_data else 0
            ...
            if 'BookConflict' in proc_name:
                ...
                issues['issues'].append("Conflict strength threshold may be too high (8.0%)")
                
            elif 'PublicFade' in proc_name:
                # Check if consensus threshold is too high
                issues['issues'].append("Public consensus threshold may be too high (65.0%)")
                
            elif 'LateFlip' in proc_name:
                # Check if flip strength threshold is too high
                issues['issues'].append("Flip strength threshold may be too high (12.0%)")
                
            # General issues
            if issues['raw_data_count'] == 0:
                issues['issues'].append("No raw data available for processing")
            elif issues['raw_data_count'] < 10:
                issues['issues'].append(f"Very low raw data count: {issues['raw_data_count']}")
    ...
    async def _analyze_confidence_scoring(self):
        ...
        try:
            ...
            print(f"   📊 Sample confidence calculation:")
            print(f"      Overall confidence: {sample_result.overall_confidence:.1f}")
            print(f"      Signal strength score: {sample_result.components.signal_strength_score:.1f}")
            ...
            if sample_result.overall_confidence < 50:
                confidence_issues.append("Confidence scores appear to be too low for strong signals")
            
            # Test with different signal strengths
            test_differentials = [5, 10, 15, 20, 25, 30]
            print(f"   🧪 Testing signal strength scoring:")
            ...
            for diff in test_differentials:
                ...
                print(f"      {diff:2d}% diff: signal={signal_score:.1f}, overall={overall_score:.1f}")
                
                # Check for issues
                if diff >= 20 and overall_score < 60:
                    confidence_issues.append(f"Strong signal ({diff}%) getting low confidence ({overall_score:.1f})")
    ...
    async def _generate_recommendations(self, signal_distributions, threshold_analysis, 
                                      processor_analysis, confidence_analysis):
    ...
    async def _implement_fixes(self, recommendations):
        ...
        await self._create_fixed_confidence_scorer(recommendations['confidence_scorer_fixes'])
    ...
    async def _generate_final_report(self, recommendations):
        ...
        print(f"   📋 Report saved: threshold_signal_analysis_report.json")
