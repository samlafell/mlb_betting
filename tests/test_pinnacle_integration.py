...
class TestPinnacleModels:
    ...
    def test_pinnacle_price_creation(self):
        ...
        price = PinnaclePrice(
            price=-110,
            designation=PriceDesignation.HOME
        )
    ...
    def test_pinnacle_market_creation(self):
        ...
        limits = [
            PinnacleLimit(amount=2000, type=LimitType.MAX_RISK_STAKE)
        ]
    ...
    def test_market_validation_errors(self):
        ...
        with pytest.raises(ValueError, match="Moneyline market must have home and away prices"):
            PinnacleMarket(
                matchup_id=1610721342,
                market_type=PinnacleMarketType.MONEYLINE,
                key="s;0;m",
...
class TestPinnacleAPIService:
    ...
    def test_market_type_determination(self, api_service):
        ...
        assert api_service._determine_market_type({"type": "moneyline"}) == PinnacleMarketType.MONEYLINE
        assert api_service._determine_market_type({"type": "spread"}) == PinnacleMarketType.SPREAD
        assert api_service._determine_market_type({"type": "total"}) == PinnacleMarketType.TOTAL
        assert api_service._determine_market_type({"type": "special"}) == PinnacleMarketType.SPECIAL
...
class TestPinnacleParser:
    ...
    async def test_parse_moneyline_market(self, parser, sample_moneyline_market):
        ...
        total = betting_split.home_or_over_bets_percentage + betting_split.away_or_under_bets_percentage
    ...
    async def test_parse_multiple_markets(self, parser, sample_moneyline_market, sample_spread_market):
