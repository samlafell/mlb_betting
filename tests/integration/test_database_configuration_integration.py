...
from src.data.pipeline.staging_action_network_history_processor import ActionNetworkHistoryProcessor
from src.data.pipeline.staging_action_network_unified_processor import ActionNetworkUnifiedStagingProcessor
from src.data.pipeline.staging_action_network_historical_processor import ActionNetworkHistoricalProcessor
from src.data.collection.mlb_stats_api_collector import MLBStatsAPICollector
from src.services.cross_site_game_resolution_service import CrossSiteGameResolutionService
...
class TestDatabaseConfigurationIntegration:
    ...
    @pytest.mark.asyncio
    async def test_action_network_history_processor_uses_centralized_config(self, mock_settings, expected_db_config):
        """Test ActionNetworkHistoryProcessor uses centralized configuration."""
        with patch('src.data.pipeline.staging_action_network_history_processor.get_settings', return_value=mock_settings):
    ...
    @pytest.mark.asyncio
    async def test_action_network_unified_processor_uses_centralized_config(self, mock_settings, expected_db_config):
        """Test ActionNetworkUnifiedStagingProcessor uses centralized configuration."""
        with patch('src.data.pipeline.staging_action_network_unified_processor.get_settings', return_value=mock_settings):
    ...
    @pytest.mark.asyncio
    async def test_action_network_historical_processor_uses_centralized_config(self, mock_settings, expected_db_config):
        """Test ActionNetworkHistoricalProcessor uses centralized configuration."""
        with patch('src.data.pipeline.staging_action_network_historical_processor.get_settings', return_value=mock_settings):
    ...
    @pytest.mark.asyncio
    async def test_mlb_stats_api_collector_uses_centralized_config(self, mock_settings, expected_db_config):
    ...
    @pytest.mark.asyncio
    async def test_cross_site_game_resolution_service_uses_centralized_config(self, mock_settings, expected_db_config):
    ...
    async def test_all_processors_initialize_without_errors(self, mock_settings):
        ...
        with patch('src.data.pipeline.staging_action_network_history_processor.get_settings', return_value=mock_settings), \
             patch('src.data.pipeline.staging_action_network_unified_processor.get_settings', return_value=mock_settings), \
             patch('src.data.pipeline.staging_action_network_historical_processor.get_settings', return_value=mock_settings):
    ...
    async def test_no_hardcoded_database_values_in_runtime(self, mock_settings):
        ...
        with patch('src.data.pipeline.staging_action_network_history_processor.get_settings', return_value=mock_settings), \
             patch('src.data.collection.mlb_stats_api_collector.get_settings', return_value=mock_settings), \
            ...
            for component_config in components:
                assert component_config["host"] != "localhost", f"Component still uses hardcoded localhost: {component_config}"
                assert component_config["user"] != "samlafell", f"Component still uses hardcoded user: {component_config}"
                assert component_config["database"] != "mlb_betting" or component_config["host"] == "test-host", \
    ...
    async def test_configuration_consistency_across_components(self, mock_settings):
        ...
        with patch('src.data.pipeline.staging_action_network_history_processor.get_settings', return_value=mock_settings), \
            ...
            for i, config in enumerate(configs[1:], 1):
                assert config == configs[0], f"Configuration mismatch between component 0 and {i}: {configs[0]} vs {config}"