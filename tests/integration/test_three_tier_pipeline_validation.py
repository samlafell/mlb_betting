...
class ThreeTierPipelineValidator:
    ...
    async def validate_phase_1_schemas(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                ...
                passed_tests=sum(1 for t in phase_results["tests"].values() if t["status"] == "passed"),
    ...
    async def _test_staging_tables(self, conn: Connection) -> Dict[str, Any]:
        ...
        try:
            ...
            result = {
                "status": "passed" if not missing_tables and not legacy_tables else "failed",
            ...
            if legacy_tables:
                result["error"] = f"Legacy core_betting tables found (should be migrated): {legacy_tables}"
    ...
    async def _test_action_network_collection(self) -> Dict[str, Any]:
        ...
        try:
            ...
            result = {
                "status": "passed" if test_result.get("status") == "success" and raw_count > 0 else "failed",
    ...
    async def _test_raw_data_validation(self) -> Dict[str, Any]:
        ...
        try:
            ...
            validation_results["action_network"] = {
                ...
                "has_external_id": an_sample and an_sample["external_game_id"] is not None,
            ...
            validation_results["sbd"] = {
                ...
                "has_external_id": sbd_sample and sbd_sample["external_matchup_id"] is not None,
                "has_raw_response": sbd_sample and sbd_sample["raw_response"] is not None,
                "has_api_endpoint": sbd_sample and sbd_sample["api_endpoint"] is not None,
            }
            
            validation_results["vsin"] = {
                "has_sample": vsin_sample is not None,
                "has_external_id": vsin_sample and vsin_sample["external_id"] is not None,
                "has_raw_response": vsin_sample and vsin_sample["raw_response"] is not None,
            ...
            if not all_sources_valid:
                failed_sources = [
                    source for source, validations in validation_results.items()
                    if not all(validations.values())
                ]
                result["error"] = f"Raw data validation failed for sources: {failed_sources}"
    ...
    async def _test_mlb_stats_api_resolution(self) -> Dict[str, Any]:
        ...
        try:
            ...
            result = {
                "status": "passed" if len(games_with_mlb_id) > 0 or mlb_api_responses > 0 else "warning",
            ...
            if len(games_with_mlb_id) == 0 and mlb_api_responses == 0:
                result["warning"] = "No recent games with MLB Stats API game IDs found - may need MLB API integration"
    ...
    async def _test_team_name_normalization(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if len(all_teams) == 0:
                result["warning"] = "No team data found across sources - may indicate data collection issues"
    ...
    async def _test_cross_source_game_matching(self) -> Dict[str, Any]:
        ...
        try:
            ...
            result = {
                ...
                "match_rate": {
                    "perfect": round(perfect_matches / matches_found * 100, 2) if matches_found > 0 else 0,
                    "partial": round(partial_matches / matches_found * 100, 2) if matches_found > 0 else 0
                },
                "sample_matches": [
                    {
                        "action_network": f"{match['an_away']} @ {match['an_home']}" if match['an_home'] else None,
                        "sbd": f"{match['sbd_away']} @ {match['sbd_home']}" if match['sbd_home'] else None,
                        "vsin": f"{match['vsin_away']} @ {match['vsin_home']}" if match['vsin_home'] else None,
            ...
            if matches_found == 0:
                result["warning"] = "No cross-source game matches found - may indicate team name normalization issues"
    ...
    async def _test_centralized_games_table(self) -> Dict[str, Any]:
        ...
        try:
            ...
            completeness_pct = {
                "teams": round((data_completeness["complete_teams"] / data_completeness["total_games"] * 100), 2) if data_completeness["total_games"] > 0 else 0,
                "dates": round((data_completeness["complete_dates"] / data_completeness["total_games"] * 100), 2) if data_completeness["total_games"] > 0 else 0,
                "status": round((data_completeness["complete_status"] / data_completeness["total_games"] * 100), 2) if data_completeness["total_games"] > 0 else 0,
            }

            result = {
                "status": "passed" if total_games > 0 and completeness_pct["teams"] >= 90 else "warning",
                "total_games": total_games,
                "unique_games": games_stats["unique_games"],
                "games_with_mlb_id": games_stats["games_with_mlb_id"],
                "mlb_id_coverage": round((games_stats["games_with_mlb_id"] / total_games * 100), 2) if total_games > 0 else 0,
                "recent_updates": recent_games,
                "date_range": {
                    "earliest": games_stats["earliest_game"].isoformat() if games_stats["earliest_game"] else None,
                    "latest": games_stats["latest_game"].isoformat() if games_stats["latest_game"] else None,
                    "unique_dates": games_stats["unique_dates"]
                },
                "data_completeness": completeness_pct,
                "high_quality_games": games_stats["high_quality_games"],
                "quality_rate": round((games_stats["high_quality_games"] / total_games * 100), 2) if total_games > 0 else 0,
            ...
            if total_games == 0:
                ...
                result["warning"] = f"Low team data completeness: {completeness_pct['teams']}%"
    ...
    async def _test_raw_to_staging_processing(self) -> Dict[str, Any]:
        ...
        try:
            ...
            result = {
                "status": "passed" if staging_total > 0 and processing_rate > 50 else "warning",
                ...
                "historical_odds": {
                    ...
                    "avg_quality": round(float(historical_odds_stats["avg_quality_score"]), 2) if historical_odds_stats["avg_quality_score"] else 0
                },
                "test_name": "Raw to Staging Processing Test",
            }

            if staging_total == 0:
                result["error"] = "No staging data found - processing pipeline may not be working"
            elif processing_rate < 50:
                result["warning"] = f"Low processing rate: {processing_rate}% - may indicate processing issues"
    ...
    async def _test_data_transformation(self) -> Dict[str, Any]:
        ...
        try:
            ...
            transformation_metrics = {
                "team_normalization_rate": round((transformation_stats["normalized_teams"] / total_games * 100), 2) if total_games > 0 else 0,
                "validation_rate": round((transformation_stats["validated_records"] / total_games * 100), 2) if total_games > 0 else 0,
                "high_quality_rate": round((transformation_stats["high_quality"] / total_games * 100), 2) if total_games > 0 else 0,
                "error_rate": round((transformation_stats["with_errors"] / total_games * 100), 2) if total_games > 0 else 0,
            }

            result = {
                "status": "passed" if transformation_metrics["team_normalization_rate"] >= 90 and transformation_metrics["validation_rate"] >= 80 else "warning",
                "total_games": total_games,
                "transformation_metrics": transformation_metrics,
                "avg_quality_score": round(float(transformation_stats["avg_quality_score"]), 2) if transformation_stats["avg_quality_score"] else 0,
                "validation_summary": {
            ...
            if transformation_metrics["team_normalization_rate"] < 90:
                result["warning"] = f"Low team normalization rate: {transformation_metrics['team_normalization_rate']}%"
    ...
    async def _test_quality_scoring_workflow(self) -> Dict[str, Any]:
        ...
        try:
            ...
            excellent_rate = (quality_summary.get("excellent", 0) / total_records * 100) if total_records > 0 else 0
            good_plus_rate = ((quality_summary.get("excellent", 0) + quality_summary.get("good", 0)) / total_records * 100) if total_records > 0 else 0
    ...
    async def _test_pipeline_execution_tracking(self) -> Dict[str, Any]:
        ...
        try:
            ...
            success_rate = (execution_stats["successful_executions"] / total_executions * 100) if total_executions > 0 else 0
            ...
            result = {
                "status": "passed" if total_executions > 0 and success_rate >= 80 else "warning",
                "execution_summary": {
            ...
            if total_executions == 0:
                result["warning"] = "No pipeline executions found - tracking may not be working"
    ...
    async def run_full_validation(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if phase_1_results["status"] == "passed":
                ...
                if phase_2_results["status"] == "passed":
                    ...
                    if phase_3_results["status"] in ["passed", "warning"]:
                        phase_4_results = await self.validate_phase_4_pipeline_processing()
            ...
            completed_phases = [
                phase for phase in full_results["phases"].values() 
...
async def test_phase_1_schema_validation():
    ...
    async with ThreeTierPipelineValidator() as validator:
        ...
        assert results["status"] in ["passed", "failed"], f"Invalid status: {results['status']}"
        
        if results["status"] == "failed":
            pytest.fail(f"Phase 1 validation failed: {results.get('error', 'Unknown error')}")
...
async def test_phase_2_source_validation():
    ...
    async with ThreeTierPipelineValidator() as validator:
        ...
        if results["status"] == "failed":
            pytest.fail(f"Phase 2 validation failed: {results.get('error', 'Unknown error')}")
...
async def test_phase_3_game_resolution():
    ...
    async with ThreeTierPipelineValidator() as validator:
        ...
        if results["status"] == "failed":
            pytest.fail(f"Phase 3 validation failed: {results.get('error', 'Unknown error')}")
...
async def test_phase_4_pipeline_processing():
    ...
    async with ThreeTierPipelineValidator() as validator:
        ...
        assert results["status"] in ["passed", "failed", "warning"], f"Invalid status: {results['status']}"
        
        # Phase 4 can have warnings (e.g., low processing rates) but still be functional
        if results["status"] == "failed":
            pytest.fail(f"Phase 4 validation failed: {results.get('error', 'Unknown error')}")
...
async def test_full_pipeline_validation():
    ...
    async with ThreeTierPipelineValidator() as validator:
        ...
        assert results["overall_status"] in ["passed", "failed"], f"Invalid status: {results['overall_status']}"
        
        # Log results for debugging
        logger.info("Full validation results", results=results)
        
        if results["overall_status"] == "failed":
            pytest.fail(f"Full pipeline validation failed: {results.get('error', 'See phase details')}")
...
async def test_end_to_end_pipeline_framework():
    ...
    async with ThreeTierPipelineValidator() as validator:
        ...
        end_to_end_summary = {
            ...
            "completed_phases": sum(1 for p in full_results["phases"].values() if p["status"] != "skipped"),
            "passed_phases": sum(1 for p in full_results["phases"].values() if p["status"] == "passed"),
            "failed_phases": sum(1 for p in full_results["phases"].values() if p["status"] == "failed"),
            "warning_phases": sum(1 for p in full_results["phases"].values() if p["status"] == "warning"),
            "skipped_phases": sum(1 for p in full_results["phases"].values() if p["status"] == "skipped"),
        ...
        logger.info("End-to-end pipeline framework validation completed", 
        ...
        framework_success = (
            ...
            full_results["phases"]["phase_1"]["status"] == "passed" or
            full_results["phases"]["phase_4"]["status"] in ["passed", "warning"] or
            # Or we have sufficient phases working to demonstrate pipeline health
            sum(1 for p in full_results["phases"].values() 
                if p["status"] in ["passed", "warning"]) >= 2
        )
        
        # Log the framework decision criteria
        logger.info("Framework success evaluation", 
                   framework_success=framework_success,
                   passed_warning_phases=sum(1 for p in full_results["phases"].values() 
                                           if p["status"] in ["passed", "warning"]),
                   pipeline_readiness=end_to_end_summary["pipeline_readiness"])
        
        if not framework_success:
            pytest.fail("End-to-end framework validation failed: Insufficient pipeline validation")
    ...
    for phase_name, phase_data in results["phases"].items():
        if phase_data["status"] == "failed":
            if "error" in phase_data:
                ...
                failed_tests = [test for test, data in phase_data["tests"].items() 
                              if data.get("status") == "failed"]
                if failed_tests:
                    issues.append(f"{phase_name}: Failed tests - {', '.join(failed_tests)}")
...
def _generate_recommendations(results: Dict[str, Any]) -> List[str]:
    ...
    if results["phases"].get("phase_1", {}).get("status") == "failed":
        ...
        if phase_1_tests.get("staging_tables", {}).get("status") == "failed":
            recommendations.append("Migrate legacy core_betting schema tables to three-tier architecture")
    ...
    if results["phases"].get("phase_4", {}).get("status") in ["failed", "warning"]:
        ...
        if phase_4_tests.get("raw_to_staging", {}).get("processing_rate", 0) < 10:
            recommendations.append("Investigate low RAW to STAGING processing rate - possible pipeline bottleneck")
        if phase_4_tests.get("execution_tracking", {}).get("execution_summary", {}).get("success_rate", 0) < 50:
            recommendations.append("Improve pipeline execution success rate - review error handling")
    
    # General recommendations
    if not recommendations:
        recommendations.append("Pipeline validation successful - monitor ongoing data quality metrics")
...
def _assess_data_flow(results: Dict[str, Any]) -> str:
    ...
    if phase_2.get("status") == "passed" and phase_4.get("status") in ["passed", "warning"]:
        return "healthy"
