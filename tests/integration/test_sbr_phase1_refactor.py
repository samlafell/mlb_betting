...
async def setup_staging_schema(conn):
    ...
    schema_path = (
        ...
        / "sql"
        / "sportsbookreview_staging_schema.sql"
    )
...
class TestPhase1Refactor:
    ...
    async def test_successful_run_stores_data_in_staging(
        self, db_connection, aiohttp_mocker, mock_sbr_html
    ):
        """
        ...
        aiohttp_mocker.get(
            f"{base_url}/?date={test_date.strftime('%Y-%m-%d')}", text=mock_sbr_html
        )
        aiohttp_mocker.get(
            f"{base_url}/pointspread/full-game/?date={test_date.strftime('%Y-%m-%d')}",
            text=mock_sbr_html,
        )
        aiohttp_mocker.get(
            f"{base_url}/totals/full-game/?date={test_date.strftime('%Y-%m-%d')}",
            text=mock_sbr_html,
        )
        ...
        try:
            ...
            # Assertions
            raw_count = await db_connection.fetchval(
                "SELECT COUNT(*) FROM sbr_raw_html"
            )
            assert raw_count == 3  # One for each bet type

            parsed_count = await db_connection.fetchval(
    ...
    async def test_validation_failure_prevents_storage(
        self, db_connection, aiohttp_mocker, mock_invalid_sbr_html
    ):
        """
        ...
        base_url = "https://www.sportsbookreview.com/betting-odds/mlb-baseball"
        aiohttp_mocker.get(
            f"{base_url}/?date={test_date.strftime('%Y-%m-%d')}",
            text=mock_invalid_sbr_html,
        )
        aiohttp_mocker.get(
            f"{base_url}/pointspread/full-game/?date={test_date.strftime('%Y-%m-%d')}",
            text=mock_invalid_sbr_html,
        ...
        try:
            ...
            assert (
                parsed_count == 0
            )  # Parsed data should be zero due to validation failure
        finally:
            await scraper.close_session()
            await pool.close()
    ...
    async def test_circuit_breaker_opens_on_failures(self, aiohttp_mocker, monkeypatch):
        ...
        scraper = SportsbookReviewScraper(
            cb_failure_threshold=2, cb_recovery_timeout=10
        )
