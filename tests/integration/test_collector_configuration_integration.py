...
class TestCollectorConfigurationIntegration:
    ...
    def test_vsin_collector_with_custom_base_url(self):
        ...
        config = CollectorConfig(
            source=DataSource.VSIN, enabled=True, base_url="https://custom-vsin.com"
        )
    ...
    def test_sbd_collector_accepts_collector_config(self):
        ...
        assert hasattr(collector, "base_url")
        assert (
            collector.base_url == "https://www.sportsbettingdime.com"
        )  # Default fallback
    ...
    @pytest.mark.asyncio
    async def test_action_network_collector_accepts_collector_config(
        self, mock_settings
    ):
        """Test Action Network collector properly accepts CollectorConfig parameter."""
        with patch(
            "src.data.collection.consolidated_action_network_collector.get_settings",
            ...
            assert collector.config == config
            assert (
                collector.source == DataSource.ACTION_NETWORK.value
            )  # Config uses enum values
    ...
    def test_mlb_stats_api_collector_accepts_collector_config(self, mock_settings):
        ...
        with patch(
            ...
            assert collector.config == config
            assert (
                collector.source == DataSource.MLB_STATS_API.value
            )  # Config uses enum values
    ...
    async def test_sbd_collector_test_collection_async_execution(self):
        ...
        with patch.object(
            collector, "collect_data", new_callable=AsyncMock
        ) as mock_collect:
    ...
    def test_all_collectors_follow_base_collector_pattern(self, mock_settings):
        ...
        collector_configs = [
            (
                ...
                CollectorConfig(source=DataSource.VSIN, enabled=True),
            ),
            (
            ...
            (
                ...
                CollectorConfig(source=DataSource.MLB_STATS_API, enabled=True),
            ),
        ]
        ...
        with patch(
            ...
            for collector_class, config in collector_configs:
                ...
                assert issubclass(collector_class, BaseCollector), (
                    f"{collector_class.__name__} should inherit from BaseCollector"
                )
    ...
    async def test_cli_compatibility_sync_vs_async_handling(self):
        ...
        with patch.object(
            sbd_collector, "collect_data", new_callable=AsyncMock
        ) as mock_collect:
            mock_collect.return_value = []
    ...
    def test_collector_config_immutability_and_thread_safety(self):
        ...
        config = CollectorConfig(
            source=DataSource.VSIN, enabled=True, base_url="https://original.com"
        )
        ...
        try:
            ...
            modified_config = config.model_copy(
                update={"base_url": "https://modified.com"}
            )
    ...
    async def test_collector_error_handling_with_config(self):
        ...
        try:
            ...
            sbd_collector = SBDUnifiedCollectorAPI(
                valid_config
            )  # Wrong source but should not crash
