...
class DataCollectionPerformanceTester:
    ...
    async def test_collection_performance(self) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                logger.warning(f"Could not initialize Action Network collector for testing: {str(e)}")
    ...
    async def _test_source_performance(
        ...
        for attempt in range(5):
            try:
                ...
                metrics["error_count"] += 1
                logger.warning(f"Performance test attempt {attempt + 1} failed for {source_name}: {str(e)}")
        ...
        if metrics["response_times"]:
            ...
            metrics["response_time_stddev"] = statistics.stdev(metrics["response_times"]) if len(metrics["response_times"]) > 1 else 0
        else:
        ...
        metrics["records_per_second"] = metrics["total_records"] / total_time if total_time > 0 else 0
        
        return metrics

    async def _test_concurrent_collection(self, sources: Dict[str, Any]) -> Dict[str, Any]:
        ...
        concurrent_metrics = {
            ...
            "sources_completed": len([r for r in results.values() if r.get("status") == "success"]),
            "sources_failed": len([r for r in results.values() if r.get("status") != "success"]),
            "source_results": results,
            "concurrent_success_rate": len([r for r in results.values() if r.get("status") == "success"]) / len(results) * 100 if results else 0
        }
    ...
    def _calculate_performance_summary(self, metrics: Dict[str, Any]) -> Dict[str, Any]:
        """Calculate overall performance summary."""
        source_metrics = {k: v for k, v in metrics.items() if k != "concurrent_collection"}
        
        if not source_metrics:
            return {"status": "no_data"}

        avg_response_times = [m.get("avg_response_time", 0) for m in source_metrics.values() if isinstance(m, dict)]
        success_rates = [m.get("success_rate", 0) for m in source_metrics.values() if isinstance(m, dict)]
        ...
        summary = {
            ...
            "fastest_source": min(source_metrics.keys(), key=lambda k: source_metrics[k].get("avg_response_time", float('inf'))) if source_metrics else None,
            "slowest_source": max(source_metrics.keys(), key=lambda k: source_metrics[k].get("avg_response_time", 0)) if source_metrics else None,
            "concurrent_performance": metrics.get("concurrent_collection", {}).get("concurrent_success_rate", 0),
    ...
    def _assess_network_resilience(self, network_tests: Dict[str, Any]) -> str:
        ...
        passed_tests = sum(1 for test in network_tests.values() if test.get("status") == "passed")
    ...
    def _calculate_error_handling_summary(self, results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for scenario_result in results.values():
            if isinstance(scenario_result, dict):
                ...
                for key in ["overall_resilience", "api_resilience", "database_resilience"]:
                    if key in scenario_result:
                        resilience_scores.append(resilience_map.get(scenario_result[key], 1))
        ...
        return {
            ...
            "resilience_score": statistics.mean(resilience_scores) if resilience_scores else 0,
            "recommendations": self._generate_error_handling_recommendations(results, overall_grade)
        }

    def _generate_error_handling_recommendations(self, results: Dict[str, Any], grade: str) -> List[str]:
        ...
        if grade == "needs_improvement":
            ...
            recommendations.append("Consider adding chaos engineering tests for production resilience")
        else:  # excellent
            recommendations.append("Error handling is excellent - maintain current practices")
    ...
    async def _test_connection_pooling(self) -> Dict[str, Any]:
        ...
        if not self.db_pool:
            return {"test": "connection_pooling", "status": "skipped", "reason": "no_pool_available"}
    ...
    def _calculate_resource_summary(self, tests: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "connection_management": tests.get("connection_pooling", {}).get("status", "unknown"),
            "cleanup_effectiveness": tests.get("resource_cleanup", {}).get("status", "unknown")
        }
...
async def test_data_collection_performance():
    ...
    async with DataCollectionPerformanceTester() as tester:
        ...
        assert results["status"] in ["completed", "failed"], f"Invalid status: {results['status']}"
        
        if results["status"] == "failed":
            pytest.fail(f"Performance testing failed: {results.get('error', 'Unknown error')}")
        
        # Validate performance criteria
        summary = results.get("summary", {})
        assert summary.get("overall_success_rate", 0) >= 60, "Collection success rate too low"
        assert summary.get("performance_grade") != "needs_improvement", "Performance needs improvement"
...
async def test_error_handling_resilience():
    ...
    async with DataCollectionPerformanceTester() as tester:
        ...
        if results["status"] == "failed":
            pytest.fail(f"Error handling testing failed: {results.get('error', 'Unknown error')}")
        
        # Validate error handling criteria
        summary = results.get("summary", {})
        assert summary.get("overall_error_handling_grade") != "needs_improvement", "Error handling needs improvement"
...
async def test_resource_management():
    ...
    async with DataCollectionPerformanceTester() as tester:
        ...
        if results["status"] == "failed":
            pytest.fail(f"Resource management testing failed: {results.get('error', 'Unknown error')}")
        
        # Validate resource management criteria
        summary = results.get("summary", {})
        assert summary.get("resource_management_grade") != "needs_improvement", "Resource management needs improvement"
        assert summary.get("success_rate", 0) >= 80, "Resource management success rate too low"
...
async def test_comprehensive_pipeline_performance():
    ...
    async with DataCollectionPerformanceTester() as tester:
        ...
        comprehensive_summary = {
            ...
            "timestamp": datetime.now().isoformat(),
            "performance_grade": performance_results.get("summary", {}).get("performance_grade", "unknown"),
            "error_handling_grade": error_results.get("summary", {}).get("overall_error_handling_grade", "unknown"),
            "resource_management_grade": resource_results.get("summary", {}).get("resource_management_grade", "unknown"),
        ...
        logger.info("Comprehensive pipeline performance testing completed", 
...
def _assess_overall_pipeline_health(performance_results: Dict, error_results: Dict, resource_results: Dict) -> str:
    ...
    grade_scores = {"excellent": 4, "good": 3, "acceptable": 2, "needs_improvement": 1, "unknown": 0}
...
def _extract_performance_issues(performance_results: Dict, error_results: Dict, resource_results: Dict) -> List[str]:
...
def _generate_performance_recommendations(performance_results: Dict, error_results: Dict, resource_results: Dict) -> List[str]:
    ...
    overall_health = _assess_overall_pipeline_health(performance_results, error_results, resource_results)
