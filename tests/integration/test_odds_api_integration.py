...
def main():
    ...
    print(f"   Used: {usage_status['used']}/{usage_status['used'] + usage_status['remaining']} credits")
    ...
    if usage_status['remaining'] >= 10:
        ...
        if response.lower() == 'y':
            ...
            odds_data = odds_service.get_mlb_odds(
                market_config=recommended_config,
                regions="us",
    ...
    print(f"\n🎯 Integration Tips:")
    print(f"   • Use 'essential' config (moneyline only) for maximum games")
    print(f"   • Use 'standard' config for full betting markets")
    print(f"   • Monitor usage with odds_service.get_usage_status()")
    print(f"   • Budget automatically resets each month")
    print(f"   • Free game list endpoint to check schedules")
