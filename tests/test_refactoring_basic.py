...
def test_base_collector_structure():
    ...
    try:
        from data.collection.base import BaseCollector, CollectorConfig, CollectionRequest
        
        # Check BaseCollector methods
        base_methods = [m for m in dir(BaseCollector) if not m.startswith('_')]
        expected_methods = ['collect_data', 'validate_record', 'normalize_record', 'test_connection']
...
def test_pydantic_models_structure():
    ...
    try:
        ...
        config_fields = CollectorConfig.__fields__ if hasattr(CollectorConfig, '__fields__') else {}
        expected_config_fields = ['source', 'enabled', 'base_url', 'api_key', 'rate_limit_per_minute']
...
def test_collector_class_inheritance():
    ...
    try:
        ...
        print(f"{'✅' if is_subclass else '❌'} SBD collector inherits from BaseCollector: {is_subclass}")
    ...
    try:
        ...
        print(f"{'✅' if is_subclass else '❌'} VSIN collector inherits from BaseCollector: {is_subclass}")
    ...
    try:
        from data.collection.consolidated_action_network_collector import ActionNetworkCollector
        from data.collection.base import BaseCollector
        
        is_subclass = issubclass(ActionNetworkCollector, BaseCollector)
        results['ActionNetwork'] = is_subclass
        print(f"{'✅' if is_subclass else '❌'} Action Network collector inherits from BaseCollector: {is_subclass}")
...
def test_collector_method_signatures():
    ...
    collectors_to_test = [
        ...
        ('ActionNetwork', 'data.collection.consolidated_action_network_collector', 'ActionNetworkCollector')
    ]
    ...
    for name, module_path, class_name in collectors_to_test:
        try:
            ...
            print(f"{status} {name} collector method check: {len(methods_present)}/{len(required_methods)} methods present")
...
def test_migration_helper_functions():
    ...
    try:
        from data.collection.migration_helper import create_collector_config, create_collection_request
        ...
        print(f"{'✅' if config_valid else '❌'} create_collector_config works: {config_valid}")
        ...
        print(f"{'✅' if request_valid else '❌'} create_collection_request works: {request_valid}")
...
def test_factory_registration():
    ...
    try:
        from data.collection.base import CollectorFactory, CollectorConfig, DataSource
        ...
        for source in sources_to_test:
            try:
                ...
                print(f"{'✅' if is_base_collector else '❌'} Factory creates {source.value} collector: {is_base_collector}")
