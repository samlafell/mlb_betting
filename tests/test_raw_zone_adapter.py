...
async def test_store_action_network_games():
    ...
    with patch(
        "src.data.pipeline.raw_zone_adapter.RawZoneProcessor"
        ...
        odds_data = [
            {
                ...
                "markets": {"moneyline": {"home": -150, "away": +130}},
            },
            {"key": "fanduel", "markets": {"moneyline": {"home": -145, "away": +125}}},
            {
                "sportsbook_key": "betmgm",
                "markets": {"spread": {"home": -1.5, "away": +1.5}},
            },
        ]

        # Store odds
        result = await adapter.store_action_network_odds(
            odds_data, game_id="game_123", source_info={"endpoint": "odds_api"}
        )
...
async def test_store_sbd_betting_splits():
    ...
    with patch(
        ...
        source_info = {"api_endpoint": "https://api.sbd.com/betting-splits"}
...
async def test_store_vsin_data():
    ...
    with patch(
        ...
        source_info = {"source_feed": "vsin_premium"}

        # Store VSIN data
        result = await adapter.store_vsin_data(
            vsin_data, data_type="analysis", source_info=source_info
        )
...
async def test_store_mlb_stats_data():
    ...
    with patch(
        ...
        source_info = {"api_endpoint": "https://statsapi.mlb.com/api/v1/games"}
...
async def test_store_betting_lines():
    ...
    with patch(
        ...
        result = await adapter.store_betting_lines(
            lines_data, bet_type="moneyline", source="oddsapi"
        )
...
async def test_store_line_movements():
    ...
    with patch(
        ...
        result = await adapter.store_line_movements(
            movements_data, source="movements_api"
        )
...
async def test_error_handling():
    ...
    with patch(
        ...
        mock_processor = AsyncMock()
        mock_processor.process_batch.side_effect = Exception(
