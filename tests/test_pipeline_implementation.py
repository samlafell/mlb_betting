...
class TestRawZoneProcessor:
    ...
    def raw_config(self):
        ...
        return create_zone_config(
            ZoneType.RAW, "raw_data", batch_size=100, validation_enabled=True
        )
    ...
    async def test_raw_validation(self, raw_processor):
        ...
        valid_record = DataRecord(
            external_id="test_123", source="action_network", raw_data={"valid": "data"}
        )
        ...
        invalid_record = DataRecord(source="action_network")
...
class TestPipelineOrchestrator:
    ...
    def orchestrator(self):
        ...
        with patch(
            "src.data.pipeline.pipeline_orchestrator.get_settings"
        ) as mock_settings:
            settings = Mock()
    ...
    async def test_zone_processing(self, orchestrator):
        ...
        with patch.object(
            orchestrator.zones[ZoneType.RAW], "process_batch"
        ) as mock_process:
    ...
    async def test_health_check(self, orchestrator):
        ...
        with patch.object(
            orchestrator.zones[ZoneType.RAW], "health_check"
        ) as mock_raw_health:
            with patch.object(
                orchestrator.zones[ZoneType.STAGING], "health_check"
            ) as mock_staging_health:
                mock_raw_health.return_value = {"status": "healthy", "zone_type": "raw"}
                mock_staging_health.return_value = {
                    "status": "healthy",
                    "zone_type": "staging",
                }
    ...
    async def test_raw_to_staging_flow(self):
        ...
        with patch.object(raw_processor, "get_connection") as mock_raw_conn:
            with patch.object(staging_processor, "get_connection") as mock_staging_conn:
                ...
                staging_processed = await staging_processor.process_record(
                    staging_record
                )
