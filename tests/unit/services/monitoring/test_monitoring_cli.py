...
class TestDashboardCommand:
    ...
    def test_dashboard_command_default_options(
        self, mock_uvicorn, monitoring_commands, cli_runner
    ):
    ...
    @patch("src.interfaces.cli.commands.monitoring.uvicorn")
    def test_dashboard_command_custom_options(
        self, mock_uvicorn, monitoring_commands, cli_runner
    ):
        ...
        result = cli_runner.invoke(
            group, ["dashboard", "--host", "0.0.0.0", "--port", "9000"]
        )
    ...
    @patch("src.interfaces.cli.commands.monitoring.uvicorn")
    def test_dashboard_command_reload_option(
        self, mock_uvicorn, monitoring_commands, cli_runner
    ):
    ...
    @patch("src.interfaces.cli.commands.monitoring.uvicorn")
    def test_dashboard_command_error_handling(
        self, mock_uvicorn, monitoring_commands, cli_runner
    ):
...
class TestStatusCommand:
    ...
    def test_status_command_success(
        self, mock_get, monitoring_commands, cli_runner, mock_dashboard_response
    ):
    ...
    @patch("src.interfaces.cli.commands.monitoring.httpx.get")
    def test_status_command_custom_url(
        self, mock_get, monitoring_commands, cli_runner, mock_dashboard_response
    ):
        ...
        group = monitoring_commands.create_group()
        result = cli_runner.invoke(
            group, ["status", "--dashboard-url", "http://custom:8080"]
        )
    ...
    def test_status_command_connection_error(
        self, mock_get, monitoring_commands, cli_runner
    ):
    ...
    @patch("src.interfaces.cli.commands.monitoring.httpx.get")
    def test_status_command_unhealthy_response(
        self, mock_get, monitoring_commands, cli_runner
    ):
    ...
    def test_status_command_detailed_flag(
        self, mock_get, monitoring_commands, cli_runner, mock_system_health_response
    ):
...
class TestLiveCommand:
    ...
    def test_live_command_basic(
        self, mock_asyncio_run, mock_ws_connect, monitoring_commands, cli_runner
    ):
    ...
    @patch("src.interfaces.cli.commands.monitoring.websockets.connect")
    @patch("src.interfaces.cli.commands.monitoring.asyncio.run")
    def test_live_command_custom_url(
        self, mock_asyncio_run, mock_ws_connect, monitoring_commands, cli_runner
    ):
        ...
        result = cli_runner.invoke(
            group, ["live", "--dashboard-url", "http://custom:8080"]
        )
    ...
    @patch("src.interfaces.cli.commands.monitoring.websockets.connect")
    @patch("src.interfaces.cli.commands.monitoring.asyncio.run")
    def test_live_command_with_filter(
        self, mock_asyncio_run, mock_ws_connect, monitoring_commands, cli_runner
    ):
    ...
    @patch("src.interfaces.cli.commands.monitoring.asyncio.run")
    def test_live_command_connection_error(
        self, mock_asyncio_run, monitoring_commands, cli_runner
    ):
...
class TestExecuteCommand:
    ...
    def test_execute_command_default_pipeline(
        self, mock_post, monitoring_commands, cli_runner
    ):
    ...
    @patch("src.interfaces.cli.commands.monitoring.httpx.post")
    def test_execute_command_specific_pipeline_type(
        self, mock_post, monitoring_commands, cli_runner
    ):
    ...
    @patch("src.interfaces.cli.commands.monitoring.httpx.post")
    def test_execute_command_force_flag(
        self, mock_post, monitoring_commands, cli_runner
    ):
    ...
    @patch("src.interfaces.cli.commands.monitoring.httpx.post")
    def test_execute_command_custom_dashboard_url(
        self, mock_post, monitoring_commands, cli_runner
    ):
        ...
        group = monitoring_commands.create_group()
        result = cli_runner.invoke(
            group, ["execute", "--dashboard-url", "http://custom:8080"]
        )
    ...
    @patch("src.interfaces.cli.commands.monitoring.httpx.post")
    def test_execute_command_pipeline_failure(
        self, mock_post, monitoring_commands, cli_runner
    ):
    ...
    @patch("src.interfaces.cli.commands.monitoring.httpx.post")
    def test_execute_command_api_error(
        self, mock_post, monitoring_commands, cli_runner
    ):
...
class TestCommandValidation:
    ...
    def test_pipeline_type_validation(self, monitoring_commands, cli_runner):
        ...
        result = cli_runner.invoke(
            group, ["execute", "--pipeline-type", "invalid_type"]
        )
    ...
    def test_valid_pipeline_types(self, monitoring_commands, cli_runner):
        ...
        for pipeline_type in valid_types:
            with patch(
                "src.interfaces.cli.commands.monitoring.httpx.post"
            ) as mock_post:
                mock_response = Mock()
                mock_response.status_code = 200
                mock_response.json.return_value = {
                ...
                result = cli_runner.invoke(
                    group, ["execute", "--pipeline-type", pipeline_type]
                )
                assert result.exit_code == 0
...
class TestLiveMonitoringWebSocket:
    ...
    def test_message_filtering(self):
        ...
        # Test with filter
        assert (
            should_display_message("pipeline_update", ["pipeline_update", "alerts"])
            is True
        )
        assert (
            should_display_message("system_health", ["pipeline_update", "alerts"])
            is False
        )
...
class TestBreakGlassIntegration:
    ...
    def test_break_glass_metrics_recording(
        self, mock_get_metrics, monitoring_commands, cli_runner
    ):
