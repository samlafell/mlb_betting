...
class TestVarcharConstraintCompliance:
    ...
    def test_all_mlb_teams_fit_varchar_constraint(self):
        ...
        test_teams = [
            "Philadelphia Phillies", "Boston Red Sox", "New York Yankees",
            "Chicago White Sox", "Los Angeles Angels", "San Francisco Giants",
            "Arizona Diamondbacks", "Cleveland Guardians", "Kansas City Royals",
            "Minnesota Twins", "Houston Astros", "Oakland Athletics",
            "Seattle Mariners", "Texas Rangers", "Atlanta Braves",
            "Miami Marlins", "New York Mets", "Washington Nationals",
            "Chicago Cubs", "Cincinnati Reds", "Milwaukee Brewers",
            "Pittsburgh Pirates", "St. Louis Cardinals", "Colorado Rockies",
            "Los Angeles Dodgers", "San Diego Padres", "Toronto Blue Jays",
            "Baltimore Orioles", "Tampa Bay Rays", "Detroit Tigers"
        ]
        
        for team_name in test_teams:
            result = normalize_team_name(team_name)
            assert len(result) <= 10, f"Team '{team_name}' normalized to '{result}' (length {len(result)}) exceeds VARCHAR(10)"
            assert len(result) >= 2, f"Team '{team_name}' normalized to '{result}' is too short"
    ...
    def test_edge_case_inputs_fit_constraint(self):
        ...
        for edge_case in edge_cases:
            ...
            assert len(result) <= 10, f"Edge case '{edge_case}' produced result '{result}' exceeding VARCHAR(10)"
...
class TestSecurityAndRobustness:
    ...
    def test_unicode_handling(self):
        ...
        unicode_inputs = [
            "Montréal Expos", "São Paulo", "北京队", "Москва", "🏈⚾🏀"
        ]
