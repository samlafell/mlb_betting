...
def test_pipeline_cli_imports():
    ...
    from src.interfaces.cli.commands.pipeline import pipeline_group, pipeline
...
def test_pipeline_run_dry_run():
    ...
    assert '--dry-run' in result.output or 'DRY RUN' in result.output or result.exit_code == 0
...
async def test_pipeline_run_full_execution():
    ...
    from src.data.pipeline.pipeline_orchestrator import PipelineExecution, PipelineMode, PipelineMetrics
    ...
    with patch('src.interfaces.cli.commands.pipeline.create_pipeline_orchestrator') as mock_create, \
         patch('src.interfaces.cli.commands.pipeline._get_sample_records') as mock_records, \
...
async def test_pipeline_run_zone_specific():
    ...
    mock_execution = PipelineExecution(
        pipeline_mode=PipelineMode.RAW_ONLY,
        status=ProcessingStatus.COMPLETED
    )
...
async def test_pipeline_status_specific_execution():
    ...
    with patch('src.interfaces.cli.commands.pipeline.create_pipeline_orchestrator') as mock_create, \
        ...
        mock_orchestrator.get_execution_status.assert_called_once_with(UUID(execution_id))
...
def test_display_functions():
    ...
    from src.interfaces.cli.commands.pipeline import (
        _display_dry_run_summary, _display_execution_results,
        _display_execution_status, _display_all_zones_status, _display_zone_status
    )
    from src.data.pipeline.pipeline_orchestrator import PipelineExecution, PipelineMode, PipelineMetrics
    from src.data.pipeline.zone_interface import ProcessingStatus, ZoneType, DataRecord
    ...
    with patch('src.interfaces.cli.commands.pipeline.console') as mock_console:
        ...
        health_status = {
            ZoneType.RAW: {
                'status': 'healthy',
                'metrics': {'records_processed': 100, 'quality_score': 0.9, 'error_rate': 0.1}
            }
        }
        ...
        _display_zone_status(ZoneType.RAW, health_status[ZoneType.RAW], True)  # Detailed
        assert mock_console.print.called
...
def test_error_handling():
    ...
    from src.interfaces.cli.commands.pipeline import run_pipeline, pipeline_status
