...
async def test_full_pipeline_execution():
    ...
    with (
        patch("src.data.pipeline.pipeline_orchestrator.ZoneFactory") as mock_factory,
        patch.object(DataPipelineOrchestrator, "_log_pipeline_execution") as mock_log,
        patch.object(
            DataPipelineOrchestrator, "_get_records_for_next_zone"
        ) as mock_get_records,
    ):
        # Mock zones
        mock_raw_zone = AsyncMock()
        ...
        mock_records = [
            DataRecord(external_id=f"record_{i}", source="test") for i in range(9)
        ]
        ...
        execution = await orchestrator.run_full_pipeline(
            test_records, execution_metadata={"test_run": True}
        )
...
async def test_partial_pipeline_modes():
    ...
    with patch("src.data.pipeline.pipeline_orchestrator.ZoneFactory") as mock_factory:
        ...
        test_records = [
            DataRecord(external_id=f"partial_test_{i}", source="test") for i in range(5)
        ]

        # Test RAW-only mode
        raw_only_execution = await orchestrator.run_single_zone_pipeline(
            ZoneType.RAW, test_records
        )
...
async def test_pipeline_error_handling():
    ...
    with (
        ...
        test_records = [
            DataRecord(external_id=f"error_test_{i}", source="test") for i in range(5)
        ]
...
async def test_pipeline_execution_tracking():
    ...
    with (
        ...
        execution = await orchestrator.run_single_zone_pipeline(
            ZoneType.RAW, test_records, execution_metadata={"test_type": "tracking"}
        )
...
async def test_pipeline_metrics_collection():
    ...
    with patch("src.data.pipeline.pipeline_orchestrator.ZoneFactory") as mock_factory:
        ...
        with patch.object(
            ...
            assert (
                overall_metrics["overall_quality_score"] >= 0.0
            )  # Can be 0.0 for testing
