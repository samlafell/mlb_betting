...
async def test_raw_data_normalization():
    ...
    raw_data = {
        ...
        "outcomes": [{"price": -150, "point": -1.5, "team": "home"}],
        "moneyline": {"home": -150, "away": 130},
    }

    record = StagingDataRecord(
        external_id="norm_test", source="action_network", raw_data=raw_data
    )
    ...
    record2 = StagingDataRecord(
        external_id="alt_test", source="generic", raw_data=alt_raw_data
    )
...
async def test_team_name_normalization():
    ...
    with patch("src.data.pipeline.staging_zone.normalize_team_name") as mock_normalize:
        ...
        mock_normalize.side_effect = lambda x: x.replace("NY", "New York").replace(
            "LA", "Los Angeles"
        )
...
async def test_sportsbook_name_normalization():
    ...
    record = StagingDataRecord(
        external_id="sb_test", source="test", sportsbook_name="draftkings"
    )
    ...
    record2 = StagingDataRecord(
        external_id="sb_test2", source="test", sportsbook_name="unknown_book"
    )

    normalized2 = await processor._normalize_sportsbook_names(record2)

    assert (
        normalized2.sportsbook_name == "Unknown_Book"
    )  # Title case applied to original
...
async def test_batch_processing():
    ...
    with (
        patch("src.data.pipeline.staging_zone.normalize_team_name") as mock_normalize,
