...
class TestZoneInterface:
    ...
    def test_zone_config_validation(self):
        ...
        config1 = ZoneConfig(
            zone_type=ZoneType.RAW, schema_name="raw_data", quality_threshold=0.5
        )
        assert config1.quality_threshold == 0.5

        config2 = ZoneConfig(
            zone_type=ZoneType.RAW, schema_name="raw_data", quality_threshold=1.0
        )
        ...
        with pytest.raises(ValueError):
            ZoneConfig(
                zone_type=ZoneType.RAW, schema_name="raw_data", quality_threshold=1.5
            )

        with pytest.raises(ValueError):
            ZoneConfig(
                zone_type=ZoneType.RAW, schema_name="raw_data", quality_threshold=-0.1
            )
    ...
    def test_data_record_with_quality_score(self):
        ...
        record = DataRecord(external_id="test_456", source="sbd", quality_score=0.95)
...
class TestCoreIntegration:
    ...
    def test_zone_config_to_metrics_workflow(self):
        ...
        config = create_zone_config(
            ZoneType.RAW, "raw_data", batch_size=100, quality_threshold=0.8
        )
        ...
        assert all(
            r.quality_score >= config.quality_threshold for r in records
        )  # Quality check
