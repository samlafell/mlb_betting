...
class TestEndToEndMLPipeline:
    ...
    async def feature_pipeline(self, test_config):
        ...
        return FeaturePipeline(
            config=test_config, feature_version="integration_test_v1.0"
        )

    @pytest.fixture
    async def redis_store(self, test_config):
        """Redis store instance"""
        store = RedisAtomicStore(redis_url=test_config.redis.url, use_msgpack=True)
    ...
    async def test_complete_feature_extraction_workflow(
        self, feature_pipeline, redis_store, database_pool, sample_game_data
    ):
    ...
    async def test_batch_feature_processing_integration(
        self, feature_pipeline, redis_store, sample_game_data
    ):
        ...
        with patch.object(
            feature_pipeline, "_load_game_data", side_effect=mock_load_data
        ):
    ...
    async def test_model_training_integration(
        self, temp_mlflow_dir, feature_pipeline, sample_game_data
    ):
        ...
        training_data = sample_game_data.with_columns(
            [
                ...
                (pl.col("home_ml_odds") < pl.col("away_ml_odds")).alias(
                    "home_favorite"
                ),
        ...
        trainer = LightGBMTrainer(
            experiment_name="integration_test", model_registry_name="test_model"
        )
    ...
    async def test_prediction_api_integration(self, test_config):
        ...
        with patch.dict(os.environ, {"ENVIRONMENT": "development"}):
            ...
            assert response.status_code in [
                ...
                422,
                503,
            ]  # 422 if validation fails, 503 if model not available

        # Test production auth requirement
        with patch.dict(
            os.environ, {"ENVIRONMENT": "production", "API_SECRET_KEY": "test_key"}
        ):
            # Without auth header
            response = client.post("/api/v1/predict", json=prediction_request)
            ...
            response = client.post(
                "/api/v1/predict", json=prediction_request, headers=headers
            )
            assert response.status_code in [200, 422, 503]

    @pytest.mark.asyncio
    async def test_ml_training_service_integration(
        self, temp_mlflow_dir, test_config, sample_game_data
    ):
    ...
    async def test_data_consistency_across_pipeline(
        ...
        with patch.object(feature_pipeline, "_load_game_data") as mock_load:
            ...
            assert (
                cached_features.minutes_before_game
                == original_features.minutes_before_game
            )

            # Verify temporal features consistency
            if (
                original_features.temporal_features
                and cached_features.temporal_features
            ):
                assert (
...
class TestConcurrentPipelineOperations:
    ...
    async def test_concurrent_feature_extraction(self):
        ...
        assert (
            len(successful_results) >= len(game_ids) * 0.8
