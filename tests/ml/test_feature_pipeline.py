...
class TestFeaturePipeline:
    ...
    def sample_game_data(self):
        ...
        return pl.DataFrame({
            ...
            'sportsbook_name': ['DraftKings', 'FanDuel', 'BetMGM', 'Caesars', 'DraftKings'],
    ...
    @pytest.mark.asyncio
    async def test_feature_extraction_success(self, feature_pipeline, mock_database_pool, sample_game_data):
        """Test successful feature extraction"""
        game_id = 12345
        cutoff_time = datetime.now() - timedelta(hours=2)
        
        # Mock database responses
        mock_database_pool.fetch.return_value = sample_game_data.to_pandas().to_dict('records')
        
        with patch.object(feature_pipeline, '_load_game_data', return_value={'temporal_data': sample_game_data}):
            result = await feature_pipeline.extract_features_for_game(game_id, cutoff_time)
    ...
    async def test_batch_processing_memory_efficiency(self, feature_pipeline):
        ...
        with patch.object(feature_pipeline, 'extract_features_for_game', side_effect=mock_extract):
    ...
    @pytest.mark.asyncio
    async def test_database_transaction_handling(self, feature_pipeline, mock_database_pool):
        ...
        with patch.object(feature_pipeline, '_get_database_pool', return_value=mock_database_pool):
        ...
        with patch.object(feature_pipeline, '_load_game_data') as mock_load:
            mock_load.return_value = {
                'temporal_data': pl.DataFrame({
                    'game_id': [12345] * 10,
                    'timestamp': [cutoff_time - timedelta(minutes=i) for i in range(10)]
                })
            }
...
class TestFeaturePipelinePerformance:
    ...
    async def test_feature_extraction_performance(self, benchmark):
        ...
        result = await benchmark(
            pipeline.extract_features_for_game,
            game_id,
            cutoff_time
        )
