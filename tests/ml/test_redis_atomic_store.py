...
class TestRedisAtomicStore:
    ...
    async def redis_store(self):
        ...
        store = RedisAtomicStore(redis_url="redis://localhost:6379/15", use_msgpack=True)  # Use test DB
    ...
    @pytest.mark.asyncio
    async def test_atomic_cache_operations(self, mock_redis_store, sample_feature_vector):
    ...
    @pytest.mark.asyncio
    async def test_messagepack_serialization_fallback(self, mock_redis_store, sample_feature_vector):
    ...
    @pytest.mark.asyncio
    async def test_batch_atomic_operations(self, mock_redis_store, sample_feature_vector):
        """Test atomic batch operations"""
        # Create batch of feature vectors
        feature_vectors = [
            (12345 + i, sample_feature_vector) for i in range(10)
        ]
        
        # Mock Redis pipeline operations
        mock_pipe = AsyncMock()
        mock_pipe.execute.return_value = [True] * 10  # All operations succeed
        mock_redis_store.redis_client.pipeline.return_value.__aenter__.return_value = mock_pipe
        
        # Test batch caching
        cached_count = await mock_redis_store.cache_batch_features_atomic(feature_vectors)
    ...
    @pytest.mark.asyncio
    async def test_concurrent_access_consistency(self, mock_redis_store, sample_feature_vector):
    ...
    @pytest.mark.asyncio
    async def test_error_handling_and_recovery(self, mock_redis_store, sample_feature_vector):
        """Test error handling and graceful recovery"""
        # Test Redis connection error
        mock_redis_store.redis_client.setex.side_effect = Exception("Connection error")
        
        success = await mock_redis_store.cache_feature_vector_atomic(12345, sample_feature_vector)
        assert success is False
        assert mock_redis_store.cache_stats['errors'] == 1
        
        # Test serialization error recovery
        with patch.object(mock_redis_store, '_serialize_feature_vector') as mock_serialize:
...
class TestRedisStoreIntegration:
    ...
    async def test_real_redis_operations(self):
        ...
        try:
            ...
            retrieved = await store.get_feature_vector_atomic(99999, "integration_test_v1.0")
...
class TestRedisStorePerformance:
    ...
    async def test_cache_operation_performance(self, benchmark):
        ...
        result = await benchmark(
            mock_store.cache_feature_vector_atomic,
            12345,
            feature_vector
        )
