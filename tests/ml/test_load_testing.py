...
class TestAPILoadTesting:
    ...
    def security_config(self):
        ...
        return SecurityConfig(
            rate_limit_per_minute=100, prediction_rate_limit=20, environment="testing"
        )

    def test_concurrent_api_requests_with_rate_limiting(
        self, test_client, rate_limiter, security_config
    ):
        ...
        p95_response_time = sorted(
            [r["response_time_ms"] for r in successful_requests]
        )[int(len(successful_requests) * 0.95)]

        # Assertions
        assert (
            len(successful_requests) >= num_concurrent_requests * 0.95
        )  # 95% success rate
        ...
        print(
            f"Success Rate: {len(successful_requests) / num_concurrent_requests * 100:.1f}%"
        )
        ...
        async def test_rate_limiting():
            ...
            for i in range(5):
                ...
                assert allowed is False, (
                    f"Request {rate_limit + i + 1} should be rate limited"
                )
    ...
    async def test_concurrent_prediction_requests(self):
        ...
        async with aiohttp.ClientSession() as session:
            ...
            async def make_prediction_request(session, request_id):
                ...
                try:
                    async with session.post(
                        f"{base_url}/api/v1/predict", json=payload, headers=headers
                    ) as response:
                        end_time = time.time()
                        return {
                            "request_id": request_id,
            ...
            # Execute concurrent requests
            num_requests = 20
            tasks = [make_prediction_request(session, i) for i in range(num_requests)]
...
class TestRedisLoadTesting:
    ...
    async def test_concurrent_redis_operations(self, redis_store):
        ...
        successful_operations = [
            ...
            and r.get("cache_success")
            and r.get("retrieve_success")
        ]

        avg_cache_time = sum(r["cache_time_ms"] for r in successful_operations) / len(
            successful_operations
        )
        avg_retrieve_time = sum(
            r["retrieve_time_ms"] for r in successful_operations
        ) / len(successful_operations)

        # Assertions
        assert (
            len(successful_operations) >= num_concurrent_operations * 0.95
        )  # 95% success rate
        ...
        print(
            f"Success Rate: {len(successful_operations) / num_concurrent_operations * 100:.1f}%"
        )
    ...
    async def test_redis_atomic_lock_contention(self, redis_store):
            ...
            for operation in range(operations_per_competitor):
                try:
                    async with redis_store.distributed_lock(
                        f"{shared_resource_key}_{operation}", timeout=1
                    ):
        ...
        assert (
            total_acquisitions <= operations_per_competitor
        )  # No double-locking per operation
...
class TestDatabaseLoadTesting:
    ...
    async def test_connection_pool_under_load(self, db_pool):
        ...
        avg_query_time = sum(r["query_time_ms"] for r in successful_queries) / len(
            successful_queries
        )
...
class TestMemoryAndResourceUsage:
    ...
    def test_memory_usage_feature_extraction(self):
        ...
        ) < 50  # Good cleanup (under 50MB residual)
