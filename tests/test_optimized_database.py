...
from mlb_sharp_betting.db.optimized_connection import (
    ConnectionConfig,
    OperationPriority,
)
...
class TestOptimizedDatabase:
    ...
    async def test_basic_write_operations(self, optimized_coordinator):
        ...
        result = coordinator.execute_write(
            "INSERT INTO test_writes (id, value) VALUES (?, ?)", (1, "test_value")
        )
    ...
    async def test_parallel_reads(self, optimized_coordinator):
        ...
        coordinator.execute_bulk_insert(
            "INSERT INTO test_parallel (id, data) VALUES (?, ?)", test_data
        )
    ...
    def test_performance_comparison(self, legacy_coordinator, optimized_coordinator):
        ...
        logger.info(
            f"Performance improvement: {((legacy_time - optimized_time) / legacy_time * 100):.1f}%"
        )

        # Verify both have same data
        legacy_count = legacy_coordinator.execute_read(
            "SELECT COUNT(*) FROM performance_test"
        )[0][0]
        optimized_count = optimized_coordinator.execute_read(
            "SELECT COUNT(*) FROM performance_test"
        )[0][0]
    ...
    async def test_concurrent_operations(self, optimized_coordinator):
        ...
        final_count = coordinator.execute_read("SELECT COUNT(*) FROM test_concurrent")[
            0
        ][0]
